...
\begin{theorem}[Maximum Modulus Principle]\label{thm:maximummodulus}
    Let \(f(z)\) holomorphic over \(U\subseteq\mathbb{C}\). If \(\exists z_0\in U\) such that \(\abs{f\paren{z_0}}\geq\abs{f(z)}\) \(\forall z\in V\), where \(V\) is some open neighborhood of \(z_0\) in \(U\), then \(f\) is a constant function.
\end{theorem}
\subsubsection{Holomorphic Automorphism Group}
