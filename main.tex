...
\begin{lemma}
    Let \(U\subseteq\mathbb{C}\) be open and let \(\qty{f_n(z)}\) be holomorphic on \(U\). For any compact set \(K\subset U\), if \(\qty{f_n(z)}\) is uniformly convergent to \(f(z)\) on \(\partial K\). Then, \(\qty{f_n(z)}\) is uniformly convergent on \(K\).
\end{lemma}
\begin{proof}
