...
\begin{example}
    Let \(f\) be entire and \(\forall z\in\mathbb{C}\), \(|f(z)|\leq Me^{|z|}\). Then \(\forall n\in\mathbb{N}\), \(|f(0)|\leq M\) and \[\abs{f^{(n)}(0)}\leq Mn!\qty(\frac{e}{n})^n.\]
\end{example}
\begin{proof}
    ...
    By letting \(R=n\), the conclusion follows. In fact, this is the tightest possible inequality. Consider \(Mn!\frac{e^R}{R^n}\) to be a function of \(R\). It attains its minimum as its derivative vanishes:
    \[Mn!\frac{e^RR^n-ne^RR^{n-1}}{R^{2n}}=0\Longleftrightarrow R^n=nR^{n-1}\Longleftrightarrow R=n.\] To confirm it as a minimum, we calculate the second order derivative:
    \[Mn!e^R\qty(\frac{1}{R^n}-\frac{2n}{R^{n+1}}+\frac{n(n+1)}{R^{n+2}})\]
\end{proof}
