...
\begin{proof}
    ...
    For the second part, let \(f(z)=f(x,y)=u(x,y)+iv(x,y)\). Assume that \(u,v\in C^1\paren{\{z_0\}}\) and satisfy the Cauchy Riemann equations at \(z_0=x_0+iy_0\). Let \[\alpha=\pdv{u}{x}\paren{x_0,y_0},\quad\beta=\dv{v}{x}\]
\end{proof}
...
\begin{theorem}\label{thm:onedimensionalpartialconjugatesolution}
    ...
    \end{equation} where \(f(z)\) is any entire \(C^1\) function.
\end{theorem}
\begin{proof}
    Differentiate equation \eqref{eq:onedimensionalpartialconjugatesolution}. By equation \eqref{eq:wirtingerderivative2}, the second term vanishes. 
\end{proof}
\subsection{The Cauchy--Goursat Theorem}
When Cauchy famously published \autoref{thm:cauchyintegralformula} and \autoref{thm:cauchyintegraltheorem}, he included the condition for \(f(z)\in C^1\paren{\overline{U}}\). It was later proved that all holomorphic functions already satisfied this and this condition was dropped by Goursat \(\)
\end{document}