...
\begin{theorem}
    Let \(V=\cbraces{z}{r<\abs{z-a}<R}\) for some \(0\leq r<R\leq\infty\). Let \(f\) be holomorphic on \(V\). Then \(f\) has the unique \textit{Laurent expansion} \[f(z)=\sum_{n=-\infty}^\infty c_n(z-a)^n\] when \(z\in V\), where \(c_n=\frac{1}{2\pi i}\oint_{\gamma}\frac{f(\zeta)\ddzeta}{(\zeta-a)^{n+1}}\), where \(\gamma\subset V\) is a simple closed curve enclosing \(a\). 
\end{theorem}
\begin{proof}
    Let \(\gamma_2,\gamma\subset V\) be two simple closed curves such that \(\gamma_2\) completely encloses \(\gamma\) without intersection and \(\gamma\) encloses \(a\). Let \(W\) be the interior of the region bounded between \(\gamma_2\) and \(\gamma\). By the Cauchy--Goursat Integral Formula (\autoref{thm:cauchygoursatformula}), \(\forall z\in W\), \[f(z)=\int_{\gamma_2}\frac{f(\zeta)\ddzeta}{\zeta-z}-\int_{\gamma}\frac{f(\zeta)\ddzeta}{\zeta-z}.\]
    \(\forall\zeta\in\gamma_2\), since \(\gamma_2\) encloses 
\end{proof}
