...
\begin{proof}
    ...
    \(\forall k\in\mathbb{Z}_{\geq0}\), \(f^{(k)}\) is continuous in \(U\) by the holomorphy of \(f\). Let \(S_k=\cbraces{z\in U}{f^{(k)}(z)=0}\). For any sequence \(\cbraces{z'_n}\in S_k\) converging to some \(z'_\infty\in U\), by the continuity of \(f\), \[\lim_{n\to\infty} f^{(k)}\paren{z'_n}=f\paren{\lim_{n\to\infty}z'_n}=f\paren{z'_\infty}=0,\] and therefore \(z'_\infty\in S_k\). Thus, \(S_k\) contains all of its accumulation points in \(U\) and is therefore closed (if \(z'_\infty\notin U\), then it is no longer relevant; we are concerned about it being closed within \(U\)). Since \(S^*=\bigcap_{k\in\mathbb{Z}_{\geq0}} S_k\)
\end{proof}
\begin{remark}
