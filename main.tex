...
\begin{lemma}
    If \(U\subseteq\mathbb{C}\) is open and simply connected, and \(f:U\to\mathbb{C}\) is holomorphic, \(\forall z\in U\), \(\forall r>0\) such that \(D(z,r)\subseteq U\), \(f(z)\) is the mean of \(f(\zeta)\) where \(\zeta\in D(z,r)\) is uniform.
\end{lemma}
