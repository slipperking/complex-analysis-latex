...
\begin{proof}
    ...
    We will now analyze the curve \(M_1'N_1'\). Observe that \begin{equation}
        \int_{\overrightarrow{M_1N_1}}f(z)\ddz=\int_{\overrightarrow{M_1'N_1'}}f\paren{z-\eta}\ddz.\label{eq:cauchygoursattheorem_boundarytointerior}
    \end{equation} 
    By \autoref{lemma:uniformcontinuousovercompactset}, \(f(z)\) is uniformly continuous over \(\overline{U}\), and therefore \(\forall\varepsilon'>0\), we can choose \(\eta>0\) to satisfy \(\abs{f\paren{z}-f\paren{z-\eta}}<\varepsilon'\) (\(\forall z\in\overrightarrow{M_1'N_1'}\)).
\end{proof}
