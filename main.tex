...
\end{theorem}
\begin{proof}
    By the Cauchy--Goursat Integral Theorem (\autoref{thm:cauchygoursattheorem}), \[\int_{\partial\paren{U\setminus D(z,\varepsilon)}}\frac{f(\zeta)}{\zeta-z}\ddzeta=\oint_{\partial U}\frac{f(\zeta)}{\zeta-z}\ddzeta-\oint_{\partial D(z,\varepsilon)}\frac{f(\zeta)}{\zeta-z}\ddzeta=0.\] Through the polar parameterization of \(\partial D(z,\varepsilon)\),\[\oint_{\partial U}\frac{f(\zeta)}{\zeta-z}\ddzeta=\int_0^{2\pi}\frac{f\paren{z+\varepsilon e^{it}}}{\varepsilon e^{it}}\cdot i\varepsilon e^{it}\dd{t}=i\int_0^{2\pi}f\paren{z+\varepsilon e^{it}}\dd{t}\]
\end{proof}
\end{document}