...
\begin{lemma}
    Let \(\Omega\subseteq\mathbb{C}\) be open, and let \(K\subset\Omega\) be compact and arbitrary. Let \(\qty{f_n(z)}\) be holomorphic on \(\Omega\), continuous in \(\overline{U}\), and uniformly convergent to \(f(z)\) on \(\partial U\). Then, \(\qty{f_n(z)}\) is uniformly convergent on \(\overline{U}\).
\end{lemma}
\begin{proof}
