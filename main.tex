...
\end{proof}
\begin{remark}
    This theorem can be generalized to simple closed curves on the Riemann sphere \(\extcomplex\). In this case, we choose the ``interior'' to be the region whose boundary is traversed counterclockwise.
\end{remark}
...
\begin{proof}
    ...
    Let \(w=f(z)\) be the biholomorphism.
\end{proof}
...
\end{example}
\begin{proof}
    In \autoref{thm:meromorphicautomorphismgrouponextendedcomplexplane}, we showed that all Möbius transformations are meromorphic automorphisms of the extended complex plane \(\widehat{\mathbb{C}}\), forming the group \(\Aut\qty(\extcomplex)\), which is isomorphic to \(\mathrm{PSL}_2(\mathbb{C})\) via the isomorphism \[\frac{az+b}{cz+d}\mapsto\qty{\mqty(a&b\\c&d),-\mqty(a&b\\c&d)}.\] 
