...
\end{equation}
Equations \eqref{eq:cauchyriemanneqs1} and \eqref{eq:cauchyriemanneqs2} are known as the Cauchy-Riemann equations. Although this condition is necessary, it is not sufficient. Consider the function \(f(z)=\sqrt{\left|\real(z)\imaginary(z)\right|}\), or $f(x,y)=\sqrt{\left|xy\right|}$. Through limits, at $(0,0)$, the derivative along both axes are 0. However, along any other direction, the derivative is not the same. 
