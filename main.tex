...
\begin{proof}
    ...
    For each \(r\in\mathbb{N}\) and each \(z\in V_r\), since \(W_r\) is an open set containing \(z\) and \(\mathfrak{B}\) is a basis, there exists \(U_{z,r}\in\mathfrak{B}\) such that \(z\in U_{z,r}\subseteq W_r\). The collection \(\cbraces{U_{z,r}}{z\in V_r}\) is an open cover of the compact set \(V_r\), so by the Heine-Borel theorem it admits a finite subcover. That is, there exist finitely many points \(z_{r,1},\ldots,z_{r,k_r}\) such that \(V_r\subset\bigcup_{i=1}^{k_r}U_{z_{r,i},r}\subseteq W_r\).
