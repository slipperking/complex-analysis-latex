...
\begin{proof}
    ...
    The disks \(\{D(\zeta, r_\zeta)\}_{\zeta \in \partial D(z_0,R)}\) cover the compact set \(\partial D(z_0, R)\). By the Heine--Borel Theorem (\autoref{thm:heineborel}), there is a finite subcover \(\qty{D\qty(\zeta_k,r_k)}_{k=1}^n\). Let \(V=\bigcup_{k=1}^n D\qty(\zeta_k,r_k)\). Therefore, \(\exists\rho>0\) such that \(A=\cbraces{z}{R-\rho\leq\abs{z-z_0}\leq R+\rho}\subseteq V\).

    Define \(\Phi:V\to\mathbb{C}\) by \(\Phi(z)=f_{\zeta_k}(z)\) if \(z\in D\qty(\zeta_k, r_k)\). This is well-defined: If \(z\in D\qty(\zeta_i, r_i)\cap D\qty(\zeta_j,r_j)\), then \(D\qty(\zeta_i,r_i)\cap D\qty(\zeta_j,r_j) \cap D\qty(z_0,R)\neq\emptyset\), and \(f_{\zeta_i}=f_{\zeta_j}=f\) there. Hence, \(f_{\zeta_i}\equiv f_{\zeta_j}\) on \(D\qty(\zeta_i,r_i) \cap D\qty(\zeta_j,r_j)\).
...
\begin{definition}\label{def:analyticchain}
    A \textit{chain of analytic continuations} from \((D_0, f_0)\) is a sequence \(\{(D_k, f_k)\}_{k=0}^n\) where \((D_k, f_k)\) is a direct analytic continuation of \((D_{k-1},f_{k-1})\) for \(1\leq k \leq n\). Then \((D_n, f_n)\) is an \textbf{analytic continuation} of \((D_0, f_0)\).
\end{definition}
