...
\begin{lemma}[Schwarz Lemma]
    ...
    When \(z\neq 0\), the two inequalities become equalities if and only if \(f(z)\) is in the form of \(ze^{i\tau}\), where \(\tau\in\mathbb{R}\). In other words, \(f\) is a pure rotation.
\end{lemma}
\begin{proof}
    ...
    For the sake of the equality condition, assume \(|f(z)|=|z|\). Then \(\abs{g(z)}\equiv 1\) on the unit open disk. By \autoref{thm:maximummodulus}, \(g(z)=e^{i\tau}\) where \(\tau\in\mathbb{R}\) and \(f(z)=ze^{i\tau}\) on \(D(0,1)\). 

    Next, assume only that \(\abs{f'(0)}=1\). It follows that \(g(0)=e^{i\tau}\)
\end{proof}
