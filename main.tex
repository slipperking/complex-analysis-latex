...
\begin{theorem}
    Let \(U\subseteq\mathbb{C}\) be open and connected such that \(f(z)\) is meromorphic on \(U\) with an isolated simple pole at \(z=a\in U\). Then \(\exists\lambda>0\) be such that \(\forall 0<\varepsilon<\lambda\), \(\partial D(a,\varepsilon)\subset U\). Let \(\gamma_\varepsilon\subseteq \partial D(a,\varepsilon)\) be a clockwise oriented connected arc with measure \(\theta\). Then, \[\lim_{\varepsilon\to 0}\int_{\gamma_\varepsilon}f(z)\ddz=i\theta\residue_{z=a}f(z)\]
\end{theorem}
