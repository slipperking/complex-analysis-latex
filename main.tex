...
\end{example}
\begin{example}
    Let \(U\subseteq\mathbb{C}\) be open and simply connected such that \(0\in U\). Suppose \(f:U\setminus\qty{0}:\mathbb{C}\) is holomorphic. If \(\forall \theta\in [0,2\pi]\), \(\lim_{r\to 0^+}f(re^{i\theta})=\infty\), prove that \(0\) cannot be an essential singularity.
\end{example}
\begin{proof}
    Although seemingly trivial at first glance, we are not guaranteed the uniform limit with respect to varying \(\theta\). Namely, for fixed \(\theta\) and \(\varepsilon\), \(\exists\delta\) such that \(\forall 0<r<\delta\), we have \(||\)
\end{proof}
