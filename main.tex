...
\begin{definition}[Residue]\label{def:residue}
    For some \(r\in\mathbb{R}_{>0}\), \(a\in U\), suppose \(f:D^*(a,r)\to \mathbb{C}\) is holomorphic. Then the \textit{residue} of \(f\) at \(a\), denoted by \(\Res(f,a)\), is equal to \begin{equation}
        \Res(f,a)=\frac{1}{2\pi i}\int_{D(a,\rho)}f(z)\ddz,\label{eq:residue}
    \end{equation}
    where \(0<\rho<r\) is arbitrary. Since \(f\) has a Laurent expansion at \(a\), being \[\sum_{n=-\infty}^\infty c_n(z-a)^n,\qquad c_n=\frac{1}{2\pi i}\int_{D(a,\rho)}\frac{f(z)}{(z-a)^{n+1}}\ddz,\] we get that the residue of \(f\) at \(a\) is equal to the first term of the principal part of its Laurent expansion.
\end{definition}
It then follows that the residue at a removable singularity is 0.
