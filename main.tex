...
\end{theorem}
\begin{proof}
    Let \(V(K,\varepsilon)=\cbraces{z\in\mathbb{C}\mid\mathrm{dist}\paren{z,K}<\varepsilon}\) where \(\mathrm{dist}\paren{z,K}\) is the distance between \(z\) and \(K\).

    Then \(\exists\varepsilon>0\) such that \(K\subset V(K,\varepsilon)\subset V(K,2\varepsilon)\subseteq V\). Let \(\Omega_1=V(K,2\varepsilon)\) and \(\Omega_2=\Omega\setminus V(K,\varepsilon)\). Then there exists a \(C^\infty\) partition of unity of \(\Omega\) subordinate to the open cover \(\cbraces{\Omega_1,\Omega_2}\).
\end{proof}
