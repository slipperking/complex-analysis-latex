...
\begin{definition}[Limit Superior]\label{def:limsup}
    ...
    \begin{enumerate}
        \item \(\forall\varepsilon>0\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(a_n<L+\varepsilon\),
        \item \(\forall\varepsilon>0\), \(\forall N\in\mathbb{N}\), \(\exists n>N\) such that \(a_n>L-\varepsilon\),
    \end{enumerate}
    then \(\limsup_{n\to\infty}a_n=a\).
\end{definition}
