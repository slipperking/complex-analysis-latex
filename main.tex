...
where \(\varphi_g\) and \(\psi_g\) are the holomorphic and principal parts of \(g\), respectively. It follows that \(f\) has the Laurent expansion at \(\infty\) of \(f(z)=\sum_{n=-\infty}^\infty c_nz^{-n}=\varphi_f(z)+\psi_f(z)\) with respect to \(\zeta\).
