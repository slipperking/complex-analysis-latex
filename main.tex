...
\begin{proof}
    ...
    Then by Morera's Theorem (\autoref{thm:morera}), \(f(z)\) is holomorphic, and \(f'(z)\) is holomorphic. We aim to show that \(f'_n(z)\to f'(z)\) uniformly.

    Let \(K\subset U\) be arbitrary and compact and \(V\supset K\) be open and relatively compact in \(U\). Since \(\cbraces{f'_n(z)}\) is holomorphic, we can define a holomorphic sequence \(\varphi_n(z)=f_n(z)-f(z)\). By \autoref{crl:nthderivativeboundedsupremum}, \[\lim_{n\to\infty}\sup_{z\in K}\abs{\varphi'_n(z)}\leq c_n\lim_{n\to\infty}\sup_{z\in V}\abs{f_n(z)-f(z)}.\]

    By the definition of uniform convergence, the right hand side approaches 0, and \(\cbraces{f'_n(z)}\) is then uniformly convergent to \(f'(z)\).
\end{proof}
