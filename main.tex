...
\begin{proof}
    ...
    \end{cases}.\]
    Let \(g(\zeta)=f\qty(\frac{1}{\zeta})=f(z)\), which has a removable singularity at \(\zeta=0\). By \autoref{thm:removablesingularitiesriemann}, \(g\) has an analytic continuation onto 0. Then \(\forall\varepsilon>0\), \(\exists\delta>0\) such that \(\forall\zeta\in D(0,\delta)\), \(\abs{g(\zeta)-g(0)}<\varepsilon\), and is bounded. In other words, \(\forall|z|>\frac{1}{\delta}\)
\end{proof}
