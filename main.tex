...
\begin{proof}
    By the Cauchy--Hadamard Theorem (\autoref{thm:cauchyhadamard}), the series has the convergence radius of \[\varlimsup_{n\to\infty}\sqrt[n]{
            \begin{cases}
                ...
                0&\text{otherwise}
    \end{cases}}=1.\]
    It follows that \(f\) is holomorphic on \(\mathbb{D}\). Let \(\qty|z_0|=\frac{1}{2}\) be arbitrary. Let \(\zeta=2z_0\). The function \(f(z)\) has a Taylor expansion at \(z=z_0\) of \[f(z)=\sum_{n=0}^\infty\frac{f^{(n)}\qty(z_0)}{n!}{\qty(z-z_0)}^n.\] Let \(\rho\) be the convergence radius of the series above. If we can show that \(\rho=\frac{1}{2}\), then it follows that by tangency, the intersection of \(\partial\mathbb{D}\) and \(\partial D\qty(z_0,\frac{1}{2})\), or \(\zeta\), is a singularity of \(f\).

    For the sake of contradiction, assume that \(\rho>\frac{1}{2}\). Hence, \(D\qty(\zeta,\rho-\frac{1}{2})\cap\mathbb{D}\neq\emptyset\). Obviously, the arc defined by \(D\qty(\zeta,\rho-\frac{1}{2})\cap\partial\mathbb{D}\) has positive length. 
\end{proof}
