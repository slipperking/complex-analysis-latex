...
\begin{theorem}[Maximum Modulus Principle]\label{thm:maximummodulus}
    Let \(f(z)\) a be holomorphic function or a real-valued harmonic function on an open connected region \(U\subseteq\mathbb{C}\). If \(\exists z_0\in U\) such that for some open neighborhood \(V\subseteq U\) of \(z_0\) and \(\forall z\in U\), \(\abs{f\paren{z_0}}\geq\abs{f(z)}\), then \(f\) is a constant function on \(U\).
\end{theorem}
\begin{proof}
...
TO BE CONTINUED
Finally, we will demonstrate that real-valued continuous functions satisfying the mean-value property is logically equivalent to being a solution to the Laplace equation. In \autoref{lemma:holomorphicmeanvalueproperty}, we showed that real solutions to the Laplace equation satisfy the mean-value property. Now we will prove the converse.
\begin{theorem}\label{thm:meanvaluepropertysolutionsareharmonic}
    Let \(f(z):U\to\mathbb{R}\) (where \(U\subseteq\mathbb{C}\) is connected) be continuous such that \(\forall z_0\in U\), \(\exists\varepsilon>0\) such that \(\overline{D(0,\varepsilon)}\subseteq U\). Then, \(\forall0<\varepsilon'<\varepsilon\), \[f(z_0)=\frac{1}{2\pi}\int_0^{2\pi}f\qty(z_0+\varepsilon'e^{it})\ddt.\]
\end{theorem}
