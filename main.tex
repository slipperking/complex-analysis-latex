...
\begin{proof}
    ...
    Since \(\abs{\varphi\qty(Re^{i\vartheta})-\varphi(\zeta)}<2M\), we want \(\int_{\abs{\vartheta-\tau}>\delta}\frac{R^2-\rho^2}{\pi R^2\qty(1-\cos(\delta))}\dd{\tau}\leq2\frac{R^2-\rho^2}{R^2\qty(1-\cos(\delta))}< \frac{\varepsilon}{2M}\), and \[R^2-\rho^2<\frac{\varepsilon}{4M}R^2\paren{1-\cos(\delta)}.\]
    ...
    We will now show that \(u(z)\) is unique. Assume that \(v\not\equiv u\) on \(\overline{D(0,R)}\) also solves the problem. Then \(u-v\) is harmonic and vanishes on \(\partial D(0,R)\). By the Poisson Integral Formula (equation~\eqref{eq:poissonintegralformula2}), \(\forall z\in D(0,R)\), \(u(z)-v(z)=\int_0^{2\pi}P(\zeta,z)\paren{u(\zeta)-v(\zeta)}\dd{\tau}=0\).
\end{proof}
