...
\end{definition}
The normality of \(H\) implies the existence of the quotient group. Let \(g_1,g_1'\in G\) form the same left coset (\(g_1H=g_1'H\)), and let \(g_2,g_2'\in G\) satisfy \(g_2H=g_2'H\). We must ensure that \begin{equation*}
...
There exists a positive integer sequence \(k_1,k_2,\ldots\) such that for every positive and finite \(R\), \(\sum_{n=1}^\infty{\qty|\frac{R}{a_n}|}^{k_n+1}\) converges. For example, let \(k_n=n-1\), and for sufficiently large \(n\), \(\frac{R}{\abs{a_n}}<1\) and the series is convergent. Consider the infinite product 
\begin{equation}
...
\begin{equation}
	f(z)=\qty(1-\frac{z}{a_1})\prod_{n=2}^\infty E_{k_n}\qty(\frac{z}{a_n})\label{eq:entirefunctionconstructedfromzeros}
...
\begin{theorem}[Weierstrass Factorization Theorem]\label{thm:weierstrassfactorizationtheorem}
	Suppose \(f(z)\) is an entire function. Let \(\qty{a_n}_{n\in\mathbb{N}}\) be a sequence of the nonzero zeros of \(f\) such that \(a_n\to\infty\) as \(n\to\infty\) and \(0<\qty|a_n|\leq\qty|a_{n+1}|\) (equality treated as multiplicities) for all \(n\). Let \(m\) be the multiplicity of \(f(z)\) at \(z=0\) (let \(m=0\) if there is no zero at \(0\)). Then there exists a sequence \(\qty{k_n}_{n\in\mathbb{N}}\) such that \(\forall R>0\), \(\sum_{n=1}^\infty{\qty|\frac{R}{a_n}|}^{k_n+1}\) converges. Then, we can write \[f(z)=z^me^{\varphi(z)}\prod_{n=1}^\infty E_{k_n}\qty(\frac{z}{a_n})\] on \(D(0,R)\), where \(E_k(z)\) is the \(k\)th Weierstrass elementary factor defined in equation~\eqref{eq:weierstrasselementaryfactor} and \(\varphi(z)\) is an entire function. By letting \(k_n=n-1\), we can write \[f(z)=z^me^{\varphi(z)}\prod_{n=1}^\infty\qty(1-\frac{z}{a_n})\exp\qty[\frac{z}{a_n}+\frac{1}{2}\qty(\frac{z}{a_n})^2+\cdots+\frac{1}{n-1}\qty(\frac{z}{a_n})^{n-1}].\]
\end{theorem}
\begin{proof}
	By the argument of equation~\eqref{eq:entirefunctionconstructedfromzeros}, construct \(\psi(z)\) to be entire and have zeros at \(a_n\) where \(n\in\mathbb{N}\). Thus, \(z^m\psi(z)\) and \(f(z)\) have the same zeros and corresponding multiplicities. 
\end{proof}
