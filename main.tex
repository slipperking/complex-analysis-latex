...
For the purpose of the following contents, a \textit{region} will denote a nonempty, open, connected subset of the complex plane.
...
\subsection{Biholomorphic Mappings}
...
\end{theorem}
\begin{proof}
    The nonemptiness of \(f(U)\) is trivial. Let \(w_1,w_2\in f(U)\) be arbitrary and distinct. Then \(\exists z_1,z_2\in U\) such that \(f\qty(z_1)=w_1\)  and \(f\qty(z_2)=w_2\).
\end{proof}
