...
\begin{definition}[Uniform Convergence]
    Over a set \(U\subseteq\mathbb{C}\), a function sequence \(\paren{f_n(z)}\) uniformly converges to a function \(f(z)\) if and only if \(\forall\varepsilon>0\),\(\exists N\in\mathbb{N}\) such that \(\forall n>N\)
\end{definition}
