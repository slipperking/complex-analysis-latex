    ...
    \item The existence of a transcendental meromorphic function \(f\) whose poles have an accumulation point at \(z=\infty\) is the concern of the following theorem:
        ...
        \begin{proof}
            ...
            \begin{enumerate}[label=(\alph*)]
                ...
                By \autoref{thm:abelradius}, This series is uniformly convergent on \({D\qty(0,\abs{\frac{a_n}{2}})}\). In other words, \(\exists \Lambda_n\in\mathbb{N}\) such that \(\forall k\geq \Lambda_n\), \[\abs{\psi_n^{(k)}(0)}\leq \varepsilon_n.\] Let \(p_n(z)=\sum_{k=0}^{\Lambda_n-1}\frac{\psi_n^{(k)}(0)}{k!}z^k\). Then, we have that
            \end{enumerate}
        \end{proof}
\end{enumerate}
