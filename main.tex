...
\end{theorem}
\begin{proof}
    Let \(\varphi:\mathbb{C}\to\mathbb{C}\) be a complex \(C^\infty\) function with compact support, identically equal to 1 over some open neighborhood of \(K\) relatively compact in \(V\) and such that \(\supp(\varphi)\subseteq V\). Since \(f\in C^1\paren{\overline{U}}\), by Pompeiu's Theorem (\autoref{thm:pompeiu}) on \(f(z)\varphi(z)\in C^\infty(\mathbb{C})\), \[f(z)\varphi(z)=\frac{1}{2\pi i}\paren{\int_{\partial U}\frac{f(\zeta)\varphi(\zeta)}{\zeta-z}\ddzeta-\int_{U}\pdv{f(\zeta)\varphi(\zeta)}{\overline{\zeta}}\cdot\frac{\dd{\overline{\zeta}}\wedge\ddzeta}{\zeta-z}}.\]
    By the product rule, \[\pdv{f(\zeta)\varphi(\zeta)}{\overline{\zeta}}=\pdv{\varphi}{\overline{\zeta}}f(\zeta),\] and since \(\partial U\subset\mathbb{C}\setminus\supp(\varphi)\), the first term vanishes, resulting in \[f(z)\varphi(z)=\frac{1}{2\pi i}\int_U\pdv{\varphi}{\overline{\zeta}}f(\zeta)\cdot\frac{\dd{\overline{\zeta}}\wedge\ddzeta}{\zeta-z}.\]
\end{proof}
