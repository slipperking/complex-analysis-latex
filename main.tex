...
\begin{definition}[Coset]\label{def:coset}
	Let \(G\) be a group, and let \(H\subseteq G\) be a subgroup. Then the \textit{left coset} of \(H\) in \(G\) with respect to \(g\in G\) is defined as \[gH=\cbraces{gh}{h\in H}.\] The \textit{right coset} is defined as \[Hg=\cbraces{hg}{h\in H}.\] The subgroup \(H\) is normal if and only if the left and right cosets are equal.
\end{definition}
...
\end{definition}
In order to ensure the existence of a quotient group, the subgroup must be normal. Assume otherwise. 
