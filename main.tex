...
Generally, if the function is even and integrated over \([0,\infty)\), then extend the integral to the entire real line. If the function seems to decay sufficiently fast in \(\mathbb{H}\), consider using a semicircular contour in the upper half-plane, as shown in \autoref{fig:semicircularcontour}. If there are singularities along the real axis, then it is customary to insert small semicircular indentations around them, as in \autoref{fig:indentedsemicircularcontour}. If the function can be written in the form of \(f\qty(xe^{i\theta})\), where \(x\in\mathbb{R}\), it is often useful to consider a wedge contour, as shown in \autoref{fig:wedgecontour}. If the function can be written in the form of \(f\qty(x+ia)\), where \(x\in\mathbb{R}\) and \(a>0\), it is often useful to consider a rectangular contour in the upper half-plane.
