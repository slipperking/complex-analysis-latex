...
\begin{proof}
    ...
    It follows that \(\varphi\in C^\infty(\mathbb{C})\) with compact support in \(\Omega_1\). Since the support of every \(\alpha_j\) where \(j\in\mathbb{N}\) lies either in \(\Omega_1\) or \(\Omega_2\), \[1=\sum_{\substack{j\in\mathbb{N}\\\supp{\alpha_j}\subseteq\Omega_2\\\supp{\alpha_j}\not\subseteq\Omega_1}}\alpha_j(z)+\varphi(z).\]
    When \(z\in\Omega_1\), the first summation vanishes, meaning that \(\varphi\equiv1\) on the neighborhood \(\Omega_1\) of \(K\).
\end{proof}
