...
\begin{definition}[Cover of a Set]
    A cover \(\mathcal{C}\) of a set \(X\) is a collection of sets \(\lbrace U_n\rbrace\) such that \[\bigcup_{n\in\mathbb{N}}U_n\supseteq X.\] A cover is \textit{open} if every set in the collection is open.
\end{definition}
...
\begin{theorem}[Partition of Unity]\label{thm:partitionofunity}
    For a nonempty open set \(\Omega\subseteq\mathbb{C}\), let \(\lbrace\Omega_i\rbrace_{i\in\mathbb{Z}_{\ge0}}\) be an open cover of \(\Omega\). Then there exists a set of bump functions known as a \(C^\infty\) partition of unity, \(\alpha_1(z),\alpha_2(z),\ldots\in C^\infty(\mathbb{C})\) with compact support within \(\Omega\) satisfying
    \begin{enumerate}
...
\end{theorem}
\begin{proof}
    By the openness of \(\Omega_i\), \(\forall z\in\Omega\), \(\exists r_z>0\) such that \(\overline{D\paren{z,r_z}}\subset\Omega_{k_z}\) for some \(k_z\in\mathbb{Z}_{\ge0}\). \(\forall z\in\Omega\), \(\forall 0<r<r_z\), \(\lbrace D\paren{z,r_z}\rbrace\) forms an open basis of \(\Omega\). Then by \autoref{lemma:locallyfiniteopencoverexistence}, there exists a locally finite open cover \(\lbrace D\paren{z_j,r_{z_j}}\rbrace\) of \(\Omega\), and since it is a subcollection of the basis, \[D\paren{z_j,r_{z_j}}\subset\overline{D\paren{z_j,r_{z_j}}}\subset\Omega_{k_{z_j}},\quad\forall j\in\mathbb{N}.\] The most trivial bump function has the form \[\theta(z)=\begin{cases}
        e^{\frac{1}{\abs{z}-1}}&\abs{z}<1\\
        0&\abs{z}\ge1
    \end{cases},\] which has the support \(\overline{D(0,1)}\) and define the function \(\theta_\varepsilon(z)=\varepsilon^{-2}\theta\paren{\frac{z}{\varepsilon}}\) with the support \(\overline{D(0,\varepsilon)}\), which satisfies that \(\forall\varepsilon>0\), \[\int_{\mathbb{C}}\theta_\varepsilon(z)\ddx\wedge\ddy=1.\]
    Define a set of functions \(\lbrace\beta_j\rbrace_{j\in\mathbb{N}}\) with \(\beta_j=\theta_{r_{z_j}}\paren{z-z_j}\), which is a bump function with the support \(\overline{D\paren{z_j, r_{z_j}}}\subset\Omega_{k_{z_j}}\). Equivalently, \(\lbrace\supp\paren{\beta_j}\rbrace_{j\in\mathbb{N}}\) is locally finite on \(\Omega\). Consequently, a point \(z\in\Omega\) lies only in a finite number of sets of \(\lbrace\supp\paren{\beta_j}\rbrace_{j\in\mathbb{N}}\). Then the sum \[S(z)=\sum_{j=1}^\infty\beta_j(z)\] is convergent, and the sequence defined by \[\lbrace\frac{\beta_j(z)}{S(z)}\rbrace\]
\end{proof}
...
Laurent Series are a generalization of Taylor Series specifically for holomorphic functions over a region punctured by a singularity. Whereas the Taylor Series applies over a disk, the Laurent Series applies over an annulus around the singularity.
