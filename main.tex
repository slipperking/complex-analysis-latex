...
\begin{proof}
    Define \[G(z)=\begin{cases}
        ...
        0&z=z_0
    \end{cases}.\]
    \(G(z)\) is continuously differentiable on \(D\paren{z_0,r}\) and satisfies the Cauchy--Riemann Equations since 
    \[\lim_{z\to z_0}\frac{G(z)-G\paren{z_0}}{z-z_0}=\frac{\paren{z-z_0}^2F(z)}{z-z_0}=\lim_{z\to z_0}\paren{z-z_0}F(z)=0,\] meaning that \(\dv{G}{z}\paren{z_0}=0\). For \(z\in\tilde{D}\paren{z_0,r}\), \(G'(z)=2\paren{z-z_0}F(z)+\paren{z-z_0}^2F'(z)\). As \(z\to z_0\), the above tends to 0, and \(G\) is holomorphic over \(D\paren{z_0,r}\).
    \end{proof}
\begin{theorem}
