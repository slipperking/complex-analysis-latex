...
\begin{definition}\label{def:analyticelement}
    An \textit{analytic element} is a pair \((D,f)\), where \(U\subseteq\mathbb{C}\) is an open set and \(f\) is a holomorphic function on \(U\).
\end{definition}

Analytic elements serve as local representations of analytic functions. The process of extending these elements is formalized through \textit{analytic continuation}, previously introduced in \autoref{def:analyticcontinuation}. By the Identity Theorem (\autoref{thm:identity}), such continuations are unique; if \((U_1, f_1)\) and \((U_2,f_2)\) are analytic elements with \(U_1\cap U_2 \neq \emptyset\), and \(f_1\equiv f_2\) on \(U_1\cap U_2\), then they are \textit{direct analytic continuations} of each other. The combined function:
\[F(z)=
    \begin{cases}
        f_1(z)&z\in U_1 \\
        f_2(z)&z\in U_2 \setminus U_1
\end{cases}\] is holomorphic on \(U_1\cup U_2\). 
\subsubsection{Continuation via Power Series}
The most straightforward method of the derivation of analytic continuations uses power series. Let \(f(z)=\sum_{n=0}^{\infty}c_n(z-z_0)^n\) have radius of convergence \(R > 0\) (by Theorem~\ref{thm:abelradius}). For \(z_1 \in D(z_0, R)\), expand \(f\) at \(z_1\):
\[f(z) = \sum_{k=0}^{\infty} \frac{f^{(k)}(z_1)}{k!} (z - z_1)^k.\]
Let \(\rho\) be the radius of convergence of this series. Then:
\[\rho \geq R - |z_1 - z_0|.\]
- If \(\rho > R - |z_1 - z_0|\), then \(f\) extends analytically to \(D(z_0, R) \cup D(z_1, \rho)\).
- If \(\rho = R - |z_1 - z_0|\), the disks \(D(z_0, R)\) and \(D(z_1, \rho)\) are tangent at a point \(\zeta_0\). Here, \(\zeta_0\) is a \textbf{singularity}, and \(f\) cannot be continued beyond \(\zeta_0\).

\subsubsection{Singularities on the Boundary}
\begin{theorem}\label{thm:boundarysingularity}
    Let \(f(z) = \sum_{n=0}^{\infty} c_n (z - z_0)^n\) have radius of convergence \(R > 0\). Then \(\partial D(z_0, R)\) contains at least one singularity of \(f\).
\end{theorem}

\begin{proof}
    Assume \(f\) can be analytically continued from every \(\zeta \in \partial D(z_0, R)\). Then for each \(\zeta\), there exists \(r_\zeta > 0\) and a holomorphic \(f_\zeta\) on \(D(\zeta, r_\zeta)\) agreeing with \(f\) on \(D(\zeta, r_\zeta) \cap D(z_0, R)\).

    The disks \(\{D(\zeta, r_\zeta)\}_{\zeta \in \partial D(z_0,R)}\) cover the compact set \(\partial D(z_0, R)\). By Heine-Borel (Theorem~\ref{thm:heineborel}), there is a finite subcover \(\{D(\zeta_k, r_k)\}_{k=1}^n\). Let \(V = \bigcup_{k=1}^n D(\zeta_k, r_k)\). Then:
    1. \(\partial D(z_0, R) \subset V\), so \(\exists \rho > 0\) such that \(A = \{z : R - \rho \leq |z - z_0| \leq R + \rho\} \subset V\).
    2. Define \(\Phi: V \to \mathbb{C}\) by \(\Phi(z) = f_{\zeta_k}(z)\) if \(z \in D(\zeta_k, r_k)\). This is well-defined: If \(z \in D(\zeta_i, r_i) \cap D(\zeta_j, r_j)\), then \(D(\zeta_i, r_i) \cap D(\zeta_j, r_j) \cap D(z_0, R) \neq \emptyset\), and \(f_{\zeta_i} = f_{\zeta_j} = f\) there. By the Identity Theorem, \(f_{\zeta_i} = f_{\zeta_j}\) on \(D(\zeta_i, r_i) \cap D(\zeta_j, r_j)\).
    3. \(\Phi\) is holomorphic on \(V\) and agrees with \(f\) on \(D(z_0, R) \cap V\).
    4. The function:
    \[
        F(z) =
        \begin{cases}
        ...
        \end{cases}
    \]
    is holomorphic on \(D(z_0, R) \cup V \supset D(z_0, R + \rho)\), contradicting the maximality of \(R\).
\end{proof}

\subsubsection{Chains and Maximal Continuations}
\begin{definition}\label{def:analyticchain}
    A \textbf{chain of analytic continuations} from \((D_0, f_0)\) is a sequence \(\{(D_k, f_k)\}_{k=0}^n\) where \((D_k, f_k)\) is a direct analytic continuation of \((D_{k-1}, f_{k-1})\) for \(1 \leq k \leq n\). Then \((D_n, f_n)\) is an \textbf{analytic continuation} of \((D_0, f_0)\).
\end{definition}

\begin{definition}\label{def:maximalanalytic}
    A \textbf{maximal analytic continuation} \((\widetilde{D}, \widetilde{f})\) is an analytic element that has no analytic continuation to a larger domain. The function \(\widetilde{f}\) is a \textbf{complete analytic function}.
\end{definition}

\subsubsection{Example: Natural Boundary}
\begin{example}\label{ex:lacunaryseries}
    The series \(f(z) = \sum_{n=0}^{\infty} z^{n!}\) has \(\mathbb{D} = D(0,1)\) as its disk of convergence, and every point on \(\partial\mathbb{D}\) is a singularity.
\end{example}

\begin{proof}
    By Cauchy-Hadamard (Theorem~\ref{thm:cauchyhadamard}), \(\limsup_{n \to \infty} |c_n|^{1/n} = 1\) since \(c_n = 1\) if \(n = k!\) and \(0\) otherwise. Thus, \(R = 1\).

    Fix \(\zeta \in \partial\mathbb{D}\). Suppose \(f\) extends analytically to a disk \(D(\zeta, \delta)\). Since roots of unity are dense in \(\partial\mathbb{D}\), there exists \(\zeta' = e^{2\pi i p/q} \in D(\zeta, \delta) \cap \partial\mathbb{D}\) for coprime integers \(p,q\). The extension \(g\) of \(f\) to \(D(\zeta, \delta)\) would satisfy:
    \[\lim_{r \to 1^{-}} f(r\zeta') = g(\zeta').\]
    However, for \(0 < r < 1\):
    \[f(r\zeta') = \underbrace{\sum_{k=0}^{q-1} (r\zeta')^{k!}}_{\text{bounded}} + \sum_{k=q}^{\infty} r^{k!} \underbrace{(\zeta')^{k!}}_{=1 \text{ since } q! \mid k! \text{ for } k \geq q}.\]
    As \(r \to 1^{-}\), \(\sum_{k=q}^{\infty} r^{k!} \to \infty\) (since terms \(\to 1\) and infinitely many), contradicting finiteness of \(g(\zeta')\). Hence, \(\zeta\) is a singularity.
\end{proof}
