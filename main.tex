...
\begin{theorem}\label{thm:holomorphycondition}
    The necessary and sufficient criterion for a function \(f(z)\) to be holomorphic on a region \(D\) is if \(f(z)\in C^1(D)\) and satisfy the Cauchy--Riemann equations.
\end{theorem}
\begin{proof}
    ...
    \[\frac{f\paren{z}-f\paren{z_0}}{z-z_0}=\alpha+i\beta+\frac{o_1\paren{\abs{\Delta z}}}{\abs{\Delta z}}+\frac{o_2\paren{\abs{\Delta z}}}{\abs{\Delta z}}.\]
...
\begin{theorem}[Looman--Menchoff Theorem]\label{thm:loomanmenchoff}
    A function \(f(z)\) is holomorphic on an open set \(U\subset\mathbb{C}\) if and only if it is continuous on \(U\), differentiable on \(U\setminus N\) for some set \(N\) with measure zero, and satisfies the \textit{Cauchy–Riemann equations}.
\end{theorem}
...
\begin{theorem}[Cauchy's Integral Formula]\label{thm:cauchyintegralformula}
    Let \(U\subset\mathbb{C}\) be an open region bounded with a piecewise smooth boundary \(\partial U \), and let \(f\in C^1(\overline{U})\) be holomorphic on \(U\). Then for all \(z\in U\),
    \begin{equation}
...
\begin{proof}
    ...
    \end{equation}
    For a fixed \(z\), the value of \[\frac{\psi(\xi+z+h)-\psi\paren{\xi+z}}{h}\] tends to \(\pdv{\psi(\xi+z)}{\xi}\) as \(h\to0\). Because \(\psi\paren{\xi+z}\) has compact support and is \(C^1\), by \autoref{thm:continuousfunctionboundedoncompact}, it is Lipschitz continuous for a constant \(M\). Then, \[\abs{\frac{\psi(\xi+z+h)-\psi\paren{\xi+z}}{h}}\leq M,\] and when \(\xi+z\notin\supp(\psi)\), \[\abs{\frac{\psi(\xi+z+h)-\psi\paren{\xi+z}}{h}}=0.\]
    Since the integrand is dominated by a function that has a convergent integral (\(\int_{\supp(\psi)}M\dd{\xi}\wedge\dd{\overline{\xi}}\)), equation \eqref{eq:onedimensionalpartialconjugatesolution_differenceexpr} uniformly converges and the limit \(h\to0\) may commute with the double integral. Let \(\zeta=\alpha+i\beta\). From the real axis, \begin{equation}
    ...
    \end{equation}
    Since \(\psi\in C^1\paren{\mathbb{C}}\) and has Lipschitz constant \(M\), equations \eqref{eq:onedimensionalpartialconjugatesolution_differenceexpr_realaxisderivative} and \eqref{eq:onedimensionalpartialconjugatesolution_differenceexpr_imaginaryaxisderivative} are both continuous (by the same argument for the continuity of \(u(z)\)). Hence, \(u\in C^1(\mathbb{C})\). It follows from the two equations that \[\pdv{u}{\overline{z}}=\frac{1}{2\pi i}\int_{\mathbb{C}}\pdv{\psi}{\overline{\zeta}}\cdot\frac{1}{\zeta-z}\ddzeta\wedge\dd{\overline{\zeta}}=\frac{1}{2\pi i}\int_{\supp(\psi)}\pdv{\psi}{\overline{\zeta}}\cdot\frac{1}{\zeta-z}\ddzeta\wedge\dd{\overline{\zeta}}.\] By the Cauchy--Pompeiu Theorem (\autoref{thm:pompeiu}), the integral above is equal to \[\pdv{u}{\overline{z}}=\psi(z)-\frac{1}{2\pi i}\oint_{\partial\supp(\psi)}\frac{\psi(\zeta)}{\zeta-z}\ddzeta.\]
    Because \(\forall\zeta\in\partial\supp(\psi)\), \(\psi(\zeta)=0\), we obtain \[\pdv{u}{\overline{z}}=\psi(z).\]
\end{proof}
\begin{remark}
    In the first part, we established that a function \(\psi(z)\in C^0(\mathbb{C})\) with compact support satisfies \[\int_{\mathbb{C}}\frac{\psi(\zeta)}{\zeta-z}\dd{\overline{\zeta}}\wedge\ddzeta=0.\] If the function is \(C^1\), then the first order derivatives of \(u(z)\) can be written in the same form (equations \eqref{eq:onedimensionalpartialconjugatesolution_differenceexpr_realaxisderivative} and \eqref{eq:onedimensionalpartialconjugatesolution_differenceexpr_imaginaryaxisderivative}) since \(\pdv{\psi}{\alpha},\pdv{\psi}{\beta}\in C^0(\mathbb{C})\), and the resulting integral is \(C^0\) as well. In general, for \(\psi(z)\in C^k(\mathbb{C})\), we can use the same process recursively to find that \(u(z)\in C^k(\mathbb{C})\) as well.
\end{remark}
\subsection{The Cauchy--Goursat Theorem}
When Cauchy famously published \autoref{thm:cauchyintegralformula} and \autoref{thm:cauchyintegraltheorem}, he included the condition for \(f(z)\in C^1\paren{\overline{U}}\). It was later proved that all holomorphic functions had holomorphic derivatives and this condition was dropped by Goursat using a broken line argument.
\end{document}