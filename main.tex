...
\begin{align*}
    ...
    & =\frac{x^0}{0!\ii^0}+\ii\frac{x^1}{1!\ii^1}-\frac{x^2}{2!\ii^2}-i\frac{x^3}{3!\ii^3}+\cdots \\
...
\begin{proof}
    ...
    \begin{gather*}
        \int_{\partial D(z,\varepsilon)}\frac{f(\zeta)\ddzeta}{\zeta-z}=\int_0^{2\pi}\frac{f\paren{z+\varepsilon \ee^{\ii t}}}{\varepsilon \ee^{\ii t}}\cdot i\varepsilon \ee^{\ii t}\dd{t}=\ii\int_0^{2\pi}f(z+\varepsilon \ee^{\ii t})\dd{t}\\
...
\begin{proof}
    ...
    \begin{gather*}
        ...
        {}+{i\paren{\int_{\psi(a)}^{y_\varphi}+\int_{y_\psi}^{\varphi(a)}}f(a+\ii y)-i\paren{\int_{\psi(a+\varepsilon)}^{y_\varphi}z+\int_{y_\psi}^{\varphi\paren{a+\varepsilon}}}f(a+\varepsilon+\ii y)}.
    \end{gather*}
...
\begin{proof}
    ...
    \[\varphi'(R)=Mn!\frac{\ee^RR^n-ne^RR^{n-1}}{R^{2n}}=0\Longleftrightarrow R^n=nR^{n-1}\Longleftrightarrow R=n.\] To confirm it as a minimum, we calculate the second order derivative:
...
\begin{proof}
    ...
    Observe that \[f(z)=\frac{1}{2\pi\varepsilon}\int_{\partial D(z,\varepsilon)}f(\zeta)\abs{\dd{\zeta}}=\frac{1}{2\pi\varepsilon}\int_{0}^{2\pi}f\paren{z+\varepsilon \ee^{\ii t}}\abs{i\varepsilon \ee^{\ii t}\dd{t}}=\frac{1}{2\pi}\int_{0}^{2\pi}f\paren{z+\varepsilon \ee^{\ii t}}\dd{t},\]
...
\begin{lemma}[Schwarz Lemma]\label{lemma:schwarz}
    ...
    Any one of the inequalities becomes equalities if and only if \(f(z)\) is in the form of \(ze^{\ii\tau}\), where \(\tau\in\mathbb{R}\). In other words, \(f\) is a pure rotation.
\end{lemma}
\begin{proof}
...
\begin{proof}
    ...
    Then, \(\abs{\psi'(0)}=1\), and by the equality statement of \autoref{lemma:schwarz}, \(\psi(z)=ze^{\ii\tau}=\rho_\tau(z)\) for some constant \(\tau\in\mathbb{R}\), and \(f(z)=\varphi_{f(0)}^{-1}\circ\rho_\tau(z)\). By equation~\eqref{eq:inversemobiustransformation}, it follows that \(f(z)=\varphi_{-f(0)}\circ\rho_\tau(z)\).
\end{proof}
...
\begin{gather}
    u(z)=\frac{1}{2\pi}\int_0^{2\pi} u(\zeta)\frac{1-|z|^2}{\abs{\zeta-z}^2}\dd{\tau} = \frac{1}{2\pi}\int_0^{2\pi}\frac{\qty(1-\rho^2)u\qty(\ee^{\ii\tau})\dd{\tau}}{\qty(\ee^{\ii\tau}-\rho \ee^{\ii\theta})\qty(\ee^{-i\tau}-\rho \ee^{-i\theta})} \nonumber \\
...
\begin{enumerate}
    ...
    \item\label{itm:isolatedsingularities_essential} If \(\lim_{z\to a}f(z)\) is nonexistent, then \(a\) is known as an \textit{essential singularity}.
        ...
        \end{example}
        \begin{proof}
            Observe that \(\lim_{\substack{z\to 0\\z\in\mathbb{R}_{>0}}}=\infty\). Similarly, \(\lim_{\substack{z\to0\\z\in\mathbb{R}_{<0}}}=0\), and \(\lim_{\substack{z\to0\\z\in i\mathbb{R}_{>0}}}\) is divergent. Therefore, the limit does not exist.
        \end{proof}
...
\begin{proof}
    ...
    \[\oint_{\gamma}\frac{f'(z)}{f(z)}\ddz=\sum_{j=1}^m2\pi i k_j+\sum_{j=1}^n\qty(-2\pi\ii k'_j)=2\pi\ii\paren{\sum_{j=1}^m k_j-\sum_{j=1}^n k'_j}=2\pi\ii\qty(k-k').\]
...
\begin{proof}
    ...
    \begin{align*}
        \frac{a\qty(\dfrac{ez+f}{gz+h})+b}{c\qty(\dfrac{ez+f}{gz+h})+d} & =\frac{aez+af+bgz+bh}{cez+cf+dgz+dh}                \\
...
\end{example}
\begin{proof}
    The zeros of \(\sin(\pi z)\) are simple at each of \(\mathbb{Z}\). Aside from the simple zero at \(z=0\), let \[a_n=\begin{cases}
...
\begin{example}[Blaschke Product]\label{ex:blaschkeproduct}
    Let \(\qty{a_k}_{k\in\mathbb{N}}\subset \mathbb{D}^*=\mathbb{D}\setminus\qty{0}\) be a sequence such that the series \(\sum_{k=1}^\infty \qty(1-\abs{a_k})\) is convergent (known as the \textit{Blaschke condition}). Then the \textit{Blaschke product}, defined by \[B(z)=\prod_{k=1}^\infty \qty[-\frac{\abs{a_k}}{a_k}\varphi_{a_k}(z)],\] where \(\varphi_a(z)\) is a Möbius transformation in the form of equation~\eqref{eq:mobiustransformationgroupofholomorphicautomorphismsunitdisk_statement}, is uniformly convergent on compact subsets of \(\mathbb{D}\) to an analytic function on \(\mathbb{D}\) such that \(\abs{B}\leq 1\) on \(\mathbb{D}\), and its only zeros are precisely at each of \(\qty{a_k}\).
\end{example}
\begin{proof}
    By the Blaschke condition, it is satisfied that \(\lim_{k\to\infty}1-\abs{a_k}\to 0\). In other words, \(\forall 0<R<1\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(1-\abs{a_k}<1-R\), or that \(\abs{a_k}>R\).
\end{proof}
...
\begin{proof}
    ...
    \end{align*}
    Let us evaluate the integral on \(\gamma_\varepsilon\) as \(\varepsilon\to 0\): \[\int_{C_\varepsilon}f(z)\ddz=i\int_{\pi}^0\exp(\varepsilon\qty(i\cos\theta-\sin\theta))\dd{\theta}=i\int_\pi^0\ee^{-\varepsilon\sin\theta}\ee^{\ii\varepsilon\cos\theta}\dd{\theta}.\]
    Obviously, \[\abs{\ee^{-\varepsilon\sin\theta}\ee^{\ii\varepsilon\cos\theta}}\leq 1,\]
    and therefore, the integral and the limit may commute:
    \[\lim_{\varepsilon\to 0}\int_{C_\varepsilon}f(z)\ddz=i\int_{\pi}^0\lim_{\varepsilon\to 0^+}\ee^{-\varepsilon\sin\theta}\ee^{\ii\varepsilon\cos\theta}\dd{\theta}=\ii\int_{\pi}^0\dd{\theta}=-\ii\pi.\]
...
\begin{theorem}
    Let \(\Omega\subseteq\mathbb{C}\) be a region, and let \(\gamma\subset\Omega\) be a rectifiable simple closed curve that is null-homotopic in \(\Omega\). Denote \(\mathrm{int}(\gamma)\) by \(U\). If \(f:\Omega\to\mathbb{C}\) is holomorphic and maps \(\gamma\) injectively to a simple closed curve \(\Gamma\), then \(w=f(z)\) is univalent in \(U\) and \(f(U)=\mathrm{int}(\Gamma)\).
\end{theorem}
\begin{proof}
...
\end{proof}
We will now give examples of conformal maps.
\begin{example}
...
\end{example}
\begin{proof}
    First assume \(z\in\mathbb{R}\). It follows that \[\abs{w}=\abs{\ee^{\ii\theta}\frac{z-a}{z-\overline{a}}}=1.\] Therefore, this transformation maps \(\mathbb{R}\) to \(\partial\mathbb{D}\). The inverse mapping is equal to \[z=\frac{w\overline{a}-a\ee^{\ii\theta}}{w-\ee^{\ii\theta}}.\]
    ...
    Hence, \(z\) maps \(\partial\mathbb{D}\) to \(\mathbb{R}\), and \(w\) maps \(\mathbb{R}\) to \(\partial\mathbb{D}\) univalently and surjectively. Since \(\mathbb{R}\cup\qty{\infty}\) is a closed curve, \(w\) is a biholomorphism from \(\mathbb{H}^+\) to \(\mathbb{D}\). 
\end{proof}
