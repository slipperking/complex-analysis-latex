...
\end{proof}
\begin{theorem}
    Let \(U\subseteq\mathbb{C}\) be open, let \(K\subseteq U\) be compact and \(V\supset K\) be open such that \(\overline{V}\subset U\) (\(V\) is a neighborhood of \(K\) that is relatively compact in \(U\)). Let \(f(z)\) be holomorphic over in \(U\). Then there exists a sequence \(\paren{c_n}\) such that \(\forall n\in\mathbb{N}\), \[\sup_{z\in K}\abs{f^{(n)}(z)}\leq c_n\left\|f\right\|_{L_1(V)},\]
    where \(\left\|f\right\|_{L_p(V)}\) denotes \[\frac{1}{\mu(V)}\int_{}\]
\end{theorem}
