...
\begin{example}[Poisson--Jensen Formula]\label{ex:poissonjensenformula}
    Let \(R>0\) be a positive and finite constant. Suppose \(f\) is a meromorphic function on the open disk \(D(0,R)\) such that \(f\) is not identically zero. Let \(k_1,\ldots,k_m\) and \(k'_1,\ldots,k'_n\) be the zeros and poles of \(f\), counted with multiplicity and order, respectively.
\end{example}
