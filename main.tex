...
\begin{definition}[Coset]\label{def:coset}
	Let \(G\) be a group, and let \(H\subseteq G\) be a subgroup. Then the \textit{left coset} of \(H\) in \(G\) with respect to \(g\in G\) is defined as \[gH=\cbraces{gh}{h\in H}.\] The \textit{right coset} is defined as \[Hg=\cbraces{hg}{h\in H}.\] The subgroup \(H\) is normal if and only if the left and right cosets are equal. Cosets, like groups and sets, are unordered.
\end{definition}
...
\end{definition}
In order to ensure the existence of a quotient group, the subgroup must be normal. Let \(g_1,g_1'\in G\) form the same left coset (\(g_1H=g_1'H\)). Then \(\exists h\in H\) such that \(g_1'=g_1h\). If \(H\) is normal, then \(h\in H\) implies \(g_1'h=g_1hg_1^{-1}g_1=g_1'\), and therefore the left coset is equal to the right coset (\(Hg_1'=g_1'H\)). Therefore, the quotient group is well-defined.
