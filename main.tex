...
\begin{proof}
    ...
    \(\forall n\in\mathbb{N}\) and \(\forall z\in V_n\), since \(W_n\) is an open set containing \(z\) and \(\mathfrak{B}\) is a basis, there exists \(U_{z,n}\in\mathfrak{B}\) such that \(z\in U_{z,n}\subseteq W_r\). The collection \(\cbraces{U_{z,n}}{z\in V_n}\) is an open cover of the compact set \(V_n\), so by the Heine-Borel theorem it admits a finite subcover. That is, there exist finitely many points \(z_{n,1},\ldots,z_{r,k_r}\) such that \(V_r\subset\bigcup_{i=1}^{k_r}U_{z_{r,i},r}\subseteq W_r\).
