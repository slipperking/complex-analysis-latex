...
\begin{proof}
    ...
    Let \(\varPsi\in C^\infty(\mathbb{C})\), and construct
    \begin{equation}
        f(z)=-\Phi(z)\varPsi(z)+\sum_{k=1}^\infty\phi_k(z)\varphi_k(z).\label{eq:generalinterpolationexistence_constructionstatement}
    \end{equation}
    Under what conditions on \(\varPsi\) will \(f\) be entire? Since the supports of each \(\varphi_k\) are disjoint, the summation \(\sum_{k=1}^\infty\phi_k(z)\varphi_k(z)\) contains at most one nonzero term and is convergent and well-defined. To construct \(f\) to be entire, we must have \(\pdv{f}{\overline{z}}=0\). In other words, we want for \[\pdv{\overline{z}}(\sum_{k=1}^\infty\phi_k\varphi_k)=\pdv{\overline{z}}[\Phi\varPsi]\Longleftrightarrow\sum_{k=1}^\infty\phi_k\pdv{\varphi_k}{\overline{z}}=\Phi\pdv{\varPsi}{\overline{z}}\] on all of \(\mathbb{C}\). Let \(g(z)=\sum_{k=1}^\infty\phi_k(z)\pdv{\varphi_k(z)}{\overline{z}}\). Since \(\varphi_k\equiv 1\) on \(\overline{D\qty(z_k,\varepsilon_k)}\), \(\pdv{\varphi_k}{\overline{z}}\equiv 0\) on \(\bigcup_{k=1}^\infty \overline{D\qty(z_k,\varepsilon_k)}\). Consequently, \(g(z)\equiv 0\) on \(\bigcup_{k=1}^\infty \overline{D\qty(z_k,\varepsilon_k)}\).

    From rearrangement, we have \(\frac{g(z)}{\Phi(z)}=\pdv{\varPsi}{\overline{z}}\), which has removable singularities at each \(z_k\). Define \(\frac{g(z)}{\Phi(z)}=0\) at \(z=z_k\). Under this assertion, we have \(\frac{g(z)}{\Phi(z)}\in C^\infty(\mathbb{C})\). Since the support of \(\frac{g(z)}{\Phi(z)}\) is the union of disjoint compact sets, by \autoref{thm:onedimensionalpartialconjugatesolution}, there exists a function \(\varPsi\in C^\infty(\mathbb{C})\) satisfying \(\frac{g(z)}{\Phi(z)}=\pdv{\varPsi}{\overline{z}}\). Since \(g\) is vanishing on \(\bigcup_{k=1}^\infty \overline{D\qty(z_k,\varepsilon_k)}\), it follows that \(\frac{g}{\Phi}\) is vanishing on \(\bigcup_{k=1}^\infty \overline{D\qty(z_k,\varepsilon_k)}\), and \(\varPsi\) is holomorphic on \(\bigcup_{k=1}^\infty{D\qty(z_k,\varepsilon_k)}\).

    Fix \(k\in\mathbb{N}\) and let \(n\in\qty{0,\ldots,n_k-1}\). For \(z\in D\qty(z_k,\varepsilon_k)\), from equation~\eqref{eq:generalinterpolationexistence_constructionstatement}, we have \[f(z)=-\Phi(z)\varPsi(z)+\phi_k(z).\] Since \(\Phi\) has a zero at \(z_k\) with multiplicity \(n_k\), we have \(\Phi(z)\varPsi(z)\) vanishing at \(z_k\) with a multiplicity of at least \(n_k\). Therefore, we have that
    \begin{align*}
...
\end{theorem}
\begin{proof}
    Let \(R>\max_{j\in\mathbb{N}_{\leq n}}\qty|z_n|\) be arbitrary. Then there exist circles centered at each isolated singularity contained in \(D(0,R)\), or that \(\forall k\in\mathbb{N}_{\leq n}\), \(\exists\delta_k>0\) such that \(\partial D\qty(z_k,\delta_k)\subset D(0,R)\). By the Cauchy--Goursat Integral Theorem,
    \begin{align*}
        \int_{\bigcup_{k=1}^nD\qty(z_k,\delta_k)^-\cup\partial D(0,R)^+}f(z)\ddz=0 & \Longleftrightarrow\qty(\sum_{k=1}^n\oint_{\partial D\qty(z_k,\delta_k)}-\oint_{\partial D(0,R)})f(z)\ddz=0 \\
        & \Longleftrightarrow\sum_{k=1}^n\residue_{z=z_k}f(z)+\residue_{z=\infty}f(z)=0,
    \end{align*}
...
\begin{proof}
    ...
    By equation~\eqref{eq:residueatpole}, it follows that \begin{align*}
...
\end{proof}
\begin{example}
    Prove that the Fourier transform of \(\sech(\pi x)\) is itself, or that \[I(\xi)=\int_{-\infty}^\infty e^{-2\pi ix\xi}\sech(\pi x)\ddx=\sech(\pi\xi).\]
\end{example}
\begin{proof}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[>=stealth,
            ...
            \node[anchor=north] at (3,0) {\(R\)};
            \node[anchor=north] at (-3,0) {\(-R\)};
            \node[anchor=south east] at (0,2) {\(2i\)};
        \end{tikzpicture}
        \caption{A rectangular contour with orientation marked.}\label{fig:rectangularcontour}
    \end{figure}Since \(\cosh(\pi z)=\cosh(\pi(z+2i))\) and \(\exp(-2\pi iz\xi)=\exp(-4\pi\xi)\exp(-2\pi i(z+2i)\xi)\), we have that \(f(z)\) is a constant multiple of \(f(z+2i)\). Therefore, we can use a rectangular contour as shown in \autoref{fig:rectangularcontour}. Let the sides be denoted by 
    \begin{gather*}
        \overleftarrow{\Gamma}=\cbraces{x+2i}{-R\leq x\leq R,x\in\mathbb{R}},\qquad\overrightarrow{\Gamma}x=\cbraces{x\in\mathbb{R}}{-R\leq x\leq R}\\
        \overset{\downarrow}{\Gamma}=\cbraces{-R+iy}{y\in[0,2]},\qquad\overset{\uparrow}{\Gamma}=\cbraces{R+iy}{y\in[0,2]}.
    \end{gather*}
    The enclosed singularities are simple poles at \(z=\frac{i}{2},\frac{3i}{2}\) (they are simple by evaluation of the Taylor expansions of the denominator). By the Residue Theorem (\autoref{thm:residuethm}), we get that \[\qty(\int_{\overrightarrow{\Gamma}}+\int_{\overset{\uparrow}{\Gamma}}+\int_{\overleftarrow{\Gamma}}+\int_{\overset{\downarrow}{\Gamma}})f(z)\ddz=2\pi i\qty[\Res(f,\frac{i}{2})+\Res(f,\frac{3i}{2})].\]
    By equation~\eqref{eq:residueatpole}, we have \[\Res[f,\frac{i}{2}]=\lim_{z\to\frac{i}{2}}\qty(z-\frac{i}{2})\frac{\exp(-2\pi iz\xi)}{\cosh(\pi z)},\qquad\Res[f,\frac{3i}{2}]=\lim_{z\to\frac{3i}{2}}\qty(z-\frac{3i}{2})\frac{\exp(-2\pi iz\xi)}{\cosh(\pi z)}.\]
    ...
    \end{align*} Similarly, \begin{align*}
        \Res[f,\frac{3i}{2}]&=\lim_{z\to\frac{3i}{2}}\qty(z-\frac{3i}{2})\frac{\exp(-2\pi iz\xi)}{\cosh(\pi z)}\\
        &=\lim_{z\to\frac{3i}{2}}\frac{\exp{-2\pi iz\xi}}{\pi\sinh(\pi z)+2\pi i\xi\cosh(\pi z)}\\
        &=-\frac{\exp(3\pi\xi)}{\pi i}.
    \end{align*}
...
\begin{proof}
    ...
    \[\int_{\gamma_\varepsilon} f(z)\ddz = \int_\alpha^\beta f\qty(a+\varepsilon e^{i\theta}) \frac{dz}{d\theta}\dd{\theta} = \varepsilon i \int_\alpha^\beta f\qty(a+\varepsilon e^{i\theta}) e^{i\theta}\dd{\theta}.\]
    ...
    \begin{align*}
        ...
        &= ic_{-1} \int_\alpha^\beta d\theta + \varepsilon i\int_\alpha^\beta\varphi\qty(a + \varepsilon e^{i\theta})e^{i\theta} \dd{\theta}\\
...
which contains \(\partial D\qty(z_0, R)\). Since \(\partial D\qty(z_0,R)\) is compact and \(V\) is open, there exists \(\rho> 0\) such that the annular region \(D\qty(z_0,R+\rho)\setminus\overline{D\qty(z_0,R-\rho)}\subset V\); equivalently, the distance from \(\partial D(z_0,R)\) to the boundary \(\partial V\) is positive.
...
\end{example}
\begin{proof}
    By the Cauchy--Hadamard Theorem (\autoref{thm:cauchyhadamard}), the series has the convergence radius of \[\varlimsup_{n\to\infty}\sqrt[n]{\begin{cases}
    1&n\in\cbraces{k!}{k\in\mathbb{N}}\\
    0&\text{otherwise}
    \end{cases}}=1.\]
It follows that \(f\) is holomorphic on \(\mathbb{D}\). Let \(\qty|z_0|=\frac{1}{2}\) be arbitrary. It follows that \(f(z)\) has a Taylor expansion at \(z=z_0\) of \[f(z)=\sum_{n=0}^\infty\frac{f^{(n)}\qty(z_0)}{n!}{\qty(z-z_0)}^n.\]
\end{proof}
