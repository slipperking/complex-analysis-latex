...
\end{example}
\begin{proof}
    It is evident that \(f(z)=\csc(\pi z)\) has double poles at each of \(z=n\in\mathbb{Z}\). The principal part of the Laurent expansion at each \(n\) is 
    \begin{align*}
        \psi_n(\zeta)&=\lim_{z\to n}\frac{\dv{z}[\qty(z-n)^2 f(z)]}{\zeta-n}+\lim_{z\to n}\frac{\qty(z-n)^2 f(z)}{\qty(\zeta-n)^2}\\
        &=\lim_{z\to n}\frac{2\frac{z-n}{\sin(\pi z)}+\qty(z-n)^2 f'(z)}{\zeta-n}+\lim_{z\to n}\frac{\frac{2\qty(z-n)}n{\pi\cos(\pi z)}}{\qty(\zeta-n)^2}
    \end{align*}
\end{proof}
