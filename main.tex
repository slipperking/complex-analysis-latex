...
\end{enumerate}
\(\sin\) and \(\cos\), through their exponential form, still satisfy the properties such as their derivatives, periodicity being \(2\pi\), parity, sum and difference, and the fundamental identities \(\sin^2(z)+\cos^2(z)=1\), \(\sin(z)=\cos\paren{\frac{\pi}{2}-z}\). However, due to the extension, some properties do not hold. For instance, \(\sin(z)\) and \(\cos(z)\) are unbounded, as along the imaginary axis, they resemble their hyperbolic counterparts, which are unbounded along the real line.

We now examine the regions over which \(\sin\) and \(\cos\) are univalent. We will begin with \(\cos(z)=\frac{e^{iz}+e^{-iz}}{2}\). Let \[\xi(z)=iz,\quad \zeta(\xi)=e^\xi,\quad w(\zeta)=\frac{\zeta+\flatfrac{1}{\zeta}}{2}.\]
Then \(\cos(z)=w\circ\zeta\circ\xi(z)\). \(\xi\) is univalent over \(\mathbb{C}\) as it is a rotation. \(\zeta\) is univalent over any region \(U\) such that \(\forall\xi_1,\xi_2\in U\) such that \(\xi_1\neq \xi_2\), \(\xi_1-\xi_2\neq2i\pi k\), where \(k\in\mathbb{Z}\). Let \(z_1,z_2\) be the preimages of \(\xi_1,\xi_2\). Then, \(z_1-z_2\neq 2\pi k\). \(w\) is univalent on a region satisfying \(\zeta_1\neq\frac{1}{\zeta_2}\), where \(\zeta_1\) and \(\zeta_2\) are the images of \(\xi_1\) and \(\xi_2\), respectively. In terms of \(z\), it is equivalent to \(e^{iz_1}e^{iz_2}\neq 1\). Then, \(z_1+z_2\neq 2\pi k\). Then \(\cos\) is univalent over any band region with thickness \(\pi\) such as \(k\pi<\real(z)<(k+1)\pi\). Let us consider the region \(\cbraces{z}{0<\real(z)<\pi}\)
...
\end{theorem}
\begin{proof}
    Let \(\cbraces{z_n}_{n\in\mathbb{N}}\) be a subset of \(\cbraces{z}{z\in U,f(z)=0}\) and assume it has an accumulation point \(z^*\) in \(U\). Since \(f\) is holomorphic over \(U\), \(\exists\varepsilon>0\) such that \(f\) is holomorphic over \(D\paren{z^*,\varepsilon}\subseteq V\). Then over this disk, \(f\) has the Taylor Series representation
    \begin{equation}\label{eq:identityaccumulationofzeroes_taylorexpansion}
        f(z)=\sum_{n=0}^\infty a_n\paren{z-z^*}^n.
    \end{equation}
    By \autoref{def:accumulationpoint}, \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(z_n\in D\paren{z^*,\varepsilon}\). Since \(z_n\) is a root of \(f\), \(f\paren{z_n}=0\). Then, by the continuity of \(f\), \[0=\lim_{n\to\infty} f\paren{z_n}=f\paren{\lim_{n\to\infty}z_n}=f\paren{z^*}.\] Using this result in comparison to equation \eqref{eq:identityaccumulationofzeroes_taylorexpansion}, we get that \(a_0=0\).

    The function \(f_1(z)=\frac{f(z)}{z-z^*}\) has a Taylor expansion over \(D\paren{z_0,\varepsilon}\) of
    \[f_1(z)=\sum_{n=0}^\infty a_{n+1}\paren{z-z^*}^n.\]
    Let \(z=z_n\neq z^*\) for some \(n>N\). Then \(f_1\) vanishes, leaving \[0=a_1+O_{n\to\infty}\paren{z_n-z^*}.\]
    Letting \(n\to\infty\), \(z_n\to z^*\), and \(a_1=0\). Define \(f_2(z)=\frac{f_1(z)}{z-z^*}\). Then, \[f_2(z)=\sum_{n=0}^\infty a_{n+2}\paren{z-z^*}^n.\]
    Similarly, \(a_2=0\). Letting \(f_n(z)=\frac{f(z)}{\paren{z-z^*}^n}\), the sequence \(\paren{a_n}_{n\in\mathbb{Z}_{\geq0}}\) vanishes, and \(f\equiv 0\) on \(D\paren{z^*,\varepsilon}\).
    
    By definition, \(D\paren{z^*,\varepsilon}\subseteq E\). For any arbitrary point in this disk, the Taylor series will also have vanishing coefficients. Then there exists another disk that \(f\) vanishes over, which is also a subset of \(E\). Then we can repeat this process until we obtain a collection of disks that cover \(U\). Then \(E=U\).
\end{proof}
\begin{remark}
...
\end{theorem}
\begin{proof}
    Let \(z_1,z_2,\ldots,z_n\) be the zeros enclosed within \(\gamma\), with multiplicities \(k_1,k_2,\ldots,k_n\) respectively. There exist disks centered around each zero with radii \(\varepsilon_1,\ldots,\varepsilon_n\) respectively, such that each disk is disjoint and contained in the interior of \(\gamma\), and \(f(z)\neq 0\) on the boundary of each disk. Since \(f\) is holomorphic and nonvanishing on and near \(\gamma\), and the zeros of \(f\) in the interior are isolated, we can write \(f(z)\) locally near each \(z_j\) as
    \[
    f(z) = (z - z_j)^{k_j}g_j(z),
    \]
    where \(g_j(z)\) is holomorphic and nonzero in a neighborhood of \(z_j\).

    Then we compute
    \[
    \frac{f'(z)}{f(z)} = \frac{d}{dz} \log f(z) = \sum_{j=1}^n \left( \frac{k_j}{z - z_j} \right) + h(z),
    \]
    where \(h(z)\) is holomorphic in a neighborhood of the region enclosed by \(\gamma\) (since the logarithmic derivative of \(g_j(z)\) is holomorphic). Now integrate \(\frac{f'(z)}{f(z)}\) along \(\gamma\):
    \[
    \frac{1}{2\pi i} \int_\gamma \frac{f'(z)}{f(z)} \, dz = \sum_{j=1}^n \frac{k_j}{2\pi i} \int_\gamma \frac{1}{z - z_j} \, dz + \frac{1}{2\pi i} \int_\gamma h(z) \, dz.
    \]
    By Cauchy's integral theorem, the second integral vanishes because \(h(z)\) is holomorphic on and inside \(\gamma\). The first term contributes \(k_j\) for each \(z_j\) inside \(\gamma\), because
    \[
    \frac{1}{2\pi i} \int_\gamma \frac{1}{z - z_j} \, dz = 1.
    \]
    Thus,
    \[
    \frac{1}{2\pi i} \int_\gamma \frac{f'(z)}{f(z)} \, dz = \sum_{j=1}^n k_j = k,
    \]
    the total number of zeros of \(f\) inside \(\gamma\), counted with multiplicity.

    For the second equality, recall that for any closed curve \(\gamma\) such that \(f(z) \ne 0\) on \(\gamma\), the total change in argument of \(f(z)\) as \(z\) traverses \(\gamma\) is given by
    \[
    \Delta_\gamma \arg f(z) = \operatorname{Im} \int_\gamma \frac{f'(z)}{f(z)} \, dz.
    \]
    Hence,
    \[
    \frac{1}{2\pi} \Delta_\gamma \arg f(z) = \frac{1}{2\pi} \operatorname{Im} \int_\gamma \frac{f'(z)}{f(z)} \, dz = \frac{1}{2\pi i} \int_\gamma \frac{f'(z)}{f(z)} \, dz = k.
    \]
