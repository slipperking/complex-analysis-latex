...
\begin{theorem}
    Let \(\prod_{n=1}^\infty\qty(1+f_n(z))\) be an infinite product. If \(\forall n\in\mathbb{N}\), \(\exists M_n\in\mathbb{R}_{\geq0}\) such that \(\abs{f_n(z)}\leq M_n\) for all \(z\in U\subseteq\mathbb{C}\) and \(\sum_{n=1}^\infty M_n\) converges, then the infinite product converges uniformly on \(U\).
\end{theorem}
\begin{proof}
    By assumption, \(\sum_{n=1}^\infty M_n\) converges. Therefore, \(\forall\varepsilon>0\), \(\exists N\in\mathbb{N}\) such that \(\forall m,n>N\), \(\sum_{k=m+1}^n M_k<\varepsilon\), and hence, \(\sum_{k=m+1}^n \abs{f_k(z)}<\varepsilon\). 
\end{proof}
