...
\begin{enumerate}
    \item If \(\lim_{z\to a}f(z)\) exists and is finite, then \(z=a\) is a removable singularity and can be analytically continued to \(D(a,r)\) by \autoref{thm:removablesingularitiesriemann}. Consequently, \(f(z)\) has a convergent Taylor expansion and the principle part of its Laurent expansion vanishes, and \(f(z)=\varphi(z)\).
    ...
    \end{proof}
    \item The necessary and sufficient for \(\lim_{z\to a} f(z)\) to not exist is that infinitely many of \(c_{-n}\) (where \(n\in\mathbb{N}\)) are nonzero. In this case, the point \(z=a\) is known as an \textit{essential singularity}.
...
\end{theorem}
In TO BE CONTINUED, we will give a generalization of this result.
