...
\newcommand{\riemannsphere}{\overline{\mathbb{C}}}
...
Let \(S^2=\left\{\paren{x_1,x_2,x_3}\in\mathbb{R}\,\middle|\,x_1^2+x_2^2+x_3^2=1\right\}\). There exists a \textit{stereographic projection} of \(S^2\) onto \(\riemannsphere\). For every point other than \((0,0,1)\), there is a corresponding complex number
\begin{equation}\label{eq:riemannsphereformula1}
...
\end{gather}
If \(u\qty(z)\) is continuous on \(\partial\mathbb{D}\), and therefore \(u\circ\exp(i\tau)\) is periodic over \(2\pi\). Thus, it can be expressed as a Fourier series
\begin{equation}
    u\qty(e^{i\tau})\sim\sum_{n=-\infty}^\infty a_ne^{in\tau},\qquad a_n=\frac{1}{2\pi}\int_{-\pi}^\pi u\qty(e^{i\tau})e^{-in\tau}\dd{\tau}.\label{eq:poissonintegralformulafourierseries}
\end{equation}
This series is not necessarily convergent (hence the usage of ``\(\sim\)''). Notice how \(u\) is continuous on a compact set and therefore is uniformly bounded by some \(M\) (\autoref{thm:continuousfunctionboundedoncompact}). Therefore, the integrand of \(a_n\) is bounded by a finite \(M\geq\abs{u\qty(e^{i\tau})e^{-in\tau}}\). It follows that \(\qty|a_n|\leq M\). By introducing factors of powers of  \(\abs{\rho}<1\), we obtain the sum converges: \[\sum_{n=-\infty}^\infty a_ne^{in\tau}\rho^{|n|},\qquad\abs{\sum_{n=-\infty}^\infty a_ne^{in\tau}\rho^{|n|}}\leq\sum_{n=-\infty}^\infty\abs{a_ne^{in\tau}\rho^{|n|}}\leq M\frac{1+|\rho|}{1-\abs{\rho}}.\]
...
\begin{theorem}\label{thm:meanvaluepropertysolutionsareharmonic}
    Let \(U\subseteq\mathbb{C}\) be open. Let \(f:U\to\mathbb{R}\) be continuous. \(\forall z_0\in U\), \(\exists\lambda>0\) such that \(\overline{D\qty(z_0,\lambda)}\subseteq U\), and if \(\forall0<\varepsilon\le\lambda\), \[f(z_0)=\frac{1}{2\pi}\int_0^{2\pi}f\qty(z_0+\varepsilon e^{it})\ddt,\]
    then \(f\) is harmonic on \(U\).
\end{theorem}
\begin{proof}
    Fix an arbitrary point \(z_0\in U\), along with a corresponding \(\lambda>0\) such that the closed disk \(\overline{D(z_0,\lambda)}\subseteq U\). Since \(f\in C^0\qty(\partial D(z_0,\lambda))\), by \autoref{thm:dirichletproblemwithlaplaceequationsolution}, there exists a unique function \(u(z)\) that is harmonic on \(D(z_0,\lambda)\), given by \[u(z)=\int_0^{2\pi}f(\zeta)P\qty(\zeta,z)\dd{\tau},\] which agrees with \(f\) on \(\partial D(z_0,\lambda)\).

    Now define \(\psi=f-u\) on \(\overline{D(z_0,\lambda)}\). Then \(\psi\) is continuous, satisfies the mean value property, and vanishes on the boundary. By an extension of the Maximum Principle (see the proof of the Maximum Modulus Principle, \autoref{thm:maximummodulus}, which only uses the mean-value property), any such function must be identically zero throughout the disk. Thus, \(f\equiv u\) on \(\overline{D(z_0,\lambda)}\), and in particular, \(f\) is harmonic at \(z_0\).
...
where \(\varphi\) and \(\psi\) are the holomorphic and principal parts of \(g\), respectively. Let \(\tilde{\varphi}(z)=\varphi\qty(\frac{1}{z})\), \(\tilde{\psi}(z)=\psi\qty(\frac{1}{z})\). At \(z=0\), \(f\) then has the Laurent expansion of \[f(z)=\sum_{n=-\infty}^\infty c_nz^n=\sum_{n=0}^\infty c_{-n}z^{-n}+\sum_{n=1}^\infty c_nz^n=\tilde{\varphi}(z)+\tilde{\psi}(z).\]
...
\begin{enumerate}
    ...
    \item If \(z=\infty\) is an essential singularity of \(f(z)\), then \(f(z)\) can be expanded as \[f(z)=\sum_{n=-\infty}^\infty c_{n}z^n,\]
        where \(\forall N\in\mathbb{N}\), \(\exists n>N\) such that \(c_{n}\neq0\) (infinitely many coefficients of \(\psi\) or \(\tilde{\psi}\) are nonzero).
\end{enumerate}
\begin{remark}
    Under stereographic projection from the point \(\mqty(0\\0\\1)\) of the unit sphere \(S_2\), a neighborhood of that point maps to a subset of the extended complex plane of the form \(\riemannsphere\setminus K\), where \(K\) is a compact and connected subset of \(\mathbb{C}\). Such sets are referred to as \textit{neighborhoods of \(\infty\)} in the Riemann sphere.
\end{remark}
\begin{example}
...
In complex analysis, three main sets of interest are \(\mathbb{D}\), \(\mathbb{C}\), and \(\riemannsphere\). We will now find \(\Aut(\mathbb{C})\).
...
\end{proof}
\subsubsection{The Group of Holomorphic Automorphisms on \texorpdfstring{\(\riemannsphere\)}{the riemannsphere}}
To make more profound conclusions on the structure of \(\Aut\qty(\riemannsphere)\), we will introduce certain concepts from group theory.
...
Let us now examine \(\Aut\qty(\riemannsphere)\). Let \(f(z)\in \Aut\qty(\riemannsphere)\) such that \(f(\infty)=\infty\). It follows that \(f\) maps \(\mathbb{C}\) to \(\mathbb{C}\) bijectively and \(f\in\Aut(\mathbb{C})<\Aut\qty(\riemannsphere)\). Therefore, \(f(z)\) has the form of \(az+b\), where \(a\in\mathbb{C}^*=\mathbb{C}\setminus\qty{0}\) and \(b\in\mathbb{C}\) are constants.

Let \(f(z)\in\Aut\qty(\riemannsphere)\) such that \(f(\infty)\neq\infty\). Then, \(g(z)=\frac{1}{f(z)-f(\infty)}\in\Aut\qty(\riemannsphere)\) and \(g(\infty)=\infty\). By the property above, \(g(z)=cz+d\) for some complex \(d\) and nonzero \(c\). Hence, \(f(z)=\frac{f(\infty)(cz+d)+1}{cz+d}\). Let \(a=cf(\infty)\), \(b=df(\infty)+1\), \(f(z)=\frac{az+b}{cz+d}\). Then in this specific construction, \(ad-bc=-c\neq0\). Let the matrix \(\mqty(a&b\\c&d)\) correspond to this transformation, where for any nonzero scalar \(k\), \(k\mqty(a&b\\c&d)\) corresponds to \(\mqty(a&b\\c&d)\). Therefore, we can arbitrarily pick \(ad-bc\) to be \(1\).

Therefore, there exists a one-to-one correspondence between \(\Aut\qty(\riemannsphere)\) and the group (under matrix multiplication) of \[\left.\cbraces{\mqty(a&b\\c&d)}{\det\mqty(a&b\\c&d)=1}\middle/\qty{\pm I},\right.\]
where \(I=\mqty(\imat{2})\). The quotient group (a group of elements in the form \(\qty{A,-A}\)) is taken because \(\mqty(a&b\\c&d)\) refers to the same transformation as \(\mqty(-a&-b\\-c&-d)\). This group, denoted by \(\mathrm{PSL}_2(\mathbb{C})=\mathrm{SL}_2(\mathbb{C})/\qty{\pm I}\cong\Aut\qty(\riemannsphere)\), is known as the \textit{projective special linear group} of order 2, and is \textit{isomorphic} (one-to-one) to the \textit{group of Möbius transformations}, consisting of all fractional linear transformations.

Therefore, any holomorphic automorphism on \(\riemannsphere\) is a composition of rotations, dilations, translations, and inversions. We will now state this formally:
\begin{theorem}[The Holomorphic Automorphism Group on \(\riemannsphere\)]\label{thm:holomorphicautomorphismgrouponextendedcomplexplane}
    \(\forall f\in \Aut\qty(\riemannsphere)\), \(f\) is a Möbius transformation. In other words, \(\exists a,b,c,d\in\mathbb{C}\) satisfying \(ad-bc\neq 0\) such that \[f(z)=\frac{az+b}{cz+d}.\]
\end{theorem}
The Möbius transformation subgroup of \(\Aut(\mathbb{D})\) is also a subgroup of \(\Aut\qty(\riemannsphere)\).

We have now introduced three of the most important regions in complex analysis: \(\mathbb{D}\), \(\mathbb{C}\), and \(\riemannsphere\). Their importance will be briefly explained by the Uniformization Theorem (\autoref{thm:uniformization}).
...
For \(x\in\mathbb{R}_{\geq0}\), since \(e^x\geq x\) and \(e^0=1\), we can integrate over \([0,x]\) to get that \(e^x\geq x+1\). Therefore, \[\exp(\sum_{k=1}^n\abs{u_k})\geq\prod_{k=1}^n \qty(1+\abs{u_k})=1+\sum_{k=1}^n\abs{u_k}+\sum_{\substack{j,k\in\qty{1,\ldots,n}\\j<k}}\abs{u_ju_k}+\cdots+\prod_{k=1}^n\abs{u_k}>\sum_{k=1}^n\abs{u_k}.\]
...
\begin{theorem}[Global Residue Theorem]\label{thm:globalresiduethm}
    If \(\qty{z_1,\ldots z_n,\infty}\) is discrete and finite, and \(f:\riemannsphere\setminus\qty{z_1,\ldots z_n,\infty}\to\mathbb{C}\) is holomorphic, and these points are the isolated singularities of \(f\), then the sum of the residues at each of these isolated singularities is zero, or \[\sum_{k=1}^n\residue_{z=z_k} f(z)+\residue_{z=\infty}f(z)=0.\]
\end{theorem}
\begin{proof}
...
\begin{proof}
    ...
    \begin{equation}
        \qty(\int_{\overrightarrow{\Gamma}}+\int_{\overset{\uparrow}{\Gamma}}+\int_{\overleftarrow{\Gamma}}+\int_{\overset{\downarrow}{\Gamma}})f(z)\ddz=2\pi i\qty[\Res(f,\frac{i}{2})].\label{eq:fouriertransformofsechpix_rectangularcontourintegral}
    \end{equation}
...
The concept of analytic continuation was previously introduced (\autoref{def:analyticcontinuation}). By the Identity Theorem of holomorphic functions (\autoref{thm:identity}), we asserted the uniqueness of analytic continuations. If \(f_1\) and \(f_2\) are analytic functions on open sets \(U_1\) and \(U_2\) respectively, satisfying \(U_3=U_1\cup U_2\neq\emptyset\), and if \(f_1\equiv f_2\) on \(U_3\), then \(f_1\) and \(f_2\) are analytic continuations of each other. The function defined by \[f(z)=
    \begin{cases}
        f_1(z)& z\in U_1\\
        f_2(z)& z\in U_2\setminus U_1
\end{cases}\] is also holomorphic.
Perhaps the most straightforward way to obtain analytic continuations is through the observation of power series. By \autoref{thm:abelradius}, there exists a unique \(R>0\) such that the power series \(f(z)=\sum_{n=0}^\infty c_n{\qty(z-z_0)}^n\) is convergent on \(D\qty(z_0,R)\) and divergent on \(\mathbb{C}\setminus \overline{D\qty(z_0,R)}\). Let \(z_1\in D(0,R)\). Then \(f\) has the Taylor expansion at \(z_1\) of \[f(z)=\sum_{n=0}^\infty\frac{f^{(n)}\qty(z_1)}{n!}\qty(z-z_1)^n.\]
Let \(\rho>0\) be its convergence radius at \(z_1\). It follows that \(\rho+\qty|z_1-z_0|\geq R\). If \(\rho+\abs{z_1-z_0}>R\), then a part of \(D\qty(z_1,\rho)\) lies outside of \(D\qty(z_0,R)\). In this case, \(f\) can be analytically continued to \(D\qty(z_1,\rho)\setminus D\qty(z_0,R)\). Under the equality case, then the two disks are tangent at some point \(\zeta_0\). Then \(f\) cannot be analytically continued from \(\zeta_0\), and \(\zeta_0\) is a \textit{singularity} of \(f\).

As a matter of fact, if \(R\) is a convergence radius, then there is at least one singularity on the boundary of the convergence disk.

Assume, for the sake of contradiction, that \( f \) can be analytically continued from every point on the boundary \( \partial D(z_0, R) \). Let \( \zeta \in \partial D(z_0, R) \) be arbitrary. Then there exists a radius \( R_\zeta > 0 \) and a function \( f_\zeta \), holomorphic on \( D(\zeta, R_\zeta) \), such that \( f_\zeta \) agrees with \(f\) on the intersection \(D(\zeta,R_\zeta)\cap D(z_0,R)\).

The collection of disks \( \{ D(\zeta, R_\zeta) \}_{\zeta\in\partial D(z_0,R)}\) forms an open cover of the compact set \( \partial D(z_0, R) \). By the Heine--Borel Theorem (\autoref{thm:heineborel}), there exists a finite subcover \( \{ D(\zeta_k, R_{\zeta_k}) \}_{k=1}^n \).

Define the open set
\[V=\bigcup_{k=1}^n D(\zeta_k, R_{\zeta_k}),\]
which contains \(\partial D\qty(z_0, R)\). Since \(\partial D\qty(z_0,R)\) is compact and \(V\) is open and the union of finitely many disks, there exists \(\rho> 0\) such that the annular region \(D\qty(z_0,R+\rho)\setminus\overline{D\qty(z_0,R-\rho)}\subset V\); equivalently, the distance from \(\partial D(z_0,R)\) to the boundary \(\partial V\) is positive.

Define a function \(\Phi:V\to\mathbb{C}\) by setting \(\Phi(z)=f_{\zeta_k}(z)\) for each \(z \in D\qty(\zeta_k,R_{\zeta_k})\). The function \(\Phi\) is single-valued and well-defined: for any \(j\neq k\) such that \(D\qty(\zeta_j, R_{\zeta_j})\cap D\qty(\zeta_k, R_{\zeta_k})\neq\emptyset\), we also have
\[D\qty(\zeta_j,R_{\zeta_j})\cap D\qty(\zeta_k,R_{\zeta_k})\cap D\qty(z_0,R)\neq\emptyset,\]
on which \(f_{\zeta_j}\), \(f_{\zeta_k}\), and \(f\) agree, and hence must agree on the union. Thus, \(\Phi\) is a well-defined holomorphic function on \(V\), and it agrees with \(f\) on \(D(z_0,R)\cap V\).

It follows that \(f\) has an analytic continuation to the set \(D(z_0,R)\cup V \), which contains the disk \(D(z_0,R+\rho)\). This contradicts the assumption that \(f\) cannot be analytically continued beyond \(D(z_0,R)\). Therefore, the assumption must be false.
\begin{example}
    The series \(f(z)=\sum_{n=0}^\infty z^{n!}\) has singularities at every point of its circle of convergence.
\end{example}
\begin{proof}
    By the Cauchy--Hadamard Theorem (\autoref{thm:cauchyhadamard}), the series has the convergence radius of \[\varlimsup_{n\to\infty}\sqrt[n]{
            \begin{cases}
                1&n\in\cbraces{k!}{k\in\mathbb{N}}\\
                0&\text{otherwise}
    \end{cases}}=1.\]
    It follows that \(f\) is holomorphic on \(\mathbb{D}\). Let \(\qty|z_0|=\frac{1}{2}\) be arbitrary. Let \(\zeta=2z_0\). The function \(f(z)\) has a Taylor expansion at \(z=z_0\) of \[\tilde{f}(z)=\sum_{n=0}^\infty\frac{f^{(n)}\qty(z_0)}{n!}{\qty(z-z_0)}^n.\] Let \(\rho\) be the convergence radius of the series above. If we can show that \(\rho=\frac{1}{2}\), then it follows that by tangency, the intersection of \(\partial\mathbb{D}\) and \(\partial D\qty(z_0,\frac{1}{2})\), or \(\zeta\), is a singularity of \(f\) and \(\tilde{f}\).

    For the sake of contradiction, assume that \(\rho>\frac{1}{2}\). Hence, \(D\qty(\zeta,\rho-\frac{1}{2})\cap\mathbb{D}\neq\emptyset\). Obviously, the arc defined by \(S=D\qty(\zeta,\rho-\frac{1}{2})\cap\partial\mathbb{D}\) has positive length. Since \(\exp(2\pi i\mathbb{Q})\) is dense in \(\partial\mathbb{D}\), \(\exists\zeta'\in S\) such that \(\zeta'=\exp(2\pi i\frac{p}{q})\) for some \(p,q\in\mathbb{Z}\). Since \(\tilde{f}\) is convergent and continuous on \(D\qty(\zeta,\rho-\frac{1}{2})\), \(\lim_{r\to 1^-}\tilde{f}(r\zeta')=\tilde{f}(\zeta')\). Since \(f\equiv\tilde{f}\) on \(\mathbb{D}\cap D\qty(z_0,\rho)\), it follows that \(\lim_{r\to 1^-}f(r\zeta')=\tilde{f}(\zeta')\). Since \[f(r\zeta')=\sum_{n=1}^{q-1}r^{n!}\zeta'^{n!}+\sum_{n=q}^\infty r^{n!}\zeta'^{n!},\]
    and for all \(N>q\), \(0<r<1\), \[\sum_{n=q}^\infty r^{n!}\zeta'^{n!}=\sum_{n=q}^\infty r^{n!}>\sum_{n=q}^N r^{n!}>\sum_{n=q}^N r^{N!}=(N-q+1)r^{N!},\]
    we have that \(\lim_{r\to 1^-}f(r\zeta')=\infty\), which contradicts the statement that \(\lim_{r\to1^-}f(r\zeta')=\tilde{f}(\zeta')\). It follows that \(\rho=\frac{1}{2}\).
\end{proof}
For an open set \(D\) and a holomorphic function \(f\) on \(D\), the pair \((D,f)\) is known as an \textit{analytic element}. Two analytic elements are \textit{direct analytic continuations} of each other if their domains overlap, on which the functions are identically equal. 

Let \(\qty(D_0,f_0)\) be an analytic element and construct a collection of elements \(\cbraces{(D_k,f_k)}_{k=1}^n\) where \(\qty(D_k,f_k)\) is a direct analytic continuation of \(\qty(D_{k-1},f_{k-1})\) for all \(k\in\mathbb{N}\). Then \(\qty(D_n,f_n)\) is an analytic continuation of \(\qty(D_0,f_0)\). Informally, we state that two analytic functions (as opposed to analytic elements) are analytic continuations of each other if their analytic elements are analytic continuations of each other. A \textit{maximal analytic continuation} \(\qty(\tilde{D},\tilde{f})\) is an element whose analytic continuations have domains fully contained in \(\tilde{D}\).
\section{The Theory of Riemann}\label{sec:riemann}
First, we will formally introduce the results in \chpref{sec:conformalityintroduction}. Then we will study one of the most famous and historically profound results in complex analysis.
