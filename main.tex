...
If a function \(f(z)\) is complex differentiable over an open neighborhood of \(z_0\), then \(f(z)\) is \textit{holomorphic} at \(z_0\). If \(f(z)\) is holomorphic for every point in an open set \(U\), then it is said to be holomorphic over \(U\).
\begin{theorem}
...
\begin{theorem}[Cauchy's Integral Formula]\label{thm:cauchyintegralformula}
    ...
    \begin{equation}
        f(z)=\frac{1}{2\pi i}\int_{\partial U}\frac{f(\zeta)}{\zeta-z}\dd{\zeta}.\label{eq:cauchyintegralformula}
    \end{equation}
\end{theorem}
\begin{proof}
...
\end{proof}
\begin{theorem}[Cauchy--Goursat Theorem]\label{thm:cauchygoursat}
    
\end{theorem}
\begin{theorem}\label{thm:onedimensionalpartialconjugateproblemsolution}
    For a compactly supported function \(\psi(z)\in C^1\paren{\mathbb{C}}\), a solution satisfying \(u(z)\in C^1\paren{\mathbb{C}}\) to the differential equation \[\pdv{u(z)}{\overline{z}}=\psi(z)\] is \[\frac{1}{2\pi i}\int_{\mathbb C}\frac{\psi(\zeta)}{\zeta-z}\ddzeta\wedge \dd{\overline{\zeta}}.\]
\end{theorem}
\end{document}