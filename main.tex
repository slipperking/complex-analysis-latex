...
\begin{proof}
    ...
    Let \(z_1,\ldots,z_n\) be the isolated poles of \(f\) in \(\overline{D(0,R)}\). \(\forall j\in\qty{1,2,\ldots,n}\), \(f\) has the Laurent expansion at \(z_j\) with a principal part of \[\psi_j(z)=\frac{c_{j,-1}}{z-z_j}+\frac{c_{j,-2}}{\qty(z-z_j)^2}+\cdots+\frac{c_{j,-m_{j}}}{\qty(z-z_j)^{m_j}}.\]
