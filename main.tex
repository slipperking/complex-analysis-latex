...
\begin{proof}
    ...
    Since \(f\) does not vanish on \(\partial K\), \(\varepsilon=\min_{z\in\partial K}\abs{f(z)}>0\). Since \(f_n\rightrightarrows f\) over \(\partial K\) (as it is a compact subset of \(U\)), \(\exists N\in\mathbb{N}\) such that \[\abs{f(z)-f_n(z)}<\varepsilon\leq\abs{f(z)}\] By Rouché's Theorem (\autoref{thm:rouche}), \(f\) and \(f_n\) have the same number of zeros (counting multiplicities) in \(K\).

    The function \(f\) has no zeros in \(K\setminus V\), which is compact (as \(V\) contains all zeros of \(f\) in \(K\)). Since 
\end{proof}
