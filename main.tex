...
\end{equation}
Let \(\phi(z)=\Phi\circ\varphi_a(z)\), which is also harmonic on \(D(0,1)\). By the univalence of \(\varphi_a\), let \(e^{i\theta}=\varphi_a\qty(e^{i\tau})\). It follows that \[ie^{i\theta}\dd{\theta}=i\frac{1-\overline{a}a}{(1-\overline{a}e^{i\tau})^2}e^{i\tau}\dd{\tau}\Longleftrightarrow\]
