...
We will now prove the primary assumption; the singularity at \(z=\infty\) cannot be an essential singularity of \(f(z)\). Let \(w\in\mathbb{C}\) be arbitrary. Then by the Casorati--Weierstrass Theorem (\autoref{thm:casoratiweierstrass}), \(\forall R>0\), \(\forall\varepsilon>0\), \(\exists |z|>R\) such that \(|f(z)-w|<\varepsilon\). Under the assumption that \(f^{-1}\) is entire, \(\forall R>0\), \(\exists\zeta\in D(w,\varepsilon)\) such that \(\abs{f^{-1}(\zeta)}>R\). It follows that the absolute value of the inverse does not attain a maximum.
