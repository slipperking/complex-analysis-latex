...
\end{theorem}
A curve is a one-dimensional manifold embedded in a higher-dimensional space. They can be parameterized with a vector \(\va{F}(t)=\begin{pmatrix}
    P(t)\\Q(t)\\R(t)
...
\begin{proof}
    ...
    Taking the limit as \(\Delta z\to0\), the infinitesimal terms on the right-hand side vanish, and \[\lim_{\Delta z\to0}\frac{f\paren{z}-f\paren{z_0}}{z-z_0}=\alpha+i\beta.\]
\end{proof}
...
Power functions are easily extendable to the complex plane by simply changing the real variable to a complex variable. The other two functions have to be redefined and reinterpreted for the complex plane. It is well known that the exponential function can be expanded as \begin{align*}
    ...
    &=\cos\paren{\frac{x}{i}}+i\sin(\frac{x}{i}).
\end{align*}
...
\end{enumerate}
The functions \(\sin\) and \(\cos\), through their exponential form, still satisfy the properties such as their derivatives, periodicity being \(2\pi\), parity, sum and difference, and the fundamental identities \(\sin^2(z)+\cos^2(z)=1\), \(\sin(z)=\cos\paren{\frac{\pi}{2}-z}\). However, due to the extension, some properties do not hold. For instance, \(\sin(z)\) and \(\cos(z)\) are unbounded, as along the imaginary axis, they resemble their hyperbolic counterparts, which are unbounded along the real line.
...
Lastly, we will examine the power function. Let \(\alpha=u+iv\) where \(u,v\in\mathbb{R}\). Then, \[z^\alpha=e^{\alpha\log(z)}=e^{(u+iv)\paren{\ln\abs{z}+i\arg\paren{z}}}=e^{u\ln\abs{z}-v\arg\paren{z}}e^{i\paren{v\ln\abs{z}+u\arg\paren{z}}}.\]
...
Then assuming \(v=0\), \(z^\alpha=\abs{z}^u\exp\paren{iu\paren{\Arg(z)+2\pi k}}\). Doing casework on \(\alpha\),
\begin{enumerate}
    ...
    Then, \[\abs{z}^{\frac{p}{q}}\exp\paren{i\frac{p}{q}\paren{\Arg(z)+2\pi k}}=\abs{z}^{\frac{p}{q}}e^{i\frac{p}{q}\Arg(z)}e^{2i\frac{p}{q}\pi k}.\]
...
Consider a holomorphic function \(f:U\to\mathbb{C}\) where \(U\subseteq\mathbb{C}\) is open and connected and a point \(z_0\in U\) such that \(f'\paren{z_0}\neq 0\). Let \(\gamma(t)\in C^1([0,1])\) be a curve with \(\gamma(0)=z_0\). Then at \(z_0\), the angle of \(\gamma\) is equal to \(\arg{\gamma'(0)}\). The image of \(\gamma\) under \(f\) is also a smooth curve, \(\sigma(t)=f(\gamma(t))\), which passes through \(w_0=f\paren{z_0}\). It follows from the chain rule that the angle of \(\sigma\) at \(w_0\) is equal to \(\arg{f'\paren{z_0}\gamma'(0)}\). By the properties of \(\arg\) since \(f'\paren{z_0}\) is nonzero and therefore has direction, \[\arg{\sigma'(0)}-\arg{\gamma'(0)}=\arg{f'\paren{z_0}}.\] Notice how the difference between the angles of an arbitrary curve and its image is independent of the curve. Then, for any two smooth curves \(\gamma_1(t)\) and \(\gamma_2(t)\) where \(0\leq t\leq1\) passing through \(z_0\) with images \(\sigma_1(t)\) and \(\sigma_2(t)\) respectively (which pass through \(w_0=f\paren{z_0}\)). Then, \[\arg{\sigma_1'(0)}-\arg{\gamma_1'(0)}=\arg{\sigma_2'(0)}-\arg{\gamma_2'(0)},\] and from rearrangement, \[\arg{\sigma_1'(0)}-\arg{\sigma_2'(0)}=\arg{\gamma_1'(0)}-\arg{\gamma_2'(0)}.\]
Geometrically, the angle at \(z_0\) between two smooth curves passing \(z_0\) in \(U\) is preserved under \(f\). Consider any arbitrary curve \(\gamma(t)\in C^1([0,1])\) which passes \(z_0\) with image \(\sigma(t)\). By the properties of the holomorphy of \(f\), \[\lim_{\substack{z\to z_0\\z\in\gamma}}\frac{\abs{f(z)-f\paren{z_0}}}{\abs{z-z_0}}=\abs{f'(z)}.\]
This implies that lengths are locally scaled by a factor of \(\abs{f'\paren{z_0}}\) under \(f\) at \(z_0\).

For points where \(\abs{f'(z)}\neq 0\), the properties above are called \textit{conformality} and \(f\) is a \textit{conformal mapping}. We will observe the geometric behavior in a later chapter for points where the derivative vanishes.
...
\end{proof}
\begin{remark}
    The Jacobian determinant of \(u,v\) with respect to \(x,y\), for a holomorphic function \(f(z)=u(x,y)+iv(x,y)\) is equal to \[\begin{vmatrix}
        u'_x&u'_y\\v'_x&v'_y
    \end{vmatrix}=\pdv{u}{x}\pdv{v}{y}-\pdv{u}{y}\pdv{v}{x}=\paren{\pdv{u}{x}}^2+\paren{\pdv{u}{y}}^2=\abs{f'(z)}^2\] by equation \eqref{eq:holomorphicderivativedecomposition}.
\end{remark}
...
\begin{lemma}[Schwarz Lemma]
    ...
    When \(z\neq 0\), the two inequalities become equalities if and only if \(f(z)\) is in the form of \(e^{i\tau}z\), where \(\tau\in\mathbb{R}\). In other words, \(f\) is a pure rotation.
\end{lemma}
Then we can consequently define \textit{the group of holomorphic automorphisms}. If \(U\subseteq\mathbb{C}\) is connected and \(f:U\to U\) is holomorphic on \(U\) and bijective, \(f\) is a \textit{holomorphic automorphism} of \(U\). The \textit{group} of holomorphic automorphisms on \(U\) is denoted by \(\Aut(U)\).


\[\varphi_a(\zeta)=\frac{z-\zeta}{1-\overline{z}\zeta}\in\Aut(U).\]
