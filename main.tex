...
\end{example}
\begin{proof}
    Although seemingly trivial at first glance, we are not guaranteed a uniform limit with respect to varying \(\theta\). Namely, for fixed \(\theta\) and \(\forall\varepsilon>0\), \(\exists\delta\) such that \(\forall 0<r<\delta\), we have \(\qty|f\qty(re^{i\theta})|>\varepsilon\). However, \(\delta\) is a function of \(\theta\). If we can show that \(\delta=\delta_\varepsilon(\theta)\) has a positive infimum, or that \(\delta_\varepsilon=\inf_{\theta\in[0,2\pi]}\delta_\varepsilon(\theta)>0\), the conclusion follows.
\end{proof}
