...
\end{example}
\begin{proof}
    Since \(z^5-1=\prod_{k=0}^4\qty(z-\omega_k)\), where \(\omega_k=e^{i\pi\frac{k}{5}}\), the integrand has singularities at every fifth root of unity. We can pentasect \(D(0,2)\) into 5 sectors each containing only one singularity (without any singularities on their boundaries). By the Cauchy--Goursat Integral Formula (\autoref{thm:cauchygoursatformula}), the integral on each sector is equal to the integral over a small disk centered around the singularity:this integral is equal to \[\sum_{k=0}^4 \int_{D(\omega_k\)}\]
\end{proof}
