...
\begin{theorem}
    If \(f(z)\) is meromorphic on \(\mathbb{C}\) and has an isolated singularity at \(z=\infty\) that is a pole or a removable singularity, then \(f\) is a rational function.
\end{theorem}
\begin{proof}
    Since the singularity at \(\infty\) is isolated, there exists a (punctured) neighborhood of \(\infty\),\(\cbraces{z\in\mathbb{C}}{R<|z|<\infty}\), on which \(f\) is holomorphic.
\end{proof}
