...
\end{example}
\begin{proof}
	Since \(z^n-1=\prod_{k=0}^{n-1}\qty(z-\omega^k_n)\), where \(\omega^k_n=e^{i\pi\frac{k}{n}}\), the integrand has singularities at every \(n\)th zero of unity. Then the integral is equal to: \begin{equation}
...
\begin{proof}
	...
	as expected.
\end{proof}
\begin{example}[The Cauchy--Goursat Integral Formula on the Exterior]\label{ex:cauchygoursatformulaexterior}
...
\begin{proof}
	\begin{enumerate}
		\item By the compactness of \(\gamma\), it can be completely contained within a sufficiently large disk centered at the origin (\(\gamma\subset D(0,R)\)). Then by applying \autoref{thm:cauchygoursatformula} or \autoref{thm:cauchygoursattheorem} on the set \(D(0,R)\cap\mathrm{ext}(\gamma)=D(0,R)\setminus\overline{\mathrm{int}(\gamma)}\), we get that \[\frac{1}{2\pi i}\int_{\partial D(0,R)}\frac{f(\zeta)}{\zeta-z}\ddzeta=\frac{1}{2\pi i}\int_\gamma\frac{f(\zeta)}{\zeta-z}\ddzeta+\begin{cases}
				      0    & z\in\mathrm{int}(\gamma)            \\
		...
		\item Under the partial fraction decomposition of equation~\eqref{eq:cauchygoursatformulaexteriorpart2_statement}, we get that \begin{align}
			      I & =\int_\gamma\frac{zf(\zeta)}{z\zeta-\zeta^2}\ddzeta=\int_\gamma\qty(\frac{f(\zeta)}{\zeta}-\frac{f(\zeta)}{\zeta-z})\ddzeta\nonumber                                              \\
			        & =\int_0^{2\pi}\qty(f\qty(Re^{i\theta})-\frac{f\qty(Re^{i\theta})}{1-\frac{z}{Re^{i\theta}}})\dd{\theta}+\begin{cases}
				                                                                                                                  0          & z\in\mathrm{int}(\gamma)            \\
	...
	\end{enumerate}
	By plugging the result into equation~\eqref{eq:cauchygoursatformulaexteriorpart2_prelimitintegral}, and letting \(R\to\infty\), we get that \[\frac{1}{2\pi i}\int_\gamma\frac{zf(\zeta)}{z\zeta-\zeta^2}\ddzeta=\begin{cases}
			0    & z\in\mathrm{int}(\gamma) \\
			f(z) & z\in\mathrm{ext}(\gamma)
		\end{cases}.\]
\end{proof}
...
\begin{example}\label{ex:legendrepolynomialintegralformula}
	A \textit{Legendre polynomial} is a polynomial whose explicit equation is given by \begin{equation}
...
\begin{proof}
	...
	\end{equation} is uniformly convergent with respect to \(\zeta\). By the boundedness of \(f\) on \(\gamma_1\) and \(\gamma_2\) from holomorphy on a compact set, and from the Weierstrass \(M\)--Test (\autoref{thm:weierstrassmtest}), we get that \begin{equation}
...
\begin{theorem}[The Casorati--Weierstrass Theorem]\label{thm:casoratiweierstrass}
...
Let \(f(z)\) be rational, written as \(f(z)=\frac{p(z)}{q(z)}\), where \(p\) and \(q\) are polynomials. Let \begin{gather*}
	p(z)=a_nz^n+a_{n-1}z^{n-1}+\cdots a_0\\
	q(z)=b_mz^m+b_{m-1}z^{m-1}+\cdots b_0,
\end{gather*} where \(a_n,b_m\neq 0\). Trivially, the poles of \(f\) are the zeros of \(q\). Since \[f(z)=\frac{z^n}{z^m}\cdot\frac{a_n+\frac{a_{n-1}}{z}+\cdots+\frac{a_0}{z^n}}{b_m+\frac{b_{m-1}}{z}+\cdots\frac{b_0}{z^m}},\] we have \[\lim_{z\to\infty} f(z)=\begin{cases}
		\frac{a_n}{b_m} & n=m \\
		0               & n<m \\
		\infty          & n>m
	\end{cases}.\]
...
\begin{theorem}[The Holomorphic Automorphism Group on \(\mathbb{C}\)]\label{thm:holomorphicautomorphismgrouponcomplexplane}
	\(\forall f\in \Aut(\mathbb{C})\), \(f\) is linear and non-constant. In other words, \(\exists a\in\mathbb{C}^*\) and \(\exists b\in\mathbb{C}\) such that \[f(z)=az+b.\]
\end{theorem}
\begin{proof}
...
\begin{definition}[Coset]\label{def:coset}
	Let \(G\) be a group, and let \(H\subseteq G\) be a subgroup (operation denoted by juxtaposition). Then the \textit{left coset} of \(H\) in \(G\) with respect to \(g\in G\) is defined as \[gH=\cbraces{gh}{h\in H}.\] The \textit{right coset} is defined as \[H*g=\cbraces{hg}{h\in H}.\] The subgroup \(H\) is normal if and only if the left and right cosets are equal. Cosets, like groups and sets, are unordered.
\end{definition}
\begin{definition}[Quotient Group]\label{def:quotientgroup}
	Let \(G\) be a group, and let \(H\subseteq G\) be a normal subgroup. Then the \textit{quotient group} of \(G\) by \(H\) is defined as \[G/H=\cbraces{gH}{g\in G}.\] The operation (\(*\)) on the quotient group is defined as \((g_1H)*(g_2H)=g_1g_2H\), where \(g_1,g_2\in G\) and \(g_1H,g_2H\in G/H\).
\end{definition}
In order to ensure the existence of a quotient group, the subgroup must be normal. Let \(g_1,g_1'\in G\) form the same left coset (\(g_1H=g_1'H\)), and let \(g_2,g_2'\in G\) satisfy \(g_2H=g_2'H\). We must ensure that \begin{equation}
	(g_1H)*(g_2H)=(g_1'H)*(g_2'H),\qquad g_1g_2H=g_1'g_2'H.\label{eq:quotientgroupcosetoperation}
\end{equation}
Let \(e\) be the identity element of \(G\) and \(H\). In order for \(g_1H=g_1'H\) to be true, it follows that \(g_1e=g_1\) is mapped to a value in \(g_1'H\). Therefore, \(\exists h_1\in H\) such that \(g_1=g_1'h_1\). Similarly, for some \(h_2\in H\), \(g_2=g_2'h_2\). Hence, \[g_1g_2=g_1'h_1g_2'h_2=g_1'\qty(h_1g_2')h_2.\] In order for the second part of equation~\eqref{eq:quotientgroupcosetoperation} to be true, there must exist an element \(h'\in H\) such that \(g_1g_2=g_1'g_2'h'\). In order for \(g_1'\qty(h_1g_2')h_2\).

Let \(f(z)\in \Aut\qty(\riemannsphere)\) such that \(f(\infty)=\infty\). It follows that \(f\) maps \(\mathbb{C}\) to \(\mathbb{C}\) bijectively and \(f\in\Aut(\mathbb{C})\subset\Aut\qty(\riemannsphere)\). Therefore, \(f(z)\) has the form of \(az+b\), where \(a\in\mathbb{C}^*=\mathbb{C}\setminus\qty{0}\) and \(b\in\mathbb{C}\) are constants.
...
\end{proof}
Let \(f(z)\) be an entire function. First, assume that \(f\) has no zeros in \(\mathbb{C}\). Then the function defined by \(z\mapsto\frac{f'(z)}{f(z)}\) is entire, it is the derivative of an entire function \(\varphi(z)\). Therefore, the function defined by \(z\mapsto f(z)e^{-\varphi(z)}\) has the vanishing derivative \(\dv{z}(f(z)e^{-\varphi(z)})=f'(z)e^{-\varphi(z)}-\varphi'(z)f(z)e^{-\varphi(z)}=0\). It follows that \(f(z)e^{-\varphi(z)}\) is constant, and therefore \(f(z)=c\exp(\varphi(z))\) for some constant \(c\in\mathbb{C}\). Since \(\varphi(z)\) is entire, it follows that \(f(z)\) is also entire. Absorb the constant \(c\) into \(\varphi(z)\), and \(f(z)=\exp(\varphi(z))\).

If \(f(z)\) is entire and has finitely many zeros in \(\mathbb{C}\), namely \(a_0=0,a_1,a_2,\ldots,a_n\) with respective multiplicities \(m_0,m_1,\ldots m_n\) (if \(0\) is not a zero, treat \(m_0=0\)), then at each zero \(a_k\), it has the local Taylor expansion of
\[f(z)=\sum_{j=m_k}^\infty c_j{\qty(z-a_k)}^{j},\] where \(c_{m_k}\neq 0\). Therefore, we can divide \(f(z)\) by \({\qty(z-a_k)}^j\) to obtain a new entire function with no additional zeros and no zero at \(a_k\). Repeating this for every root, we can define \(\psi(z)=\frac{f(z)}{p(z)}\), which is entire and has no zeros, where \[p(z)=z^{m_0}{\qty(1-\frac{z}{a_1})}^{m_1}\cdots{\qty(1-\frac{z}{a_n})}^{m_n}.\] We write \(p(z)\) in the above form rather than that of \(z^{m_0}\prod_{k=1}^n{\qty(z-a_k)}^{m_k}\) as we aim to generalize the construction to infinite products to study convergence. By the previous argument, \(\psi(z)=\exp(\varphi(z))\) for some entire function \(\varphi(z)\). Therefore, we can write \[f(z)=p(z)\exp(\varphi(z)),\] where \(p(z)\) is a polynomial with roots at \(a_k\) with respective multiplicities of \(m_k\). The entire functions \(p(z)\) and \(f(z)\) both have the same zeros with matching multiplicities.

Lastly, suppose that \(f(z)\) is entire and has infinitely many zeros such that \(f\) is not identically zero. It follows that \(f\) has countably many zeros (since the zeros are isolated). Let the zeros be indexed by \(\mathbb{N}\), namely \(a_1,a_2,\ldots\). Without loss of generality, assume that \(\forall n\in\mathbb{N}\), \(0<\abs{a_n}<\abs{a_{n+1}}\), and \(\lim_{n\to\infty}a_n=\infty\). The case for a zero at \(0\) will be treated differently.

For every positive and finite \(R\), there exists a positive integer sequence \(k_1,k_2,\ldots\) such that \(\sum_{n=1}^\infty{\qty(\frac{R}{\abs{a_n}})}^{k_n+1}\) converges. For example, let \(k_n=n-1\), and for sufficiently large \(n\), \(\frac{R}{\abs{a_n}}<1\) and the series is convergent. Consider the infinite product 
\begin{equation}
	\prod_{n=2}^\infty{\qty(1-\frac{z}{a_n})}\exp\qty[\frac{z}{a_n}+\frac{1}{2}\qty(\frac{z}{a_n})^2+\cdots+\frac{1}{k_n}\qty(\frac{z}{a_n})^{k_n}].\label{eq:infiniteproductweierstrassfactorizationintermediate}
\end{equation}
Let \begin{gather}
	P_k(z)=z+\frac{1}{2}z^2+\cdots+\frac{1}{k}z^{k}\nonumber\\
	Q_k(z)=\log(1-z)+P_k(z)\nonumber\\
	E_k(z)=\exp\qty[Q_k(z)]=(1-z)\exp\qty[P_k(z)].\label{eq:weierstrasselementaryfactor}
\end{gather}
Therefore, we can rewrite equation~\eqref{eq:infiniteproductweierstrassfactorizationintermediate} as 
\begin{equation*}
	\prod_{n=2}^\infty E_{k_n}\qty(\frac{z}{a_n}).
\end{equation*}
The expression in equation~\eqref{eq:weierstrasselementaryfactor} is known as the \(k\)th \textit{Weierstrass elementary factor}.

By \(\varepsilon\)--\(N\), for a fixed \(R>0\), \(\exists N\in\mathbb{N}\) such that \(\forall n\geq N\), \(\abs{a_n}>2R\) (the coefficient is an arbitrary constant greater than one). Consider the product \(\prod_{n=N}^\infty E_{k_n}\qty(\frac{z}{a_n})\). For \(z\in \overline{D(0,R)}\) and \(n\ge N\), we have \(\qty|\frac{z}{a_n}|\le\frac{1}{2}\). The Taylor expansion \(\log(1-w)=-\sum_{k=1}^\infty\frac{w^k}{k}\) has a convergence disk of \(D(0,1)\). Then, \begin{align}
	\abs{Q_{k_n}\qty(\frac{z}{a_n})}&=\abs{-\sum_{j=1}^\infty\frac{1}{j}\qty(\frac{z}{a_n})^j+\sum_{j=1}^{k_n}\frac{1}{j}\qty(\frac{z}{a_n})^j}\leq\sum_{j=k_n+1}^\infty\frac{1}{j}\abs{\frac{z}{a_n}}^j\nonumber\\
	&\leq\sum_{j=k_n+1}^\infty\abs{\frac{z}{a_n}}^j=\abs{\frac{z}{a_n}}^{k_n+1}\frac{1}{1-\abs{\dfrac{z}{a_n}}}\leq 2\abs{\frac{R}{a_n}}^{k_n+1}(\leq 1).\label{eq:infiniteproductweierstrassfactorizationuniformbound}
\end{align}
By the definition of \(\qty{k_n}_{n\in\mathbb{N}}\), the series \(2\sum_{k=1}^\infty\abs{\frac{R}{a_n}}^{k_n+1}\) converges. Therefore, \(\sum_{k=1}^\infty Q_{k_n}\qty(\frac{z}{a_n})\) is uniformly and absolutely convergent on \(\overline{D(0,R)}\) by the Weierstrass \(M\)--Test (\autoref{thm:weierstrassmtest}). Thus, we get that \(\prod_{n=N}^\infty E_{k_n}\qty(\frac{z}{a_n})=\exp(\sum_{n=N}^\infty Q_{k_n}\qty(\frac{z}{a_n}))\) and \textit{uniformly converges} (on \(\overline{D(0,R)}\)) to a nonzero holomorphic function \(f(z)\) on \({D(0,R)}\) by \autoref{thm:weierstrassuniformlimit} and \autoref{thm:hurwitzsimplecase}. 

The zeros of \[\qty(1-\frac{z}{a_1})\prod_{n=2}^\infty E_{k_n}\qty(\frac{z}{a_n})\] are \(a_1,\ldots, a_{N-1}\) and lie in \(\overline{D(0,2R)}\). To prove the absolute convergence of \(\prod_{n=N}^\infty E_{k_n}\qty(\frac{z}{a_n})\) on \(\overline{D(0,R)}\), we will show that \(\sum_{n=N}^\infty\abs{E_{k_n}\qty(\frac{z}{a_n})-1}\) is convergent. Trivially, when \(\zeta\in\overline{\mathbb{D}}\), we have \(\abs{\exp(\zeta)-1}\leq\exp\qty|\zeta|-1\leq\paren{e-1}\abs{\zeta}\). By equation~\eqref{eq:infiniteproductweierstrassfactorizationuniformbound} above, we get that \(\qty|Q_{k_n}\qty(\frac{z}{a_n})|\leq 1\) when \(n\geq N\).

Therefore, we have \[\abs{E_{k_n}\qty(\frac{z}{a_n})-1}=\abs{\exp(Q_{k_n}\qty(\frac{z}{a_n}))-1}\leq (e-1)\abs{Q_{k_n}\qty(\frac{z}{a_n})}\leq2(e-1)\abs{\frac{R}{a_n}}^{k_n+1},\]
which has a convergent series by definition. Therefore, \(\prod_{n=N}^\infty E_{k_n}\qty(\frac{z}{a_n})\) is absolutely convergent on \(\overline{D(0,R)}\). We can let \(R\to\infty\) and adjust the sequence \(\qty{k_n}_{n\in\mathbb{N}}\) accordingly. 

Therefore, for any such sequence \(\qty{a_n}_{n\in\mathbb{N}}\), we can define the entire function \[f(z)=\qty(1-\frac{z}{a_1})\prod_{n=2}^\infty E_{k_n}\qty(\frac{z}{a_n})\] to have zeros at each element of the sequence, and for any compact disk \(\overline{D(0,R)}\), the product above is uniformly convergent.
...
\begin{definition}[Residue]\label{def:residue}
	For some \(r\in\mathbb{R}_{>0}\), \(a\in U\), suppose \(f:D^*(a,r)\to \mathbb{C}\) is holomorphic. Then the \textit{residue} of \(f\) at \(a\), denoted by \(\residue_{z=a}f(z)\), is equal to \begin{equation}
...
By the definition of a Taylor series, we find that \begin{equation}
...
\end{theorem}
\begin{proof}
	Let \(R>\max_{j\in\mathbb{N}_{\leq n}}\qty|z_n|\) be arbitrary. Then there exist circles centered at each isolated singularity contained in \(D(0,R)\), or that \(\forall k\in\mathbb{N}_{\leq n}\), \(\exists\delta_k>0\) such that \(\partial D\qty(z_k,\delta_k)\subset D(0,R)\). By the Cauchy--Goursat Integral Theorem, \begin{align*}
...
\begin{proof}
	...
	\end{figure}Consider \(\gamma\) to be a closed semicircle with radius \(R\geq 2\) as in \autoref{fig:semicircularcontour}. Notice that the function \(z\mapsto\frac{1}{\qty(z^2+1)^{n+1}}\) has singularities at only \(z=i\) and \(z=-i\), both of which are poles of order \(n+1\). By equation~\eqref{eq:residueatpole}, the residue at \(z=i\) is \begin{align*}
	...
	\end{align*}
	The singularity at \(z=-i\) is not relevant, as it is not enclosed by the contour. By the Residue Theorem (\autoref{thm:residuethm}), we have \begin{align*}
...
\begin{proof}
	...
	Notice that \(\frac{2}{\pi}\theta\leq\sin(\theta)\leq\theta\) over the integration range. We want to observe the behavior as \(R\to\infty\): \begin{align*}
...
\begin{theorem}[The Great Picard Theorem]\label{thm:greatpicard}

\end{theorem}
\begin{corollary}[The Little Picard Theorem]\label{thm:littlepicard}
