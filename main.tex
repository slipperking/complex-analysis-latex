...
\begin{proof}
    ...
    Since both derivatives are positive, \(\abs{f'(z_1)}\leq\frac{1-\overline{w_1}w_1}{1-\overline{z_1}z_1}\). By the Schwarz Lemma (\autoref{lemma:schwarz}), under the equality condition \(\abs{\varphi_{w_1}'\qty(w_1)f'\qty(z_1)\varphi_{-z_1}'(0)}=1\), \(\varphi_{w_1}\circ f\circ\varphi_{-z_1}=e^{i\tau}\), where \(\tau\in\mathbb{R}\) is constant. It follows that \[f=\varphi_{-w_1}\circ e^{i\tau}\circ\varphi_{z_1}\in\Aut(D(0,1)).\]
\end{proof}
