...
The Residue Theorem is extremely important as it allows for simple evaluation of definite (most commonly improper) real-valued integrals. This is because oftentimes, residues are easy to calculate. We can integrate over a contour (a smooth closed curve) that contains the important part of the real interval. Oftentimes this is the most non-trivial step.
\begin{example}
...
\begin{proof}
	...
	\end{figure}Consider \(\gamma\) to be a closed semicircle with radius \(R\) as in \autoref{fig:semicircularcontour}. Notice that the function \(z\mapsto\frac{1}{\qty(z^2+1)^{n+1}}\) has singularities at only \(z=i\) and \(z=-i\), both of which are poles of order \(n+1\). By equation~\eqref{eq:residueatpole}, the residue at \(z=i\) is \begin{align*}
	...
	\end{align*}
	The singularity at \(z=-i\) is not relevant, as it is not enclosed by the contour. By the Residue Theorem (\autoref{thm:residuethm}), we have \[\int_{\gamma} \frac{1}{\qty(z^2+1)^{n+1}}\ddz=2\pi i\cdot\residue_{z=i}\frac{1}{\qty(z^2+1)^{n+1}}=\frac{(2n)!}{2^{2n}(n!)^2}.\]
\end{proof}
