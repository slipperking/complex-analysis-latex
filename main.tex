...
\end{theorem}
\begin{proof}
    By the same logic as with the proof of \autoref{thm:hurwitzsimplecase}, \(f(z)\) is holomorphic over \(U\). Since \(f\not\equiv 0\) on \(U\), the zeros in \(K\), \(\cbraces{z_n}\), are disjoint, and there exist disjoint open disks around each zero such that \(f\) does not vanish on their positively oriented boundaries \(\cbraces{\gamma_n}\).
\end{proof}
