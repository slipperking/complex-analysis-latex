...
\end{example}
\begin{proof}
    It is evident that \(\csc(\pi z)\) has double poles at each of \(z=n\in\mathbb{Z}\). The principal part of the Laurent expansion at each \(n\) is 
    \begin{align*}
        \widetilde{\psi}_n(\zeta)&=\lim_{z\to n}\frac{\dv{z}[\qty(z-n)^2 \csc(\pi z)]}{\zeta-n}+\lim_{z\to n}\frac{\qty(z-n)^2 \csc(\pi z)}{\qty(\zeta-n)^2}\\
        &=\lim_{z\to n}\frac{\frac{2}{\pi\cos(\pi z)}-\frac{2\qty(z-n)}{\pi^2\qty[\cos(\pi z)\tan(\pi z)+\sin(\pi z)\sec^2(\pi z)]}\csc(z)\cot(z)}{\zeta-n}+\lim_{z\to n}\frac{\frac{2\qty(z-n)}{\pi\cos(\pi z)}}{\qty(\zeta-n)^2}\\
        &=\frac{\frac{2}{\pi}(-1)^n-\frac{2}{\pi^3\qty[-\sin(\pi z)\tan(\pi z)+\sec(\pi z)+\sec(\pi z)+2\sin(\pi z)\tan(\pi z)\sec^2(\pi z)]}}{\zeta-n}
    \end{align*}
\end{proof}
...
The sequence of partial sums \[f_n(x)=\sum_{k=1}^n e^{-xk}x^{s-1}\]
\end{document}
