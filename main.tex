...
We will prove later in section [TO BE CONTINUED] that the complex derivative of a holomorphic function \(f(z)=u(z)+iv(z)\) is holomorphic. Under this assumption, \(f(z)\) has continuous second order partial derivatives, and therefore, \[\pdv[2]{u}{x}{y}=\pdv[2]{u}{y}{x},\quad\pdv[2]{v}{x}{y}=\pdv[2]{v}{y}{x},\] and by the Cauchy--Riemann equations, \[\pdv[2]{u}{x}=\pdv[2]{v}{x}{y},\quad\pdv[2]{u}{y}=-\pdv[2]{v}{y}{x},\]
...
\end{remark}
\subsection{Analyticity and Holomorphy}
...
\begin{theorem}[Cauchy--Goursat Differentiation Formula]\label{thm:cauchydifferentiationformula}
    ...
    \begin{equation}
        f(z)=\sum_{k=0}^\infty a_k\paren{z-z_0}^k,\label{eq:cauchydifferentiationformula_taylorseries}
    \end{equation}
    where \begin{equation}
        a_k=\frac{1}{2\pi i}\oint_{\partial U}\frac{f(\zeta)}{\paren{\zeta-z}^{k+1}}\ddzeta.\label{eq:cauchydifferentiationformula_taylorseriescoefficients}
    \end{equation}
\end{theorem}
\begin{proof}
    \(\forall z_0\in U\), \(\forall z\in D\paren{z_0,r}\subset U\), by \autoref{thm:cauchygoursatformula}, \[f(z)-f\paren{z_0}=\frac{1}{2\pi i}\int_{\partial U}\paren{\frac{f(\zeta)}{\zeta-z}-\frac{f(\zeta)}{\zeta-z_0}}\ddzeta=\frac{z-z_0}{2\pi i}\int_{\partial U}\frac{f(\zeta)\ddzeta}{\paren{\zeta-z}\paren{\zeta-z_0}},\]
    and dividing by \(z-z_0\), the above is equal to \[\frac{f(z)-f\paren{z_0}}{z-z_0}=\frac{1}{2\pi i}\int_{\partial U}\frac{f(\zeta)\ddzeta}{\paren{\zeta-z}\paren{\zeta-z_0}}.\]
    Since \begin{align}
        \frac{f(z)-f\paren{z_0}}{z-z_0}-\frac{1}{2\pi i}\int_{\partial U}\frac{f(\zeta)\ddzeta}{\paren{\zeta-z_0}^2}&=\frac{1}{2\pi i}\int_{\partial U}\frac{f\paren{\zeta}}{\zeta-z_0}\paren{\frac{1}{\zeta-z}-\frac{1}{\zeta-z_0}}\ddzeta\nonumber\\
        &=\frac{z-z_0}{2\pi i}\int_{\partial U}\frac{f(\zeta)}{(\zeta-z)\paren{\zeta-z_0}^2}\ddzeta.\label{eq:cauchydifferentiationformula_differenceoffirstorderdifferences}
    \end{align}
    Let \(d\) be the distance from \(z_0\) to \(\partial U\), and choose \(r=\frac{d}{2}\). Then, since \(\abs{z-z_0}<\frac{d}{2}\) and \(\abs{\zeta-z_0}\geq d\), \(\abs{\zeta-z}\geq\frac{d}{2}\). Then the absolute value of the integrand of \eqref{eq:cauchydifferentiationformula_differenceoffirstorderdifferences} is bounded above by \(\frac{2M}{d^3}\), where \(M\) is the maximum of \(f(\zeta)\), which exists by \autoref{thm:continuousfunctionboundedoncompact}. Then, \[\abs{\frac{z-z_0}{2\pi i}\int_{\partial U}\frac{f(\zeta)}{(\zeta-z)\paren{\zeta-z_0}^2}\ddzeta}\leq\frac{\abs{z-z_0}}{\pi}\frac{M}{d^3}\int_{\partial U}\abs{\ddzeta}\]
\end{proof}
