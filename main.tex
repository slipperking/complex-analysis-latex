...
\begin{proof}
    ...
    \begin{equation}
        \abs{\int_{\Gamma}f(z)\ddz-\int_{P}f(z)\ddz}<\varepsilon.\label{eq:integralpiecewisesmoothtopolygonalchain_chaindefinition}
    \end{equation} The statement we aim to prove is equivalent to proving that \[\int_{P}f(z)\ddz=0.\]
    \begin{figure}
        \centering
        \begin{tikzpicture}
            \coordinate (A) at (0, 4);
    ...
    \end{figure}
    Since \(P\) is a closed polygonal chain, we can triangulate the interior. For example, consider \autoref{fig:closedpolygonalchaintriangulation}. Then, \begin{align*}
    ...
    \end{align*} Thus, if the integral over every triangle in \(U\) vanishes, then \eqref{eq:cauchyintegraltheoremoversimplyconnectedset_statement} follows. Consider a triangle in \(U\) with boundary \(\Delta\). Then define \(M\) to be \[M=\abs{\int_{\Delta}f(z)\ddz}.\]
    \begin{figure}
        \centering
        \begin{tikzpicture}
        ...
        \label{fig:}
    \end{figure}
    Quadrisect the triangle bounded by \(\Delta\) into four triangles with boundaries \(\Delta_1,\Delta_2,\Delta_3,\Delta_4\) as in
\end{proof}
\end{document}