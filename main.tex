...
\begin{proof}
    ...
    \end{figure}Since \(\cosh(\pi z)=\cosh(\pi(z+2i))\), and \(\exp(-2\pi iz\xi)=\exp(-4\pi\xi)\exp(-2\pi i(z+2i)\xi)\), we have that \(f(z)\) is a constant multiple of \(f(z+2i)\). Therefore, we can use a rectangular contour as shown in \autoref{fig:rectangularcontour}. 
