...
\end{lemma}
\begin{proof}
    Let \(K\subset U\) be an arbitrary compact set. By the uniform convergence of \(\sum_{k=1}^\infty \abs{f_k(z)}\) on \(K\), it follows that the uniform limit is continuous by the Uniform Limit Theorem (\autoref{thm:uniformlimit}). By continuity on a compact set, it follows that the limit is bounded by some constant \(M\). Additionally, \(\forall\varepsilon\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(\sum_{k=1}^n \abs{f_k(z)}<M+\varepsilon\). It follows that the partial sums are uniformly bounded in \(K\) by \(\max\qty[\max_{k=1}^N\qty[\max_{z\in K}f_k(z)], M+\varepsilon]\). Similarly, by earlier discussion of partial products, we have \[\prod_{k=1}^n\qty(1+\abs{f_k(z)})\leq\exp\]
\end{proof}
