...
\end{proof}
\begin{remark}
    In \autoref{sec:differentialgeometry}, we will introduce the \textit{hyperbolic metric} on \(\mathbb{D}\), defined as \[\dd{s}^2=\frac{4\abs{\ddz}^2}{\qty(1-|z|^2)^2}.\]
...
where \(0<\rho<1\). By the uniform continuity of \(\Phi\) on \(\overline{\mathbb{D}}\) (\autoref{lemma:uniformcontinuousovercompactset}), \(\forall\varepsilon>0\), \(\exists\delta>0\) such that \(\forall\rho\in\qty(\frac{1}{2},1)\) satisfying \(1-\rho<\delta\), \(\forall\theta\in[0,2\pi]\), \(\abs{\Phi\qty(e^{i\theta})-\Phi\qty(\rho e^{i\theta})}<\varepsilon\). It then follows that \[\abs{\frac{1}{2\pi\rho}\int_0^{2\pi}\Phi\qty( e^{i\theta})\dd{\theta}-\frac{1}{2\pi\rho}\int_0^{2\pi}\Phi\qty(\rho e^{i\theta})\dd{\theta}}<\frac{\varepsilon}{\rho}<2\varepsilon.\]
...
\end{equation}
Let \(u(\zeta)=\Phi\circ\varphi_z(\zeta)\), which is also harmonic on \(\mathbb{D}\). By the univalence of \(\varphi_z\), let \(e^{i\theta}=\varphi_z\qty(e^{i\tau})\). It follows that \[ie^{i\theta}\dd{\theta}=i\frac{1-\overline{z}z}{(1-\overline{z}e^{i\tau})^2}e^{i\tau}\dd{\tau}\Longleftrightarrow\dd{\theta}=\frac{1-\overline{z}z}{(1-\overline{z}e^{i\tau})^2}\frac{1-\overline{z}e^{i\tau}}{e^{i\tau}-z}e^{i\tau}\dd{\tau}=\frac{1-\abs{z}^2}{\abs{1-\overline{z}e^{i\tau}}^2}\dd{\tau}.\]
