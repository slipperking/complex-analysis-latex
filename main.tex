...
Let \(f:\mathbb{C}\setminus\overline{D(0,R)}\to\mathbb{C}\) be holomorphic for some \(R>0\). Then \(z=\infty\) is an \textit{isolated singularity} of \(f\). To analyze the nature of the singularity, let \(\zeta=\frac{1}{z}\). We define a new function \(g(\zeta)=f\qty(\frac{1}{\zeta})=f(z)\), which is holomorphic on \(D^*\qty(0,\frac{1}{R})\). Then at \(\zeta=0\), \(g(\zeta)\) has the Laurent expansion of \[g(\zeta)=\sum_{n=-\infty}^\infty c_n\zeta^n=\sum_{n=0}^\infty c_{n}\zeta^n+\sum_{n=1}^\infty c_{-n}\zeta^{-n}=\varphi_g(\zeta)+\psi_g(\zeta),\]
where \(\varphi_g\) and \(\psi_g\) are the holomorphic and principal parts of \(g\), respectively. It follows that \(f\) has the Laurent expansion at \(\infty\)
