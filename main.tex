...
\end{gather}
If \(u\qty(z)\) is continuous on \(\partial\mathbb{D}\), and therefore \(u\circ\exp(i\theta)\) is periodic over \(2\pi\). Thus, it can be expressed as a Fourier series
\begin{equation}
...
By substituting the coefficients, we have \begin{align*}
    \sum_{n=-\infty}^\infty a_ne^{in\theta}\rho^{|n|}&=\sum_{n=-\infty}^\infty \qty(\frac{1}{2\pi}\int_0^{2\pi}u\qty(e^{i\tau})e^{-in\tau}\dd{\tau})e^{in\theta}\rho^{|n|}\\
...
\begin{align*}
    ...
    &=1+2\Re\qty[\frac{\rho\exp[i(\theta-\tau)]}{1-\rho\exp[i(\theta-\tau)]}]
\end{align*}
