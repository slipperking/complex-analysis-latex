...
\begin{proof}
    ...
    with a singularity at \(\zeta=z\). Consider the region \(U\setminus D(z,\varepsilon)\). Applying the Stokes--Cartan Theorem, \[\int_{U\setminus D(z,\varepsilon)} \dd(\frac{f(\zeta)\ddzeta}{\zeta-z})=\int_U\frac{f(\zeta)\ddzeta}{\zeta-z}-\int_{D(z,\varepsilon)}\frac{f(\zeta)\ddzeta}{\zeta-z}\]
\end{proof}
\end{document}