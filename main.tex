...
\begin{example}\label{ex:cauchygoursatformula5poles}
    Evaluate the integral \(\int_{D(0,2)}\frac{\ddz}{z^5-1}\).
\end{example}
\begin{proof}
    Since \(z^5-1=\prod_{k=0}^4\qty(z-\omega_k)\), where \(\omega_k=e^{i\pi\frac{k}{5}}\), the integrand has singularities at every fifth root of unity. We can pentasect \(D(0,2)\) into 5 sectors \(S_0,S_1,\ldots,S_4\) each containing only one singularity (without any singularities on their boundaries). Then the integral is equal to: \begin{equation}
        \sum_{k=0}^4 \int_{S_k}\frac{\ddz}{\prod_{j=0}^4\qty(z-\omega_j)}=\sum_{k=0}^4 \int_{S_k}\sum_{j=0}^4\frac{c_j}{z-\omega_j}\ddz,\label{eq:cauchygoursatformula5poles}
    \end{equation}
    where \(\cbraces{c_j}\) are the coefficients of the partial fraction decomposition. When \(k\neq j\), since \(\omega_j\notin S_k\), \(\frac{c_j}{z-\omega_j}\) vanishes by holomorphy. By the Cauchy--Goursat Integral Formula (\autoref{thm:cauchygoursatformula}), equation~\eqref{eq:cauchygoursatformula5poles} becomes: \[\sum_{k=0}^4\int_{S_k}\frac{c_k}{z-\omega_k}\ddz=2\pi i\sum_{k=0}^4c_k.\]
    Observe that \(\sum_{k=0}^4 c_k=\lim_{z\to\infty}\sum_{k=0}^4\frac{zc_k}{z-\omega_k}=\lim_{z\to\infty}\frac{z}{z^5-1}=0\),
    Therefore, \[\int_{D(0,2)}\frac{\ddz}{z^5-1}=0.\]
\end{proof}
...
\begin{example}
    Prove that \(\forall n\in\mathbb{N}\), \[\int_{0}^{2\pi}\cos^{2n}(\theta)\dd{\theta}=2\pi\frac{}{}\]
\end{example}
...
\begin{proof}
    ...
    \[\abs{f^{(n)}\paren{z_0}}\leq\frac{n!}{2\pi}\int_0^{2\pi}\frac{M}{\paren{e^{i\theta}R}^{n+1}}e^{i\theta}R\dd{\theta}=\frac{n!M}{R^n}.\]
\end{proof}
...
We have touched on the subject of Weierstrass' Theorem for the uniform convergence of the derivatives of a holomorphic function sequence, and we will give the full result below.
