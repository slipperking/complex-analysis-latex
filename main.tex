...
\end{definition}
It then follows that the residue at a removable singularity is 0. Let \(z=\infty\) be an isolated singularity of \(f(z)\), which is holomorphic in \(\mathbb{C}\setminus\overline{D(0,R)}\), for sufficiently large finite \(R\). Then for finite \(\rho>R\), the residue at \(z=\infty\) is \textit{defined} as \[\residue_{z=\infty}f(z)=-\frac{1}{2\pi i}\oint_{\partial D(0,\rho)}f(z)\ddz.\]
