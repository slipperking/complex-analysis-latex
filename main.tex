...
\begin{proof}
    ...
    \[\mathcal{U}=\left\{\mathbb{C}^n\setminus\overline{B^n}\left(z_0,\frac{1}{k}\right)\;\middle|\; \forall k\in\mathbb{N}\right\}.\]
    There then exists a finite subcover \(\mathcal{C}=\left\{\mathbb{C}^n\setminus\overline{B^n}\left(z_0,\frac{1}{k_i}\right)\;\middle|\; i=1,2,\ldots,m\right\}\). Then, \[X\subseteq\mathbb{C}^n\setminus\overline{B^n}\left(z_0,\frac{1}{\max(k_1,\ldots,k_m)}\right),\]
    and that \(X\cap\overline{B^n}\left(z_0,\frac{1}{\max(k_1,\ldots,k_m)}\right)=\emptyset\). However, by the definition of the accumulation point, every open neighborhood of the accumulation point must intersect \(X\). Therefore, by contradiction, \(X\) is closed.
...
Let \(D(a,r)=B^1(a,r)=\cbraces{z\in\mathbb{C}}{\abs{z-a}<r}\) denote the \textit{open disk} centered at \(a\) with radius \(r\). We will now observe the convergence of power series.
\begin{theorem}[Abel's Theorem]\label{thm:abelradius}
    ...
    \begin{enumerate}
        ...
        \item \(f(z)\) is holomorphic over \(D()\) and \(f'(z)\) can be obtained by termwise differentiation \(f'(z)=\sum_{n=1}^\infty na_nz^{n-1}\) and also has the radius of convergence \(R\).\label{itm:abelradius_differentiation}
    \end{enumerate}
\end{theorem}
