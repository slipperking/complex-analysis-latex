...
\end{proof}
We will introduce a trivial application of Liouville's Theorem:
...
\end{lemma}
\begin{theorem}
    Let \(U\subseteq\mathbb{C}\) be open, let \(K\subseteq U\) be compact and \(V\supset K\) be open such that \(\overline{V}\subset U\) (\(V\) is a neighborhood of \(K\) that is relatively compact in \(U\)). Let \(f(z)\) be holomorphic in \(U\). Then there exists a sequence \(\paren{c_n}\) such that \(\forall n\in\mathbb{N}\), \[\sup_{z\in K}\abs{f^{(n)}(z)}\leq c_n\left\|f\right\|_{L_1(V)},\]
    where \(\left\|f\right\|_{L_p(V)}\) denotes \[\paren{\int_V\abs{f(z)}^p\ddx\wedge\ddy}^{\flatfrac{1}{p}}.\]
\end{theorem}
\begin{proof}
    Let \(\varphi\in C^\infty(V)\)  satisfy \(\supp(\varphi)\in V\) and be identically equal to 1 over some open neighborhood \(W\) of \(K\) relatively compact in \(V\). Since \(f\in C^1\paren{\overline{U}}\), by Pompeiu's Theorem (\autoref{thm:pompeiu}) on \(f(z)\varphi(z)\in C^\infty(\mathbb{C})\), \[f(z)\varphi(z)=\frac{1}{2\pi i}\paren{\int_{\partial U}\frac{f(\zeta)\varphi(\zeta)}{\zeta-z}\ddzeta-\int_{U}\pdv{f(\zeta)\varphi(\zeta)}{\overline{\zeta}}\cdot\frac{\dd{\overline{\zeta}}\wedge\ddzeta}{\zeta-z}}.\]
    ...
    We can differentiate within the integral as \(f(\zeta)\cdot\pdv{\varphi(\zeta)}{\overline{\zeta}}\) is \(C^\infty\) and bounded over \(K^1\), and thus the integrand is termwise dominated by an integrable function independent of \(\zeta\):
    ...
    Notice that over \(W\), \(\psi=1\), \(\psi'=0\), and is disjoint from \(K_1\) (or that \(W\cap K_1=\emptyset\)). Since \(K\) is a compact subset of \(W\), which is open, their boundaries have positive distance. Then, the distance between \(W\) and \(K\) is positive. Therefore, \(\exists M>0\) such that \[\frac{1}{\abs{\zeta-z}}\leq M,\] and thus, \[\abs{\pdv{\varphi(\zeta)}{\overline{\zeta}}}\frac{1}{\abs{\zeta-z}^{n+1}}\] can be bounded by a sequence \(\paren{c'_n}\), independent of \(f\) and only of \(n\) and the geometries of \(K\) and \(V\). Then, \[\abs{f^{(n)}(z)}\leq\frac{n!}{2\pi}\int_{K_1}c'_n\abs{f(\zeta)}{\abs{\ddzeta\wedge\dd{\overline{\zeta}}}}=\frac{n!}{2\pi}\int_{K_1}c'_n\abs{f(\zeta)}{\abs{\ddzeta\wedge\dd{\overline{\zeta}}}}.\] Because \(K_1\) is bounded, it has a finite area, and 
\end{proof}
