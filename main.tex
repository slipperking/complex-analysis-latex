...
\begin{proof}
    ...
    \qty|I_1|<\varepsilon\int_{\abs{\vartheta-\tau}<\delta}P(\zeta,z)\dd{\tau}<\varepsilon.
    \]
    ...
    Since \(\abs{\varphi\qty(Re^{i\vartheta})-\varphi(\zeta)}<2M\), we can set \(\int_{\abs{\vartheta-\tau}>\delta}\frac{R^2-\rho^2}{\frac{1}{2}R^2\qty(1-\cos(\delta))}\)
\end{proof}
