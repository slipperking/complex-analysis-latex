...
\end{proof}
Due to the Maximum Modulus Principle (\autoref{thm:maximummodulus}), we have:
\begin{lemma}
    Let \(\Omega\subseteq\mathbb{C}\) be open, and let \(U\subseteq\Omega\) be open, bounded, and relatively compact in \(\Omega\). Let \(\qty{f_n(z)}\) be holomorphic on \(U\), continuous in \(\overline{U}\), and uniformly convergent to \(f(z)\) on \(\partial U\). Then, \(\qty{f_n(z)}\) is uniformly convergent on \(\overline{U}\).
\end{lemma}
\begin{proof}
