...
\begin{theorem}\label{thm:onedimensionalpartialconjugateproblemsolution}
    For a compactly supported function \(\psi(z)\in C^1\paren{\mathbb{C}}\), a solution satisfying \(u(z)\in C^1\paren{\mathbb{C}}\) to the non-homogeneous Cauchy--Riemann equation \[\pdv{u(z)}{\overline{z}}=\psi(z)\] is \[u(z)=\frac{1}{2\pi i}\int_{\mathbb C}\frac{\psi(\zeta)}{\zeta-z}\ddzeta\wedge \dd{\overline{\zeta}}+f(z),\]
    where \(f(z)\) is any entire function.
\end{theorem}
\begin{proof}
