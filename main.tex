...
\end{theorem}
\begin{proof}
    Let \(\qty{a_n}\) be the bounded sequence. In other words, \(\exists m,M\in\mathbb{R}\) such that \(\qty{a_n}\subset[m,M]\). By the infiniteness of \(\qty{a_n}\), at least one of \(\qty[m,\frac{m+M}{2}]\) and \(\qty[\frac{m+M}{2},M]\) contains an infinite subsequence. 
    ...
    Continuing this process, we obtain a sequence of intervals \(\qty{m_n, M_n}\) satisfying \[m_1\leq m_2\leq m_3\leq\cdots\leq M_3\leq M_2\leq M_1.\]
    The length of the \(n\)th interval is equal to \[M_n-m_n=\frac{1}{2^n}\qty(M-m).\] Then since \(\lim_{n\to\infty}M_n-m_n=0\), \[\lim_{n\to\infty}M_n=\lim_{n\to\infty}m_n=a.\]

    \(\forall n\in\mathbb{N}\), choose \(a_{n_k}\) to be any term in \(\qty[m_n,M_n]\)
    
    Assume sequence $a_n$ is bounded within $[A,B]$. Bisecting the interval into $\left[A, \frac {A+B} {2}\right]$ and $\left[\frac{A+B}{2},B\right]$. At least one of the two subintervals contains infinite terms from $[A,B]$. Select one and name its bounds $A_1$ and $B_1$. Iterating this $k$ times, as $k\to\infty$, the length of the nested subinterval $\left[A_k, B_k\right]=\frac{B-A}{2^k}\to0$. At this limit, $\lim_{k\to\infty} A_k=\lim_{n\to\infty}B_k=L$. For each $k$, infinite elements exist between $A_k$ and $B_k$. Let $a_{n_k}$ form a sequence, where $n_k$ chooses an index such that $a_{n_k}\in\left[A_k, B_k\right]$ This subsequence converges to $L$. Note that $n_k$ must be strictly increasing.
\end{proof}
...
\begin{proof}
    ...
    \[k_g-k_f=\int_{\gamma}\frac{g'(z)}{g(z)}\ddz-\int_\gamma\frac{f'(z)}{f(z)}\ddz=\int_\gamma\mfrac{g'(z)f(z)-f'(z)g(z)}{g(z)f(z)}\ddz=\int_\gamma\frac{\paren{\mfrac{g(z)}{f(z)}}'}{\mfrac{g(z)}{f(z)}}\ddz.\]
...
\begin{proof}
    ...
    Next, assume only that \(\abs{f'(0)}=1\). It follows that \(\abs{g(0)}=1\). Since \(\abs{g(z)}\leq 1\) over \(D(0,1)\), it follows from \autoref{thm:maximummodulus} that \(g\) is constant with magnitude 1, or in the form of \(\exp(i\tau)\), where \(\tau\in\mathbb{R}\) is a constant. Consequently, \(f(z)=z\exp(i\tau)\).
\end{proof}
...
\begin{theorem}[Weierstrass' Uniform Limit Theorem]\label{thm:weierstrassuniformlimit}
    Let \(\cbraces{f_n(z)}_{n\in\mathbb{N}}\) be a sequence of holomorphic functions over an open region \(U\subseteq\mathbb{C}\) that converges uniformly to \(f(z)\) on every compact subset of \(U\). Then \(f(z)\) is holomorphic on \(U\), and \(\forall k\in\mathbb{N}\), the sequence \(\cbraces{f^{(k)}_n(z)}_{n\in\mathbb{N}}\) uniformly converges to \(f^{(k)}(z)\) on all compact subsets of \(U\).
\end{theorem}
\begin{proof}
