...
\begin{proof}
    Since \(z^5-1=\prod_{k=0}^4\qty(z-\omega_k)\), where \(\omega_k=e^{i\pi\frac{k}{5}}\), the integrand has singularities at every fifth root of unity. We can pentasect \(D(0,2)\) into 5 sectors \(S_0,S_1,\ldots,S_4\) each containing only one singularity (without any singularities on their boundaries). Then the integral is equal to: \begin{equation}
        \sum_{k=0}^4 \int_{S_k}\frac{\ddz}{\prod_{j=0}^4\qty(z-\omega_j)}=\sum_{k=0}^4 \int_{S_k}\sum_{j=0}^4\frac{A_j}{z-\omega_j}\ddz,\label{eq:cauchygoursatformula5poles}
    \end{equation}
    where \(\cbraces{A_j}\) are the coefficients of the partial fraction decomposition. When \(k\neq j\), since \(\omega_j\notin S_k\), \(\frac{A_j}{z-\omega_j}\) vanishes by holomorphy. By the Cauchy--Goursat Integral Formula (\autoref{thm:cauchygoursatformula}), equation~\eqref{eq:cauchygoursatformula5poles} becomes: \[\sum_{k=0}^4\int_{S_k}\frac{A_k}{z-\omega_k}\ddz=2\pi i\sum_{k=0}^4 A_k.\]
    Observe that \(\sum_{k=0}^4 A_k=\lim_{z\to\infty}\sum_{k=0}^4\frac{zA_k}{z-\omega_k}=\lim_{z\to\infty}\frac{z}{z^5-1}=0\),
