...
\begin{proof}
    Define the auxiliary function \[g(z)=\begin{cases}
        ...
        f'(0)&z=0
    \end{cases}.\]
    Because \(\lim_{z\to0}\frac{f(z)}{z}=f'(0)\), \(g(z)\) is holomorphic on \(D(0,1)\). We can then apply the Maximum Modulus Principle (\autoref{thm:maximummodulus}) on a sufficiently small disk (\(\varepsilon>0\)) in the form \(D(0,1-\varepsilon)\), we get \[\abs{g(z)}\leq\max_{\abs{z}=\varepsilon}\frac{\abs{f(z)}}{\varepsilon}\]
\end{proof}
