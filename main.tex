...
Lastly, we will examine the power function. Let \(\alpha=u+iv\). Then, \[z^\alpha=e^{\alpha\log(z)}=e^{(u+iv)\paren{\ln\abs{z}+i\arg\paren{z}}}=e^{u\ln\abs{z}-v\arg\paren{z}}e^{i\paren{v\ln\abs{z}+u\arg\paren{z}}}.\]
Let \(r_k=\exp\paren{u\ln\abs{z}-v\paren{\Arg(z)+2\pi k}}\) and \(\theta_k=v\ln\abs{z}+u\paren{\Arg{z}+2\pi k}\). Then, \(z^\alpha=r_ke^{i\theta_k}\), where \(k\in\mathbb{Z}\).
