...
Let \(a\in D(0,1)\) and notice that \(\varphi_a(z)=\frac{z-a}{1-\overline{a}z}\in\Aut(D(0,1))\) maps \(\partial D\) to \(\partial D\) bijectively. Let \(U:D(0,1)\to\mathbb{R}\) be harmonic such that \(U\) is continuous on \(\overline{D(0,1)}\). By the mean-value property introduced in \autoref{lemma:holomorphicmeanvalueproperty}, we have \[U(0)=\frac{1}{2\pi}\int_0^{2\pi}U\qty(e^{i\theta})\dd{\theta}.\]
