...
\end{theorem}
\begin{proof}
    Since \(U\) is open, there exists a small disk centered at each isolated singularity \(z_k\) of radii \(\delta_k\). By the Cauchy--Goursat Integral Theorem (\autoref{thm:cauchygoursattheorem}), we get that \[\int_{\partial U^+\cup\bigcup_{k=1}^nD\qty(z_k,\delta_k)^-}f(z)\ddz=0.\]
    From rearrangement, \(\int_{\partial U}f(z)\ddz=\sum_{k=1}^n\int_{D\qty(z_k,\delta_k)}\)
\end{proof}
