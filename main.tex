...
\begin{theorem}\label{thm:identityaccumulationofzeroes}
    Let \(U\subseteq\mathbb{C}\), and \(f:U\to\mathbb{C}\) be holomorphic over \(U\). Then the set \(\cbraces{z\in U\mid f(z)=0}\) has no accumulation points in \(U\), unless \(f\equiv0\) over \(U\).
\end{theorem}
\begin{proof}
    ...
    Now, take any point \(z'\) in \(D(z_0,\varepsilon)\). Since \(f\equiv 0\) in this disk, the Taylor expansion of \(f\) around \(z'\) also has all zero coefficients, so \(f\equiv 0\) in some neighborhood of \(z'\). This allows us to extend the vanishing of \(f\) to neighboring disks. Because \(U\) is connected and these overlapping disks form a cover of \(U\), \(f\equiv 0\) on all of \(U\).
\end{proof}
\begin{remark}
...
\end{theorem}
\begin{proof}
    Let \(h=f-g\) be holomorphic over \(U\). Since \(E\) has an accumulation point in \(U\),
\end{proof}
