...
\begin{proof}
    ...
    where \(\zeta=Re^{i\tau}\) and \(z=\rho e^{i\theta}\), assume that \(\abs{\vartheta-\tau}>\delta\). Then \(\exists\eta>0\) such that when \(|\xi-z|<\eta\), \(\abs{\theta-\tau}>\delta\) and \(\rho>\frac{R}{2}\). Then by the Law of Cosines, \(|\zeta-z|^2>4\rho^2\sin[2](\frac{\delta}{2})>\frac{1}{2}R^2\qty(1-\sin(\delta))\). 
\end{proof}
