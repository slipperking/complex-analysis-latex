...
The two results above resemble the curl and divergence of \(\begin{pmatrix}
    P \\Q\\R
\end{pmatrix}\). A differential form \(\omega\) is \textit{closed} if \(\dd{\omega}=0\), and is exact if there exists \(\eta\) such that \(\omega=\dd{\omega}\). 
...
\end{align*}
Then \(\arccos\) is also a multi-valued function. We can define \(\arcsin(z)=\frac{\pi}{2}-\arccos{z}\). 
