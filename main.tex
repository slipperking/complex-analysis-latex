...
\begin{proof}
    ...
    Since \(f\in C^0(\partial D(z,\varepsilon))\), as \(\varepsilon\to0\), \begin{align*}
        ...
        &\leq2\pi\sup\abs{f\paren{z+\varepsilon e^{it}}-f(z)}\to0.
    \end{align*} In the proof of \autoref{thm:pompeiu}, we used Lipschitz continuity for a smooth function, which was a stronger condition than necessary. The true necessity of smoothness was to be able to apply Green's Theorem (\autoref{thm:complexgreen}).
