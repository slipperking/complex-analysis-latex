...
\begin{proof}
    ...
    \[\frac{f(z)-f\qty(z_0)}{z-z_0}-f_1\paren{z}=\qty[\frac{S_n(z)-S_n\qty(z_0)}{z-z_0}-S'_n(z)]+S'_n(z)-f_1(z)+\frac{R_n(z)-R_n\qty(z_0)}{z-z_0}.\]
    Since \(S'_n(z)\to f_1(z)\) as \(n\to\infty\), \(\forall\varepsilon>0\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(\abs{S'_n(z)-f_1(z)}<\frac{\varepsilon}{3}\). Since \(\frac{R_n(z)-R_n\qty(z_0)}{z-z_0}=\sum_{k=n}^\infty a_k\frac{z^k-z_0^k}{z-z_0}\)
\end{proof}
