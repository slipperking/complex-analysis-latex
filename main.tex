...
\end{example}
\begin{proof}
    It is evident that \(\csc^2(\pi z)\) has double poles at each of \(z=n\in\mathbb{Z}\). Therefore, \((z-n)^2\csc^2(\pi z)\) is holomorphic (or has an analytic continuation that is holomorphic) on \(D(n,1)\). Therefore, by repeatedly applying L'Hôpital's Rule, we can write its Taylor expansion on \(D(n,1)\):
    \begin{align*}
        ...
        &=\lim_{\zeta\to n}\frac{2\qty(\zeta-n)}{\pi\sin(2\pi\zeta)}+(z-n)\lim_{\zeta\to n}\frac{2(\zeta-n)-2\pi(\zeta-n)^2\cot(\pi\zeta)}{\sin[2](\pi\zeta)}\\
        &\quad+\order{(z-n)^2}\\
        &=\order{(z-n)^2}+\lim_{\zeta\to n}\frac{2}{2\pi^2\cos(2\pi\zeta)}\\
        &\quad+(z-n)\lim_{\zeta\to n}\frac{2-4\pi(\zeta-n)\cot(\pi\zeta)+2\pi^2\qty(\zeta-n)^2\csc^2(\pi\zeta)}{\pi\sin(2\pi\zeta)}\\
        &=\order{(z-n)^2}+\frac{1}{\pi^2}\\
        &\quad-\frac{2}{\pi}(z-n)\lim_{\zeta\to n}\qty[\qty(\pi^2\qty(\zeta-n)^2+1)\cot(\pi\zeta)\csc[2](\pi\zeta)-2\pi\qty(\zeta-n)\csc[2](\pi\zeta)]
    \end{align*}
\end{proof}
