...
Power functions are easily extendable to the complex plane by simply changing the real variable to a complex variable. The other two functions have to be redefined and reinterpreted for the complex plane. It is well known that the exponential function can be expanded as \begin{align*}
    ...
    &=\cos\paren{\frac{x}{i}}+i\sin\paren{\frac{x}{i}}.
\end{align*}
...
Consider a holomorphic function \(f:U\to\mathbb{C}\) where \(U\subseteq\mathbb{C}\) is open.
...
In a topological space \(X\), a subset can either be open, closed (\(X\setminus U\), where \(U\) is open in \(X\)), both (clopen), or neither.

Two clopen sets in any topological space \(X\) are \(\emptyset\) and \(X\). \(X\) is \textit{connected} if and only if \(X\) and \(\emptyset\) are the only clopen subsets of \(X\). The proof is not very trivial and not in the scope of this work.

However, we will give examples for both cases. The topological space of \((\mathbb{R},\tau)\) where \(\tau\) is the standard topology only contains two clopen sets: \(\mathbb{R}\) and \(\emptyset\).

The topological space of \(X=(\mathbb{R},\tau)\), where \(\tau\) is the unique discrete topology formed by the basis \(\cbraces{(n,n+1)}{n\in 2\mathbb{Z}}\cup\cbraces{\emptyset,\mathbb{R}}\). Consider \((0,1)\subset X\). Obviously, \((0,1)\in\tau\) so it is open. However, \(\mathbb{R}\setminus (0,1)=\bigcup_{\substack{n\in 2\mathbb{Z}\\n\neq 0
}}(n,n+1)\in\tau\), and therefore, \((0,1)\) is also closed under \(\tau\). In fact, every set in this topology is clopen.
...
\begin{theorem}\label{thm:identityaccumulationofzeros}
    Let \(U\subseteq\mathbb{C}\) be open and connected, and \(f:U\to\mathbb{C}\) be holomorphic over \(U\). Then if the set defined by \(E=\cbraces{z\in U\mid f(z)=0}\) has an accumulation point in \(U\), then \(f\equiv0\) over \(U\).
\end{theorem}
\begin{proof}
    Let \(\cbraces{z_n}_{n\in\mathbb{N}}\) be a subset of \(\cbraces{z}{z\in U,f(z)=0}\) and assume it has an accumulation point \(z_\infty\) in \(U\). Since \(f\) is holomorphic over \(U\), \(\exists\varepsilon>0\) such that \(f\) is holomorphic over \(D\paren{z_\infty,\varepsilon}\subseteq V\). Then over this disk, \(f\) has the Taylor expansion
    \begin{equation}\label{eq:identityaccumulationofzeros_taylorexpansion}
        f(z)=\sum_{n=0}^\infty a_n\paren{z-z_\infty}^n.
    \end{equation}
    By \autoref{def:accumulationpoint}, \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(z_n\in D\paren{z_\infty,\varepsilon}\). Since \(z_n\) is a zero of \(f\), \(f\paren{z_n}=0\). Then, by the continuity of \(f\), \[0=\lim_{n\to\infty} f\paren{z_n}=f\paren{\lim_{n\to\infty}z_n}=f\paren{z_\infty}.\] Using this result in comparison to equation \eqref{eq:identityaccumulationofzeros_taylorexpansion}, we get that \(a_0=0\).
    ...
    Similarly, \(a_2=0\). Letting \(f_n(z)=\frac{f(z)}{\paren{z-z_\infty}^n}\), the sequence \(\paren{a_n}_{n\in\mathbb{Z}_{\geq0}}\) vanishes, and \(f\equiv 0\) on \(D\paren{z_\infty,\varepsilon}\).
    
    By definition, \(D\paren{z_\infty,\varepsilon}\subseteq E\). For any arbitrary point in this disk, the Taylor series will also have vanishing coefficients. Then there exists another disk that \(f\) vanishes over, which is also a subset of \(E\). Then we can repeat this process until we obtain a collection of disks that cover \(U\). Then \(E=U\).
\end{proof}
\begin{remark}
...
\begin{theorem}[Identity Theorem]\label{thm:identity}
    Let \(U\subseteq\mathbb{C}\) be open and connected, and define \(f(z)\) and \(g(z)\) to be two holomorphic functions on \(U\). For a set \(E\subseteq U\) with an accumulation point in \(U\), if \(f\equiv g\) on \(E\), then \(f\equiv g\) on \(U\).
\end{theorem}
\begin{proof}
    Let \(h=f-g\) be holomorphic over \(U\). Since \(E\) has an accumulation point in \(U\), and \(h\equiv 0\) over \(E\), then by \autoref{thm:identityaccumulationofzeros}, \(h\equiv 0\) over \(U\).
\end{proof}
...
This preludes to the following fact: a non-constant holomorphic function cannot attain a local minimum or maximum of its modulus inside its domain (with respect to an open neighborhood of the point).
...
\begin{theorem}[Maximum Modulus Principle]\label{thm:maximummodulus}
    Let \(f(z)\) be holomorphic over a connected region \(U\subseteq\mathbb{C}\). If \(\exists z_0\in U\) such that for some open neighborhood \(V\in U\) of \(z_0\) and \(\forall z\in U\), \(\abs{f\paren{z_0}}\geq\abs{f(z)}\), then \(f\) is a constant function on \(U\).
\end{theorem}
\begin{proof}
    Assume that \(z_0\) exists. We aim to prove that the set \(S=\cbraces{z}{f(z)=f\paren{z_0},z\in U}\) is all of \(U\). This is equivalent to proving that \(S\) is nonempty, open, and closed in \(U\).

    Since \(z_0\in S\), the first condition is satisfied (nonemptiness). Since \(f\) is continuous, \(S\) is closed (if \(S\) is not closed, then at ).
\end{proof}
...
To help us better understand the properties of the \(\wp\) functions, we will first introduce the concept of a lattice.
