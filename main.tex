...
\end{theorem}
\begin{proof}
    Assume that $\forall R>0$, $\exists x\in X$ where $|x|>R$, which implies that \(X\) is unbounded. Consider the collection of open sets \[U=\{B(0,k)\mid k\in\mathbb{N}\}\] where $B(a,b)$ is the open ball with radius $b$ centered at $a$. $U$ forms an open cover of $X$.
\end{proof}
