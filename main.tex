...
\begin{proof}
    ...
    \]
    where \(h(z)\) is holomorphic in a neighborhood of \(a\) and \(c_{-1} = \residue_{z=a}f(z)\).

    Then for \(z = a + \varepsilon e^{i\theta}\),
    \[
    f(a + \varepsilon e^{i\theta}) = \frac{c_{-1}}{\varepsilon e^{i\theta}} + h(a + \varepsilon e^{i\theta}).
    \]
    So,
    \begin{align*}
        \int_{\gamma_\varepsilon} f(z)\,dz 
        &= \varepsilon i \int_\alpha^\beta \left( \frac{c_{-1}}{\varepsilon e^{i\theta}} + h(a + \varepsilon e^{i\theta}) \right) e^{i\theta} \, d\theta \\
        &= i c_{-1} \int_\alpha^\beta d\theta + \varepsilon i \int_\alpha^\beta h(a + \varepsilon e^{i\theta}) e^{i\theta} \, d\theta \\
        &= i c_{-1} (\beta - \alpha) + \varepsilon i \int_\alpha^\beta h(a + \varepsilon e^{i\theta}) e^{i\theta} \, d\theta.
    \end{align*}

    As \(\varepsilon \to 0\), \(h(a + \varepsilon e^{i\theta}) \to h(a)\) uniformly in \(\theta\), so the second term vanishes:
    \[
    \lim_{\varepsilon \to 0} \varepsilon i \int_\alpha^\beta h(a + \varepsilon e^{i\theta}) e^{i\theta} \, d\theta = 0.
    \]

    Therefore,
    \[
    \lim_{\varepsilon \to 0} \int_{\gamma_\varepsilon} f(z)\,dz = i c_{-1} (\beta - \alpha) = i \vartheta \cdot \residue_{z=a} f(z).
    \]
\end{proof}
