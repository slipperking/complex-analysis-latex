...
\begin{definition}[Epsilon Delta]\label{def:epsilondelta}
    ...
    If \(f:U\to\mathbb{R}\) is defined on an open interval  \(\exists L_1\in\mathbb{R}\) such that $\forall\varepsilon>0$, \(\exists\delta>0\) such that $\forall 0<x-a<\delta$ where \(x\in U\) and \(|f(x)-L_1|<\varepsilon\), then \(\lim_{x\to a^+}f(x)=L_1\).
\end{definition}
