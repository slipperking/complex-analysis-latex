...
\begin{theorem}[Bolzano-Weierstrass Theorem]\label{theorem:bolzanoweierstrass}
    Every infinite subset of a compact set \(X\subset\mathbb{C}^n\) has an accumulation point in \(X\).
\end{theorem}
\begin{proof}
Let \(X \subset \mathbb{C}^n\) be compact, and let \(A \subset X\) be an infinite subset. Since \(X\) is bounded, there exists a closed cube \(Q \subset \mathbb{C}^n\) that contains \(X\), and hence also contains \(A\).

    We will use a bisection argument similar to the one used in the Heine–Borel theorem.

    Divide \(Q\) into \(2^{2n}\) congruent subcubes by bisecting each real and imaginary coordinate axis. Since \(A\) is infinite and the subcubes are finite in number, by the pigeonhole principle, at least one of the subcubes contains infinitely many points of \(A\). Call this subcube \(Q_1\).

    Repeat the process: bisect \(Q_1\) into \(2^{2n}\) subcubes, and choose a subcube \(Q_2 \subset Q_1\) that contains infinitely many points of \(A\). Continue this process inductively, constructing a nested sequence of closed cubes
    \[
    Q_0 \supset Q_1 \supset Q_2 \supset \cdots
    \]
    such that:
    \begin{itemize}
        \item each \(Q_k\) contains infinitely many points of \(A\),
        \item the side length of \(Q_k\) tends to zero as \(k \to \infty\).
    \end{itemize}
    ...
    We claim that \(z_\infty\) is an accumulation point of \(A\). To see this, observe that for each \(k\), \(Q_k\) contains infinitely many points of \(A\), and \(Q_k\) is a neighborhood of \(z_\infty\) for sufficiently small \(k\). Moreover, since the side length of \(Q_k\) tends to zero, for any \(\varepsilon > 0\), there exists \(N\) such that for all \(k \geq N\), \(Q_k \subset D(z_\infty, \varepsilon)\). Therefore, \(D(z_\infty, \varepsilon)\) contains infinitely many points of \(A\), proving that \(z_\infty\) is indeed an accumulation point of \(A\).

    It remains to show that \(z_\infty \in X\). Since each \(Q_k \cap A \subset X\), and \(X\) is closed, any limit of a sequence in \(X\) must lie in \(X\). In particular, choose any point \(z_k \in Q_k \cap A\) for each \(k\). Then \((z_k)\) is a sequence in \(X\) converging to \(z_\infty\), so \(z_\infty \in X\) because \(X\) is closed.

    Thus, every infinite subset of a compact set \(X\) has an accumulation point in \(X\).
\end{proof}
...
\begin{proof}
    ...
    First we will show that \(X\) is bounded. Suppose that $\forall R>0$, $\exists z\in X$ where \(\|z\|>R\). Consider the collection of open sets \[\mathcal{U}=\{D(0,k)\mid \forall k\in\mathbb{N}\}\] where $D(a,b)$ is the open ball with radius $b$ centered at \(a\). $\mathcal{U}$ forms an open cover of $X$. Then there exists a finite subcover $\{D(0,k_1),\ldots,D(0,k_m)\}$ covering $X$. Then, \[X\subseteq\bigcup_{i=1}^mD(0,k_i)=D(0,\max(k_1,\ldots k_m)).\] By contradiction, \(X\) must be bounded.
