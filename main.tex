...
\end{lemma}
\begin{proof}
    Because \(f\in C^0(G)\), there is a compact set \(D\subseteq G\) that \(f\) is continuous over. By \autoref{lemma:uniformcontinuousovercompactset}, \(\forall\varepsilon>0\), \(\exists\delta>0\) such that \(\forall z',z''\in D\) satisfying \(\abs{z''-z'}<\delta\), \(\abs{f\paren{z''}-f\paren{z'}}<\varepsilon\). Partition \(\Gamma\) into \(n\in\mathbb{N}\) curves \(s_0,s_1,\ldots,s_{n-1}\) between points \(z_0,z_1,\ldots z_n\) such that the arc length of \(z_k\), \(\forall k\in\) is less than \(\delta\). Connect each pair of adjacent boundary points with a straight line to construct a polygonal chain \(l_0,l_1,\ldots,l_{n-1}\) with lengths each less than \(\delta\) as well. Over the partition formed with \(s_k\), the integral \[\int_{\Gamma}f(z)\ddz\] can be approximated with the Riemann sum \[\sum_{k=0}^{n-1}f\paren{z_k}\Delta z_k\] where \(\Delta z_k=\)
\end{proof}
\end{document}