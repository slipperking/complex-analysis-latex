...
\end{proof}
For an open set \(D\) and a holomorphic function \(f\) on \(D\), the pair \((f,D)\) is known as an \textit{analytic element}. Two analytic elements are \textit{direct analytic continuations} of each other if their domains overlap, on which the functions are identically equal. 

Let \(\qty(f_0,D_0)\) be an analytic element element and construct a collection of elements \(\cbraces{(f_k,D_k)}_{k=1}^n\) where \(\qty(f_k,D_k)\) is a direct analytic continuation of \(\qty(f_{k-1},D_{k-1})\) for all \(k\in\mathbb{N}\). Then \(\qty(f_n,D_n)\) is an analytic continuation of \(\qty(f_0,D_0)\). Informally, we state that two analytic functions (as opposed to analytic elements) are analytic continuations of each other if their domains are implied. 
