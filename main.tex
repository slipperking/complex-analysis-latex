...
\end{example}
\begin{proof}
    For the sake of contradiction, assume that \(f(x)\) is uniformly continuous over \(\paren{0,1}\). Then \(\forall\varepsilon>0\), \(\exists\delta>0\) such that \(\forall x,y\in\paren{0,1}\), where $|x-y|<\delta$, \(\abs{f(y)-f(x)}<\varepsilon\). Choose $\varepsilon=1$. We aim to show that \(\forall\delta>0\)
\end{proof}
\begin{lemma}
