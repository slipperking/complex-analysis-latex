...
\begin{example}
    Evaluate the improper integral \(\int_{-\infty}^\infty \frac{1}{\qty(x^2+1)^2}\).
\end{example}
\begin{proof}
    \begin{figure}
		\centering
		\begin{tikzpicture}[>=stealth,
			...
			\node[anchor=north, xshift=-2pt] at (5, 0) {\(x\)};
			\node[anchor=east, yshift=-2pt] at (0, 5) {\(y\)};
	...
	\end{figure}
    Consider \(\gamma\) to be a closed semicircle with radius \(R\) as in \autoref{fig:semicircularcontour}. We can split the curve into two separate curves. 
\end{proof}
