...
\begin{proof}
    ...
    From \autoref{fig:dirichletproblemwithlaplaceequationsolution_secondintegral}, it is evident that \(R-\rho<|\xi-z|<\eta\). In order for \(|\theta-\tau|>\delta\) to be true, we need to ensure that \(\eta<R^2+\rho^2-2R\rho\cos(\vartheta-\tau-\delta)\). Obviously, this will be satisfied if \(\eta<\frac{R^2}{2}\qty(1-\cos(\vartheta-\tau-\delta))<2\rho^2\qty(1-\cos(\vartheta-\tau-\delta))\). Therefore, we can choose \(\eta=\min\qty(\frac{\varepsilon}{6M}R\paren{1-\cos(\delta)},\frac{R^2}{2}\qty(1-\cos(\vartheta-\tau-\delta)))\)
    
    Since \(\exists\eta>0\) satisfying the criteria above for \(\abs{\xi-z}<\eta\), we have \(\abs{\varphi(\xi)-u(z)}<2\varepsilon\). Then equation~\eqref{eq:dirichletproblemwithlaplaceequationsolution_limittoboundary} follows.
