...
\end{theorem}
\begin{proof}
    By the same logic as with the proof of \autoref{thm:hurwitzsimplecase}, \(f(z)\) is holomorphic over \(U\). Since \(f\not\equiv 0\) on \(U\), the zeros in \(K\), \(\cbraces{z_j}\), are disjoint.
    
    Since \(f\) does not vanish on \(\partial K\), \(\abs{f}\) attains its positive infimum on \(\partial K\). Since \(f_n\rightrightarrows f\) over \(\partial K\) (as it is a compact subset of \(U\)), \(\exists N\in\mathbb{N}\) such that \[\abs{f(z)-f_n(z)}<\abs{f(z)}\] By Rouché's Theorem (\autoref{thm:rouche}), \(f\) and \(f_n\) have the same number of zeros (counting multiplicities) in \(K\).
    ...
    This region can be split into 
    Then \(f\) and \(f_n\) have the same number
\end{proof}
