...
\end{proof}
The following is also a direct result from Rouché's Theorem (\autoref{thm:rouche}). Unlike \autoref{thm:hurwitz}, which concerns the stability of zeros under perturbations of \(f_n(z)\) for increasing \(n\), we can directly consider perturbations of \(f(z)+c\) for vanishing \(c\). Observe their similarities in formulation.
...
For comparison, consider the real function \(x^2\). Shifting it upward removes the zero of multiplicity 2 at \(x=0\) in \(\mathbb{R}\), while shifting it downward causes that zero to split into two simple zeros. This second behavior agrees with \autoref{thm:holomorphiczerosstabilityundershifts}, and is a good way to visualize it in two dimensions. However, the invalidity of the upwards shift highlights a key difference between real differentiable functions and holomorphic functions.
