...
\begin{theorem}[Cauchy's Estimate]\label{thm:cauchysestimate}
    For a function \(f:U\to\mathbb{C}\) holomorphic over \(U\subseteq\mathbb{C}\) and \(\overline{D\paren{z_0,R}}\subseteq{U}\), then \(\forall n\in\mathbb{N}\), \[f^{(n)}\paren{z_0}\leq\frac{n!M}{R^n},\]
    where \[M=\max_{z\in\overline{D\paren{z_0,R}}}\abs{f(z)}.\]
\end{theorem}
\begin{proof}
    By the Cauchy--Goursat Differentiation Formula (\autoref{thm:cauchydifferentiationformula}), \(\forall z\in D\paren{z_0,R}\), \(\forall n\in\mathbb{N}\), \[f^{(n)}(z)=\frac{1}{2\pi i}\oint_{\partial D\paren{z_0,R}}\frac{f(\zeta)}{\paren{\zeta-z}^{n+1}}\ddzeta\]
\end{proof}
\begin{theorem}[Liouville's Theorem]\label{liouville}
    Any bounded entire function is constant.
\end{theorem}
\begin{proof}
    Let \(f:\mathbb{C}\to\mathbb{C}\) be entire. Then, \(\forall z_0\in\mathbb{C}\), \(\forall R>0\), \(f\) is holomorphic over \(\overline{D\paren{z_0,R}}\). By \autoref{thm:cauchysestimate}, \[f'\paren{z_0}\leq\frac{M}{R},\] where \(M=\sup_{z\in\mathbb{C}}\abs{f(z)}\). By letting \(r\to\infty\), \(f'\paren{z_0}\) where \(z_0\) is any arbitrary value in \(\mathbb{C}\). Therefore, \(f(z)\) is constant.
\end{proof}
