...
\begin{proof}
    ...
    As are integrals defined with limits, we aim to prove that for fixed \(\xi=Re^{i\vartheta}\in\partial D(0,R)\), \[\lim_{\substack{z\to\xi\\z\in D(0,R)}}u(z)=\varphi(\xi).\]
    ...
    where \(\zeta=Re^{i\tau}\) and \(z=\rho e^{i\theta}\), assume that \(\abs{\vartheta-\tau}>\delta\). Then \(\exists\eta>0\) such that when \(|\xi-z|<\eta\), \(\abs{\theta-\tau}>\delta\) and \(\rho>\frac{R}{2}\). Then by the Law of Cosines, \(|\zeta-z|^2>4\rho^2\sin[2](\frac{\delta}{2})>\frac{1}{2}R^2\qty(1-\cos(\delta))\). 
