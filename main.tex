...
\end{example}
\begin{proof}
    Since \(z^n-1=\prod_{k=0}^{n-1}\qty(z-\omega^k_n)\), where \(\omega^k_n=e^{i\pi\frac{k}{n}}\), the integrand has singularities at every \(n\)th root of unity. We can divide \(D(0,2)\) into \(n\) sectors \(S_0,S_1,\ldots,S_{n-1}\) each containing only one singularity (without any singularities on their boundaries). Then the integral is equal to: \begin{equation}
        \sum_{k=0}^{n-1} \int_{\partial S_k}\frac{\ddz}{\prod_{j=0}^{n-1}\qty(z-\omega_j)}=\sum_{k=0}^{n-1}\int_{\partial S_k}\sum_{j=0}^{n-1}\frac{c_j}{z-\omega_j}\ddz,\label{eq:cauchygoursatformularootsofunity}
    \end{equation}
    where \(\cbraces{c_j}\) are the coefficients of the partial fraction decomposition. When \(k\neq j\), since \(\omega_j\notin S_k\), \(\frac{c_j}{z-\omega_j}\) vanishes by holomorphy. By the Cauchy--Goursat Integral Formula (\autoref{thm:cauchygoursatformula}), equation~\eqref{eq:cauchygoursatformularootsofunity} becomes: \[\sum_{k=0}^{n-1}\int_{\partial S_k}\frac{c_k}{z-\omega_k}\ddz=2\pi i\sum_{k=0}^{n-1}c_k.\]
    Observe that \(\sum_{k=0}^{n-1} c_k=\lim_{z\to\infty}\sum_{k=0}^{n-1}\frac{zc_k}{z-\omega_k}=\lim_{z\to\infty}\frac{z}{z^n-1}=0\),
...
\begin{proof}
    ...
    Letting \(z=e^{i\theta}\), we get \(\int_{\partial\mathbb{D}}\qty(e^{i\theta}+e^{-i\theta})^{2n}e^{-i\theta}\ddz=2^{2n}i\int_0^{2\pi}\cos^{2n}\theta\dd{\theta}\). Alternatively, we can expand the integrand and get \[\int_{\partial\mathbb{D}}\sum_{k=0}^{2n}\binom{2n}{k}z^{2k-n}\frac{\ddz}{z}=\sum_{k=0}^{2n}\int_{\partial\mathbb{D}}\binom{2n}{k}z^{2k-n-1}\ddz.\]
    When \(2k-n-1\ge0\), the integrand is holomorphic. The integral is then equal to \[\int_{\partial\mathbb{D}}z^{-n-1}+\int_{\partial\mathbb{D}}2nz^{-n+1}+\cdots+\int_{\partial\mathbb{D}}\frac{\ddz}{z}.\]
\end{proof}
