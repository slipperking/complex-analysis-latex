...
\end{theorem}
\begin{proof}
    Let \(\cbraces{z_n}_{n\in\mathbb{N}}\) be a subset of \(\cbraces{z}{z\in U,f(z)=0}\) and assume it has an accumulation point \(z^*\) in \(U\). Since \(f\) is holomorphic over \(U\), \(\exists\varepsilon>0\) such that \(f\) is holomorphic over \(D\paren{z_0,\varepsilon}\subseteq V\). Then over this disk, \(f\) has the Taylor Series representation
    ...
    Letting \(n\to\infty\), \(z_n\to z^*\), and \(a_1=0\). Define \(f_2(z)=\)
