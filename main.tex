...
\begin{lemma}
    Let \(U\subseteq\mathbb{C}\) be open and bounded, and let \(\qty{f_n(z)}\) be holomorphic on \(U\). Let \(K\subseteq\Omega\) be compact. If \(\qty{f_n(z)}\)  If \(f_n\rightrightarrows f\) on \(\partial U\), then \(f_n\rightrightarrows f\) on \(\overline{U}\).
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{remark}
    Therefore, the statement "uniformly converges on every compact subset"
\end{remark}
