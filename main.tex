...
\begin{theorem}[Maximum Modulus Principle]\label{thm:maximummodulus}
    Let \(f(z)\) be holomorphic over an open connected region \(U\subseteq\mathbb{C}\). If \(\exists z_0\in U\) such that for some open neighborhood \(V\in U\) of \(z_0\) and \(\forall z\in U\), \(\abs{f\paren{z_0}}\geq\abs{f(z)}\), then \(f\) is a constant function on \(U\).
\end{theorem}
\begin{proof}
    ...
    Since \(z_0\in S\), the first condition is satisfied (nonemptiness). For any sequence \(\cbraces{z_n}\in S\) converging to some \(z_\infty\in U\), by the continuity of \(f\), \[\lim_{n\to\infty} f\paren{z_n}=f\paren{\lim_{n\to\infty}z_n}=f(z)=f\paren{z_0}.\] Thus, \(S\) contains all of its accumulation points in \(U\) and is therefore closed (if \(z_\infty\notin U\), then it is no longer relevant; we are concerned with it being closed within \(U\)).

    Since \(S\subseteq U\) and \(U\) is open, \(\forall z\in S\), \(\exists\varepsilon>0\) such that \(D(z,\varepsilon)\subseteq U\). By \autoref{lemma:holomorphicmeanvalueproperty}, \(\)
\end{proof}
