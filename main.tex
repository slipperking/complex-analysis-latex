...
In normal calculus, we worked with functions $f: \mathbb{R}\to\mathbb R$. In complex analysis the focus is functions of the form $f: \mathbb C\to\mathbb C$. The criteria for functions to be \textit{analytic} is that the direction of approach for calculating the derivative does not affect the result. To demonstrate this, set a complex function $f(z)=u(z)+v(z)i$ where $z=x+yi$ and $u$ and $v$ can have inputs $x$ and $y$. Approaching the function from the complex axis where $h$ is real:
\begin{align*}
    \frac{\partial f}{\partial z}=\lim_{h=0} \frac{f(z+hi)-f(z)}{hi}&=\lim_{h=0} \frac{u(x+(y+h)i)+iv(x+(y+h)i)-u(x+yi)-iv(x+yi)}{hi}\\
    &=\frac{\partial u}{\partial y}+i\frac{\partial v}{\partial y}\text{.}
\end{align*}
Approaching the function from the real axis,
\begin{align*}
    \frac{\partial f}{\partial z}=\lim_{h=0} \frac{f(z+h)-f(z)}{h}&=\lim_{h=0} \frac{u((x+h)+yi)+iv((x+h)+yi)-u(x+yi)-iv(x+yi)}{h}\\
