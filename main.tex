...
\begin{proof}
    ...
    Define a set of functions \(\lbrace\beta_j\rbrace_{j\in\mathbb{N}}\) with \(\beta_j=\theta_{r_{z_j}}\paren{z-z_j}\), which is a bump function with the support \(\overline{D\paren{z_j, r_{z_j}}}\subset\Omega_{k_{z_j}}\). Equivalently, \(\lbrace\supp\paren{\beta_j}\rbrace_{j\in\mathbb{N}}\) is locally finite.
\end{proof}
