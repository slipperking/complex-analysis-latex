...
\end{theorem}
\begin{proof}
    It is obvious that \(g(z)\) has no zeros on \(\gamma\). Otherwise, \(\exists z\in\gamma\) such that \(g(z)=0\), implying that \(\abs{f(z)}<\abs{f(z)}\) which is impossible. 
    
    Let \(k_f\) and \(k_g\) denote the number of zeros of \(f\) and \(g\) enclosed by \(\gamma\), respectively. By the argument principle,
    \[k_f-k_g=\int_{\gamma}\frac{f'(z)}{f(z)}\]
\end{proof}
