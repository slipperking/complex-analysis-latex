...
\begin{proof}
    ...
    The Maximum Principle can be generalized to \(\psi\) since it satisfies the mean-value property (refer to the proof of the Maximum Modulus Principle (\autoref{thm:maximummodulus}), which uses the mean-value property of holomorphic functions). It follows that the maximum of \(\psi\) is assumed to be on \(\partial D\qty(z_0,\varepsilon)\), and \(\psi\equiv 0\) on \(\overline{D\qty(z_0,\varepsilon)}\). Then \(f\) is harmonic on the closed disk (and on \(z_0\)). Since \(z_0\in U\) was arbitrary, the harmonicity is extendable to all of \(U\).
\end{proof}
