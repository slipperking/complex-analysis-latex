...
Let \(C\) be a positively orientedsmooth curve in \(\mathbb{R}^3\) parametrized by \(\mathbf{r}(t) = (x(t), y(t), z(t))\) for \(t \in [a,b]\), and let \(f(x,y,z)\) be differentiable. Then
\[\int_C \pdv{f}{x} \ddx + \pdv{f}{y} \ddy + \pdv{f}{z} \ddz = f(x(b), y(b), z(b)) - f(x(a), y(a), z(a)).\]
\end{theorem}
