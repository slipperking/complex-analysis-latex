...
\begin{equation}
    u\qty(e^{i\theta}) \sim \sum_{n=-\infty}^\infty a_n e^{in\theta},\quad a_n=\frac{1}{2\pi}\int_{-\pi}^\pi u\qty(e^{i\tau})e^{-in\tau}\dd{\tau}=\frac{1}{2\pi}\int_0^{2\pi} u\qty(e^{i\tau})e^{-in\tau}\dd{\tau}. \label{eq:poissonintegralformulafourierseries}
\end{equation}
...
\end{theorem}
\begin{proof}
    Fix \(z_0 \in U\) and choose \(\lambda>0\) such that \(\overline{D(z_0,\lambda)}\subseteq U\). Since \(f \in C^0\qty(\partial D(z_0,\lambda))\), \autoref{thm:dirichletproblemwithlaplaceequationsolution} guarantees a unique harmonic function \(u\) on \(D(z_0,\lambda)\) satisfying
    \[
        u(z) = \int_0^{2\pi} f(\zeta) P\qty(\zeta,z) \dd{\tau},
    \]
    with \(u = f\) on \(\partial D(z_0,\lambda)\). Define \(\psi = f - u\) on \(\overline{D(z_0,\lambda)}\). Then \(\psi\) is continuous, satisfies the mean-value property, and vanishes on \(\partial D(z_0,\lambda)\). By the extended Maximum Principle (cf. proof of \autoref{thm:maximummodulus}, which relies solely on the mean-value property), \(\psi \equiv 0\) on \(\overline{D(z_0,\lambda)}\). Thus \(f \equiv u\) on \(\overline{D(z_0,\lambda)}\), implying \(f\) is harmonic at \(z_0\). The arbitrariness of \(z_0\) establishes harmonicity on \(U\).
\end{proof}
