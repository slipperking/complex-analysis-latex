...
\end{example}
\begin{proof}
    If \(f(x)\) is not uniformly continuous, then $\exists\varepsilon>0$ such that $\forall\delta>0$, \(\exists x,y\in(0,1)\) such that \(|x-y|<\delta\) and \(\abs{f(x)-f(y)}\geq\varepsilon\). Let \[x=\frac{1}{n},\quad y=\frac{1}{n+\min{\paren{\varepsilon,1}}}\]
\end{proof}
\begin{lemma}
