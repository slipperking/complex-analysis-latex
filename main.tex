...
\end{proof}
\begin{lemma}
    Given a holomorphic function \(f(z)\) on a simply connected set \(U\subseteq\mathbb{C}\), for any piecewise smooth closed curve \(\Gamma\subset G\), \[\int_\Gamma f(\zeta)\ddzeta=0.\]
\end{lemma}
\begin{proof}
    By \autoref{lemma:integralpiecewisesmoothtopolygonalchain}, \(\forall\varepsilon>0\), there is a polygonal chain \(P\) where \[\abs{\int_{\Gamma}f(z)\ddz-\int_{P}f(z)\ddz}<\varepsilon.\] The statement we aim to prove is equivalent to proving that \[\int_{P}f(z)\ddz=0.\]
    \begin{figure}
        \centering
        \begin{tikzpicture}
            \coordinate (A) at (0, 4);
            ...
            \node[left] at (A) {A};
            \node[above, xshift=3pt, yshift=5pt] at (B) {B};
            \node[left, xshift=-3pt] at (C) {C};
            \node[below left] at (D) {D};
            \node[left, xshift=-2pt] at (E) {E};
        \end{tikzpicture}
        \caption{A closed polygonal chain internally triangulated, with orientations shown.}
        \label{fig:closedpolygonalchaintriangulation}
    \end{figure}
    Since \(P\) is a closed polygonal chain, we can triangulate the interior. For simplicity's sake, consider \autoref{fig:closedpolygonalchaintriangulation}. Then, \[\int_{P}f(z)\ddz=\int_{\overrightarrow{AB}}f(z)\ddz+\int_{\overrightarrow{AB}}f(z)\ddz+\int_{\overrightarrow{AB}}f(z)\ddz+\]
\end{proof}
\end{document}