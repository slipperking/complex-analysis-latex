...
\end{proof}
\begin{remark}
    The Jacobian determinant of \(u,v\) with respect to \(x,y\), for a holomorphic function \(f(z)=u(x,y)+iv(x,y)\) is equal to \[J=\begin{vmatrix}
        \pdv*{u}{x}&\pdv*{u}{y}\\\pdv*{v}{x}&\pdv*{v}{y}
    \end{vmatrix}=\pdv{u}{x}\pdv{v}{y}-\pdv{u}{y}\pdv{v}{x}.\]
    By the Cauchy--Riemann Equations, define \(\alpha=\pdv{u}{x}=\pdv{v}{y}\) and \(\beta=-\pdv{u}{y}=\pdv{v}{x}\). It follows that \(f'(z)=\alpha\)Then, \[J=\alpha^2+\beta^2\]
\end{remark}
