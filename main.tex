...
\begin{proof}
    ...
    From \eqref{eq:rouche}, by dividing both sides by \(f(z)\), we obtain \(\abs{w-1}<1\). Then \(\Gamma\) lies in the open disk \(D(1,1)\), which will never intersect or enclose 0. Then by the Cauchy--Goursat Integral Theorem (\autoref{thm:cauchygoursattheorem}), \[k_g-k_f=\int_\Gamma\frac{\dd{w}}{w}=0,\]Then, \(k_g-k_f=0\), as desired.
\end{proof}
