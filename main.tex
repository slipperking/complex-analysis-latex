...
\end{theorem}
\begin{proof}
    Let \(\qty{a_n}\) be the bounded sequence. In other words, \(\exists m,M\in\mathbb{R}\) such that \(\qty{a_n}\subset[m,M]\). By the infiniteness of \(\qty{a_n}\), at least one of \(\qty[m,\frac{m+M}{2}]\) and \(\qty[\frac{m+M}{2},M]\) have infinite elements of \(\)
