...
\begin{proof}
    ...
    When \(w_0\) lies on \(\Gamma\), \(f-w_0\) has no zeros in \(U\). Indeed, for the sake of contradiction, assume that \(\exists z_0\in U\) such that \(f\qty(z_0)=w_0\). By the Open Mapping Theorem (\autoref{thm:openmapping}), \(\exists\delta>0\) such that \(D\qty(w_0,\delta)\subseteq f(U)\). Then \(\forall w'\in D\qty(w_0,\delta)\), \(f-w'\) has 
\end{proof}
