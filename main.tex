...
The Argument Principle (\autoref{thm:argumentprincipleholomorphic}) can be generalized for meromorphic functions.
\begin{theorem}[Generalized Argument Principle]\label{thm:argumentprinciplemeromorphic}
    Let \(U\subseteq\mathbb{C}\) be a region and \(f:U\to\mathbb{C}\) be meromorphic. Let \(\gamma\subset U\) be a closed, positively oriented curve homotopic to a constant curve (the homotopy completely lies in \(U\)). If \(f\) has no zeros and poles on \(\gamma\), then \(f\) has finitely many poles and zeros in the region bounded by \(\gamma\). Let \(k_z\) and \(k_p\) denote the number of zeros and poles of \(f\) enclosed by \(\gamma\), counted with multiplicity and order, respectively. Then, \[k_z-k_p=\frac{1}{2\pi i}\oint_{\gamma}\frac{f'(z)}{f(z)}\ddz.\]
    Let \(\Gamma\) be the image of \(\gamma\) under \(f\). Then, \(k_z-k_p=\frac{1}{2\pi}\Delta_\Gamma\arg(f)\), where \(\Delta_\Gamma\arg(f)=\Delta_{\gamma}\arg(f(z))\) is the total change in argument of \(f(z)\) as \(z\) traverses \(\gamma\).
\end{theorem}
\begin{proof}
    Let \(z_1,\ldots,z_n\) be the poles enclosed by \(\gamma\) with orders \(k_1,\ldots,k_n\) respectively. Then there exists disks centered at each pole with radii \(\varepsilon_1,\ldots,\varepsilon_n\) fully contained within the interior of \(\gamma\) that do not intersect the zeros of \(f\) enclosed by \(\gamma\) such that their closures are disjoint. 
\end{proof}
