...
\begin{theorem}\label{thm:meanvaluepropertysolutionsareharmonic}
    Let \(U\subseteq\mathbb{C}\) be open. Let \(f(z):U\to\mathbb{R}\) be continuous. \(\forall z_0\in U\), \(\exists\varepsilon>0\) such that \(\overline{D\qty(z_0,\varepsilon)}\subseteq U\) If \(\forall0<\varepsilon'<\varepsilon\), \[f(z_0)=\frac{1}{2\pi}\int_0^{2\pi}f\qty(z_0+\varepsilon'e^{it})\ddt,\]
