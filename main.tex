...
\begin{theorem}[Uniform Limit Theorem]
    Let \(\cbraces{f_n(z)}\) be continuous on \(U\subseteq\mathbb{C}\) and \(\sum_{n=1}^\infty f_n(z)\) uniformly converge to \(S(z)\). Then \(S(z)\) is continuous on \(U\).
\end{theorem}
\begin{proof}
    \(\forall n\in\mathbb{N}\), \(\forall z_0\in U\), \(\forall\varepsilon>0\), \(\exists\delta>0\) such that \(\forall z\in D\paren{z_0,\delta}\subseteq U\), \(\abs{f(z)-f\paren{z_0}}<\varepsilon\). Then
\end{proof}
