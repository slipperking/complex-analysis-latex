...
If \(U\subseteq\mathbb{C}\) is connected and \(f:U\to U\) is holomorphic on \(U\) and bijective, \(f\) is a \textit{holomorphic automorphism} of \(U\). The \textit{group of holomorphic automorphisms} on \(U\) is denoted by \(\Aut(U)\), which is the set of all holomorphic automorphisms such as \(f\), with the operation of composition.

First we will show that \(\forall z\in D(0,1)\),
\begin{equation}
    \varphi_z(\zeta)=\frac{z-\zeta}{1-\overline{z}\zeta}\in\Aut( D(0,1)).\label{eq:mobiustransformationgroupofholomorphicautomorphismsstatement}
\end{equation}
Firstly, the function is holomorphic on \(\overline{D(0,1)}\) as \(|\zeta|\le1\), \(\abs{\overline{z}}<1\), the denominator never vanishes. 

First, we will observe the image of \(\partial D(0,1)\). Let \(\abs{\zeta}=1\). Then,
\[\abs{\varphi_z(\zeta)}=\abs{\frac{1}{\zeta}}\abs{\frac{z-\zeta}{\frac{1}{\zeta}-\overline{z}}}=\abs{\frac{z-\zeta}{\overline{\zeta}-\overline{z}}}=1.\]
Therefore, the image of \(\partial D(0,1)\) lies on \(\partial D(0,1)\), and since \(f\) is holomorphic and non-constant, by the Maximum Modulus Principle (\autoref{thm:maximummodulus}), for \(\abs{\zeta}<1\), \(\abs{\varphi(\zeta)}<1\). Therefore, \(f\) maps \(D(0,1)\) to \(D(0,1)\). We next aim to show that \(f:D(0,1)\to D(0,1)\) is bijective.

Let us first confirm injectivity. \(\forall\zeta_1,\zeta_2\in D(0,1)\), we will observe when \[\frac{z-\zeta_1}{1-\overline{z}\zeta_1}=\frac{z-\zeta_2}{1-\overline{z}\zeta_2}\] is satisfied. It follows that \[\qty(z-\zeta_1)\qty(1-\overline{z}\zeta_2)=\qty(z-\zeta_2)\qty(1-\overline{z}\zeta_1)\Longleftrightarrow z+\overline{z}\zeta_1\zeta_2-\abs{z}^2\zeta_2-\zeta_1=z+\overline{z}\zeta_1\zeta_2-\abs{z}^2\zeta_1-\zeta_2.\]
Then, \[\abs{z}^2\paren{\zeta_2-\zeta_1}=\zeta_2-\zeta_1.\Longleftrightarrow\qty\big(\qty|z|^2-1)\qty(\zeta_2-\zeta_1)=0.\] Since \(\abs{z}<1\), we get \(\zeta_2-\zeta_2=0\). This proves the injectivity of \(\varphi_z(\zeta)\).

Next, we will solve for the inverse of \(\varphi_z\). Let \(\zeta=\varphi_z(\xi)=\frac{z-\xi}{1-\overline{z}\xi}\). Then,
\[\zeta-\overline{z}\zeta\xi=z-\xi\Longleftrightarrow\xi=\frac{z-\zeta}{1-\overline{z}\zeta}.\]
It follows that \(\varphi_z=\qty(\varphi_z)^{-1}\). Thus \(\varphi_z\) is surjective and consequently bijective. It follows that equation \eqref{eq:mobiustransformationgroupofholomorphicautomorphismsstatement} is true. Functions in the form of \(\varphi_z\) are known as \textit{Möbius transformations}, and the group of all such transformations is known as the \textit{Möbius transformation group} and is a subgroup of \(\Aut(D(0,1))\). 
