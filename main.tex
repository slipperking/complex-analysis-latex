...
\newcommand{\ccwarrow}[2]{
  % Vector perpendicular to side
  \path let \p1 = ($(#2)-(#1)$), \n1 = {veclen(\x1,\y1)}, 
             \p2 = ({-\y1/\n1}, {\x1/\n1}) % Unit normal vector (left-hand rule)
  in coordinate (Start) at ($(#1)!0.5!(#2) + ({\x2*\offset}, {\y2*\offset})$)
     coordinate (End) at   ($(#2)!0.5!(#1) + ({\x2*\offset}, {\y2*\offset})$);
  \draw[->, thick] (Start) -- (End);
}

% Triangle BCE (CCW: B -> C -> E)
\ccwarrow{B}{C}
\ccwarrow{C}{E}
\ccwarrow{E}{B}

% Triangle CDE (CCW: C -> D -> E)
\ccwarrow{C}{D}
\ccwarrow{D}{E}
\ccwarrow{E}{C}

% Triangle ABE (CCW: A -> B -> E)
\ccwarrow{A}{B}
\ccwarrow{B}{E}
\ccwarrow{E}{A}
        \end{tikzpicture}
    ...
    Since \(P\) is a closed polygonal chain, we can triangulate the interior as in .
\end{proof}
\end{document}