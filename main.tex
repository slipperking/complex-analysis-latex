...
\end{example}
\begin{proof}
    Although seemingly trivial at first glance, we are not guaranteed a uniform limit with respect to varying \(\theta\). Namely, for fixed \(\theta\) and \(\forall\varepsilon>0\), \(\exists\delta\) such that \(\forall 0<r<\delta\), we have \(\qty|f\qty(re^{i\theta})|>\varepsilon\). However \(\delta\) is a function of \(\theta\). If we can show that \(\delta=\delta(\theta)\) has a positive infimum of \(\delta'\), we can conclude that \(\lim_{z\to0}f(z)=\infty\). This is because we need to be able to select a \(\delta'\) neighborhood of \(0\) for a given \(\varepsilon\). If \(\delta(\theta)\) is continuous, the conclusion follows (since \([0,2\pi]\) is compact).

    Fix \(\theta\) and choose \(\delta_\theta\) such that \(\abs{f\qty(re^{i\theta})}>\varepsilon\) for all \(0<r<\delta_\theta\). By the continuity of \(f\) at \(re^{i\theta}\), \(\exists\eta>0\) such that \(\forall |\vartheta-\theta|<\eta\), 
\end{proof}
