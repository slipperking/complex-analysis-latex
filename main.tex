...
First, we will observe the result from image of \(\partial D(0,1)\). \(\forall\varepsilon<1\), let \(\abs{\zeta}=\varepsilon\). 
\[\abs{\varphi_z(\zeta)}=\abs{\overline{\zeta}}\abs{\frac{z-\zeta}{\overline{\zeta}-\overline{z}}}=\varepsilon.\]
Therefore, the image of \(\partial D(0,\varepsilon)\) lies on \(\partial D(0,\varepsilon)\), and therefore \(f\) maps \(D(0,1)\) to \(D(0,1)\). We next aim to show that \(f:D(0,1)\to D(0,1)\) is bijective.

Let us first confirm injectivity. \(\forall\zeta_1,\zeta_2\in D(0,1)\), and observe the results from \(\frac{z-\zeta_1}{}\)
