...
\end{example}
\begin{proof}
    We choose a branch of the logarithm that encloses the positive real axis, or the principal branch when \(-\pi<\Arg(z)\leq \pi\). Let \(f(z)=\frac{\Log\qty(z^2+1)}{z^2+1}\). The function has singularities at \(z=\pm i\). By the evenness of \(f\), we can consider a semicircular contour. Without loss of generality, choose the contour as in \autoref{fig:semicircularcontour}, made of \[\Gamma=\cbraces{r\in\mathbb{R}}{-R\leq r\leq R},\qquad C_R=\cbraces{Re^{i\theta}}{0\leq\theta\leq\pi},\] where \(R>1\). Without loss of generality, we can let \(R\ge2\). 
\end{proof}
