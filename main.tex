...
We will start by introducing the concept of \textit{infinite products}. Let \(\prod_{k=1}^n\qty(1+u_k)\) be an infinite product. For \(x\in\mathbb{R}_{\geq0}\), since \(e^x\geq x\) and \(e^0=1\), we can integrate over \([0,x]\) to get that \(e^x\geq x+1\). Therefore, \(\prod_{k=1}^n \qty(1+u_k)=\)
