...
For a region \(U\subseteq\mathbb{C}\) and a holomorphic function \(f:U\to\mathbb{C}\), a point \(z_0\in U\) is a \textit{zero} of \(f\) if and only if \(f\paren{z_0}=0\). Furthermore, if \(f\) has the Taylor expansion at \(z_0\) of \[a_m\paren{z-z_0}^m+a_{m+1}\paren{z-z_0}^{m+1}+\cdots,\quad\quad m\in\mathbb{N},\] then the zero at \(z_0\) has multiplicity \(m\).
...
\begin{theorem}[Argument Principle for Holomorphic Functions]\label{thm:argumentprincipleholomorphic}
    ...
    Let \(\Gamma\) be the image of \(\gamma\) under \(f\). Then, \(k=\frac{1}{2\pi}\Delta_\Gamma\arg(f)\), where \(\Delta_\Gamma\arg f=\Delta_{\gamma}\arg f(z)\) is the total change in argument as \(z\) traverses \(\gamma\).
\end{theorem}
\begin{proof}
    Let \(z_0,z_1,\ldots z_n\) be the zeros enclosed within \(\gamma\) (multiplicities counted as multiple zeros).
\end{proof}
