...
\end{theorem}
\begin{proof}
    Assume that \(U\) never attains its supremum \(M\). Then, \(\abs{f(x)}<M\). Define \(\psi(x)=\frac{1}{M-f(x)}\), which is positive and continuous as the denominator never reaches 0. By \autoref{thm:continuousfunctionboundedoncompact}, \(\psi(x)\) is bounded with some value of \(\mu>0\) satisfying \(\psi(x)<\mu\). \(f(x)\) also has the representation \(M-\frac{1}{\psi(x)}\), and therefore, \[f(x)<M-\frac{1}{\mu}\]
\end{proof}
