...
Assume that \(f:\mathbb{C}\to\mathbb{C}\) is an entire automorphism, or that \(f\in\Aut(\mathbb{C})\). 
...
Therefore, there exists a one-to-one correspondence between \(\Aut\qty(\riemannsphere)\) and the group (under matrix multiplication) of \[\cbraces{\mqty(a&b\\c&d)}{\det{\mqty(a&b\\c&d)}=1}/\qty{\pm I},\]
where \(I=\mqty(\imat{2})\). The quotient group (for a valid matrix \(A\), this group contains the cosets \(\qty{A, -A}\). The operation here for two cosets \(\cbraces{A,-A}\) and \(\cbraces{B,-B}\) is coset) is taken because \(\mqty(a&b\\c&d)\) refers to the same transformation as \(\mqty(-a&-b\\-c&-d)\). This group, denoted by \(\mathrm{PSL}(2,\mathbb{C})=\mathrm{SL}(2,\mathbb{C})/\qty{\pm I}\cong\Aut\qty(\riemannsphere)\), is known as the \textit{group of Möbius transformations}, consisting of all fractional linear transformations.
