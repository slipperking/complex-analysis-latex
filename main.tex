...
Let \(U\subset\mathbb{R}\) be open and let \(f(x,t)\) defined for \(x\in U\) and \(t\in[a(x),b(x)]\), where \(a(x)\), \(b(x)\) are differentiable functions on \(U\), and \(a(x)<b(x)\) for all \(x\in U\). Suppose that:
\begin{itemize}
    \item \(f(x,t)\) and \(\pdv{f}{x}(x,t)\) are continuous on the set \(\{(x,t) \in I \times \mathbb{R} \mid a(x) \leq t \leq b(x)\}\),
    \item \(a(x)\), \(b(x)\) are differentiable on \(I\).
\end{itemize}
Then the function \[F(x)=\int_{a(x)}^{b(x)}f(x,t)\dd{t}\] is differentiable on \(I\), and
