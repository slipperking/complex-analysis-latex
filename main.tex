...
\begin{equation}
    u\qty(e^{i\theta})\sim\sum_{n=-\infty}^\infty a_n e^{in\theta},\qquad a_n=\frac{1}{2\pi}\int_0^{2\pi} u\qty(e^{i\tau})e^{-in\tau}\dd{\tau}. \label{eq:poissonintegralformulafourierseries}
\end{equation}
...
\begin{example}[Poisson--Jensen Formula]\label{ex:poissonjensenformula}
    Let \(R>0\) be a positive and finite constant. Suppose \(f\) is a meromorphic function on the open disk \(D(0,R)\) such that \(f\) is not identically zero. Let \(a_1,\ldots,a_m\) and \(b_1,\ldots,b_n\) be the zeros and poles of \(f\), counted with multiplicity and order, respectively (multiplicities and orders count as multiple zeros or poles). Then it follows that \[\log\abs{f(z)}=\frac{1}{2\pi}\int_0^{2\pi}\log\abs{f\qty(Re^{i\theta})}P(z,\theta)\dd{\theta}-\sum_{j=1}^m\log||\]
\end{example}
...
If \(f(z)\) is a constant multiple of \(f(z+iy)\) for some \(y\in\mathbb{R}\), it is a strong indication to use a rectangular contour. If \(f(z)\) is a constant multiple of \(f\qty(ze^{i\tau})\) for some \(\tau\in\mathbb{R}\), a wedge-shaped contour is an appropriate choice.
...
\end{theorem}
\begin{proof}
    Let \(w_0\in\mathbb{C}\). First \(w_0\) does not lie on \(\Gamma\) itself. By the Argument Principle (\autoref{thm:argumentprincipleholomorphic}), the number of zeros of \(f-w_0\) enclosed by \(\gamma\) is equal to \[k=\frac{1}{2\pi i}\oint_\gamma \frac{f'(z)}{f(z)-w_0}\ddz=\frac{1}{2\pi i}\oint_\Gamma\frac{\dd{w}}{w-w_0}=\frac{1}{2\pi}\Delta_\Gamma\arg(w-w_0).\] If \(w_0\in\mathrm{ext}(\Gamma)\), the integral vanishes by the Cauchy--Goursat Integral Theorem (\autoref{thm:cauchygoursattheorem}). On the contrary, if \(w_0\in\mathrm{int}(\Gamma)\), then \(\Gamma\) winds around \(w_0\) exactly once, and hence, in other words, \(\forall w_0\in\mathrm{int}(\Gamma)\), \(f(z)=w_0\) has a unique solution in \(U\). This verifies the univalence of \(f\) in \(U\).
...
\begin{example}
    ...
    \begin{equation}
        w=e^{i\theta}\frac{z-a}{1-\overline{a}z},\quad a\in\mathbb{D},\theta\in\mathbb{R}.\label{eq:biholomorphismunitdiskautomorphism}
    \end{equation}
...
\begin{example}
    ...
    \begin{equation}
        w=e^{i\theta}\frac{z-a}{z-\overline{a}},\quad a\in\mathbb{H}^+,\theta\in\mathbb{R}.\label{eq:biholomorphismunitdisktoupperhalfplane}
    \end{equation}
\end{example}
\begin{proof}
    First assume \(z\in\mathbb{R}\). It follows that \[\abs{w}=\abs{e^{i\theta}\frac{z-a}{z-\overline{a}}}=1.\] Therefore, this transformation maps \(\mathbb{R}\) to \(\partial\mathbb{D}\). The inverse mapping is equal to \[z=\frac{w\overline{a}-ae^{i\theta}}{w-e^{i\theta}}.\]
    It can be 
\end{proof}
...
\begin{equation}
    \Gamma(z)=\int_0^\infty e^{-t}t^{z-1}\dd{t},\label{eq:gammafunction}
\end{equation} where \(z\in\mathbb{C}\). By letting \(z=x+iy\) where \(x,y\in\mathbb{R}\), we have \(\abs{e^{-t}t^{z-1}}=e^{-t}t^{x-1}\). Notice that for \(x>0\),
\begin{align*}
    \abs{\Gamma(x)}&=\int_0^1 e^{-t}t^{x-1}\ddt+\int_1^\infty e^{-t}t^{x-1}\ddt\\
    &\leq \int_0^1 t^{x-1}\ddt+\int_1^\infty e^{-t}t^{x-1}\ddt\\
    &=\frac{1}{x}+\int_1^\infty e^{-t}t^{x-1}\ddt.
\end{align*}
Since \(\int_1^\infty\frac{\ddt}{t^2}\) is convergent and \(\lim_{t\to\infty}\frac{e^{-t}t^{z-1}}{t^{-2}}=0\), then by comparison, the second integral is convergent.
...
\begin{align*}
    \Gamma(z+1)&=\int_0^\infty e^{-t}t^z\ddt\\
    &=-\eval{e^{-t}t^z}_0^\infty+z\int_0^\infty e^{-t}t^{z-1}\ddt.\\
    &=z\Gamma(z).
\end{align*}
Additionally, \[\Gamma(1)=\int_0^\infty e^{-t}\ddt=-\eval{e^{-t}}_0^\infty=1.\]
...
\end{align*}
Let us now analyze the difference \[\lim_{n\to\infty}\qty[\int_0^n e^{-t}t^{z-1}\ddt-f_n(z)]=\lim_{n\to\infty}\int_0^n e^{-t}t^{z-1}\qty[1-e^{t}{\qty(1-\frac{t}{n})}^n]\ddt.\]
\end{document}
