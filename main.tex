...
Assume \(f:\mathbb{C}\to\mathbb{C}\) is an entire automorphism (\(f\in\Aut(\mathbb{C})\)). Obviously, \(f(z)\) cannot be the constant function. Therefore, by Liouville's Theorem (\autoref{thm:liouville}), \(f\) is unbounded. Therefore, \(f\) cannot have a removable singularity at \(z=\infty\). Since \(f\) is an automorphism, \(\infty\) cannot be an essential singularity. It follows that \(\lim_{z\to\infty}f(z)=\infty\).
