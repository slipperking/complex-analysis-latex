...
We will now observe the convergence of power series. Let \(D(a,r)=\cbraces{z\in\mathbb{C}}{\abs{z-a}<r}\) denote the \textit{open disk}.
\begin{theorem}[Abel's Theorem]\label{thm:abelradius}
    ...
    \begin{enumerate}
        \item \(f(z)\) absolutely converges for all \(\abs{z}<R\), and \(\forall 0\leq\rho<R\), uniformly converges on \(\abs{z}\leq\rho\).\label{itm:abelradius_absoluteanduniformconvergence}
