...
\end{example}
\begin{proof}
    By the Blaschke condition, it is satisfied that \(\lim_{k\to\infty}1-\abs{a_k}\to 0\). In other words, \(\forall 0<R<1\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(1-\abs{a_n}<1-R\), or that \(\abs{a_n}>R\). If we can prove that \[\sum_{k=1}^\infty\abs{\frac{\abs{a_k}}{a_k}\frac{z-a_k}{1-\overline{a_k}z}-1}\] locally uniformly converges, we can use \autoref{lemma:infiniteproductlocallyuniformconvergencecriterion2} to show that the infinite product converges uniformly on compact subsets of \(\mathbb{D}\). Let \(K\subset\mathbb{D}\) be an arbitrary compact subset. 
\end{proof}
