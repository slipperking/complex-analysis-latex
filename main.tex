...
\begin{proof}
    ...
    \[\Delta^1\geq\frac{M}{4},\]
    and recursively, choose \(\Delta^2\geq\frac{M}{4^2},\ldots,\Delta^n\geq\frac{M}{4^n}\). Let \(L\) denote the perimeter of \(\Delta\). Then, the perimeters of \(\Delta_1,\Delta_2,\ldots\) are \(\frac{L}{2},\frac{L}{2^2},\ldots\) respectively. As \(n\to\infty\), \(\Delta_n\) shrinks to a single point \(z_0\). Then, \(\forall n\in\mathbb{N}\), \(z_0\in\Delta_n\). 

    By the definition of holomorphy, \(\forall\varepsilon>0\), \(\exists\delta>0\) such that \(\forall z\in D\paren{z_0,\delta}\), \[\abs{\frac{f\paren{z}-f\paren{z_0}}{z-z_0}-f'\paren{z_0}}<\varepsilon,\] and \(\exists N\in\mathbb{N}\) such that \(\forall n\in \mathbb{N}_{>N}\), \(\Delta_n\subset D\paren{z_0,\delta}\).
\end{proof}
\end{document}