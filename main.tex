...
\begin{proof}
    ...
    \item Under the partial fraction decomposition of equation~\eqref{eq:cauchygoursatformulaexteriorpart2}, we get that \begin{align*}
    ...
    We will analyze the first integral as \(R\to\infty\): \[\qty|\int_0^{2\pi}\qty(f\qty(Re^{i\theta})-\frac{f\qty(Re^{i\theta})}{1-\frac{z}{Re^{i\theta}}})\dd{\theta}|\leq \int_0^{2\pi}\qty|\qty(f\qty(Re^{i\theta})-\frac{f\qty(Re^{i\theta})}{1-\frac{z}{Re^{i\theta}}})|\dd{\theta}\]
    \end{enumerate}
\end{proof}
