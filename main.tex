...
\end{gather}
Therefore, we can rewrite equation~\eqref{eq:infiniteproductweierstrassfactorizationintermediate} as \[\prod_{n=2}^\infty E_{k_n}\qty(\frac{z}{a_n}).\]
...
\end{align*}
By the definition of \(\qty{k_n}_{n\in\mathbb{N}}\), the series \(2\sum_{k=1}^\infty\abs{\frac{R}{a_n}}^{k_n+1}\) converges. Therefore, \(\sum_{k=1}^\infty Q_{k_n}\qty(\frac{z}{a_n})\) is uniformly and absolutely convergent on \(\overline{D(0,R)}\) by the Weierstrass \(M\)--Test (\autoref{thm:weierstrassmtest}).
