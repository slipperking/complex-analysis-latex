...
\begin{proof}
    ...
    \end{equation} is uniformly convergent with respect to \(\zeta\). By the boundedness of \(f\) on \(\gamma_1\) and \(\gamma_2\) by holomorphy, by the Weierstrass \(M\)--Test, Therefore, we get that \[f(z)=\frac{1}{2\pi i}\qty(\sum_{n=0}^\infty\int_{\gamma_2}\frac{(z-a)^n}{(\zeta-a)^{n+1}}f(\zeta)\ddzeta+\sum_{n=0}^\infty\int_{\gamma_1}\frac{(\zeta-a)^n}{(z-a)^{n+1}}f(\zeta)\ddzeta).\] 
