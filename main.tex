...
\begin{proof}
    ...
    Conversely, \(\qty{a_n}\) is bounded (fixing \(N\), \(\forall n>N\), \(\abs{a_n-a_{N+1}}<\varepsilon\)). By the Bolzano--Weierstrass Theorem (\autoref{thm:bolzanoweierstrass}), \(\qty{a_n}\) has a convergent subsequence \(\qty{a_{n_k}}_{k\in\mathbb{N}}\) converging to \(a_\infty\). Therefore, \(\forall\varepsilon>0\), \(\exists N\in\mathbb{N}\) such that \(\forall n,n_k>N\), \(\abs{a_n-a_{n_k}}<\frac{\varepsilon}{2}\) and \(\abs{a_{n_k}-a_\infty}<\frac{\varepsilon}{2}\). 
\end{proof}
