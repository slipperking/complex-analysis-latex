...
\begin{lemma}
    Let \(\Omega\subseteq\mathbb{C}\) be an open set and \(\mathfrak{B}\) be an (open) basis of \(\Omega\). Then there exists a sequence of sets \(U_1,U_2,\cdots\in\mathfrak{B}\) such that \begin{enumerate}
        \item \(\bigcup_{n\in\mathbb{N}}U_n=\Omega\).
        \item \(\forall K\subset\Omega\) that is compact, \(K\) intersects only a finite number of sets in \(\mathfrak{B}\).
    \end{enumerate}
\end{lemma}
