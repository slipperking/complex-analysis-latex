...
\begin{proof}
    ...
    Let \(E=\cbraces{z\in U}{\forall n\in\mathbb{Z}_{\geq0}, f^{(n)}(z)=0}\). \(\forall z\in D\paren{z_\infty,\varepsilon}\), since \(f(z)\) locally vanishes (and has vanishing derivatives as a consequence), \(D\paren{z_\infty,\varepsilon}\subseteq E\). Furthermore, \(\forall z'\in E\), \(\exists\varepsilon'>0\) such that \(f(z)\) has a convergent Taylor Series with vanishing coefficients on \(D(z',\varepsilon')\subseteq U\). Then \(f\equiv0\) on \(D\paren{z',\varepsilon'}\). Then \(\forall z\in D(z',\varepsilon')\), since \(f\) is constant at \(z\), it also has vanishing derivatives. It follows that \(D(z',\varepsilon')\subseteq E\). Since every point in \(E\) has an open neighborhood also in \(E\), \(E\) is open.

    \(\forall k\in\mathbb{Z}_{\geq0}\), \(f^{(k)}\) is continuous. 
\end{proof}
\begin{remark}
