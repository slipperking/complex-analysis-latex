...
\begin{proof}
    ...
    As are integrals defined with limits, we aim to prove that \(\forall\xi=Re^{i\vartheta}\in\partial D(0,R)\), \[\lim_{\substack{z\to\xi\\z\in D(0,R)}}u(z)=\varphi(\xi).\]
    ...
    For a constant harmonic function identically equal to 1, we get \(1=\int_0^{2\pi}P(\zeta,z)\dd{\tau}\) from equation~\eqref{eq:poissonintegralformula2}.
\end{proof}
