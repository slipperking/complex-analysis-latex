...
\begin{definition}[Epsilon-Delta]\label{def:epsilondelta}
    ...
    Likewise, the **left-hand limit** \(\lim_{x\to a^-}f(x)=L\) exists if for every \(\varepsilon>0\), there exists \(\delta>0\) such that for all \(x\in U\) with \(-\delta<x-a<0\), we have \(|f(x)-L|<\varepsilon\).
\end{definition}
