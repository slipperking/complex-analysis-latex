...
In the theory of partial differential equations, an important problem is to find a function continuous on \(\overline{\Omega}\) that solves Laplace's equation on \(\Omega\) that is identically equal to a given continuous function on \(\partial\Omega\). 

The real-valued solution \(u\in C^0\paren{\overline{D(0,R)}}\) that solves \(\Delta u=0\) on \(D(0,R)\) and \(u\equiv\varphi\) on \(\partial D(0,R)\), where \(\varphi\in C^0\paren{\partial D(0,R)}\) is:
\[u(z)=\int_0^{2\pi}\varphi(\zeta)P(\zeta,z)\dd{\tau}.\]
We will now verify this seemingly trivial solution:
...
More precisely, for a continuous function \(\varphi\in C^0(\partial D(0,R))\), we aim to find a real-valued solution \(u\in C^0\qty(\overline{D(0,R)})\) solving:
\[\Delta u(z)=0\quad\forall z\in D(0,R),\qquad u(z)=\varphi(z) \quad\forall z\in\partial D(0,R).\]
The unique solution is given by the Poisson integral formula:
\[u(z)=\int_0^{2\pi}\varphi(\zeta)P(\zeta, z)\dd{\tau},\]
where \(\zeta=Re^{i\tau}\).
