...
\begin{proof}
    ...
    Assume that there exists an infinite open cover \(\mathcal{U}\) of \(X\) without finite subcovering. Bisect $Q$ into $2^{2n}$ sub-boxes. Choose $Q_1$ such that $Q_1\cup X$ has no finite subcover of $\mathcal{U}$. Under the previous assumptions, this is possible since if every $\text{sub-box}\cap X$ had finite subcovering, then $Q\cap X=X$ would have finite subcovering. Similarly, choose $Q_2$ by bisecting $Q_1$ in a similar way, and recursively obtain a sequence of boxes:
    \[Q\supset Q_1\supset Q_2\supset\ldots\]
\end{proof}
