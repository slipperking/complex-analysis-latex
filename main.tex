...
For every positive and finite \(R\), there exists a positive integer sequence \(k_1,k_2,\ldots\) such that \(\sum_{n=1}^\infty{\qty(\frac{R}{\abs{a_n}})}^{k_n+1}\) converges. For example, let \(k_n=n-1\), and for sufficiently large \(n\), \(\frac{R}{\abs{a_n}}<1\) and the series is convergent.
