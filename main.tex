...
\begin{definition}[Basis of a Topology]
    For a topological space \((X,\tau)\), a \textit{basis} \(\mathcal{B}\) is a collection of subsets of \(X\) such that:
    \begin{enumerate}
        \item \(\forall x\in X\), \(x\in\bigcup_{U\in\tau} A\).
        \item A set \(U\) is in \(\tau\) if and only if for every \(x\in U\),  there exists \(B\in\mathcal{B}\) such that \(x\in B\).
    \end{enumerate}
\end{definition}
\begin{lemma}
