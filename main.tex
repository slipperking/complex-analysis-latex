...
A univariate function \(f(z)\) formed by taking compositions, sums, products, and powers of finitely many functions of the following form:
\begin{enumerate}
    \item Power functions in the form of \(z^w\), \(w\in\mathbb{C}\).
    \item Exponential functions in the form of \(e^z\).
    \item Logarithmic functions in the form of \(\ln z\).
\end{enumerate}
Trigonometric functions such as \(\sin(z)\), \(\cos(z)\), and \(\tan(z)\), and hyperbolic functions such as \(\sinh(z)\), \(\cosh(z)\), and \(\tanh(z)\) are also elementary functions as they can be written in terms of exponentials. Their inverses can also be written in logarithms. The complex logarithm, denoted \(\log(z)\)
