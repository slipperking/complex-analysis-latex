...
We now examine the regions over which \(\sin\) and \(\cos\) are univalent. We begin with the cosine function, expressed as \(\cos(z)=\frac{e^{iz}+e^{-iz}}{2}\). Define the auxiliary functions \[\xi(z)=iz,\quad\zeta(\xi)=e^\xi,\quad w(\zeta)=\frac{\zeta+\flatfrac{1}{\zeta}}{2}.\] Then, \(\cos(z)=(w\circ\zeta\circ\xi)(z)\).
...
Combining these constraints, we conclude that \(\cos(z)\) is univalent on any vertical strip in the complex plane of width \(\pi\), that is, a region of the form \[\{z\in\mathbb{C}\mid k\pi<\Re(z)<(k+1)\pi\},\quad k\in\mathbb{Z}.\] Let us now consider the specific region \(\{z\in\mathbb{C}\mid 0<\Re(z)<\pi\}\), and analyze how it is mapped under \(\cos(z)\).

\begin{enumerate}
    \item \(\xi(z)=iz\) maps the region \(\{z\mid 0<\Re(z)<\pi\}\) to the horizontal strip \(\{\xi\in\mathbb{C}\mid 0<\Im(\xi)<\pi\}\).
    \item \(\zeta(\xi)=e^\xi\) maps this horizontal strip conformally onto the open annulus \(\{\zeta\in\mathbb{C}\mid 1<|\zeta|<e^{\pi}\}\), since \(\Re(\xi)\) is unbounded and \(\Im(\xi)\in(0,\pi)\), so \(|\zeta|=e^{\Re(\xi)}\), and \(\arg(\zeta)=\Im(\xi)\in(0,\pi)\).
    \item \(w(\zeta)=\frac{\zeta+\frac{1}{\zeta}}{2}\) maps this annulus onto the complex plane minus the real interval \([-1,1]\), forming a slit plane. This mapping is known to be univalent on such annuli not symmetric under inversion.
\end{enumerate}

Thus, the composition \(\cos(z)=w\circ\zeta\circ\xi\) is univalent on the strip \(\{z\in\mathbb{C}\mid 0<\Re(z)<\pi\}\), and the image of this strip under \(\cos\) is the complex plane with a slit along the real interval \([-1,1]\) removed.
