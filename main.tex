...
\begin{proof}
    ...
    \]
    Since \(\varphi\) is continuous on a compact set \(\partial D(0,R)\), it is bounded by \autoref{lemma:uniformcontinuousovercompactset}, or \(M=\sup_{\qty|\zeta|=R}\abs{\varphi(\zeta)}\) that is finite, and thus, \(\abs{\varphi\qty(Re^{i\vartheta})-\varphi(\zeta)}<2M\).
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \draw[thin] (z) arc[start angle=-100, end angle=62.35, radius=2];
            ...
            \node[anchor=east] at ([yshift=-5pt, xshift=-2pt] $(z)!0.5!(zeta)$) {\(|\zeta-z|\)};
        \end{tikzpicture}
        \caption{\(\zeta\), \(\xi\), and \(z\) when \(\abs{\vartheta-\tau}>\delta\), with angles and distances marked.}
        \label{fig:dirichletproblemwithlaplaceequationsolution_secondintegral}
    \end{figure}

    Since \[P(\zeta,z)=\frac{R^2-\rho^2}{2\pi\qty|\zeta-z|^2},\]
    where \(\zeta=Re^{i\tau}\) and \(z=\rho e^{i\theta}\), assume that \(\abs{\vartheta-\tau}>\delta\). Then \(\exists\eta>0\) such that when \(|\xi-z|<\eta\), \(\abs{\theta-\tau}>\delta\) and \(\rho>\frac{R}{2}\). Then \(|\zeta-z|^2>4\rho^2\sin[2](\frac{\delta}{2})>R^2\frac{1-\cos[2](\delta)}{2}\).
\end{proof}
