...
\begin{proof}
    ...
    \item Under the partial fraction decomposition of equation~\eqref{eq:cauchygoursatformulaexteriorpart2}, we get that \[I=\int_\gamma\frac{zf(\zeta)}{z\zeta-\zeta^2}\ddzeta=\int_\gamma\qty(\frac{f(\zeta)}{\zeta}-\frac{f(\zeta)}{\zeta-z})\ddzeta=\frac{1}{2\pi i}\int_\gamma\frac{f(\zeta)}{\zeta-z}\ddzeta=\lim_{R\to\infty}\int_0^{2\pi}\frac{f\qty(Re^{i\theta})}{1-\frac{z}{Re^{i\theta}}}\dd{\theta}-\begin{cases}
        0&z\in\mathrm{int}(\gamma)\\
        f(z)&z\in\mathrm{ext}(\gamma)
    \end{cases}\]
    \end{enumerate}
