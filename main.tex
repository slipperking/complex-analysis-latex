...
\begin{enumerate}
    ...
    \item If \(\lim_{z\to a}f(z)=\infty\), then \(z=a\) is a \textit{pole} of \(f\) (from the stereographic projection, the values of \(+\infty\) and \(-\infty\) in the extended real number line represent approaching the \(\infty\) point from different directions, analogous to \(0^-\) and \(0^+\)).
\end{enumerate}
