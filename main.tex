...
\end{definition}
If a topological space \(X\) can be written as the union between two nonempty disjoint open sets, then it is \textit{disconnected}.
...
\end{theorem}
\begin{proof}
    Suppose that \(X\) is connected. For the sake of contradiction, suppose that \(U\subseteq X\) is clopen such that \(U\neq\emptyset\) and \(U\neq X\). Notice that \(U\) and \(X\setminus U\) are both open in \(X\) by clopenness and are disjoint from each other. Since \(X=U\cup\qty(X\setminus U)\), it follows that \(X\) is disconnected.
...
\begin{theorem}\label{thm:meanvaluepropertysolutionsareharmonic}
    Let \(U\subseteq\mathbb{C}\) be open. Let \(f(z):U\to\mathbb{R}\) be continuous. \(\forall z_0\in U\), \(\exists\varepsilon>0\) such that \(\overline{D\qty(z_0,\varepsilon)}\subseteq U\), and if \(\forall0<\varepsilon'<\varepsilon\), \[f(z_0)=\frac{1}{2\pi}\int_0^{2\pi}f\qty(z_0+\varepsilon'e^{it})\ddt,\]
