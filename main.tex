...
\[z-\overline{a}zw=a-w\Longleftrightarrow w=\frac{a-z}{1-\overline{a}z}.\]
...
\begin{proof}
    ...
    Then, \(\abs{\psi'(0)}=1\), and by the equality statement of \autoref{lemma:schwarz}, \(\psi(z)=ze^{i\tau}=\rho_\tau(z)\) for some constant \(\tau\in\mathbb{R}\), and \(f(z)=\varphi_{f(0)}^{-1}\circ\)
\end{proof}
