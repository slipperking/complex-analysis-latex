...
\begin{theorem}[The Holomorphic Automorphism Group on \(\mathbb{C}\)]\label{thm:holomorphicautomorphismgrouponunitdisk}
    \(\forall f\in \Aut(\mathbb{C})\), \(f\) is linear and non-constant. In other words, \(\exists a\in\mathbb{C}^*\) and \(\exists\tau\in\mathbb{R}\) such that \[f(z)=\varphi_a\circ\rho_\tau(z).\]
\end{theorem}
Let \(f:\mathbb{C}\to\mathbb{C}\) be an entire automorphism, or that \(f\in\Aut(\mathbb{C})\). First, assume that \(\infty\) is not an essential singularity. Then \(\infty\) must be a pole by trichotomy, as a removable singularity implies boundedness (\autoref{lemma:removablesingularityatinftyentireconstant}). Therefore, \(f(z)\) is a polynomial of degree \(m\), where \(m\in\mathbb{N}\).
...
Therefore, any holomorphic automorphism on \(\riemannsphere\) is a composition of rotations, dilations, translations, and inversions.
