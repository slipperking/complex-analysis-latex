...
\end{equation}
Let \(\phi(z)=\Phi\circ\varphi_a(z)\), which is also harmonic on \(D(0,1)\). By the univalence of \(\varphi_a\), \(z=e^{i\tau}\) is mapped bijectively to \(e^{i\theta}\) with \(e^{i\theta}=\varphi_{-a}\qty(e^{i\tau})\).
