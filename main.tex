...
\end{gather}
Since \(u\qty(z)\) is continuous on \(\partial\mathbb{D}\) and \(u\circ\exp(i\theta)\) is periodic over \(2\pi\), it can be expressed as a Fourier series,
\begin{equation}
    u\qty(e^{i\theta})\sim\sum_{n=-\infty}^\infty a_ne^{in\theta},\qquad a_n=\frac{1}{2\pi}\int_{-\pi}^\pi u\qty(e^{i\tau})e^{-in\tau}\dd{\tau}=\frac{1}{2\pi}\int_0^{2\pi} u\qty(e^{i\tau})e^{-in\tau}\dd{\tau}.\label{eq:poissonintegralformulafourierseries}
\end{equation}
This series is not necessarily convergent. Notice that \(u\) is continuous on a compact set and therefore is uniformly bounded by some \(M\) (\autoref{thm:continuousfunctionboundedoncompact}). Therefore, the integrand of \(a_n\) is bounded by a finite \(M\geq\abs{u\qty(e^{i\theta})e^{-in\theta}}\). It follows that \(\qty|a_n|\leq M\). By introducing factors of powers of \(\abs{\rho}<1\) and a change of variables, we obtain the convergent sum: \[\sum_{n=-\infty}^\infty a_ne^{in\theta}\rho^{|n|},\qquad\abs{\sum_{n=-\infty}^\infty a_ne^{in\theta}\rho^{|n|}}\leq\sum_{n=-\infty}^\infty\abs{a_ne^{in\theta}\rho^{|n|}}\leq M\frac{1+|\rho|}{1-\abs{\rho}}.\]
By substituting the coefficients, we have \begin{align*}
    \sum_{n=-\infty}^\infty a_n e^{in\theta}\rho^{|n|}&=\sum_{n=-\infty}^\infty \qty(\frac{1}{2\pi}\int_0^{2\pi}u\qty(e^{i\tau})e^{-in\tau}\dd{\tau})e^{in\theta}\rho^{|n|}\\
    &=\frac{1}{2\pi}\sum_{n=-\infty}^\infty\int_0^{2\pi}\rho^{|n|}u\qty(e^{i\tau})e^{in(\theta-\tau)}\dd{\tau}.
\end{align*}
By the Weierstrass \(M\)--Test (\autoref{thm:weierstrassmtest}) and \autoref{thm:limitintegralswitch}, we have
\begin{equation}
    \frac{1}{2\pi}\sum_{n=-\infty}^\infty\int_0^{2\pi}\rho^{|n|}u\qty(e^{i\tau})e^{in(\theta-\tau)}\dd{\tau}=\frac{1}{2\pi}\int_0^{2\pi}u\qty(e^{i\tau})\sum_{n=-\infty}^\infty\rho^{|n|}e^{in(\theta-\tau)}\dd{\tau}.\label{eq:poissonintegralformulafourierseriespostintegralsummationswitch}
\end{equation}
This summation can be further simplified:
\begin{align*}
    \sum_{n=-\infty}^\infty\rho^{|n|}e^{in(\theta-\tau)}&=\sum_{n=0}^\infty\rho^n e^{in(\theta-\tau)}+\sum_{n=1}^\infty\rho^n e^{-in(\theta-\tau)}\\
    &=1+2\sum_{n=1}^\infty\rho^n\cos[n(\theta-\tau)]\\
    &=1+2\Re\sum_{n=1}^\infty\rho^n e^{in(\theta-\tau)}\\
    &=1+2\Re\qty[\frac{\rho\exp[i(\theta-\tau)]\qty(1-\rho\exp[-i(\theta-\tau)])}{\qty(1-\rho\exp[i(\theta-\tau)])\qty(1-\rho\exp[-i(\theta-\tau)])}]\\
    &=1+2\qty[\frac{\rho\Re\qty[e^{i(\theta-\tau)}]-\rho^2}{1+\rho^2-2\rho\cos(\theta-\tau)}]\\
    &=\frac{1-\rho^2}{1+\rho^2-2\rho\cos(\theta-\tau)}.
\end{align*}
Substituting this into equation~\eqref{eq:poissonintegralformulafourierseriespostintegralsummationswitch}, we have that \[\sum_{n=-\infty}^\infty a_n e^{in\theta}\rho^{|n|}=\frac{1}{2\pi}\int_0^{2\pi}\frac{\qty(1-\rho^2)u\qty(e^{i\tau})}{1+\rho^2-2\rho\cos(\theta-\tau)}\dd{\tau}=u\qty(\rho e^{i\theta}).\]
Additionally by the proof of \autoref{thm:dirichletproblemwithlaplaceequationsolution} (specifically the proof of equation~\eqref{eq:dirichletproblemwithlaplaceequationsolution_limittoboundary}), we have \[\lim_{\rho\to1^-}\sum_{n=-\infty}^\infty a_ne^{in\theta}\rho^{\abs{n}}=u\qty(e^{i\theta}).\]
Therefore, for any arbitrary continuous function on \(\partial\mathbb{D}\), its Fourier series is \textit{Abel summable} as \(\rho\to1^-\).

Finally, we will demonstrate that real-valued continuous functions satisfying the mean-value property are equivalent to being a solution to the Laplace equation. In \autoref{lemma:holomorphicmeanvalueproperty}, we showed that real solutions to the Laplace equation satisfy the mean-value property. Now we will prove the converse.
...
\end{lemma}
\begin{proof}
    From simple substitution, we have \begin{align*}
...
\begin{proof}
    ...
    since \(y>0\). Therefore, \(\varphi\) maps \(\mathbb{H}^+\) to \(\mathbb{D}\). Since \(\varphi\in\Aut\qty(\extcomplex)\), it is univalent. Furthermore, \(\varphi^{-1}(z)=i\frac{1+z}{1-z}\) is also univalent. Let \(\abs{z}<1\) be arbitrary. Then, \begin{align*}
