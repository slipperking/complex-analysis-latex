...
\begin{theorem}[Looman–Menchoff Theorem]
    The necessary and sufficient criterion for a function \(f(z)\) to be holomorphic on a region \(D\) is if it is continuous over \(D\), \(f(x,y)\) is differentiable over \(D\setminus N\), where \(N\) is a countable set, and satisfy the Cauchy--Riemann equations.
\end{theorem}
