...
Let \(\qty{z_k}_{k=1}^n\subset\mathbb{C}\) be a sequence of distinct complex numbers and \(\qty{w_k}_{k=1}^n\subset\mathbb{C}\) be a sequence of complex numbers. We can then construct a polynomial \(f(z)\) such that \(\forall k\in\qty{1,\ldots,n}\), \(f(z_k)=w_k\). One such explicit formula is given by the \textit{Lagrange interpolation formula}:
