...
\end{example}
\begin{proof}
    Since \(z^5-1=\prod_{k=0}^4\qty(z-\omega_k)\), where \(\omega_k=e^{i\pi\frac{k}{5}}\), the integrand has singularities at every fifth root of unity. We can pentasect \(D(0,2)\) into 5 sectors each containing only one singularity (without any singularities on their boundaries). By the Cauchy--Goursat Integral Formula (\autoref{thm:cauchygoursatformula}), the integral over each sector is equal to the integral over a small disk with radius \(\varepsilon<2\sin\qty(\frac{\pi}{5})\) centered around the singularity: \[\sum_{k=0}^4 \int_{D(\omega_k,\varepsilon)}\frac{\ddz}{\prod_{j=0}^4\qty(z-\omega_j)}=\sum_{k=0}^4 \int_{D(\omega_k,\varepsilon)}\sum_{j=0}^4\frac{\ddz}{\prod_{j=0}^4\qty(z-\omega_j)}.\]
\end{proof}
