...
\begin{lemma}[Existence of Bump Functions]\label{thm:bumpfunctionexistence}
    For any open set \(U\subset\mathbb{C}\) and a compact set \(K\subset U\), there exists a bump function \(\phi\in C^\infty(\mathbb{C})\) with support \(\supp(\phi)\subseteq\overline{U}\) such that \(\phi\) is identically 1 over \(K\).
\end{lemma}
