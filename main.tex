...
\begin{theorem}[Green's Theorem]\label{theorem:realgreen}
    Let \(\partial R\) be a positively oriented, piecewise smooth, simple closed curve embedded in $\mathbb{R}^2$, and let \(R\) be the positively oriented region bounded by \(\partial R\). For \(P(x,y),Q(x,y)\in C^1\left(\overline{R}\right)\),
...
\begin{theorem}[Gauss' Theorem]\label{theorem:divergencegauss}
    ...
    \[\iiint_V  = \iint_S \vec{F} \cdot d\vec{S}.\]
\end{theorem}
