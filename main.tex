...
\begin{definition}[Residue]\label{def:residue}
    For some \(r\in\mathbb{R}_{>0}\), \(a\in U\), suppose \(f:D^*(a,r)\to \mathbb{C}\) is holomorphic. Then the \textit{residue} of \(f\) at \(a\), denoted by \(\residue_{z=a}f(z)\), is equal to
    \begin{equation}
...
Multiplying by \((z-a)^{m}\), we obtain that \[(z-a)^mf(z)=c_{-m}+c_{1-m}(z-a)+\cdots+c_{-1}(z-a)^{m-1}+\cdots.\]
By the definition of a Taylor series, we find that
\begin{equation}
    c_{-1}=\residue_{z=a}f(z)=\frac{1}{(m-1)!}\left.\dv[m-1]{z}((z-a)^mf(z))\right|_{z=a}.\label{eq:residueatpole}
\end{equation}
...
\begin{theorem}[Residue Theorem]\label{thm:residuethm}
    Let \(U\subset\mathbb{C}\) be an open set with a simple closed boundary \(\partial U\). Suppose \(\cbraces{z_n}\subset U\) is a finite set and \(f(z)\) is holomorphic on \(U\setminus\cbraces{z_n}\) and continuous on \(\overline{U}\setminus\cbraces{z_n}\). Then, \[\int_{\partial U}f(z)\ddz=2\pi i\sum_{k=1}^n\residue_{z_k}(f)\]
\end{theorem}
\begin{proof}
...
\begin{proof}
    ...
    \end{figure}Let \(f(z)=\frac{e^{iz}}{z}\). Consider a closed contour \(\gamma\) in the form of \autoref{fig:indentedsemicircularcontour}, consisting of a semicircle of radius \(R\) in the upper half-plane \(\mathbb{H}\) (call this \(\gamma_R\)), a line segment from \(-R\) to \(-\varepsilon\), a smaller semicircle of radius \(\varepsilon\) in the upper half-plane (call this \(\gamma_\varepsilon\)), and a line segment from \(\varepsilon\) to \(R\).

    By the Cauchy--Goursat Integral Theorem (\autoref{thm:cauchygoursattheorem}), we have that \[\int_{\gamma}f(z)\ddz=\int_{\gamma_R}f(z)\ddz+\int_{-R}^{-\varepsilon}f(z)\ddz+\int_{\gamma_\varepsilon}f(z)\ddz+\int_{\varepsilon}^{R}f(z)\ddz=0.\]
    We will now analyze each integral. The first integral is \[\int_{\gamma_R}f(z)\ddz=\int_0^\pi\frac{\exp(iRe^{i\theta})}{Re^{i\theta}}Rie^{i\theta}\dd{\theta}=i\int_0^{\pi}e^{iR\cos\theta}e^{-R\sin\theta}\dd{\theta}.\]
    ...
    \end{align*}
    Let us evaluate the integral on \(\gamma_\varepsilon\) as \(\varepsilon\to 0\): \[\int_{\gamma_\varepsilon}f(z)\ddz=i\int_{\pi}^0\exp(\varepsilon\qty(i\cos\theta-\sin\theta))\dd{\theta}=i\int_\pi^0e^{-\varepsilon\sin\theta}e^{i\varepsilon\cos\theta}\dd{\theta}.\]
    Obviously, \[\abs{e^{-\varepsilon\sin\theta}e^{i\varepsilon\cos\theta}}\leq 1,\]
    and therefore, the integral and the limit may commute:
    \[\lim_{\varepsilon\to 0}\int_{\gamma_\varepsilon}f(z)\ddz=i\int_{\pi}^0\lim_{\varepsilon\to 0^+}e^{-\varepsilon\sin\theta}e^{i\varepsilon\cos\theta}\dd{\theta}=i\int_{\pi}^0\dd{\theta}=-i\pi.\]
...
\begin{proof}
    \begin{figure}
        ...
        \begin{tikzpicture}[>=stealth,
            ...
            \node[anchor=north] at (3.6,1.5) {\(\gamma_3\)};
            \node[anchor=north] at (3.5,0) {\(R\)};
            \node[anchor=south] at (2.5,2.5) {\(R\)};
            \node[anchor=west] at (0.65,0.35) {\(\tfrac{\pi}{4}\)};
        \end{tikzpicture}
        \caption{A wedge contour with orientation marked.}\label{fig:wedgecontour}
    \end{figure}Let \(f(z)=e^{iz^2}\). Choose the wedge contour composed of \[\gamma_1=\cbraces{r\in\mathbb{R}}{0\leq r\leq R},\quad\gamma_2=\cbraces{re^{i\frac{\pi}{4}}}{0\leq r\leq R},\quad\gamma_3=\cbraces{Re^{i\theta}}{0\leq\theta\leq\frac{\pi}{4}}\] as in \autoref{fig:wedgecontour}. By the Cauchy--Goursat Integral Theorem (\autoref{thm:cauchygoursattheorem}), we have that 
    \begin{equation}
        \int_{\gamma_1}f(z)\ddz+\int_{\gamma_2}f(z)\ddz+\int_{\gamma_3}f(z)\ddz=0.\label{eq:fresnelwedgecontourintegral}
    \end{equation} The third integral can be written as \[\int_{\gamma_3}f(z)\ddz=Ri\int_0^{\frac{\pi}{4}}e^{i{\qty(Re^{i\theta})}^2}e^{i\theta}\dd{\theta}.\]
    Using the fact that \(\frac{4}{\pi}\theta<\sin(2\theta)\) on the integration range, it can be bounded as \[\abs{\int_{\gamma_3}f(z)\ddz}\leq R{\int_0^{\frac{\pi}{4}}e^{-R^2\sin(2\theta)}\dd{\theta}}<R\int_0^{\frac{\pi}{4}}e^{-\frac{4}{\pi}R^2\theta}\dd{\theta}=-\frac{\pi}{4R}\eval{e^{-\frac{4}{\pi}R^2\theta}}_0^{\frac{\pi}{4}}=\frac{\pi}{4R}\qty(1-e^{-R^2}).\]
...
\end{proof}
\begin{example}
    Evaluate \(\int_0^\infty\frac{x^\alpha}{1+x^\beta}\ddx\), where \(0<\alpha+1<\beta\).
\end{example}
\begin{proof}
    Let \(f(z)=\frac{z^\alpha}{1+z^\beta}\) and let \(-\pi<\Arg(z)\leq \pi\) in the principal branches of \(z^\alpha=e^{\alpha\Log(z)}\) and \(z^\beta=e^{\beta\Log(z)}\). Then except for at the zeros of \(1+z^\beta\), \(f\) is
\end{proof}
\begin{example}
...
where \(z\in\mathbb{C}\). By letting \(z=x+iy\) where \(x,y\in\mathbb{R}\), 
\end{document}
