...
\end{theorem}
\begin{proof}
    Let \(z_1,z_2,\ldots,z_n\) be the zeros enclosed within \(\gamma\), with multiplicities \(k_1,k_2,\ldots,k_n\) respectively. There exist disks centered around each zero with radii \(\varepsilon_1,\ldots,\varepsilon_n\) respectively, such that each disk is disjoint and contained in the interior of \(\gamma\). The function \[\frac{f'(z)}{f(z)}\] is holomorphic over \[\setminus\bigcup_{k=1}^n D\paren{z_k,\varepsilon_k}.\]
    This region has the boundary of \(\)
\end{proof}
