...
Let \(f(x,t)\) be a function defined on a domain \(U=[a(x), b(x)]\times[c,d]\subset\mathbb{R}^2\), where \(x\) is a parameter and \(t\) is the variable of integration. Suppose:
\begin{itemize}
    \item \(f(x,t)\) and \(\pdv{f}{x}(x,t)\) are continuous on \(U\),
    \item \(a(x)\), \(b(x)\) are differentiable functions of \(x\).
\end{itemize}
Then the function \[F(x)=\int_{a(x)}^{b(x)}f(x,t)\dd{t}\] is differentiable with:
\[\dv{F}{x}=\int_{a(x)}^{b(x)}\frac{\partial f}{\partial x}(x,t)\ddt + f(x, b(x)) \cdot \frac{db}{dx} - f(x, a(x)) \cdot \frac{da}{dx}.\]
\end{theorem}
