...
\begin{proof}
    ...
    Since \(f\) is continuous, \(\exists\delta>0\) such that \(\forall z\in\gamma\), \(\abs{f(z)}>\delta\). By the uniform convergence of \(\cbraces{f_n(z)}\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(\forall z\in\gamma\), \(\abs{f_n(z)-f(z)}<\frac{\delta}{2}\). Then \(\abs{f_n(z)}>\frac{\delta}{2}\) on \(\gamma\). It follows that \(\frac{1}{f_n(z)}\) and its limit are uniformly bounded: \[\abs{\frac{1}{f(z)}}<\frac{1}{\delta},\quad\abs{\frac{1}{f_n(z)}}<\frac{2}{\delta},\quad\forall z\in\gamma,\forall n>N.\]
    ...
    \begin{align*}
        ...
        &<M\delta^2\frac{\delta/2+\abs{f'-f'_n}}{2}
    \end{align*}
\end{proof}
