...
\begin{definition}[Cover of a Set]
    A cover \(\mathcal{C}\) of a set \(X\) is any collection of sets such that \[\bigcup_{A\in\mathcal{C}}A\supseteq X.\] A cover is \textit{open} if every set in the collection is open.
\end{definition}
...
\begin{definition}[Topological]
    ...
    \end{enumerate}
    The collection \(\tau\) is called a \emph{topology} on \(X\), and its elements are referred to as \emph{open sets}.
\end{definition}
...
For example, in a topological space, a function \(f:X\to Y\) between two topological spaces is said to be continuous if the preimage of every open set in \(Y\) is an open set in \(X\). This generalizes the familiar \(\varepsilon\)-\(\delta\) definition of continuity from calculus. To illustrate this, consider the function \(f : \mathbb{R} \to \mathbb{R}\) defined by
\[f(x)= \begin{cases}
1 & x \geq 0, \\
0 & x < 0.
\end{cases}
...
Let us examine the open set \(V = (0.5, 1.5) \subseteq \mathbb{R}\). Then the preimage of \(V\) is
