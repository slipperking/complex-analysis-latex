...
\begin{theorem}[Cauchy--Goursat Differentiation Formula]\label{thm:cauchydifferentiationformula}
    ...
    \end{equation}
    Additionally, since \(U\) is open, \(\forall z_0\in U\), \(\exists\varepsilon>0\) such that the closed disk \(\overline{D\paren{z_0,\varepsilon}}\subset U\). Then \(\forall z\in D\paren{z_0,\varepsilon}\), \(f(z)\) has the Taylor expansion 
    \begin{equation}
        f(z)=\sum_{k=0}^\infty a_k\paren{z-z_0}^k,\label{eq:cauchydifferentiationformula_taylorseries}
    \end{equation}
    where \begin{equation}
        a_k=\frac{k!}{2\pi i}\int_{\partial U}\frac{f(\zeta)}{\paren{\zeta-z}^{k+1}}\ddzeta.\label{eq:cauchydifferentiationformula_taylorseriescoefficients}
    \end{equation}
\end{theorem}
\begin{proof}
