...
\begin{proof}
    ...
    Similarly, \(a_1=0\). We recursively obtain that \(a_0,a_1,\ldots=0.\) Then \(f\equiv0\) over \(D\paren{z_0,\varepsilon}\). For any point \(z'\) in the disk, the Taylor Series centered around it is also identically 0, (and depending on the choice of \(z'\), the convergence radius will be larger than \(\varepsilon\)). We can continue to choose disks around arbitrary points in previous disks until they cover \(U\). Since \(U\) is connected, then \(f\equiv0\) over \(U\).
\end{proof}
