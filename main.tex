...
\begin{proof}
    ...
    \end{align*}
    where the linear term vanishes since \(\frac{\qty(\zeta-n)^2}{\sin[2](\pi\zeta)}\) is even around \(\zeta=n\). Hence, the principal part of \(\csc^2(\pi z)\) at \(z=n\) is \(\psi_n(z)=\frac{1}{\pi^2(z-n)^2}\). Since \[\sum_{n=-\infty}^\infty\psi_n(z)=\frac{1}{\pi^2}\sum_{n=-\infty}^\infty\frac{1}{\qty(z-n)^2}\] grows at the same rate as \(2\sum_{n=1}^\infty\frac{1}{n^2}\) (except at the poles), it converges uniformly on compact subsets of \(\mathbb{C}\setminus\mathbb{Z}\) by the Weierstrass \(M\)--Test (\autoref{thm:weierstrassmtest}). 
\end{proof}
