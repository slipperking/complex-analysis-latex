...
\begin{proof}
    ...
    Since \(f\) is continuous, \(\exists\delta>0\) such that \(\forall z\in\gamma\), \(\abs{f(z)}\ge\delta\). By the uniform convergence of \(\cbraces{f_n(z)}\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(\forall z\in\gamma\), \(\abs{f_n(z)-f(z)}<\frac{\delta}{2}\). By the reverse triangle inequality, \(\)
\end{proof}
