...
\(\forall a\in D(0,R)\), \(\forall R>0\), we can apply the transformation \(\tilde{\varphi}_a\qty(z)=R\varphi_{\flatfrac{a}{R}}\qty(\frac{z}{R})\) to extend the automorphism to \(D(0,R)\). Let \(\Phi\) instead be harmonic on \(D(0,R)\) and continuous on \(\overline{D(0,R)}\). Then, \[\Phi(0)=\frac{1}{2\pi R}\int_0^{2\pi}\Phi\qty(Re^{i\theta})\dd{\theta}.\] It follows that \(u=\Phi\circ\tilde{\varphi}_a\) is also harmonic on \(D(0,R)\) with \(\Phi(0)=u\circ\tilde{\varphi}_{-a}(0)=u(a)\), and from the bijectivity of  \(Re^{i\theta}=\tilde{\varphi}_a\qty(Re^{i\tau})\), 
...
Then, because \(\Phi(Re^{i\theta})=u\circ\tilde{\varphi}_a^{-1}\qty(Re^{i\theta})\), and \(\tilde{\varphi}_a^{-1}(z)=R\varphi_{-\flatfrac{a}{R}}\)
\[u(z)=\frac{1}{2\pi}\int_0^{2\pi}u\qty(Re^{i\tau})\frac{R^2-\abs{z}^2}{\abs{Re^{i\tau}-z}^2}\dd{\tau}.\]
The expression \(P(\zeta,z)=\frac{R^2-\abs{z}^2}{2\pi\abs{\zeta-z}^2}\) is also a form of the Poisson kernel. Then with \(\zeta=R\exp\qty(i\tau)\),
