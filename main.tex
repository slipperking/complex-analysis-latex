...
\begin{proof}
    ...
    Bisect \(\mathbb{C}\) into \(\mathbb{C}\setminus D\paren{z,\varepsilon}\) and \(D\paren{z,\varepsilon}\). \(\forall\varepsilon>0\), the integral \[-\frac{1}{2\pi i}\int_{\mathbb{C}\setminus D\paren{z,\varepsilon}}\frac{\psi(\zeta)}{\zeta-z}\dd{\overline{\zeta}}\wedge\ddzeta\] is continuous. Since \(\psi(\zeta)\) is compactly supported over \(\mathbb{C}\) and continuous, by \autoref{thm:continuousfunctionboundedoncompact}, it is bounded. Then the limit \[\lim_{\varepsilon\to0}\paren{-\frac{1}{2\pi i}\int_{D\paren{z,\varepsilon}}\frac{\psi(\zeta)}{\zeta-z}\dd{\overline{\zeta}}\wedge\ddzeta}=0.\] Therefore, \eqref{eq:onedimensionalpartialconjugatesolution} is continuous. Let \(\xi=\zeta-z\) and observe that \[u(z)=\frac{1}{2\pi i}\int_{\mathbb{C}}\frac{\psi(\xi+z)}{\xi}\dd{\xi}\wedge\dd{\overline{\xi}}\] Then, \begin{equation}
        \frac{u(z)-u\paren{z_0}}{z-z_0}=\frac{1}{2\pi i}\int_{\mathbb{C}}\frac{\psi(\xi+z)-\psi\paren{\xi+z_0}}{\xi\paren{z-z_0}}\dd{\xi}\wedge\dd{\overline{\xi}}.\label{eq:onedimensionalpartialconjugatesolution_differenceexpr}
    \end{equation}
    The value of \[\frac{\psi(\xi+z)-\psi\paren{\xi+z_0}}{z-z_0}\] tends to \(\pdv{\psi(\xi+z)}{\xi}\) as \(z_0\to z\). Because \(\psi\paren{\xi+z}\) has compact support and is \(C^1\), by \autoref{thm:continuousfunctionboundedoncompact}, it is Lipschitz continuous for a constant \(M\). Then, \[\abs{\frac{\psi(\xi+z)-\psi\paren{\xi+z_0}}{z-z_0}}\leq M,\] and when \(\xi+z\notin\supp(\psi)\), \[\abs{\frac{\psi(\xi+z)-\psi\paren{\xi+z_0}}{z-z_0}}=0.\]
    Since the integrand is dominated by a function that has a convergent integral (\(\int_{\supp(\psi)}M\dd{\xi}\wedge\dd{\overline{\xi}}\)), equation \eqref{eq:onedimensionalpartialconjugatesolution_differenceexpr} uniformly converges and the limit \(z_0\to z\) may commute with the double integral. From the real axis \[\pdv{u}{x}(z)=\frac{1}{2\pi i}\int_{\mathbb{C}}\pdv{\psi(\xi+z)}{z}\cdot\frac{1}{\xi}\dd{\xi}\wedge\dd{\overline{\xi}}.\] Since \(\pdv{\psi(\xi+z)}{z}\) is continuous over a compact region, it is bounded, and we can invoke the same argument as we did to prove the continuity of \(u(z)\).
\end{proof}
