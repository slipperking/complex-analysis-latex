...
\begin{proof}
    ...
    By definition, \(D\paren{z^*,\varepsilon}\subseteq U\). For any point \(z'\) in this disk, since 
\end{proof}
\begin{remark}
...
\begin{theorem}[Identity Theorem]\label{thm:identity}
    Let \(U\subseteq\mathbb{C}\) be a region and \(f(z)\) and \(g(z)\) be two holomorphic functions on \(U\). For a set \(E\subseteq U\) with an accumulation point in \(U\), if \(f\equiv g\) on \(E\), then \(f\equiv g\) on \(U\).
\end{theorem}
\begin{proof}
