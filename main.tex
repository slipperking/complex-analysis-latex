...
Indeed, we can apply the transformation \(R\varphi_a\qty(\frac{z}{R})\) to extend the automorphism to \(D(0,R)\). Let \(\Phi\) instead be harmonic on \(D(0,R)\) and continuous on \(\overline{D(0,R)}\). Then, \[\Phi(0)=\frac{1}{2\pi R}\int_0^{2\pi}\Phi\qty(Re^{i\theta})\dd{\theta}.\] It follows that \(\phi=\Phi\circ\varphi_a\) is also harmonic on \(D(0,R)\), and under \(Re^{i\theta}=\varphi_a\qty(Re^{i\tau})\),
