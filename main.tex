...
\begin{proof}
    ...
    \begin{align*}
            \abs{\varphi(\xi)-u(z)}&=\abs{\qty(\int_{\abs{\vartheta-\tau}<\delta}+\int_{\abs{\vartheta-\tau}\ge\delta})P(\zeta,z)\paren{\varphi\qty(Re^{i\vartheta})-\varphi(\zeta)}\dd{\tau}}\\&=\abs{I_1+I_2}\leq\abs{I_1}+\abs{I_2}.
    \end{align*}
    Since the Poisson kernel is nonnegative, \[
    \qty|I_1|<\varepsilon\int_{\abs{\vartheta-\tau}<\delta}P(\zeta,z)\dd{\tau}<\varepsilon.
    \]
    Informally, the Poisson kernel becomes small when \(z\) and \(\zeta\) are sufficiently far apart. Since \(\varphi\) is continuous on a compact set \(\partial D(0,R)\), it is bounded by \autoref{lemma:uniformcontinuousovercompactset}, or \(\exists M>0\) that is finite, such that \(\)
\end{proof}
