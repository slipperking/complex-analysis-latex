...
\begin{example}[Cauchy's Integral Formula on the Exterior]\label{ex:cauchygoursatformulaexterior}
    ...
    \begin{enumerate}
        ...
        \item If \(\gamma\) encloses the origin, then \(\forall z\in\mathbb{C}\setminus\gamma\), then \begin{equation}
            \frac{1}{2\pi i}\int_\gamma\frac{f(\zeta)}{z\zeta-\zeta^2}\ddzeta=\begin{cases}
...
\begin{proof}
    ...
    \item Under the partial fraction decomposition of equation~\eqref{eq:cauchygoursatformulaexteriorpart2}, we get that \[\int_\gamma\frac{f(\zeta)}{z\zeta-\zeta^2}\ddzeta=\int_\gamma\qty[\frac{f(\zeta)}{\zeta}-\frac{f(\zeta)}{\zeta-z}]\ddzeta\]
    \end{enumerate}
\end{proof}
