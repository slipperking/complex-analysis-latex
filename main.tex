...
\begin{proof}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \draw[thin] ($(0,0)!0.1!(zeta)$) arc[start angle=10, end angle=62.35, radius=0.5];
            \draw[thin] (2,0) arc[start angle=0, end angle=10, radius=2];
            \draw[thin] (1.2,0) arc[start angle=0, end angle=80, radius=1.2];
            ...
            \node[anchor=north] at ($(z)!0.5!(xi)$) {\(\eta^-\)};
            \node[anchor=west] at ($(0,0)!0.5!(xi)$) {\(R\)};
            \node[anchor=west] at ($(z)!0.5!(0,0)$) {\(\rho\)};
            \node[anchor=north] at (0.7,0.7) {\(\delta^+\)};
            \node[anchor=north] at (1.1,1.1) {\(\vartheta\)};
    ...
    \end{figure}Since \(\varphi\) is continuous on a compact set \(\partial D(0,R)\), it is bounded by \autoref{lemma:uniformcontinuousovercompactset}, or \(M=\sup_{\qty|\zeta|=R}\abs{\varphi(\zeta)}\) that is finite.

    The kernel can be rewritten as \[P(\zeta,z)=\frac{R^2-\rho^2}{2\pi\qty|\zeta-z|^2},\]
    where \(\zeta=Re^{i\tau}\) and \(z=\rho e^{i\theta}\), with \(\abs{\vartheta-\tau}>\delta\). Then \(\exists\eta>0\) such that when \(|\xi-z|<\eta\), \(\abs{\theta-\tau}>\delta\) and \(\rho>\frac{R}{2}\) (this can be arbitrarily chosen for different resulting bounds) as in \autoref{fig:dirichletproblemwithlaplaceequationsolution_secondintegral}. Then, \[|\zeta-z|^2>4\rho^2\sin[2](\frac{\delta}{2})>\frac{1}{2}R^2\qty(1-\cos(\delta)).\]
    
    Since \(\abs{\varphi\qty(Re^{i\vartheta})-\varphi(\zeta)}<2M\), we want \(\int_{\abs{\vartheta-\tau}>\delta}\frac{R^2-\rho^2}{\pi R^2\qty(1-\cos(\delta))}\dd{\tau}<2\frac{R^2-\rho^2}{R^2\qty(1-\cos(\delta))}< \frac{\varepsilon}{2M}\), and \[R^2-\rho^2<\frac{\varepsilon}{4M}R^2\paren{1-\cos(\delta)}\Longrightarrow R-\rho<\frac{\varepsilon}{6M}R\paren{1-\cos(\delta)}.\]
    From \autoref{fig:dirichletproblemwithlaplaceequationsolution_secondintegral}, it is evident that \(R-\rho<|\xi-z|<\eta\). In order for \(|\theta-\tau|>\delta\) to be true, we need to ensure that \(\eta<R^2+\rho^2-2R\rho\cos(\vartheta-\tau-\delta)\). Obviously, this will be satisfied if \(|\xi-z|<\frac{R^2}{2}\qty(1-\cos(\vartheta-\tau-\delta))<2\rho^2\qty(1-\cos(\vartheta-\tau-\delta))\). Therefore, we can choose \(\eta=\min\qty(\frac{\varepsilon}{6M}R\paren{1-\cos(\delta)},\frac{R^2}{2}\qty(1-\cos(\vartheta-\tau-\delta)))\), the necessary requirements are satisfied.
