...
By \(\varepsilon\)--\(N\), for a fixed \(R>0\), \(\exists N\in\mathbb{N}\) such that \(\forall n\geq N\), \(\abs{a_n}>2R\) (the coefficient is an arbitrary constant greater than one). Consider the product \(\prod_{n=N}^\infty E_{k_n}\qty(\frac{z}{a_n})\). For \(z\in D(0,R)\), we have \(\qty|\frac{z}{a_n}|<\frac{1}{2}\). Notice that \begin{align*}
	\abs{Q_{k_n}\qty(\frac{z}{a_n})}\leq
\end{align*}
