...
\begin{definition}[Continuity]\label{def:continuity}
    ...
    \[\lim_{x\to a}f(x)=f(a).\]
\end{definition}
\begin{theorem}\label{thm:continuousfunctionbounded}
...
\end{lemma}
\begin{proof}
    Let \(f:U\to\mathbb{R}\) be \(C^1\). By \autoref{thm:continuousfunctionbounded}, since \(U\) is compact and \(f'\) is continuous, \(\exists M>0\) such that \(\forall x\in U\), \(|f'(x)|\leq M\).
...
Let \(S^2=\left\{\paren{x_1,x_2,x_3}\in\mathbb{R}\,\middle|\,x_1^2+x_2^2+x_3^2=1\right\}\). There exists a \textit{stereographic projection} of \(S^2\) onto \(\riemannsphere\). For every point except for \((0,0,1)\), there is a corresponding complex number \[z=\frac{x_1+ix_2}{1-x_3}.\]
...
By letting \(\infty\) correspond to \(\paren{0,0,1}\), the bijection is complete. The sphere \(S^2\) is also called the \textit{Riemann sphere}. The region given by the disk \(\abs{z}<1\) is given by \(x_3<0\), and the region \(\abs{z}>1\) is given by \(x_3>0\). In this representation, \(\infty\in\riemannsphere\) is no longer considered to be special.
...
\begin{theorem}
    A sufficient criterion for a function \(f(z)\) to be holomorphic on a region \(D\) is if \(f(z)\in C^1(D)\) and satisfy the Cauchy--Riemann equations.
\end{theorem}
\begin{proof}
    
\end{proof}
A variant that is much harder to prove gives the logical equivalence statement:
\begin{theorem}[Looman–Menchoff Theorem]
    The necessary and sufficient criterion for a function \(f(z)\) to be holomorphic on a region \(D\) is if it is continuous over \(D\), differentiable over \(D\setminus N\) for a countable set \(N\), and satisfy the Cauchy--Riemann equations.
\end{theorem}
...
\begin{theorem}[Green's Theorem, Complex Form]\label{thm:complexgreen}
    Let \(U\subset\mathbb{C}\) be bounded with a piecewise smooth boundary \(\partial U\). For two scalar functions \(\omega_1=\omega_1\paren{z,\overline{z}}\) and \(\omega_2=\omega_2\paren{z,\overline{z}}\) satisfy \(\omega_1,\omega_2\in C^1\paren{\overline{U}}\), define the 1-form \(\omega=\omega_1\ddz+\omega_2\dd{\overline{z}}\). Then,
    \begin{equation}
...
\begin{proof}
    ...
    Then,
    \begin{align}
        \omega&=\paren{\xi_1+i\eta_1}\ddz+\paren{\xi_2+i\eta_2}\dd{\overline{z}}\label{eq:complexgreenomegaexpansionintermediate}\\
        ...
        &=\brackets{\paren{\xi_1+\xi_2}\ddx+\paren{\eta_2-\eta_1}\ddy}+i\brackets{\paren{\eta_1+\eta_2}\ddx+\paren{\xi_1-\xi_2}\ddy}\label{eq:complexgreenrealandcomplexdxdyintermediate}
    \end{align}
    Each of \(\xi_1,\xi_2,\eta_1,\eta_2\) are real valued functions that can be represented with a domain of \(\mathbb{R}^2\). We then will apply the \(d=\partial+\overline{\partial}\) definition of the exterior derivative and relate it to \eqref{eq:complexgreen}. Starting with equation \eqref{eq:complexgreenomegaexpansionintermediate}, \begin{align*}
        ...
        &=\paren{\pdv{\eta_2}{x}-\pdv{\xi_1}{y}-\pdv{\eta_1}{x}-\pdv{\xi_2}{y}}\ddx\wedge\ddy+i\paren{\pdv{\xi_1}{x}-\pdv{\eta_1}{y}-\pdv{\xi_2}{x}-\pdv{\eta_2}{y}}\ddx\wedge\ddy.
    \end{align*}
    From equation \eqref{eq:complexgreenrealandcomplexdxdyintermediate}, we can apply \autoref{thm:realgreen}. For the real component of \(\omega\), we obtain
    \[\int_{\partial U} \paren{\xi_1+\xi_2}\ddx+\paren{\eta_2-\eta_1}\ddy=\iint_U\paren{\pdv{\eta_2}{x}-\pdv{\xi_1}{y}-\pdv{\eta_1}{x}-\pdv{\xi_2}{y}}\ddx\wedge\ddy,\]
...
\begin{proof}
    ...
    We then aim to prove that \begin{equation}
        \lim_{\varepsilon\to0}\int_{D(z,\varepsilon)}\pdv{f}{\overline{\zeta}}\cdot\frac{\dd{\overline{\zeta}}\wedge\ddzeta}{\zeta-z}=0.\label{eq:pompeiuproofareadiskstatement}
    \end{equation}
    Notice that since \(f\in C^1\paren{\overline{U}}\), by \autoref{thm:continuousfunctionbounded}, \(\exists M'\in\mathbb{R}_{>0}\) such that \(\forall\zeta\in\overline{U}\), \(\abs{\pdv{f}{\overline{\zeta}}}\leq M'\). Then,
...
\begin{theorem}[Cauchy's Integral Formula]\label{thm:cauchyintegralformula}
    Let \(U\subset\mathbb{C}\) be bounded with piecewise smooth boundary \(\partial U \), and let \(f\in C^1(\overline{U})\) be holomorphic on \(U\). Then for all \(z\in U\),
    \begin{equation}
...
\end{proof}
\begin{theorem}[Cauchy's Integral Theorem]\label{thm:cauchygoursat}
...
\begin{theorem}\label{thm:onedimensionalpartialconjugatesolution}
    For a compactly supported function \(\psi(z)\in C^1\paren{\mathbb{C}}\), a solution satisfying \(u(z)\in C^1\paren{\mathbb{C}}\) to the non-homogeneous Cauchy--Riemann equation \[\pdv{u(z)}{\overline{z}}=\psi(z)\] is \begin{equation}
        u(z)=\frac{1}{2\pi i}\int_{\mathbb C}\frac{\psi(\zeta)}{\zeta-z}\ddzeta\wedge \dd{\overline{\zeta}}+f(z),\label{eq:onedimensionalpartialconjugatesolution}
    \end{equation} where \(f(z)\) is any entire function.
\end{theorem}
\begin{proof}
    Differentiate equation \eqref{eq:onedimensionalpartialconjugatesolution}. By equation \eqref{eq:wirtingerderivative2}, the second term vanishes. Because \(\supp\paren{\psi(z)}\) 
\end{proof}
\end{document}