...
\begin{definition}[Residue]\label{def:residue}
    For some \(r\in\mathbb{R}_{>0}\), \(a\in U\), suppose \(f:D^*(a,r)\to \mathbb{C}\) is holomorphic. Then the \textit{residue} of \(f\) at \(a\), denoted by \(\Res(f,a)\), is equal to \begin{equation}
        \Res_{z=a}f(z)=\frac{1}{2\pi i}\int_{\partial D(a,\rho)}f(z)\ddz,\label{eq:residue}
    \end{equation}
...
\end{definition}
It then follows that the residue at a removable singularity is 0. Let \(z=\infty\) be an isolated singularity of \(f(z)\), which is holomorphic in \(\mathbb{C}\setminus\overline{D(0,R)}\), for sufficiently large finite \(R\). Then for finite \(\rho>R\), the residue at \(z=\infty\) is \textit{defined} as \[\Res_{z=\infty}f(z)=-\int_{\partial D(0,\rho)}f(z)\ddz.\]
At a neighborhood of \(\infty\), if \(f\) has the Laurent expansion \[f(z)=\sum_{n=-\infty}^\infty c_nz^n,\]
then we have \(\Res_{z=\infty}f(z)=-c_{-1}\). If \(\zeta=\frac{1}{z}\) and \(g(\zeta)=f\qty(\frac{1}{\zeta})=f\qty(z)\), from division, we obtain the expansion \[\frac{1}{\zeta^2}f(\zeta)=\sum_{n=-\infty}^\infty c_n\zeta^{-n-2}.\] In terms of residues, we have \[-c_{-1}=\Res_{\zeta=0}\qty(-\frac{f(\zeta)}{\zeta^2})\]
