...
\begin{lemma}\label{lemma:poincare}
    For any differential form \(\omega\), the exterior derivative applied twice satisfies: \[dd\omega=0.\]
\end{lemma}
...
We can consider \(f(z)\) to be a bivariate function \(f(x,y)\) for \(z=x+iy\). Two main cases we are concerned with are when $h$ approaches 0 from the :
\[\lim_{\substack{h\to0\\h\in\mathbb{R}}}\frac{f()-f(z)}{h}.\]
