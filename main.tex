...
\begin{proof}
    ...
    From rearrangement, \(\oint_{\partial U}f(z)\ddz=\sum_{k=1}^n\oint_{D\qty(z_k,\delta_k)}f(z)\ddz\), and the conclusion follows.
\end{proof}
...
\end{theorem}
\begin{proof}
    Let \(R>\max_{j\in\mathbb{N}_{\leq n}}\qty|z_n|\) be arbitrary. Then there exists circles centered at each isolated singularity \(z_k\) with radius \(\delta_k\) respectively such that \(\partial D\qty(z_k,\delta_k)\subset\)
\end{proof}
