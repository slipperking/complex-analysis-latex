...
\end{proof}
If \(f\) is a nonzero holomorphic function on a domain, \(\flatfrac{1}{f}\) is also a nonzero holomorphic function on the domain. Then applying the Maximum Modulus Principle to \(\flatfrac{1}{f}\), we can obtain the Minimum Modulus Principle (for nonzero modulus).
...
Therefore, the image of \(\partial D(0,1)\) lies on \(\partial D(0,1)\), and since \(f\) is holomorphic and non-constant, by the Maximum Modulus Principle (\autoref{thm:maximummodulus}), \(\abs{}\) \(f\) maps \(D(0,1)\) to \(D(0,1)\). We next aim to show that \(f:D(0,1)\to D(0,1)\) is bijective.

Let us first confirm injectivity. \(\forall\zeta_1,\zeta_2\in D(0,1)\), and observe the results from \(\frac{z-\zeta_1}{\overline{\zeta}-\overline{z}}\)
