...
where \(I=\mqty(\imat{2})\). The quotient group (a group of "cosets" in the form \(\qty{A,-A}\)) is taken because \(\mqty(a&b\\c&d)\) refers to the same transformation as \(\mqty(-a&-b\\-c&-d)\). This group, denoted by \(\mathrm{PSL}(2,\mathbb{C})=\mathrm{SL}(2,\mathbb{C})/\qty{\pm I}\cong\Aut\qty(\riemannsphere)\), is known as the \textit{projective special linear group} of order 2, and is \textit{isomorphic} to the \textit{group of Möbius transformations}, consisting of all fractional linear transformations.
...
We will start by introducing the concept of \textit{infinite products}. Let \(\prod_{k=1}^n\qty(1+u_k)\) be an infinite product. For \(x\in\mathbb{R}_{\geq0}\), since \(e^x\geq x\) and \(e^0=1\), we can integrate over \([0,x]\) to get that \(e^x\geq x+1\). Therefore, \[\exp(\sum_{k=1}^n\abs{u_k})\geq\prod_{k=1}^n \qty(1+\abs{u_k})=1+\sum_{k=1}^n\abs{u_k}+\sum_{\substack{j,k\in\qty{1,\ldots,n}\\j<k}}\abs{u_ju_k}+\cdots+\prod_{k=1}^n\abs{u_k}>\sum_{k=1}^n\abs{u_k}.\]
Since the convergence of \(\sum_{k=1}^\infty\qty|u_k|\) is the same as that of \(\exp(\sum_{k=1}^\infty\abs{u_k})\), it follows that the convergence of \(\sum_{k=1}^\infty\abs{u_k}\) is equivalent to that of \(\prod_{k=1}^\infty\qty()\)
...
\end{definition}
It then follows that the residue at a removable singularity is 0. As a direct consequence of equation~\eqref{eq:residue}, we can derive explicit formulae for the calculation of residues at poles. TO BE CONTINUED
...
\end{figure}As a matter of fact, if we take a neighborhood of an arbitrary point in \(\mathbb{C}\) on the Riemann sphere and traverse its boundary clockwise (from the perspective of outside the sphere), its projection onto \(\mathbb{C}\) will be counterclockwise. However, the boundary of a neighborhood of \(\infty\) in \(S^2\) will have a clockwise projection (hence the difference in orientation). We define its equality with the residue of \(-\frac{f\qty(\frac{1}{\zeta})}{\zeta^2}\), rather than \(f\qty(\frac{1}{\zeta})\), because we compose the differential form \(f(z)\ddz\) with the inversion, as opposed to \(f(z)\).
