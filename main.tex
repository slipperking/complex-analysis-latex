...
\begin{proof}
    ...
    \end{equation}
    Notice that since \(f\in C^1\paren{\overline{U}}\), by \autoref{theorem:continuousfunctionbounded}, \(\exists M'\in\mathbb{R}_{>0}\) such that \(\forall\zeta\in\overline{U}\), \(\abs{\pdv{f}{\overline{\zeta}}}\leq M'\). Then,
...
\begin{theorem}[Cauchy Integral Formula]\label{theorem:cauchyintegralformula}
    Let \(U\subset\mathbb{C}\) be bounded with piecewise smooth boundary \(\partial U \), and let \(f\in C^1(\overline{U})\) be holomorphic on \(U\). Then for all \(z\in U\), \[f(z)=\frac{1}{2\pi i}\int_{\partial U}\frac{f(\zeta)}{\zeta-z}\dd{\zeta}.\]
\end{theorem}
\begin{proof}
    By equation \eqref{eq:wirtingerderivative2}, for \(f\paren{\zeta,\overline{\zeta}}\), \(\pdv{f}{\o}\)
\end{proof}
\end{document}