...
Indeed, we can apply the transformation \(R\varphi_a\qty(\frac{z}{R})\) to extend the automorphism to \(D(0,R)\). Let \(\Phi\) instead be harmonic on \(D(0,R)\) and continuous on \(\overline{D(0,R)}\). Then, \[\Phi(0)=\frac{1}{2\pi}\int_0^{2\pi}\Phi\qty( e^{i\theta})\dd{\theta}\]
