...
\begin{proof}
    ...
    The function \(f\) has no zeros in \(K\setminus V\) (as \(V\) contains all zeros of \(f\) in \(K\)). Since \(K\setminus V\) is compact, \(f_n\rightrightarrows f\) over this set. Similarly, \(\exists N_2\in\mathbb{N}\) such that \(\forall n>N_2\), \[\abs{f(z)-f_n(z)}<\abs{f(z)},\quad\forall z\in K\setminus V.\]
...
\end{theorem}
\begin{proof}
    The proof is exactly the same as in \autoref{thm:hurwitz}. Obviously, \(|f|\) attains its infimum on \(\partial K\) by its continuity, or that \(\delta=\min_{z\in\partial K}\abs{f(z)}\). Then \(\forall\abs{\lambda}<\delta\), \(\forall z\in \partial K\),
    \[\abs{f(z)-\paren{f(z)+\lambda}}<\delta\leq\abs{f(z)}.\]
