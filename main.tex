...
\begin{theorem}[Differentiation under the Integral Sign]\label{thm:leibnizintegralrule}
    Let \( f(x,t) \) be a function defined on a rectangle \( [a,b] \times [c,d] \subset \mathbb{R}^2 \), and suppose:
    \begin{enumerate}
        \item For each fixed \( x \in [a,b] \), the function \( t \mapsto f(x,t) \) is integrable on \( [c,d] \),
        \item The partial derivative \( \frac{\partial f}{\partial x}(x,t) \) exists for all \( (x,t) \in [a,b] \times [c,d] \),
        \item There exists an integrable function \( M(t) \) on \( [c,d] \) such that 
        \[
        \left| \frac{\partial f}{\partial x}(x,t) \right| \leq M(t)
        \quad \text{for all } x \in [a,b],\ t \in [c,d].
        \]
    \end{enumerate}
    Then the function
    \[
    ...
    \]
    is differentiable on \( [a,b] \), and its derivative is given by
    \[
    F'(x) = \frac{d}{dx} \int_c^d f(x,t)\,dt = \int_c^d \frac{\partial f}{\partial x}(x,t)\,dt.
    \]
\end{theorem}
...
\end{theorem}
\begin{proof}
    Since \(P(\zeta,z)=\frac{1}{4\pi}\paren{\frac{\zeta+z}{\zeta-z}+\frac{\overline{\zeta}+\overline{z}}{\overline{\zeta}-\overline{z}}}\), it follows that from equation~\eqref{eq:laplaciancomplexform}, \(\Delta_z P(\zeta,z)=4\pdv{P(\zeta,z)}{z}{\overline{z}}=0\) (since each term is independent of either \(z\) or \(\overline{z}\)). Then equation~\eqref{eq:dirichletproblemwithlaplaceequationsolution} becomes \[\Delta u(z)=\Delta\int_0^{2\pi}\varphi(\zeta)P(\zeta,z)\dd{\tau}=\int_0^{2\pi}\Delta\qty[\varphi(\zeta)P(\zeta,z)]\dd{\tau}=0.\]
\end{proof}
