...
\begin{proof}
    ...
    \[\int_{\partial\mathbb{D}}z^{2k-2n-1}\ddz=\int_0^{2\pi} e^{i\theta\qty(2k-2n-1)}ie^{i\theta}\dd{\theta}=i\int_0^{2\pi}e^{2i\theta(k-n)}=\begin{cases}
        ...
        2\pi i&k=n
    \end{cases}.\]
    Therefore, \[2^{2n}i\int_0^{2\pi}\cos^{2n}\theta\dd{\theta}=2\pi i\binom{2n}{n}\Longleftrightarrow\int_0^{2\pi}\cos^{2n}\theta\dd{\theta}=2\pi\frac{\qty(2n)!}{2^{2n}\qty(n!)^2}=2\pi\frac{(2n)(2n-1)...(n+1)}{n(n-1)(n-2)...1*2^{2n}}\]
\end{proof}
