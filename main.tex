...
\begin{proof}
    ...
    Fix \(\theta\in[0,2\pi]\) arbitrarily. We will prove that \(\delta_\varepsilon\) is continuous at \(\theta\) by contradiction, so first assume otherwise. That means \(\exists\varepsilon'>0\) such that \(\forall\delta>0\), for all \(\vartheta\) satisfying \(|\vartheta-\theta|<\delta\),
\end{proof}
