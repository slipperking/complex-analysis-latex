...
\begin{proof}
    ...
    \end{equation}
    The value of \[\frac{\psi(\xi+z+h)-\psi\paren{\xi+z}}{h}\] tends to \(\pdv{\psi(\xi+z)}{\xi}\) as \(h\to0\). Because \(\psi\paren{\xi+z}\) has compact support and is \(C^1\), by \autoref{thm:continuousfunctionboundedoncompact}, it is Lipschitz continuous for a constant \(M\). Then, \[\abs{\frac{\psi(\xi+z+h)-\psi\paren{\xi+z}}{h}}\leq M,\] and when \(\xi+z\notin\supp(\psi)\), \[\abs{\frac{\psi(\xi+z+h)-\psi\paren{\xi+z}}{h}}=0.\]
    Since the integrand is dominated by a function that has a convergent integral (\(\int_{\supp(\psi)}M\dd{\xi}\wedge\dd{\overline{\xi}}\)), equation \eqref{eq:onedimensionalpartialconjugatesolution_differenceexpr} uniformly converges and the limit \(h\to0\) may commute with the double integral. Let \(\xi=\alpha+i\beta\). From the real axis \[\pdv{u}{x}(z)=\frac{1}{2\pi i}\int_{\mathbb{C}}\pdv{\psi(\xi+z)}{\partial\alpha}\cdot\frac{1}{\xi}\dd{\xi}\wedge\dd{\overline{\xi}}.\] Since \(\pdv{\psi(\xi+z)}{z}\) is continuous over a compact region, it is bounded, and we can invoke the same argument as we did to prove the continuity of \(u(z)\).
\end{proof}
