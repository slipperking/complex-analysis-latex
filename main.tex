...
\begin{gather*}
    ...
    =\frac{1}{2\pi}\int_0^{2\pi}\frac{\qty(1-\rho^2)u\qty(e^{i\tau})\dd{\tau}}{1+\rho^2-2\rho\cos(\theta-\tau)}.
\end{gather*}
...
\begin{proof}
    ...
    From rearrangement, we have \(\frac{g(z)}{\Phi(z)}=\pdv{\varPsi}{\overline{z}}\), which has removable singularities at each \(z_k\). Define \(\frac{g(z)}{\Phi(z)}=0\) at \(z=z_k\). Under this assertion, we have \(\frac{g(z)}{\Phi(z)}\in C^\infty(\mathbb{C})\). Since the support of \(\frac{g(z)}{\Phi(z)}\) is the union of disjoint compact sets, by \autoref{thm:onedimensionalpartialconjugatesolution}, there exists a function \(\varPsi\in C^\infty(\mathbb{C})\) satisfying \(\frac{g(z)}{\Phi(z)}=\pdv{\varPsi}{\overline{z}}\). Since \(g\) is vanishing on \(\bigcup_{k=1}^\infty \overline{D\qty(z_k,\varepsilon_k)}\), it follows that \(\frac{g}{\Phi}\) is vanishing on \(\bigcup_{k=1}^\infty \overline{D\qty(z_k,\varepsilon_k)}\), and \(\varPsi\) is holomorphic on \(\bigcup_{k=1}^\infty \overline{D\qty(z_k,\varepsilon_k)}\).
...
\end{proof}
\begin{example}[Gaussian Integral]\label{ex:gaussianintegral}
    Evaluate the improper integral \(I=\int_{-\infty}^\infty e^{-x^2}\ddx\).
\end{example}
\begin{proof}
    This is a standard integral that can be evaluated using polar coordinates. However, we will now provide a solution via contour integration. Let \(\gamma\) be a closed semicircle as in \autoref{fig:semicircularcontour}, with radius \(R\geq 2\). By the Cauchy--Goursat Integral Theorem (\autoref{thm:cauchygoursattheorem}), we have that \[\int_{\gamma}e^{-z^2}\ddz=\int_{-R}^R e^{-x^2}\ddx+\int_0^\pi e^{-R^2e^{2i\theta}}Rie^{i\theta}\dd{\theta}=0.\]
    On the real axis, \(1-\frac{4}{\pi}\theta\leq\cos(2\theta)\) on \(\qty[0,\frac{\pi}{4}]\), \(\frac{4}{\pi}\theta-3\leq\cos(2\theta)\) on \(\qty[\frac{3\pi}{4},\pi]\). 
    \[\abs{\int_0^\pi e^{-R^2e^{2i\theta}}Rie^{i\theta}\dd{\theta}}\leq R\int_0^\pi e^{-R^2\cos(2\theta)}\dd{\theta}\]
\end{proof}
...
\begin{proof}
    ...
    From equation~\eqref{eq:fresnelwedgecontourintegral}, we have that \[\int_0^\infty e^{ir^2}\dd{r}=e^{i\frac{\pi}{4}}\int_0^\infty e^{-r^2}\dd{r}.\] By \autoref{ex:gaussianintegral}, we know that \(\int_0^\infty e^{-r^2}\dd{r}=\frac{\sqrt{\pi}}{2}\). Therefore, we have \[\int_0^\infty e^{ir^2}\dd{r}=\qty(\frac{\sqrt{2}}{2}+i\frac{\sqrt{2}}{2})\frac{\sqrt{\pi}}{2}.\]
