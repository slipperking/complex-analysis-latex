...
We have previously introduced the concept of expressing a complex function as a function of \(x\) and \(y\). It can also be expressed in terms of \(z\) and \(\overline{z}\), where \(z=x+iy\) and \(\overline{z}=x-iy\). Then \(|z|=z\overline{z}\), \(x=\frac{z+\overline{z}}{2}\), and \(y=\frac{z-\overline{z}}{2i}\). By the rules of the derivative,
\begin{equation}
...
and
\[\pdv{f}{\overline{z}}=\frac{1}{2}\paren{\pdv{u}{x}+i\pdv{v}{x}+i\pdv{u}{y}-\pdv{v}{y}}.\] If \(f\) is holomorphic,
...
Consider \(z=\frac{e^{iw}+e^{-iw}}{2}\). Then,
\begin{align*}
...
\end{remark}
\subsection{The Conformality of Holomorphic Mappings}\label{sec:conformalityintroduction}
...
\begin{proof}
    ...
    The rightmost term in equation~\eqref{eq:pompeiu_directintermediate} can be parameterized with \(\zeta=z+\varepsilon e^{it}\), \(t\in[0,2\pi]\). Then,
    \begin{gather*}
...
\begin{proof}
    ...
    Split \(\mathbb{C}\) into \(\mathbb{C}\setminus D\paren{z,\varepsilon}\) and \(\overline{D\paren{z,\varepsilon}}\). \(\forall\varepsilon>0\), the integral \[-\frac{1}{2\pi i}\int_{\mathbb{C}\setminus D\paren{z,\varepsilon}}\frac{\psi(\zeta)}{\zeta-z}\dd{\overline{\zeta}}\wedge\ddzeta\] is continuous. Since \(\psi(\zeta)\) is compactly supported over \(\mathbb{C}\) and continuous, by \autoref{thm:continuousfunctionboundedoncompact}, it is bounded. Then the limit \[\lim_{\varepsilon\to0}\paren{-\frac{1}{2\pi i}\int_{D\paren{z,\varepsilon}}\frac{\psi(\zeta)}{\zeta-z}\dd{\overline{\zeta}}\wedge\ddzeta}=0.\] Therefore, equation~\eqref{eq:onedimensionalpartialconjugatesolution} is continuous. Let \(\xi=\zeta-z\) and observe that \[u(z)=\frac{1}{2\pi i}\int_{\mathbb{C}}\frac{\psi(\xi+z)}{\xi}\dd{\xi}\wedge\dd{\overline{\xi}}\] Then,
    \begin{equation}
    ...
    As shown above, the integrand is uniformly bounded by \(M\), which has a convergent integral of \(\int_{K}M\dd{\xi}\wedge\dd{\overline{\xi}}\), the limit \(h\to0\) may commute with the integral in equation~\eqref{eq:onedimensionalpartialconjugatesolution_differenceexpr}. Let \(\zeta=\alpha+i\beta\). From the real axis,
    \begin{equation}
...
\end{example}
\begin{proof}
    Since \(z^n-1=\prod_{k=0}^{n-1}\qty(z-\omega^k_n)\), where \(\omega^k_n=e^{i\pi\frac{k}{n}}\), the integrand has singularities at every \(n\)th zero of unity. Then the integral is equal to:
    \begin{equation}
    ...
    \end{equation}
    where \(\cbraces{c_j}\) are the coefficients of the partial fraction decomposition. By the Cauchy--Goursat Integral Formula (\autoref{thm:cauchygoursatformula}), equation~\eqref{eq:cauchygoursatformulazerosofunity} becomes: \[\sum_{k=0}^{n-1}\int_{\partial D(0,2)}\frac{c_k}{z-\omega_k}\ddz=2\pi i\sum_{k=0}^{n-1}c_k.\]
...
\begin{proof}
    ...
    \[\int_{\partial\mathbb{D}}z^{2k-2n-1}\ddz=\int_0^{2\pi} e^{i\theta\qty(2k-2n-1)}ie^{i\theta}\dd{\theta}=i\int_0^{2\pi}e^{2i\theta(k-n)}\dd{\theta}=
        \begin{cases}
            0      & k< n \\
            2\pi i & k=n
    \end{cases}.\]
    Therefore, \[2^{2n}i\int_0^{2\pi}\cos^{2n}\theta\dd{\theta}=2\pi i\binom{2n}{n}\Longleftrightarrow\int_0^{2\pi}\cos^{2n}\theta\dd{\theta}=2\pi\frac{\qty(2n)!}{2^{2n}\qty(n!)^2}=2\pi\frac{\prod_{k=1}^{2n}k}{\prod_{k=1}^{n}\qty(2k)^2}.\]
...
\begin{example}[Cauchy--Goursat Integral Formula on the Exterior]\label{ex:cauchygoursatformulaexterior}
    ...
    \begin{enumerate}
        \item If \(f\) has a removable singularity at \(\infty\), or if \(w=\lim_{z\to\infty} f(z)\) exists and is finite, then \(\forall z\in\mathbb{C}\setminus\gamma\), \[\frac{1}{2\pi i}\int_\gamma\frac{f(\zeta)}{\zeta-z}\ddzeta=
                \begin{cases}
        ...
        \item If \(\gamma\) encloses the origin, then \(\forall z\in\mathbb{C}\setminus\gamma\), then
            \begin{equation}
                \frac{1}{2\pi i}\int_\gamma\frac{zf(\zeta)}{z\zeta-\zeta^2}\ddzeta=
                \begin{cases}
...
\begin{proof}
    \begin{enumerate}
        \item By the compactness of \(\gamma\), it can be completely contained within a sufficiently large disk centered at the origin (\(\gamma\subset D(0,R)\)). Then by applying \autoref{thm:cauchygoursatformula} or \autoref{thm:cauchygoursattheorem} on the set \(D(0,R)\cap\mathrm{ext}(\gamma)=D(0,R)\setminus\overline{\mathrm{int}(\gamma)}\), we get that \[\frac{1}{2\pi i}\int_{\partial D(0,R)}\frac{f(\zeta)}{\zeta-z}\ddzeta=\frac{1}{2\pi i}\int_\gamma\frac{f(\zeta)}{\zeta-z}\ddzeta+
                \begin{cases}
                    0    & z\in\mathrm{int}(\gamma)            \\
                    f(z) & z\in D(0,R)\cap\mathrm{ext}(\gamma)
            \end{cases}.\]
            By letting \(R\to\infty\) and letting \(\zeta=Re^{i\theta}\), we get that \[\frac{1}{2\pi i}\int_\gamma\frac{f(\zeta)}{\zeta-z}\ddzeta=\frac{1}{2\pi}\lim_{R\to\infty}\int_0^{2\pi}\frac{f\qty(Re^{i\theta})}{1-\frac{z}{Re^{i\theta}}}\dd{\theta}-
                \begin{cases}
                    0    & z\in\mathrm{int}(\gamma) \\
                    f(z) & z\in\mathrm{ext}(\gamma)
            \end{cases}.\]
            By the continuity of \(f\) on \(\partial D(0,R)\), it attains its maximum \(M\). For sufficiently large \(R\), \(\abs{1-\frac{z}{Re^{i\theta}}}\) attains a positive minimum. Then the integrand is uniformly bounded with respect to \(R\) under \(\theta\), and we can commute the limit with the integral. Hence,
            \begin{align*}
                \frac{1}{2\pi i}\int_\gamma\frac{f(\zeta)}{\zeta-z}\ddzeta & =\frac{1}{2\pi}\int_0^{2\pi}\frac{w}{1-\lim_{R\to\infty}\frac{z}{Re^{i\theta}}}\dd{\theta}-
                \begin{cases}
                    0    & z\in\mathrm{int}(\gamma) \\
                    f(z) & z\in\mathrm{ext}(\gamma)
                \end{cases} \\&=
        ...
        \item Under the partial fraction decomposition of equation~\eqref{eq:cauchygoursatformulaexteriorpart2_statement}, we get that
            \begin{align}
                ...
                & =\int_0^{2\pi}\qty(f\qty(Re^{i\theta})-\frac{f\qty(Re^{i\theta})}{1-\frac{z}{Re^{i\theta}}})\dd{\theta}+
                \begin{cases}
                    0          & z\in\mathrm{int}(\gamma)            \\
                    2\pi if(z) & z\in\mathrm{ext}(\gamma)\cap D(0,R)
                \end{cases},\label{eq:cauchygoursatformulaexteriorpart2_prelimitintegral}
            \end{align} when \(\gamma\subset D(0,R)\).
            We will analyze the first integral as \(R\to\infty\). By the triangle and reverse triangle inequalities, \[\qty|\int_0^{2\pi}\qty(f\qty(Re^{i\theta})-\frac{f\qty(Re^{i\theta})}{1-\frac{z}{Re^{i\theta}}})\dd{\theta}|\leq\int_0^{2\pi}\qty|\frac{z}{Re^{i\theta}-z}|\dd{\theta}\leq\int_0^{2\pi}\frac{|z|}{R-|z|}\dd{\theta}=\frac{2\pi|z|}{R-|z|}\to0.\]
    \end{enumerate}
    By plugging the result into equation~\eqref{eq:cauchygoursatformulaexteriorpart2_prelimitintegral}, and letting \(R\to\infty\), we get that \[\frac{1}{2\pi i}\int_\gamma\frac{zf(\zeta)}{z\zeta-\zeta^2}\ddzeta=
        \begin{cases}
            0    & z\in\mathrm{int}(\gamma) \\
            f(z) & z\in\mathrm{ext}(\gamma)
    \end{cases}.\]
...
\begin{example}\label{ex:legendrepolynomialintegralformula}
    ...
    \end{equation}
    Prove the integral form \[P_n(z)=\frac{1}{2\pi i}\oint_\gamma\frac{\qty(\zeta^2-1)^n}{2^n(\zeta-z)^{n+1}}\ddzeta,\] where \(\gamma\) is a simple closed curve enclosing \(z\).
\end{example}
\begin{proof}
...
\begin{theorem}[Argument Principle for Holomorphic Functions]\label{thm:argumentprincipleholomorphic}
    ...
    \[k=\frac{1}{2\pi i}\oint_{\gamma}\frac{f'(z)}{f(z)}\ddz.\]
...
\begin{proof}
    ...
    is holomorphic on the domain \[\mathrm{int}\paren{\gamma}\setminus\bigcup_{j=1}^n D\qty(z_j,\varepsilon_j),\]
    ...
    \[\oint_{\partial D\paren{z_j,\varepsilon_j}}\frac{k_j}{z-z_j}\ddz=2\pi ik_j.\]
    Combining results,
    \[\oint_{\gamma}\frac{f'(z)}{f(z)}\ddz=\sum_{j=1}^n2\pi ik_j=2\pi ik.\]
    Finally, parametrize \(\Gamma\) by \(w=f(z)\). Then \(\dd{w}=f'(z)\ddz\), and
    \[k=\frac{1}{2\pi i}\oint_{\Gamma}\frac{\dd{w}}{w}=\frac{1}{2\pi i}\Delta_\Gamma\log(w)=\frac{1}{2\pi}\Delta_\Gamma\arg(w),\] which proves the result.
\end{proof}
...
\begin{equation}
    \varphi_a(z)=\frac{z-a}{1-\overline{a}z}\in\Aut( \mathbb{D}).\label{eq:mobiustransformationgroupofholomorphicautomorphismsunitdiskstatement}
\end{equation}
...
\(\forall z\in D(0,R)\), \(\forall R>0\), we can apply the transformation \(\widetilde{\varphi}_z\qty(\zeta)=R\varphi_{\flatfrac{z}{R}}\qty(\frac{\zeta}{R})\) to extend the automorphism to \(D(0,R)\). Let \(\Phi\) instead be harmonic on \(D(0,R)\) and continuous on \(\overline{D(0,R)}\). Then, \[\Phi(0)=\frac{1}{2\pi}\int_0^{2\pi}\Phi\qty(Re^{i\psi})\dd{\psi}.\]
It follows that \(u=\Phi\circ\widetilde{\varphi}_z\) is also harmonic on \(D(0,R)\) with \(\Phi(0)=u\circ\widetilde{\varphi}_{-z}(0)=u(z)\), and from the bijectivity of  \(Re^{i\psi}=\widetilde{\varphi}_z\qty(Re^{i\tau})\),
...
\begin{proof}
    ...
    As integrals are defined with limits, we aim to prove that for fixed \(\xi=Re^{i\vartheta}\in\partial D(0,R)\),
    \begin{equation}
    ...
    where \(\zeta=Re^{i\tau}\) and \(z=\rho e^{i\theta}\), with \(\abs{\vartheta-\tau}>\delta\). Then \(\exists\eta>0\) such that \(\forall z\) with \(|\xi-z|<\eta\),
    \begin{equation}
...
Consider \(R=1\), \(\zeta=e^{i\tau}\), and \(z=\rho e^{i\theta}\) in equation~\eqref{eq:poissonintegralformula2}:
\begin{gather}
    u(z)=\frac{1}{2\pi}\int_0^{2\pi} u(\zeta)\frac{1-|z|^2}{\abs{\zeta-z}^2}\dd{\tau} = \frac{1}{2\pi}\int_0^{2\pi}\frac{\qty(1-\rho^2)u\qty(e^{i\tau})\dd{\tau}}{\qty(e^{i\tau}-\rho e^{i\theta})\qty(e^{-i\tau}-\rho e^{-i\theta})} \nonumber \\
    =\frac{1}{2\pi}\int_0^{2\pi} \frac{\qty(1-\rho^2)u\qty(e^{i\tau})\dd{\tau}}{1+\rho^2-2\rho\cos(\theta-\tau)}.\label{eq:poissonintegralformulatrigonometricsubstitution}
\end{gather}
...
\begin{equation}
    u\qty(e^{i\theta})\sim\sum_{n=-\infty}^\infty a_n e^{in\theta},\qquad a_n=\frac{1}{2\pi}\int_{-\pi}^\pi u\qty(e^{i\tau})e^{-in\tau}\dd{\tau}=\frac{1}{2\pi}\int_0^{2\pi} u\qty(e^{i\tau})e^{-in\tau}\dd{\tau}. \label{eq:poissonintegralformulafourierseries}
\end{equation}
This series may diverge. Observe that continuity of \(u\) on the compact set \(\partial\mathbb{D}\) implies uniform boundedness: \(\exists M>0\) such that \(\abs{u\qty(e^{i\theta})} \leq M\) for all \(\theta\) (\autoref{thm:continuousfunctionboundedoncompact}). Consequently, \(\abs{a_n} \leq M\). Introducing factors \(\rho^{\abs{n}}\) with \(\abs{\rho}<1\) yields a convergent series:
\[\sum_{n=-\infty}^\infty a_n e^{in\theta}\rho^{\abs{n}}, \quad\abs{\sum_{n=-\infty}^\infty a_n e^{in\theta}\rho^{\abs{n}}}\leq\sum_{n=-\infty}^\infty\abs{a_n}\rho^{\abs{n}}\leq M \frac{1+\abs{\rho}}{1-\abs{\rho}}.\]
Substituting the coefficients gives
\begin{align*}
    \sum_{n=-\infty}^\infty a_n e^{in\theta}\rho^{\abs{n}}&=\sum_{n=-\infty}^\infty \qty(\frac{1}{2\pi}\int_0^{2\pi}u\qty(e^{i\tau})e^{-in\tau}\dd{\tau}) e^{in\theta}\rho^{\abs{n}}\\
    &=\frac{1}{2\pi} \sum_{n=-\infty}^\infty \int_0^{2\pi}\rho^{\abs{n}} u\qty(e^{i\tau}) e^{in(\theta-\tau)}\dd{\tau}.
\end{align*}
By the Weierstrass \(M\)-Test (\autoref{thm:weierstrassmtest}) and \autoref{thm:limitintegralswitch},
\begin{equation}
    \frac{1}{2\pi}\sum_{n=-\infty}^\infty \int_0^{2\pi} \rho^{\abs{n}}u\qty(e^{i\tau}) e^{in(\theta-\tau)} \dd{\tau}=\frac{1}{2\pi} \int_0^{2\pi} u\qty(e^{i\tau})\sum_{n=-\infty}^\infty \rho^{\abs{n}} e^{in(\theta-\tau)}\dd{\tau}.\label{eq:poissonintegralformulafourierseriespostintegralsummationswitch}
\end{equation}
The summation simplifies as follows:
\begin{align*}
    \sum_{n=-\infty}^\infty \rho^{\abs{n}} e^{in(\theta-\tau)}&=\sum_{n=0}^\infty \rho^n e^{in(\theta-\tau)}+\sum_{n=1}^\infty \rho^n e^{-in(\theta-\tau)}\\
    &=1+2\sum_{n=1}^\infty\rho^n\cos[n(\theta-\tau)]\\
    &=1+2\Re\sum_{n=1}^\infty\rho^n e^{in(\theta-\tau)} \\
    &=1+2\Re\qty[\frac{\rho e^{i(\theta-\tau)}}{1-\rho e^{i(\theta-\tau)}}] \\
...
\[\sum_{n=-\infty}^\infty a_n e^{in\theta}\rho^{\abs{n}}=\frac{1}{2\pi}\int_0^{2\pi}\frac{\qty(1-\rho^2)u\qty(e^{i\tau})}{1+\rho^2-2\rho\cos(\theta-\tau)}\dd{\tau}=u\qty(\rho e^{i\theta}).\]
...
\[\lim_{\rho\to1^-}\sum_{n=-\infty}^\infty a_n e^{in\theta}\rho^{\abs{n}}=u\qty(e^{i\theta}).\]
...
\begin{theorem}\label{thm:meanvaluepropertysolutionsareharmonic}
    ...
    \[f(z_0)=\frac{1}{2\pi}\int_0^{2\pi} f\qty(z_0+\varepsilon e^{it})\dd{t}.\]
...
\[k-k'=\frac{1}{2\pi i}\oint_{\gamma}\frac{f'(z)}{f(z)}\ddz.\]
...
\[\frac{f'(z)}{f(z)}=\frac{k_j}{z-z_j}+\frac{h_j'(z)}{h_j(z)}.\]
Integration yields \(\oint_{\partial D\paren{z_j,\varepsilon_j}}\frac{f'(z)}{f(z)}\ddz=2\pi i k_j\) since \(\frac{h_j'}{h_j}\) is holomorphic. Near each pole \(p_j\), write \(f(z)=\paren{z-p_j}^{-k'_j}g_j(z)\) with \(g_j\) holomorphic and non-vanishing. Differentiation gives
\[\frac{f'(z)}{f(z)}=-\frac{k'_j}{z-p_j}+\frac{g_j'(z)}{g_j(z)}.\]
Integration yields \(\oint_{\partial D\paren{p_j,\varepsilon'_j}}\frac{f'(z)}{f(z)}\ddz=-2\pi i k'_j\). Combining results,
\[\oint_{\gamma}\frac{f'(z)}{f(z)}\ddz=\sum_{j=1}^m2\pi i k_j+\sum_{j=1}^n\qty(-2\pi i k'_j)=2\pi i\paren{\sum_{j=1}^m k_j-\sum_{j=1}^n k'_j}=2\pi i\qty(k-k').\]
Parametrize \(\Gamma\) by \(w=f(z)\). Then \(\dd{w}=f'(z)\ddz\), and
\[k-k'=\frac{1}{2\pi i}\oint_{\Gamma}\frac{\dd{w}}{w}=\frac{1}{2\pi i}\Delta_\Gamma\log(w)=\frac{1}{2\pi}\Delta_\Gamma\arg(w),\]
...
\end{proof}
\subsubsection{The Group of Holomorphic Automorphisms on \texorpdfstring{\(\extcomplex\)}{the extcomplex}}
...
\begin{definition}[Quotient Group]\label{def:quotientgroup}
    Let \(G\) be a group, and let \(H\trianglelefteq G\) be a normal subgroup. Then the \textit{quotient group} of \(G\) by \(H\) is defined as \[G/H=\cbraces{gH}{g\in G}.\] The operation (denoted with juxtaposition as well) on the quotient group is defined as \((g_1H)(g_2H)=g_1g_2H\), where \(g_1,g_2\in G\) and \(g_1H,g_2H\in G/H\).
\end{definition}
The normality of \(H\) implies the existence of the quotient group. Let \(g_1,g_1'\in G\) form the same left coset (\(g_1H=g_1'H\)), and let \(g_2,g_2'\in G\) satisfy \(g_2H=g_2'H\). We must ensure that
\begin{equation*}
...
where \(I=\mqty(\imat{2})\). The quotient group (a group of elements in the form \(\qty{A,-A}\)) is taken because \(\mqty(a&b\\c&d)\) refers to the same transformation as \(\mqty(-a&-b\\-c&-d)\). This group, denoted by \(\mathrm{PSL}_2(\mathbb{C})=\left.\mathrm{SL}_2(\mathbb{C})\middle/\qty{\pm I}\right.\cong\Aut\qty(\extcomplex)\), is known as the \textit{projective special linear group} of order 2, and is \textit{isomorphic} (one-to-one) to the \textit{group of Möbius transformations}, consisting of all fractional linear transformations.

Therefore, any holomorphic automorphism on \(\extcomplex\) is a composition of rotations, dilations, translations, and inversions. We will now state this formally:
\begin{theorem}[The Holomorphic Automorphism Group on \(\extcomplex\)]\label{thm:holomorphicautomorphismgrouponextendedcomplexplane}
...
\end{proof}
We have now introduced three of the most important regions in complex analysis: \(\mathbb{D}\), \(\mathbb{C}\), and \(\extcomplex\). Their importance will be briefly explained by the Uniformization Theorem (\autoref{thm:uniformization}).

Let \(\mathbb{H}^+=\cbraces{z\in\mathbb{C}}{\Im(z)>0}\) and let \(\overline{\mathbb{H}^+}=\mathbb{H}^+\cup\mathbb{R}\).
\begin{example}
    Find the group of holomorphic automorphisms on \(\mathbb{H}^+\).
\end{example}
\begin{proof}
    Let \(z=x+iy\), for \(x,y\in\mathbb{R}\). For \(\Im(z)>0\), the function defined by \(\varphi(z)=\frac{z-i}{z+i}\), known as the \textit{Cayley transform}, satisfies \[\abs{\varphi(z)}=\sqrt{\frac{x^2+(y-1)^2}{x^2+{(y+1)}^2}}<1\]
    since \(y>0\). Therefore, \(\varphi\) maps \(\mathbb{H}^+\) to \(\mathbb{D}\). Since \(\varphi\in\Aut\qty(\extcomplex)\), it is univalent. Furthermore, \(\varphi^{-1}(z)=i\frac{1+z}{1-z}\) is also univalent. Let \(\abs{z}<1\) be arbitrary. Then,
    \begin{align*}
        \Im\qty(\varphi^{-1}(z))=\Re\qty(\frac{z+1}{1-z})=\Re\qty(\frac{(1+z)\qty(1-\overline{z})}{\abs{1-z}^2})=\Re\qty(\frac{1-\Im(z)^2-\Re(z)^2+2i\Im(z)}{\abs{1-z}^2}),
    \end{align*}
    which is positive since \(\Im(z)^2+\Re(z)^2<1\) and the last term is purely imaginary. Hence, \(\varphi\) maps \(\mathbb{H}^+\) to \(\mathbb{D}\) bijectively.

    Let \(f\in\Aut\qty(\mathbb{H}^+)\) be arbitrary. It follows that \(\varphi\circ f\circ\varphi^{-1}\in\Aut\qty(\mathbb{D})\). By \autoref{thm:holomorphicautomorphismgrouponunitdisk}, for some \(|a|<1\) and for some \(\tau\in\mathbb{R}\), we have \[\varphi\circ f\circ\varphi^{-1}=\varphi_a\circ\rho_\tau(z),\]
    where \(\varphi_a\) is in the form of equation~\eqref{eq:mobiustransformationgroupofholomorphicautomorphismsunitdiskstatement} and \(\rho_\tau(z)=ze^{i\tau}\). Then we have \[f=\varphi^{-1}\circ\varphi_a\circ\rho_\tau(z)\circ\varphi.\]
    TO BE CONTINUED
\end{proof}
...
For \(x\in\mathbb{R}_{\geq0}\), since \(e^x\geq x\) and \(e^0=1\), we can integrate over \([0,x]\) to get that \(e^x\geq x+1\). Therefore, \[\exp(\sum_{k=1}^n\abs{u_k})\geq\prod_{k=1}^n \qty(1+\abs{u_k})=1+\sum_{k=1}^n\abs{u_k}+\sum_{\substack{j,k\in\qty{1,\ldots,n}\\j<k}}\abs{u_j u_k}+\cdots+\prod_{k=1}^n\abs{u_k}>\sum_{k=1}^n\abs{u_k}.\]
...
\begin{enumerate}
    \item If \(f\) has no zeros in \(\mathbb{C}\), then the function defined by \(z\mapsto\frac{f'(z)}{f(z)}\) is entire, it is the derivative of an entire function \(\varphi(z)\). Therefore, the function defined by \(z\mapsto f(z)e^{-\varphi(z)}\) has the vanishing derivative \(\dv{z}(f(z)e^{-\varphi(z)})=f'(z)e^{-\varphi(z)}-\varphi'(z)f(z)e^{-\varphi(z)}=0\). It follows that \(f(z)e^{-\varphi(z)}\) is constant, and therefore \(f(z)=c\exp(\varphi(z))\) for some constant \(c\in\mathbb{C}\). Since \(\varphi(z)\) is entire, it follows that \(f(z)\) is also entire. Absorb the constant \(c\) into \(\varphi(z)\), and \(f(z)=\exp(\varphi(z))\).\label{itm:entirefunctionconstructednonvanishing}
    ...
    \item If \(f(z)\) is entire and has infinitely many zeros such that \(f\) is not identically zero. It follows that \(f\) has countably many zeros (since the zeros are isolated). Let the zeros be indexed by \(\mathbb{N}\), namely \(a_1,a_2,\ldots\). Without loss of generality, assume that \(\forall n\in\mathbb{N}\), \(0<\abs{a_n}\leq\abs{a_{n+1}}\) (repeated elements representing multiplicities), and \(\lim_{n\to\infty}a_n=\infty\). The case for a zero at \(0\) will be treated differently.
        ...
        \end{align}
        By the definition of \(\qty{p_n}_{n\in\mathbb{N}}\), the series \(2\sum_{k=1}^\infty\abs{\frac{R}{a_n}}^{p_n+1}\) converges. Therefore, \(\sum_{k=1}^\infty Q_{p_n}\qty(\frac{z}{a_n})\) is uniformly and absolutely convergent on \(\overline{D(0,R)}\) by the Weierstrass \(M\)--Test (\autoref{thm:weierstrassmtest}). Thus, we get that \(\prod_{n=N}^\infty E_{p_n}\qty(\frac{z}{a_n})=\exp(\sum_{n=N}^\infty Q_{p_n}\qty(\frac{z}{a_n}))\) and \textit{uniformly converges} on \(\overline{D(0,R)}\) to a nonzero holomorphic function \(f(z)\) on \({D(0,R)}\) by \autoref{thm:weierstrassuniformlimit} and \autoref{thm:hurwitzsimplecase}.

        The zeros of \[\prod_{n=1}^\infty E_{p_n}\qty(\frac{z}{a_n})\] are \(a_1,\ldots, a_{N-1}\) and lie in \(\overline{D(0,2R)}\). To prove the absolute convergence of \(\prod_{n=N}^\infty E_{p_n}\qty(\frac{z}{a_n})\) on \(\overline{D(0,R)}\), we will show that \(\sum_{n=N}^\infty\abs{E_{p_n}\qty(\frac{z}{a_n})-1}\) is convergent. Trivially, when \(\zeta\in\overline{\mathbb{D}}\), we have \(\abs{\exp(\zeta)-1}\leq\exp\qty|\zeta|-1\leq\paren{e-1}\abs{\zeta}\). By equation~\eqref{eq:infiniteproductweierstrassfactorizationuniformbound} above, we get that \(\qty|Q_{p_n}\qty(\frac{z}{a_n})|\leq 1\) when \(n\geq N\).

        Therefore, we have \[\abs{E_{p_n}\qty(\frac{z}{a_n})-1}=\abs{\exp(Q_{p_n}\qty(\frac{z}{a_n}))-1}\leq (e-1)\abs{Q_{p_n}\qty(\frac{z}{a_n})}\leq2(e-1)\abs{\frac{R}{a_n}}^{p_n+1},\]
        which has a convergent series by definition. Therefore, \(\prod_{n=N}^\infty E_{p_n}\qty(\frac{z}{a_n})\) is absolutely convergent on \(\overline{D(0,R)}\). We can let \(R\to\infty\) and adjust the sequence \(\qty{p_n}_{n\in\mathbb{N}}\) accordingly.

        Therefore, for any such sequence \(\qty{a_n}_{n\in\mathbb{N}}\), we can define the entire function
        \begin{equation}
        ...
        \end{theorem}
        \begin{proof}
            By the argument of equation~\eqref{eq:entirefunctionconstructedfrominfinitelymanyzeros}, construct \(\psi(z)\) to be entire and have zeros at \(\qty{a_n}_{n\in\mathbb{N}}\). Thus, \(z^m\psi(z)\) and \(f(z)\) have the same zeros and corresponding multiplicities. Then the function \(z^m\frac{\psi(z)}{f(z)}\) has removable singularities on all of \(\qty{a_n}_{n\in\mathbb{N}}\cup\qty{0}\) and has an analytic continuation (\autoref{thm:removablesingularitiesriemann}) to an entire and non-vanishing function. Therefore, it can be written as \(z^m\frac{\psi(z)}{f(z)}=e^{\phi(z)}\), where \(\phi\). Let \(\varphi=-\phi\), and from rearrangement, we obtain equation~\eqref{eq:weierstrassfactorization_statement}
        \end{proof}
\end{enumerate}
...
\begin{definition}[Residue]\label{def:residue}
    ...
    \begin{equation}
        \residue_{z=a}f(z)=\frac{1}{2\pi i}\int_{\partial D(a,\rho)}f(z)\ddz,\label{eq:residue}
    \end{equation}
    where \(0<\rho<r\) is arbitrary. Since \(f\) has a Laurent expansion at \(a\), being \[\sum_{n=-\infty}^\infty c_n(z-a)^n,\qquad c_n=\frac{1}{2\pi i}\int_{\partial D(a,\rho)}\frac{f(z)\ddz}{(z-a)^{n+1}},\] we get that the residue of \(f\) at \(a\) is equal to the first term \(c_{-1}\) of the principal part of its Laurent expansion.
\end{definition}
...
Let \(z=\infty\) be an isolated singularity of \(f(z)\), which is holomorphic in \(\mathbb{C}\setminus\overline{D(0,R)}\), for sufficiently large finite \(R\). Then for finite \(\rho>R\), the residue at \(z=\infty\) is \textit{defined} as (notice the orientation) \[\residue_{z=\infty}f(z)=\frac{1}{2\pi i}\ointclockwise_{\partial D(0,\rho)}f(z)\ddz.\]
Let \(\zeta=\frac{1}{z}\). Then we get that \[\residue_{z=\infty}f(z)=-\frac{1}{2\pi i}\ointctrclockwise_{\partial D(0,\rho)}f\qty(\frac{1}{\zeta})\dd(\frac{1}{\zeta})=\frac{1}{2\pi i}\ointclockwise_{\partial D\qty(0,\frac{1}{\rho})}\frac{f\qty(\frac{1}{\zeta})}{\zeta^2}\ddzeta=-\residue_{\zeta=0}\frac{f\qty(\frac{1}{\zeta})}{\zeta^2}.\]
...
\begin{theorem}[Residue Theorem]\label{thm:residuethm}
    Let \(U\subset\mathbb{C}\) be an open set with a simple closed boundary \(\partial U\). Suppose \(\cbraces{z_n}\subset U\) is a finite set and \(f(z)\) is holomorphic on \(U\setminus\cbraces{z_n}\) and continuous on \(\overline{U}\setminus\cbraces{z_n}\). Then, \[\int_{\partial U}f(z)\ddz=2\pi i\sum_{k=1}^n\Res(f,z_k)\]
\end{theorem}
\begin{proof}
...
\end{theorem}
\begin{proof}
    Let \(R>\max_{j\in\mathbb{N}_{\leq n}}\qty|z_n|\) be arbitrary. By the Residue Theorem (\autoref{thm:residuethm}), \[-\residue_{z=\infty}f(z)=\frac{1}{2\pi i}\int_{\partial D(0,R)}f(z)\ddz=\sum_{k=1}^n\residue_{z=z_k}f(z)\]
...
\begin{figure}
    ...
    \caption{The orientation of a neighborhood that does not enclose \(\infty\) after projection.}
    \label{fig:stereographicprojectionofneighborhood}
...
\begin{proof}
    ...
    We will now analyze each integral. The first integral is \[\int_{C_R}f(z)\ddz=\int_0^\pi\frac{\exp(iRe^{i\theta})}{Re^{i\theta}}Rie^{i\theta}\dd{\theta}=i\int_0^{\pi}e^{iR\cos\theta}e^{-R\sin\theta}\dd{\theta}.\]
    Notice that \(\frac{2}{\pi}\theta\leq\sin(\theta)\leq\theta\) over the integration range. We want to observe the behavior as \(R\to\infty\):
    \begin{align*}
        \abs{i\int_0^\pi e^{iR\cos\theta}e^{-R\sin\theta}\dd{\theta}} & \leq\int_0^{\pi}e^{-R\sin\theta}\dd{\theta}=2\int_0^{\frac{\pi}{2}}e^{-R\sin\theta}\dd{\theta}                                                                  \\
        & <2\int_0^{\frac{\pi}{2}}e^{-R\frac{2}{\pi}\theta}\dd{\theta}=\eval{-\frac{\pi}{R}e^{-R\frac{2}{\pi}\theta}}_0^{\frac{\pi}{2}}=\frac{\pi}{R}\qty(1-e^{-R})\to 0.
    \end{align*}
    Let us evaluate the integral on \(\gamma_\varepsilon\) as \(\varepsilon\to 0\): \[\int_{C_\varepsilon}f(z)\ddz=i\int_{\pi}^0\exp(\varepsilon\qty(i\cos\theta-\sin\theta))\dd{\theta}=i\int_\pi^0e^{-\varepsilon\sin\theta}e^{i\varepsilon\cos\theta}\dd{\theta}.\]
    Obviously, \[\abs{e^{-\varepsilon\sin\theta}e^{i\varepsilon\cos\theta}}\leq 1,\]
    and therefore, the integral and the limit may commute:
    \[\lim_{\varepsilon\to 0}\int_{C_\varepsilon}f(z)\ddz=i\int_{\pi}^0\lim_{\varepsilon\to 0^+}e^{-\varepsilon\sin\theta}e^{i\varepsilon\cos\theta}\dd{\theta}=i\int_{\pi}^0\dd{\theta}=-i\pi.\]
    Evaluating the integral over the line segments, we have
    \begin{align*}
        \int_{-R}^{-\varepsilon}f(z)\ddz+\int_{\varepsilon}^{R}f(z)\ddz & =\int_{-R}^{-\varepsilon}\frac{e^{iz}}{z}\dd{z}+\int_{\varepsilon}^{R}\frac{e^{iz}}{z}\dd{z}=\int_{-R}^{-\varepsilon}\frac{e^{iz}}{z}\dd{z}-\int_{-R}^{-\varepsilon}\frac{e^{-iz}}{z}\dd{z} \\
        & \to2i\int_{-\infty}^0\frac{\sin{z}}{z}\ddz=2i\int_0^{\infty}\frac{\sin{z}}{z}\ddz.
    \end{align*}
    Hence, \[-i\pi+2i\int_{0}^\infty\frac{\sin(z)}{z}\ddz=0\Longleftrightarrow\int_{0}^\infty\frac{\sin{z}}{z}\ddz=\frac{\pi}{2}.\]
\end{proof}
...
\begin{proof}
    ...
    \end{figure}Let \(f(z)=e^{iz^2}\). Choose the wedge contour composed of \[\Gamma_1=\cbraces{r\in\mathbb{R}}{0\leq r\leq R},\quad\Gamma_2=\cbraces{re^{i\frac{\pi}{4}}}{0\leq r\leq R},\quad C_R=\cbraces{Re^{i\theta}}{0\leq\theta\leq\frac{\pi}{4}}\] as in \autoref{fig:wedgecontour}. By the Cauchy--Goursat Integral Theorem (\autoref{thm:cauchygoursattheorem}), we have that
    \begin{equation}
        \int_{\Gamma_1}f(z)\ddz+\int_{\Gamma_2}f(z)\ddz+\int_{C_R}f(z)\ddz=0.\label{eq:fresnelwedgecontourintegral}
    \end{equation} The third integral can be written as \[\int_{C_R}f(z)\ddz=Ri\int_0^{\frac{\pi}{4}}e^{i{\qty(Re^{i\theta})}^2}e^{i\theta}\dd{\theta}.\]
    Using the fact that \(\frac{4}{\pi}\theta<\sin(2\theta)\) on the integration range, it can be bounded as \[\abs{\int_{C_R}f(z)\ddz}\leq R{\int_0^{\frac{\pi}{4}}e^{-R^2\sin(2\theta)}\dd{\theta}}<R\int_0^{\frac{\pi}{4}}e^{-\frac{4}{\pi}R^2\theta}\dd{\theta}=-\frac{\pi}{4R}\eval{e^{-\frac{4}{\pi}R^2\theta}}_0^{\frac{\pi}{4}}=\frac{\pi}{4R}\qty(1-e^{-R^2}).\]
    As \(R\to\infty\), this integral tends to 0. Let \(z=re^{i\frac{\pi}{4}}\) on \(\Gamma_2\). Then, we have \[\lim_{R\to\infty}\int_{\Gamma_2}f(z)\ddz=\int_\infty^0\exp[i\qty(re^{i\frac{\pi}{4}})^2]e^{i\frac{\pi}{4}}\dd{r}=e^{i\frac{\pi}{4}}\int_\infty^0\exp(-r^2)\dd{r}.\]
    From equation~\eqref{eq:fresnelwedgecontourintegral}, we have that \[\int_0^\infty e^{ir^2}\dd{r}=e^{i\frac{\pi}{4}}\int_0^\infty e^{-r^2}\dd{r}.\] Since \(\int_0^\infty e^{-r^2}\dd{r}=\frac{\sqrt{\pi}}{2}\), we have \[\int_0^\infty e^{ir^2}\dd{r}=\qty(\frac{\sqrt{2}}{2}+i\frac{\sqrt{2}}{2})\frac{\sqrt{\pi}}{2}.\]
    Since \(e^{ir^2}=\cos(r^2)+i\sin(r^2)\), we have \[\int_0^\infty\cos(r^2)\dd{r}=\Re\qty[\int_0^\infty e^{ir^2}\dd{r}]=\frac{\sqrt{2\pi}}{4},\qquad\int_0^\infty\sin(r^2)\dd{r}=\Im\qty[\int_0^\infty e^{ir^2}\dd{r}]=\frac{\sqrt{2\pi}}{4},\]
    as desired.
\end{proof}
\begin{example}
...
\end{example}
\begin{proof}
    Let \(z=e^{i\theta}\). Consequently, we have \(\cos\theta=\frac{z+z^{-1}}{2}\), \(\sin\theta=\frac{z-z^{-1}}{2i}\), and \(\ddz=ie^{i\theta}\dd{\theta}\), implying that \(\dd{\theta}=\frac{\ddz}{iz}\). Therefore, by the Residue Theorem (\autoref{thm:residuethm}), letting \(f(z)=\frac{1}{iz}\Phi\qty(\frac{z+z^{-1}}{2},\frac{z-z^{-1}}{2i})\), we have \[\int_0^{2\pi}\Phi(\cos\theta,\sin\theta)\dd{\theta}=\int_{\partial\mathbb{D}}f(z)\ddz=2\pi i\sum_{k=1}^n\residue_{z=z_k}f(z),\]
...
\end{example}
\begin{proof}
    Let \(f(z)=\frac{z^\alpha}{1+z^\beta}\) and let \(-\pi<\Arg(z)\leq \pi\) in the principal branches of \(z^\alpha=e^{\alpha\Log(z)}\) and \(z^\beta=e^{\beta\Log(z)}\). Then except for at the zeros of \(1+z^\beta\), \(f\) is holomorphic.

    The solutions to \(z^\beta=-1\) are \(z=\exp(i\frac{\pi}{\beta}+2ik\frac{\pi}{\beta})\). Choose a wedge contour with an angle of \(\frac{2\pi}{\beta}\) (as in \autoref{fig:wedgecontour}). The only singularity it encloses is \(\exp(i\frac{\pi}{\beta})\). Since it is a simple zero of \(\frac{1}{f}\), this singularity is a simple pole.

    The contour is the union of the following curves:
    \[\Gamma_1=\cbraces{r\in\mathbb{R}}{0\leq r\leq R},\quad\Gamma_2=\cbraces{r\exp(i\frac{2\pi}{\beta})}{0\leq r\leq R},\quad C_R=\cbraces{Re^{i\theta}}{0\leq\theta\leq \frac{2\pi}{\beta}},\]
    where \(R>1\). By the Residue Theorem (\autoref{thm:residuethm}), we get that \[\lim_{R\to\infty}\int_{\Gamma_1}f(z)\ddz+\int_{\Gamma_2}f(z)\ddz+\int_{C_R}f(z)\ddz=2\pi i\Res[f,\exp(i\frac{\pi}{\beta})].\]
    ...
    \begin{align*}
        ...
        &=\lim_{z\to\exp(i\frac{\pi}{\beta})}\frac{z^{\alpha+1}}{(\beta-\alpha)z^{\beta}-\alpha}\\
        &=-\frac{1}{\beta}\exp(i\frac{\pi}{\beta}(\alpha+1)).
    \end{align*}
    We can write the integral on \(\Gamma_2\) in terms of \(I\): \[\int_{\Gamma_2}f(z)\ddz=\int_R^0 f\qty[r\exp(i\frac{2\pi}{\beta})]\exp(i\frac{2\pi}{\beta})\dd{r}=-\exp[i\frac{2\pi}{\beta}(1+\alpha)]\int_0^R\frac{r^\alpha}{1+r^\beta}\dd{r}.\]
    We also have \[\int_{C_R}f(z)\ddz=Ri\int_{0}^{\frac{2\pi}{\beta}}f\qty(Re^{i\theta})e^{i\theta}\dd{\theta}=i\int_0^{\frac{2\pi}{\beta}}\frac{R^{\alpha+1}}{1+R^\beta e^{i\beta\theta}}\exp[i\theta(1+\alpha)]\dd{\theta}.\] We can show that the integrand can be dominated as follows:
    \[\abs{\int_0^{\frac{2\pi}{\beta}}\frac{R^{\alpha+1}}{1+R^\beta e^{i\beta\theta}}\exp[i\theta(1+\alpha)]\dd{\theta}}\leq\int_0^{\frac{2\pi}{\beta}}\frac{R^{\alpha+1}}{R^\beta-1}\dd{\theta}.\]
    ...
    By letting \(R\to\infty\), we have \[\qty[1-\exp(i\frac{2\pi}{\beta}(1+\alpha))]I=-\frac{2\pi i}{\beta}\exp(i\frac{\pi}{\beta}(\alpha+1)).\]
    It follows that \[I=\frac{2\pi i}{\beta}\brackets{\exp(i\frac{\pi}{\beta}(\alpha+1))-\exp(-i\frac{\pi}{\beta}(\alpha+1))}^{-1}=\frac{\pi}{\beta}\csc(\frac{\pi}{\beta}(\alpha+1)).\]
\end{proof}
\begin{example}
    Prove that the Fourier transform of \(\sech(\pi x)\) is itself, or that \[I(\xi)=\int_{-\infty}^\infty\exp(-2\pi ix\xi)\sech(\pi x)\ddx=\sech(\pi\xi).\]
\end{example}
\begin{proof}
    Fix \(\xi\in\mathbb{R}\) and let \(f(z)=\frac{\exp(-2\pi iz\xi)}{\cosh(\pi z)}\). Its poles in \(\mathbb{C}\) occur when \(e^{\pi z}+e^{-\pi z}=0\), or equivalently, when \(z=i\qty(n+\frac{1}{2})\), where \(n\in\mathbb{Z}\).
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[>=stealth,
            ...
            \node[anchor=north] at (3,0) {\(R\)};
            \node[anchor=north] at (-3,0) {\(-R\)};
            \node[anchor=south east] at (0,2) {\(i\)};
        \end{tikzpicture}
        \caption{A rectangular contour with orientation marked.}\label{fig:rectangularcontour}
    \end{figure}Since \[\cosh(\pi(z+i))=-\cosh(\pi z),\qquad\exp(-2\pi i(z+i)\xi)=\exp(2\pi\xi)\exp(-2\pi iz\xi),\] we have that \(f(z)\) is a constant multiple of \(f(z+i)\). In particular, \(f(z+i)=-\exp(2\pi\xi)f(z)\). Therefore, we can use a rectangular contour as shown in \autoref{fig:rectangularcontour}. Let the sides be denoted by
    \begin{gather*}
        \overleftarrow{\Gamma}=\cbraces{x+i}{-R\leq x\leq R,x\in\mathbb{R}},\qquad\overrightarrow{\Gamma}x=\cbraces{x\in\mathbb{R}}{-R\leq x\leq R}\\
        \overset{\downarrow}{\Gamma}=\cbraces{-R+iy}{y\in[0,1]},\qquad\overset{\uparrow}{\Gamma}=\cbraces{R+iy}{y\in[0,1]}.
    \end{gather*}
    The only enclosed singularity is a simple pole at \(z=\frac{i}{2}\) (simple by evaluation of the Taylor expansion of the denominator). By the Residue Theorem (\autoref{thm:residuethm}), we get that
    \begin{equation}
        \qty(\int_{\overrightarrow{\Gamma}}+\int_{\overset{\uparrow}{\Gamma}}+\int_{\overleftarrow{\Gamma}}+\int_{\overset{\downarrow}{\Gamma}})f(z)\ddz=2\pi i\Res(f,\frac{i}{2}).\label{eq:fouriertransformofsechpix_rectangularcontourintegral}
    \end{equation}
    By equation~\eqref{eq:residueatpole}, we have
    \begin{align*}
        \Res(f,\frac{i}{2})&=\lim_{z\to\frac{i}{2}}\qty(z-\frac{i}{2})\frac{\exp(-2\pi iz\xi)}{\cosh(\pi z)}\\
        &=\lim_{z\to\frac{i}{2}}\dv{z}(\frac{\cosh(\pi z)}{\exp(-2\pi iz\xi)})^{-1}\\
        &=\lim_{z\to\frac{i}{2}}\frac{\exp(-2\pi iz\xi)}{\pi\sinh(\pi z)+2\pi i\xi\cosh(\pi z)}\\
        &=\frac{\exp(\pi\xi)}{\pi i}.
    \end{align*}
    The sum of the horizontal line integrals is equal to \[\int_{-R}^R f(z)\ddz+\int_R^{-R}f(z+i)\ddz=\int_{-R}^R f(z)\ddz-\int_R^{-R}e^{2\pi\xi}f(z)\ddz=\qty(1+e^{2\pi\xi})\int_{-R}^R f(z)\ddz.\] As \(R\to\infty\), we have \(\int_{\overrightarrow{\Gamma}}f(z)\ddz+\int_{\overleftarrow{\Gamma}}f(z)\ddz\to\qty(1+e^{2\pi\xi})I(\xi)\).
    The remaining two integrals can be written as \[\int_{\overset{\uparrow}{\Gamma}}f(z)\ddz=\int_0^1 \frac{\exp(-2\pi i(R+iz)\xi)}{\cosh(\pi(R+iz))}\ddz,\qquad\int_{\overset{\downarrow}{\Gamma}}f(z)\ddz=\int_1^0 \frac{\exp(2\pi i(R-iz)\xi)}{\cosh(\pi(-R+iz))}\ddz.\]
    They can be bounded with \[\abs{\int_0^1 \frac{\exp(-2\pi i(R+iz)\xi)}{\cosh(\pi(R+iz))}\ddz}\leq 2\int_0^1\frac{\exp(2\pi z\xi)}{\abs{e^{\pi R}e^{\pi iz}+e^{-\pi R}e^{-\pi iz}}}\ddz\leq2\int_0^1\frac{\exp(2\pi z\xi)}{\abs{e^{\pi R}-e^{-\pi R}}}\ddz\] and \[\abs{\int_1^0 \frac{\exp(2\pi i(R-iz)\xi)}{\cosh(\pi(-R+iz))}\ddz}\leq 2\int_0^1\frac{\exp(2\pi z\xi)}{\abs{e^{-\pi R}e^{\pi iz}+e^{\pi R}e^{-\pi iz}}}\ddz\leq2\int_0^1\frac{\exp(2\pi z\xi)}{\abs{e^{\pi R}-e^{-\pi R}}}\ddz\]
    Since the integrands are continuous and uniformly convergent to \(0\) with respect to \(z\), we have \[\int_{\overset{\uparrow}{\Gamma}}f(z)\ddz+\int_{\overset{\downarrow}{\Gamma}}f(z)\ddz\to 0\] as \(R\to\infty\). By rearrangement of equation~\eqref{eq:fouriertransformofsechpix_rectangularcontourintegral}, \[I(\xi)\qty(1+e^{2\pi\xi})=2\exp(\pi\xi),\]
    or that \[I(\xi)=\frac{2}{e^{-\pi\xi}+e^{\pi\xi}}=\sech(\pi\xi),\] which proves the result.
\end{proof}
Contour integration is a powerful tool for evaluating real improper integrals. Often, the most challenging aspect is selecting an appropriate contour that encloses the relevant singularities.

Generally, if the function is even and integrated over \(\mathbb{R}_{\geq0}\), then extend the integral to the entire real line. If the function seems to decay sufficiently fast in \(\mathbb{H}^+\), consider using a semicircular contour in the upper half-plane, as shown in \autoref{fig:semicircularcontour}. If there are singularities along the real axis, then it is customary to insert small semicircular indentations around them, as in \autoref{fig:indentedsemicircularcontour}. If a function is a constant multiple of the function over an imaginary shift and the function vanishes near the ends of the real axis, then it is common to consider a rectangular contour.

The following result is useful for evaluating integrals on such indentations.
\begin{theorem}\label{thm:residueoverarc}
    ...
    \[\lim_{\varepsilon\to 0}\int_{\gamma_\varepsilon}f(z)\ddz=i\vartheta\cdot\residue_{z=a}f(z).\]
\end{theorem}
\begin{proof}
    Parameterize \(\gamma_\varepsilon\) with \(z=a+\varepsilon e^{i\theta}\), where \(\theta\in[\alpha,\beta]\) and \(\beta-\alpha=\vartheta\). Then,
    \[\int_{\gamma_\varepsilon} f(z)\ddz = \int_\alpha^\beta f\qty(a+\varepsilon e^{i\theta}) \dv{z}{\theta}\dd{\theta}=\varepsilon i \int_\alpha^\beta f\qty(a+\varepsilon e^{i\theta})e^{i\theta}\dd{\theta}.\]
    ...
    Then for \(z=a+\varepsilon e^{i\theta}\),
    \[f\qty(a+\varepsilon e^{i\theta})=\frac{c_{-1}}{\varepsilon e^{i\theta}}+\varphi(a+\varepsilon e^{i\theta}).\]
    So,
    \begin{align*}
        \int_{\gamma_\varepsilon}f(z)\ddz
        &= \varepsilon i\int_\alpha^\beta \qty(\frac{c_{-1}}{\varepsilon e^{i\theta}}+\varphi(a+\varepsilon e^{i\theta}))e^{i\theta}\dd{\theta} \\
        &= ic_{-1}\int_\alpha^\beta\dd{\theta}+\varepsilon i\int_\alpha^\beta\varphi\qty(a+\varepsilon e^{i\theta})e^{i\theta}\dd{\theta}\\
        &= ic_{-1}\vartheta+\varepsilon i\int_\alpha^\beta\varphi\qty(a+\varepsilon e^{i\theta})e^{i\theta}\dd{\theta}.
    \end{align*}
    ...
    \[\lim_{\varepsilon\to 0} \varepsilon i \int_\alpha^\beta\varphi\qty(a+\varepsilon e^{i\theta}) e^{i\theta}\dd{\theta}=\lim_{\varepsilon\to 0}\varepsilon i\int_\alpha^\beta\varphi(a)e^{i\theta}\dd{\theta}=0.\]
    Therefore,
    \[\lim_{\varepsilon \to 0}\int_{\gamma_\varepsilon}f(z)\ddz=i\vartheta\residue_{z=a}f(z),\] as desired.
\end{proof}
...
\begin{proof}
    ...
    Fix \(\zeta\in\partial\mathbb{D}\). Suppose \(f\) extends analytically to a disk \(D(\zeta,\delta)\). Since \(\exp(2\pi i\mathbb{Q})\) is dense in \(\partial\mathbb{D}\), there exists \(\zeta'=\exp(2\pi i\frac{p}{q})\in D(\zeta,\delta)\cap\partial\mathbb{D}\) for coprime integers \(p,q\). The extension \(g\) of \(f\) to \(D(\zeta, \delta)\) would satisfy:
...
\begin{example}
    Let \(\Log(z)\) denote the principal branch of \(\log(z)\), with \(-\pi<\Arg(z)\leq\pi\). The analytic elements \[\qty(D(1,1),\Log)\qand\qty(D(1,1),\Log+2\pi i)\] are analytic continuations of each other.
\end{example}
\begin{example}
    Show that the analytic functions defined by the series \(f(z)=\sum_{n=0}^\infty\alpha^n z^n\) and \(\widetilde{f}(z)=\sum_{n=0}^\infty\frac{(\alpha-1)^n z^n}{(1-z)^{n+1}}\) are analytic continuations of each other.
\end{example}
\begin{proof}
...
\end{theorem}
\begin{proof}
    Let \(w_0\) be a point such that \(w_0\) does not lie on \(\Gamma\) itself. By the Argument Principle (\autoref{thm:argumentprincipleholomorphic}), the number of zeros of \(f-w_0\) enclosed by \(\gamma\) is equal to \[k=\frac{1}{2\pi i}\oint_\gamma \frac{f'(z)}{f(z)-w_0}\ddz=\frac{1}{2\pi i}\oint_\Gamma\frac{\dd{w}}{w-w_0}=\frac{1}{2\pi}\Delta_\Gamma\arg(w-w_0).\] If \(w_0\notin U\), 
\end{proof}
...
The Riemann--\(\zeta\) function is one of the most important aspects in analytic number theory due to its connection with the distribution of prime numbers. We will also introduce one of the most profound theorems in number theory on the distribution of prime numbers.

Let us first introduce the \(\Gamma\) function, which generalizes the factorial. Let \[\Gamma(z)=\int_0^\infty e^{-t}t^{z-1}\dd{t},\]
where \(z\in\mathbb{C}\). By letting \(z=x+iy\) where \(x,y\in\mathbb{R}\), we have \(\abs{e^{-t}t^{z-1}}=e^{-t}t^{x-1}\)
\end{document}
