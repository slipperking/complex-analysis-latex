...
\end{theorem}
\begin{proof}
    Assume that \(z_0\) exists. We aim to prove that the set \(S=\cbraces{z}{f(z)=f\paren{z_0},z\in V}\) is all of \(V\). This is equivalent to proving that \(S\) is nonempty, open, and closed in \(V\).

    Since \(z_0\in S\), the first condition is satisfied (nonemptiness). For any sequence \(\cbraces{z_n}\in S\) converging to some \(z_\infty\in V\), by the continuity of \(f\), \[\lim_{n\to\infty} f\paren{z_n}=f\paren{\lim_{n\to\infty}z_n}=f\paren{z_\infty}=f\paren{z_0}.\] Thus, \(S\) contains all of its accumulation points in \(V\) and is therefore closed (if \(z_\infty\notin V\), then it is no longer relevant; we are concerned with it being closed within \(V\)).

    Since \(S\subseteq V\) and \(V\) is open, \(\forall z\in S\), \(\exists\varepsilon>0\) such that \(D(z,\varepsilon)\subseteq V\). By \autoref{lemma:holomorphicmeanvalueproperty}, \(\forall 0<\varepsilon'<\varepsilon\),
    \begin{align*}
    ...
    \end{align*}
    From the equality of the last two integrals, \(\int_{0}^{2\pi}\brackets{\abs{f(z)}-\abs{f\paren{z+\varepsilon'e^{it}}}}\dd{t}=0\). Since this integrand is strictly nonnegative, we have equality. Thus, \(\forall \zeta\in D(z,\varepsilon)\), \(\abs{f(z)}=\abs{f(\zeta)}\).
\end{proof}
