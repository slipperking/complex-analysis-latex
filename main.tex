...
\end{lemma}
\begin{proof}
    Because \(f\in C^0(G)\), there is a compact set \(D\subseteq G\) that \(f\) is continuous over. By \autoref{lemma:uniformcontinuousovercompactset}, \(\forall\varepsilon>0\), \(\exists\delta>0\) such that \(\forall z_0,z_1\in D\) satisfying \(\abs{z_1-z_0}<\delta\), \(\abs{f\paren{z_1}-f\paren{z_0}}<\varepsilon\). Dissect \(\Gamma\) into \(n\in\mathbb{N}\) curves \(s_0,s_1,\ldots,s_{n-1}\) such that the arc length for each curve is less than \(\delta\). Connect each pair of adjacent boundary points with a straight line to construct a polygonal chain \(l_0,l_1,\ldots,l_{n-1}\) with lengths each less than \(\delta\).
\end{proof}
\end{document}