...
\end{proof}
\begin{remark}
    In the first part, we established that a function \(\psi(z)\in C^0(\mathbb{C})\) with compact support satisfies \[\int_{\mathbb{C}}\frac{\psi(\zeta)}{\zeta-z}\dd{\overline{\zeta}}\wedge\ddzeta\in C^0(\mathbb{C})\]
    ...
    Then using the same argument, In general, for \(\psi(z)\in C^k(\mathbb{C})\), we can use the same process recursively to find that \(u(z)\in C^k(\mathbb{C})\) as well.
\end{remark}
