...
where \(\varphi_g\) and \(\psi_g\) are the holomorphic and principal parts of \(g\), respectively. Let \(\varphi\) It follows that \(f\) has the Laurent expansion at \(\infty\) of \(f(z)=\sum_{n=-\infty}^\infty c_nz^{-n}=\sum_{n=0}^\infty c_nz^{-n}+\sum_{n=1}^\infty c_{-n}z^n=\varphi_f(z)+\psi_f(z)\).

The classification of the singularity at \(\infty\) is then reduced to the classification of the singularity of \(g\) at \(0\):
\begin{enumerate}
    \item If \(z=\infty\) is a removable singularity of\(f(z)\), then \(f(z)\) has the form \[f(z)=c\]
\end{enumerate}
