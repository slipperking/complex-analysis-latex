...
\begin{proof}
    ...
    Since \(\abs{\varphi\qty(Re^{i\vartheta})-\varphi(\zeta)}<2M\), we want \(\int_{\abs{\vartheta-\tau}>\delta}\frac{R^2-\rho^2}{\pi R^2\qty(1-\cos(\frac{\delta}{2}))}\dd{\tau}<2\frac{R^2-\rho^2}{R^2\qty(1-\cos(\frac{\delta}{2}))}< \frac{\varepsilon}{2M}\), and \[R^2-\rho^2<\frac{\varepsilon}{4M}R^2\paren{1-\cos(\frac{\delta}{2})}\Longrightarrow R-\rho<\frac{\varepsilon}{6M}R\paren{1-\cos(\frac{\delta}{2})}.\]
    From \autoref{fig:dirichletproblemwithlaplaceequationsolution_secondintegral}, it is evident that \(R-\rho<|\xi-z|<\eta\). In order for \(|\theta-\tau|>\frac{\delta}{2}\) to be true, we can enforce that \(\abs{\vartheta-\theta}<\frac{\delta}{2}\). In other words \(\eta<R^2+\rho^2-2R\rho\cos(\vartheta-\tau-\delta)\). Obviously, this will be satisfied if \(|\xi-z|<\frac{R^2}{2}\qty(1-\cos(\vartheta-\tau-\delta))<2\rho^2\qty(1-\cos(\vartheta-\tau-\delta))\). Therefore, we can choose \(\eta=\min\qty(\frac{\varepsilon}{6M}R\paren{1-\cos(\delta)},\frac{R^2}{2}\qty(1-\cos(\vartheta-\tau-\delta)))\), the necessary requirements are satisfied.
