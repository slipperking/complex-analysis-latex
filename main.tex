...
\end{example}
\begin{proof}
    Since \(z^5-1=\prod_{k=0}^4\qty(z-\omega_k)\), where \(\omega_k=e^{i\pi\frac{k}{5}}\), the integrand has singularities at every fifth root of unity. We can pentasect \(D(0,2)\) into 5 sectors \(S_1,S_2,\ldots,S_5\) each containing only one singularity (without any singularities on their boundaries). By the Cauchy--Goursat Integral Formula (\autoref{thm:cauchygoursatformula}), the integral is equal to: \begin{equation}
