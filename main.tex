...
where \(I=\mqty(\imat{2})\). The quotient group (a group of "cosets" in the form \(\qty{A,-A}\)) is taken because \(\mqty(a&b\\c&d)\) refers to the same transformation as \(\mqty(-a&-b\\-c&-d)\). This group, denoted by \(\mathrm{PSL}(2,\mathbb{C})=\mathrm{SL}(2,\mathbb{C})/\qty{\pm I}\cong\Aut\qty(\riemannsphere)\), is known as the \textit{projective special linear group} of order 2, and is isomorphic to the \textit{group of Möbius transformations}, consisting of all fractional linear transformations.
