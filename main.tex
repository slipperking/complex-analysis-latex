...
\begin{proof}
    ...
    Since \(f\) is continuous, \(f(x_{n_k})\to f(x_\infty)\), so the sequence \(\cbraces{f\paren{x_{n_k}}}\) is bounded. But this contradicts \(\abs{f\paren{x_{n_k}}}>n_k\to\infty\), hence \(f\) must be bounded on \(U\).
\end{proof}
