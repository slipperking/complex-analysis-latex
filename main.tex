...
\begin{proof}
    ...
    By properties of the exhaustion, \(W_n\supseteq V_n\). \(\lbrace V_n\rbrace\) forms compact shells around \(K_{n-1}\), and therefore, \[\bigcup_{n\in\mathbb{N}} V_n=\Omega.\]
    ...
    Let \(\lbrace U_j\rbrace\) be a collection formed by enumerating \(U_{z_{r,i},r}\) where \(r\in\mathbb{N}\) and \(i=1,\ldots,k_r\). The first clause of the statement is true since each of \(V_r\) is covered by some subcollection of \(\left\{U_j\right\}\), which means that the entire collection covers \(\Omega\). Since \(\forall K\subset\Omega\) that is compact, \(K\) intersects each set of some finite subcollection of \(\left\{W_j\right\}\), with each set of the subcollection containing the union of only a finite subcollection of \(\left\{U_j\right\}\). This property is known as \textit{local finiteness}.
\end{proof}
