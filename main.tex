...
\begin{proof}
    ...
    First we will show that $X$ is bounded. Suppose that $\forall R>0$, $\exists x\in X$ where $\|x\|>R$. Consider the collection of open sets \[U=\{B(0,k)\mid k\in\mathbb{N}\}\] where $B(a,b)$ is the open ball with radius $b$ centered at $a$. $U$ forms an open cover of $X$. Then there exists a finite subcover $\{B(0,k_1),\ldots,B(0,k_m)\}$ covering $X$. Then, \[X\subseteq\bigcup_{i=1}^mB(0,k_i)=B(0,\max(k_1,\ldots k_m)).\] By contradiction, \(X\) must be bounded.
