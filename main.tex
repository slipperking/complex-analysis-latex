...
\end{lemma}
\begin{proof}
    Because \(f\in C^0(G)\), there is a compact set \(D\subseteq G\) that \(f\) is continuous over. By \autoref{lemma:uniformcontinuousovercompactset}, \(\forall\varepsilon>0\), \(\exists\delta>0\) such that \(\forall z_0,z_1\in D\) satisfying \(\abs{z_1-z_0}<\delta\), \(\abs{f\paren{z_1}-f\paren{z_0}}<\varepsilon\). Dissect \(\Gamma\) into points such that 
\end{proof}
\end{document}