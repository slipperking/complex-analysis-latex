...
\begin{proof}
    ...
    We then prove the converse. By the assumption that \(X\) is bounded, \(\exists R>0\) such that the $X$ is contained within the closed box \[X\subseteq \left\{z\;\middle|\; z\in\mathbb{C}^n, \max_{i\in\{1,\ldots,n\}}\left|\Re(z_i)\right|\le R,\max_{i\in\{1,\ldots,n\}}\left|\Im(z_i)\right|\le R\right\}.\] 

    Assume that there exists an infinite open cover \(\mathcal{U}\) of \(X\) without a finite subcover. Bisect the cube into $2^{2n}$ sub-boxes. To cover each 
\end{proof}
