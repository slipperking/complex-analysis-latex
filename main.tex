...
Since \[\int_0^\infty\sum_{n=1}^\infty\abs{\ee^{-xn}x^{s-1}}\ddx=\int_0^\infty\sum_{n=1}^\infty \ee^{-xn}x^{\sigma-1}\ddx=\int_0^\infty \frac{x^{\sigma-1}}{e^{-x}-1}\ddx\] is convergent, by Fubini's Theorem (with a counting measure on \(\mathbb{N}\) and a Lebesgue measure on \(\mathbb{R}_{\geq 0}\)), the operations of integration and summation may be exchanged (a statement in measure theory that we now take for granted). Therefore, we have
\begin{equation}
    \Gamma(s)\zeta(s)=\int_0^\infty\frac{x^{s-1}}{e^{-x}-1}\ddx.\label{eq:gammazetaproduct}
\end{equation}
\end{document}
