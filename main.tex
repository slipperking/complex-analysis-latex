...
\begin{proof}
	...
	\end{figure}Consider \(\gamma\) to be a closed semicircle with radius \(R\) as in \autoref{fig:semicircularcontour}. Notice that the function \(z\mapsto\frac{1}{\qty(z^2+1)^{n+1}}\) has singularities at only \(z=i\) and \(z=-i\), both of which are poles of order \(n+1\). By equation~\eqref{eq:residueatpole}, the residue at \(z=i\) is \[\residue_{z=i}\frac{1}{\qty(z^2+1)^{n+1}}=\left.\frac{1}{n!}\dv[n]{z}((z+i)^{-n-1})\right|_{z=i}=\frac{1}{n!}\frac{(-1)^n\prod_{k=1}^n (n+k)}{(2i)^{2n+1}}\right=\frac{(-1)^n(2n)!}{(n!)^2(2i)^{2n+1}}=\frac{i(2n!)}{2^{2n+1}(n!)^2}.\] Similarly, the residue at \(z=-i\) is \(\residue_{z=-i}\frac{1}{\qty(z^2+1)^{n+1}}=-\frac{1}{(n!)2i}\).
\end{proof}
