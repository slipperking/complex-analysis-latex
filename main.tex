...
\begin{theorem}[Weierstrass Factorization Theorem]\label{thm:weierstrassfactorizationtheorem}
	Let \(\qty{a_n}_{n\in\mathbb{N}}\cap\qty{0}=\emptyset\) be a sequence of complex numbers such that \(a_n\to\infty\) as \(n\to\infty\) and \(\qty|a_n|\leq\qty|a_{n+1}|\) (multiplicities counted as multiple roots) for every \(n\). Let \(m\) be the
\end{theorem}
