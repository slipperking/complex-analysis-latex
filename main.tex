...
\begin{proof}
    ...
    Since \(z_j\) has multiplicity \(k_j\), \(f(z)\) can be locally written as \(\paren{z-z_j}^{k_j}h(z)\), where \(\forall z\in D\paren{z_j,\varepsilon_j}\), \(h\paren{z}\neq 0\). Analyzing the integrand's numerator,
    \[f'(z)=k_j\paren{z-z_j}^{k_j-1}\]
\end{proof}
