...
\begin{proof}
    ...
    Since \(z_0\in S\), the first condition is satisfied (nonemptiness). Since \(f\) is continuous in \(U\), \(S\) is closed in \(U\), as \(\cbraces{f\paren{z_0}}\) is closed, and so is its preimage on \(f\). 
\end{proof}
