...
\begin{proof}
   ...
   We now show that \(z_\infty\in X\). Suppose for contradiction that \(z_\infty \notin X\). Since \(X\) is closed, its complement is open. So there exists an open neighborhood \(D(z_\infty, \delta)\) such that
    \[
    D(z_\infty, \delta) \subset \mathbb{C}^n \setminus X.
    \]
    But then, for sufficiently large \(k\), we have \(Q_k \subset D(z_\infty, \delta)\), and hence \(Q_k \cap X = \emptyset\). This contradicts the construction of \(Q_k\), which ensures that \(Q_k\) contains infinitely many points of \(A \subset X\).

    Therefore, every infinite subset of a compact set \(X\) has an accumulation point in \(X\).
\end{proof}
