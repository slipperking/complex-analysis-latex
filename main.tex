...
\begin{theorem}[Weierstrass \(M\)--Test]\label{thm:weierstrassmtest}
    Let \(U\subseteq\mathbb{C}\), and suppose that \(\forall n\in\mathbb{N}\), \(f_n:U\to\mathbb{C}\) is a function such that \[\qty|f_n(z)|\le M_n\]
where \( M_n \in \mathbb{R} \) and the series \( \sum_{n=1}^\infty M_n \) converges. 
Then the series \( \sum_{n=1}^\infty f_n(z) \) converges \textbf{uniformly and absolutely} on \( U \).
\end{theorem}
\begin{proof}
