...
\begin{definition}[Quotient Group]\label{def:quotientgroup}
	Let \(G\) be a group, and let \(H\leq G\) be a normal subgroup. Then the \textit{quotient group} of \(G\) by \(H\) is defined as \[G/H=\cbraces{gH}{g\in G}.\] The operation (\(*\)) on the quotient group is defined as \((g_1H)*(g_2H)=g_1g_2H\), where \(g_1,g_2\in G\) and \(g_1H,g_2H\in G/H\).
\end{definition}
