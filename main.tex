...
\begin{proof}
    ...
    We can define the function \[\varphi_\varepsilon(x)=\begin{cases}
        ...
        1&\abs{x}<\varepsilon
    \end{cases},\]
    which satisfies \(\forall\varepsilon>0\), \[\int_{-\infty}^\infty \varphi_\varepsilon(x)\ddx=1,\] \(\varphi_\varepsilon\in C^\infty(\mathbb{R})\) and \(\supp\paren{\varphi_\varepsilon}\) is compact. We can extend this to \(\varphi_\varepsilon:\mathbb{C}\to\mathbb{R}\) with \[\varphi_\varepsilon(z)=\begin{cases}
        \phi\paren{1+\varepsilon-\abs{z}}&\abs{z}\ge\varepsilon\\
        1&\abs{z}<\varepsilon
    \end{cases}.\] The same properties apply as the real case.

    Choose \(r\in\mathbb{R}\) such that \(V\subset D(0,r)\) and define \(\varphi(z)=\varphi_r(z)\). Since \(f\in C^1\paren{\overline{U}}\), by Pompeiu's Theorem (\autoref{thm:pompeiu}), \[f(z)\varphi(z)=\int_{\partial V}f(\zeta)\varphi(\zeta)\ddzeta-\int_{V}\pdv{f(\zeta)\phi(\zeta)}{}\]
\end{proof}
