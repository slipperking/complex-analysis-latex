...
\end{proof}
Contour integration is a powerful tool for evaluating real improper integrals. Often, the most challenging aspect is selecting an appropriate contour that encloses the relevant singularities. We will now give a general overview of commonly used contours, depending on the type of integral being considered.

Firstly, if the function is even and integrated over \([0,\infty)\), then extend the function to the entire real line. If the function seems to decay sufficiently fast in \(\mathbb{H}\), consider using a semicircular contour in the upper half-plane, as shown in \autoref{fig:semicircularcontour}. If there are singularities along the real axis, then insert small semicircular indentations around them, as in \autoref{fig:indentedsemicircularcontour}
\subsection{Analytic Continuation}
We
\section{The Theory of Riemann}
