...
We now turn to a fundamental result in complex analysis due to Weierstrass, which establishes conditions under which the limit of a sequence of holomorphic functions is itself holomorphic. This theorem not only guarantees the holomorphy of the limit function but also the uniform convergence of its derivatives.
...
\end{proof}
\begin{remark}
    In the statement on \autoref{thm:weierstrassuniformlimit}, the uniform convergence on every compact subset can therefore be loosened to the uniform convergence on every simple closed curve.
\end{remark}
