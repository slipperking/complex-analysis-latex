...
\begin{proof}
    ...
    Because \(\psi(\zeta)\) is compactly supported and continuous, by \ref{thm:continuousfunctionbounded}, it is bounded. Bisect \(\mathbb{C}\) into \(\mathbb{C}\setminus D\paren{z,\varepsilon}\) and \(D\paren{z,\varepsilon}\). \(\forall\varepsilon>0\), the integral \[-\frac{1}{2\pi i}\int_{\mathbb{C}\setminus D\paren{z,\varepsilon}}\frac{\psi(\zeta)}{\zeta-z}\dd{\overline{\zeta}}\wedge\ddzeta\] is continuous. Since \(\supp\paren{\psi(\zeta)}\) is bounded, pompeiu_weaksingularityvanishes
\end{proof}
