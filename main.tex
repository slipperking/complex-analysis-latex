...
The most straightforward method of the derivation of analytic continuations uses power series. Let \(f(z)=\sum_{n=0}^{\infty}c_n(z-z_0)^n\) have radius of convergence \(R>0\) (by \autoref{thm:abelradius}). For \(z_1\in D(z_0, R)\), we can expand \(f\) at \(z_1\):
\[f(z)=\sum_{k=0}^{\infty} \frac{f^{(k)}\qty(z_1)}{k!}\qty(z-z_1)^k.\]
Let \(\rho\) be the radius of convergence of this series. Then:
\[\rho\geq R - |z_1 - z_0|.\]
If \(\rho>R-\abs{z_1-z_0}\), then \(f\) extends analytically to \(D\qty(z_0,R) \cup D(z_1,\rho)\). In the case that \(\rho=R-\abs{z_1-z_0}\), the disks \(D(z_0,R)\) and \(D(z_1,\rho)\) are tangent at a point \(\zeta_0\). Here, \(\zeta_0\) is a \textit{singularity}, and \(f\) cannot be continued beyond \(\zeta_0\).
...
\begin{proof}
    ...
    The disks \(\{D(\zeta, r_\zeta)\}_{\zeta \in \partial D(z_0,R)}\) cover the compact set \(\partial D(z_0, R)\). By Heine-Borel (Theorem~\ref{thm:heineborel}), there is a finite subcover \(\{D(\zeta_k, r_k)\}_{k=1}^n\). Let \(V=\bigcup_{k=1}^n D(\zeta_k, r_k)\). Therefore, \(\exists\rho>0\) such that \(A=\cbraces{z}{R-\rho\leq\abs{z-z_0}\leq R+\rho}\subseteq V\).
