...
For a set $A\in\mathbb{C}^n$, define the closure of $A$, or $\overline{A}$ to be the intersection of all closed sets containing $A$. In other words, it is the union of $A$ and its boundary.
\end{definition}
...
\begin{proof}
    ...
    First we will show that $X$ is bounded. Suppose that $\forall R>0$, $\exists x\in X$ where $\|x\|>R$. Consider the collection of open sets \[\mathcal{U}=\{B(0,k)\mid k\in\mathbb{N}\}\] where $B(a,b)$ is the open ball with radius $b$ centered at $a$. $\mathcal{U}$ forms an open cover of $X$. Then there exists a finite subcover $\{B(0,k_1),\ldots,B(0,k_m)\}$ covering $X$. Then, \[X\subseteq\bigcup_{i=1}^mB(0,k_i)=B(0,\max(k_1,\ldots k_m)).\] By contradiction, \(X\) must be bounded.

    $X$ must also be a closed set. For the sake of contradiction, assume that there exists a point $x_0\in\overline{X}\setminus X$. Construct a collection of open sets
    \[\mathcal{U}=\left\{\left.\mathbb{C}^n\setminus\overline{B}\left(x_0,\frac{1}{n}\right)\middle|n\in\mathbb{N}\right.\right\}.\] This collection covers the set $X$
\end{proof}
...
\begin{theorem}[Pompeiu's Theorem]\label{theorem:pompeiu}
    Let \(U\subseteq\mathbb{C}\) where $\partial U$ is piecewise smooth. Let \(f(z)\in C^1(\overline{U})\), where $\overline{U}$ represents the closure of \(U\). Then $\forall z\in U$, \begin{equation}
