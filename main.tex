...
\end{theorem}
\begin{proof}
    By \autoref{thm:cauchygoursattheorem}, \[\int_{\partial\paren{U\setminus D(z,\varepsilon)}}\frac{f(\zeta)}{\zeta-z}\ddzeta=\oint_{\partial U}\frac{f(\zeta)}{\zeta-z}\ddzeta-\oint_{\partial D(z,\varepsilon)}\frac{f(\zeta)}{\zeta-z}\ddzeta=0.\] Through the polar parameterization of \(\partial D(z,\varepsilon)\),\[\int_0^{2\pi}\frac{f(z+e^{it})}{}\]
\end{proof}
\end{document}