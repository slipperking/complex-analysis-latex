...
\end{proof} We will now provide the following assertion on the locally uniform convergence of infinite products:
\begin{lemma}\label{lemma:infiniteproductlocallyuniformconvergencecriterion}
    If \(f(z)=\prod_{n=1}^\infty f_n(z)\) is an infinite product of holomorphic functions \(\qty{f_n}\) on some open set \(U\subseteq\mathbb{C}\) such that each \(\log\qty(f_n)\) is holomorphic and \(\sum_{n=1}^\infty\log\qty(f_n(z))\) converges uniformly on every compact subset \(K\subset U\), then \(f(z)\) converges uniformly on each \(K\).
\end{lemma}
\begin{proof}
    Let \(K\) be a compact subset of \(U\). Since \(\sum_{n=1}^\infty\log\qty(f_n(z))\) converges uniformly on \(K\), it follows that \(\forall\varepsilon>0\), \(\exists N\in\mathbb{N}\) such that \(\forall n>m>N\), \(\abs{\sum_{k=m+1}^n\log\qty(f_k(z))}<\varepsilon\) for all \(z\in K\). Additionally, we have: \[\abs{\prod_{k=1}^n f_k(z)-\prod_{k=1}^m f_k(z)}=\abs{\exp[\sum_{k=1}^n \log\qty(f_k(z))]-\exp[\sum_{k=1}^m \log\qty(f_k(z))]}.\]
    By \autoref{thm:weierstrassuniformlimit}, the uniform limit \(\sum_{k=1}^\infty\log\qty(f_k(z))\) is holomorphic on \(U\). By continuity and \autoref{thm:continuousfunctionboundedoncompact}, this limit is bounded on \(K\). It follows that each partial sum is uniformly bounded on \(K\). Since the exponential function is Lipschitz continuous on compact subsets of \(\mathbb{C}\), there exists a finite constant \(M>0\) such that \[\abs{\exp[\sum_{k=1}^n \log\qty(f_k(z))]-\exp[\sum_{k=1}^m \log\qty(f_k(z))]}\leq M\abs{\sum_{k=m+1}^n\log\qty(f_k(z))}<M\varepsilon.\] The criterion of uniform convergence on compact subsets is also known as \textit{locally uniform convergence}. 
\end{proof}
...
\end{example}
\begin{proof}
    By the Blaschke condition, it is satisfied that \(\lim_{k\to\infty}1-\abs{a_k}\to 0\). In other words, \(\forall 0<R<1\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(1-\abs{a_n}<1-R\), or that \(\abs{a_n}>R\). If we can prove that \[\sum_{k=1}^\infty \log\qty[\frac{\abs{a_k}}{a_k}\frac{a_k-z}{1-\overline{a_k}z}]\] is uniformly convergent on \(\overline{D(0,R)}\) for any \(0<R<1\), then by \autoref{lemma:infiniteproductlocallyuniformconvergencecriterion}, we can show that equation~\eqref{eq:blaschkeproduct_statement} is locally uniformly convergent on \(\mathbb{D}\). For any \(\varepsilon>0\), \(\exists N'\in\mathbb{N}\) such that \(\forall m>n>N'\), \(\sum_{k=m+1}^n\qty(1-\abs{a_k})<\varepsilon\). Notice that \[\log\qty[\prod_{k=m+1}^n\qty[\frac{\abs{a_k}}{a_k}\frac{a_k-z}{1-\overline{a_k}z}]]\]
...
Since \[\int_0^\infty\sum_{n=1}^\infty\abs{\ee^{-xn}x^{s-1}}\ddx=\int_0^\infty\sum_{n=1}^\infty \ee^{-xn}x^{\sigma-1}\ddx=\int_0^\infty \frac{x^{\sigma-1}}{\ee^{-x}-1}\ddx\] is convergent, by Fubini's Theorem (with a counting measure on \(\mathbb{N}\) and a Lebesgue measure on \(\mathbb{R}_{\geq 0}\)), the operations of integration and summation may be exchanged (a statement in measure theory that we now take for granted). Therefore, we have
\begin{equation}
    \Gamma(s)\zeta(s)=\int_0^\infty\frac{x^{s-1}}{\ee^{-x}-1}\ddx.\label{eq:gammazetaproduct}
\end{equation}
\end{document}
