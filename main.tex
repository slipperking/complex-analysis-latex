...
\begin{lemma}
    Let \(U\subseteq\mathbb{C}\) be open and bounded, and let \(\qty{f_n(z)}\) be holomorphic on \(U\) and continuous on \(\overline{U}\). If \(f_n\rightrightarrows f\) on \(\partial U\), then \(f_n\rightrightarrows f\) on \(\overline{U}\).
\end{lemma}
\begin{proof}
