...
Then \(e^z\) is periodic over \(2\pi i\). The complex logarithm, denoted \(w=\Log(z)\), is the solution to \(z=e^w\). We can then define the inverse trigonometric and hyperbolic functions. 
...
The exponential function never vanishes as \(\abs{e^z}=e^x>0\). It is also an entire function with \(\paren{e^z}'=e^z\). By analysis of the real and imaginary parts,
...
\begin{theorem}\label{thm:identityaccumulationofzeroes}
    Let \(U\subseteq\mathbb{C}\), and \(f:U\to\mathbb{C}\) be holomorphic over \(U\). Then if the set defined by \(E=\cbraces{z\in U\mid f(z)=0}\) has an accumulation point in \(U\), then \(f\equiv0\) over \(U\).
\end{theorem}
\begin{proof}
    Let \(\cbraces{z_n}_{n\in\mathbb{N}}\) be a subset of \(\cbraces{z}{z\in U,f(z)=0}\) and assume it has an accumulation point \(z_0\) in \(U\). Since \(f\) is holomorphic over \(U\), \(\exists\varepsilon>0\) such that \(f\) is holomorphic over \(D\paren{z_0,\varepsilon}\subseteq V\). Then over this disk, \(f\) has the Taylor Series representation
    \begin{equation}\label{eq:identityaccumulationofzeroes_taylorexpansion}
        f(z)=\sum_{n=0}^\infty a_n\paren{z-z_0}^n.
    \end{equation}
    By \autoref{def:accumulationpoint}, \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(z_n\in D\paren{z_0,\varepsilon}\). Since \(z_n\) is a root of \(f\), \(f\paren{z_n}=0\). Then, by the continuity of \(f\), \[0=\lim_{n\to\infty} f\paren{z_n}=f\paren{\lim_{n\to\infty}z_n}=f\paren{z_0}.\] Using this result in comparison to equation \eqref{eq:identityaccumulationofzeroes_taylorexpansion}, we get that \(a_0=0\).

    The function \(f_1(z)=\frac{f(z)}{z-z_0}\) can be analytically continued by \autoref{thm:analyticcontinuationremovablesingularities} with a Taylor expansion over \(D\paren{z_0,\varepsilon}\) of
    \[f_1(z)=\sum_{n=0}^\infty a_{n+1}\paren{z-z_0}^n.\]
    Similarly, \(a_1=0\). We recursively obtain that \(a_0,a_1,\ldots=0.\) Then \(f\equiv0\) over \(D\paren{z_0,\varepsilon}\). To extend this to \(U\), TO BE CONTINUED
\end{proof}
\begin{remark}
...
\begin{theorem}[Argument Principle for Holomorphic Functions]\label{thm:argumentprincipleholomorphic}
    Let \(U\subseteq\mathbb{C}\) be a region and \(f:U\to\mathbb{C}\) be holomorphic. Let \(\gamma\subset U\) be a closed (positively oriented) curve homotopic to a constant curve (the homotopy lies in \(U\)). If \(f\) has no zeroes on \(\gamma\), then \(f\) has finitely many zeroes in the region bounded by \(\gamma\), and number is equal to \[Z=\frac{1}{2\pi i}\int_{\gamma}\frac{f'(z)}{f(z)}\ddz.\]
\end{theorem}
