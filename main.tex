...
\begin{definition}[Uniform Continuity]\label{def:uniformcontinuity}
    A function \(f:U\to\mathbb{R}\), defined on a set \(U\subseteq\mathbb{R}\), is uniformly continuous if and only if $\forall\varepsilon>0$, $\exists\delta>0$ such that $\forall x,y\in U$ where \(|x-y|<\delta\), \(\left|f(x)-f(y)\right|<\varepsilon\).
\end{definition}
\begin{lemma}
