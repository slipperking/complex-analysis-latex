...
\begin{proof}
    ...
    Notice that over \(W\), \(\psi=1\), \(\psi'=0\), and is disjoint from \(K_1\) (or that \(W\cap K_1=\emptyset\)). Since \(K\) is a compact subset of \(W\), which is open, their boundaries have positive distance. Then, the distance between \(W\) and \(K\) is positive. Therefore, \(\exists M>0\) such that \[\frac{1}{\abs{\zeta-z}}\leq M,\] and thus, \[\]
\end{proof}
