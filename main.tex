    ...
    \item The existence of a transcendental meromorphic function \(f\) whose poles have an accumulation point at \(z=\infty\) is the concern of the following theorem:
        ...
        \begin{proof}
            ...
            \begin{enumerate}[label=(\alph*)]
                ...
                \item Let \(\qty{\varepsilon_n}_{n\in\mathbb{N}}\) be a positive sequence such that \(\sum_{n=1}^\infty\varepsilon_n\) is convergent. If \(a_1=0\), we can choose \(p_1(z)=0\) (this can actually be any arbitrary polynomial). For \(n\geq 2\) (or \(n\geq 1\) if \(a_1\) is nonzero), \(\psi_n\) is a polynomial in terms of \(\frac{1}{z-a_n}\) and has its only pole at \(z=a_n\). Therefore, \(\psi_n(z)\) is holomorphic on \(D\qty(0,\abs{a_n})\) and can be written as \[\psi_n(z)=\sum_{k=0}^\infty \frac{\psi_n^{(k)}(0)}{k!}z^k.\]
                
                By \autoref{thm:abelradius}, This series is uniformly convergent on \({D\qty(0,\abs{\frac{a_n}{2}})}\subset\overline{D\qty(0,\abs{\frac{a_n}{2}})}\) 
            \end{enumerate}
        \end{proof}
\end{enumerate}
