...
\begin{theorem}[Hurwitz's Theorem]\label{thm:hurwitz}
    ...
    Then, for any open set \(W\subset U\), and any open neighborhood \(V\) of the set \(\cbraces{z\in W}{f(z)=0}\) relatively compact in \(U\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\),
    \begin{enumerate}
        \item The zeros of \(f_n(z)\) in \(W\) lie in \(V\), that is, \(\cbraces{z\in K}{f_n(z)=0}\subset V\).
        \item The number of zeros (counting multiplicity) of \(f_n(z)\) in \(K\) is the same as that of \(f(z)\) in \(W\).
    \end{enumerate}
\end{theorem}
