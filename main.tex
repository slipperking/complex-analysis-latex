...
\end{example}
\begin{proof}
    If \(f(x)\) is not uniformly continuous over $(0,1)$, then $\exists\varepsilon>0$ such that $\forall\delta>0$, \(\exists x,y\in(0,1)\) such that \(|x-y|<\delta\) and \(\abs{f(x)-f(y)}\geq\varepsilon\). Choose $\varepsilon=1$ and \[x=\frac{1}{n},\quad y=\frac{1}{n+1}.\] Then \(\forall\delta\), \(\exists n>1\) where \(\abs{x-y}<\delta\).
\end{proof}
\begin{lemma}
