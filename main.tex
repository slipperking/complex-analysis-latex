...
Let \(I\subset\mathbb{R}\) be an open, and let \(a:I\to\mathbb{R}\) and \(b:I\to\mathbb{R}\) be differentiable on \(I\) such that \(\forall x\in I\), \(a(x)<b(x)\). Let \(U=\cbraces{(x,t)\in U\times\mathbb{R}}{a(x) \leq t \leq b(x)}\). If \(f(x,t)\)\(U\), and \(a(x)<b(x)\) for all \(x\in U\). Suppose that:
\begin{itemize}
    \item \(f(x,t)\) and \(\pdv{f}{x}(x,t)\) are continuous on the set \(\),
    \item \(a(x)\), \(b(x)\) are differentiable on \(U\).
\end{itemize}
Then the function \[F(x)=\int_{a(x)}^{b(x)}f(x,t)\dd{t}\] is differentiable on \(U\), and
