...
\begin{theorem}[Hurwitz's Theorem]
    Let \(\cbraces{f_n(z)}\) be a sequence of holomorphic functions on \(U\subseteq\mathbb{C}\) that uniformly converges to \(f(z)\) on every compact subset of \(U\). If \(\forall n\in\mathbb{N}\), \(f_n(z)\) has no roots in \(U\), then \(f\) is either identically 0 or has no roots in \(U\).
\end{theorem}
