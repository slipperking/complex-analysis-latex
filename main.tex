...
\begin{proof}
    ...
    Because \(f(\zeta)\) is continuous over \(\partial U\), it is bounded by a constant \(M\). Additionally, since \(\abs{z-z_0}<\abs{\zeta-z_0}\), the sum is termwise bounded by the convergent series \[\sum_{j=0}^\infty\frac{Mr^j}{\abs{\zeta-z_0}^{j+1}}.\]
...
\begin{proof}
    ...
    \end{equation} is uniformly convergent with respect to \(\zeta\). Therefore, we get that \[f(z)=\frac{1}{2\pi i}\qty(\sum_{n=0}^\infty\int_{\gamma_2}\frac{(z-a)^n}{(\zeta-a)^{n+1}}f(\zeta)\ddzeta+\sum_{n=0}^\infty\int_{\gamma_1}\frac{(\zeta-a)^n}{(z-a)^{n+1}}f(\zeta)\ddzeta).\] 
    Since \(\frac{1}{\zeta-a}\) is holomorphic on \(V\)
\end{proof}
