...
\end{proof}
\begin{remark}
    This is the most specific form of \autoref{thm:bolzanoweierstrassforsets}. In \autoref{thm:bolzanoweierstrass}, Let \(U\subset\mathbb{R}\) be a compact interval. Every infinite subset \(\qty{a_n}\subset U\) has a subset \(\qty{a_{n_k}}\) with a limit in \(U\).
\end{remark}
...
Then, \[\abs{a}^2\paren{z_2-z_1}=z_2-z_1.\Longleftrightarrow\qty\big(\qty|a|^2-1)\qty(z_2-z_1)=0.\] Since \(\abs{a}<1\), then \(\abs{a}^2-1\neq0\), and we get \(z_2-z_2=0\). This proves the univalence of \(\varphi_a(z)\).
