...
\begin{proof}
    ...
    For the sake of contradiction, assume that \(\rho>\frac{1}{2}\). Hence, \(D\qty(\zeta,\rho-\frac{1}{2})\cap\mathbb{D}\neq\emptyset\). Obviously, the arc defined by \(S=D\qty(\zeta,\rho-\frac{1}{2})\cap\partial\mathbb{D}\) has positive length. Since \(\exp(2\pi i\mathbb{Q})\) is dense in \(\partial\mathbb{D}\), \(\exists\zeta'\in S\) such that \(\zeta'=\exp(2\pi i\frac{p}{q})\) for some \(p,q\in\mathbb{Z}\). Since \(\tilde{f}\) is convergent and continuous on \(D\qty(\zeta,\rho-\frac{1}{2})\), it follows that \(\lim_{z\to\zeta'}\tilde{f}(z)=\tilde{f}(\zeta')\), which implies that \(\lim_{r\to 1^-} \tilde{f}(r\zeta')=\tilde{f}(\zeta')\). Since \[f(r\zeta')=\sum_{n=1}^{q-1}r^{n!}\zeta'^{n!}+\sum_{n=q}^\infty r^{n!}\zeta'^{n!},\]
