...
\begin{align*}
    \frac{\partial f}{\partial z}=\lim_{h=0} \frac{f(z+hi)-f(z)}{hi}&=\lim_{h=0} \frac{u(x,y+h)+iv(x,y+h)-u(x,y)-iv(x,y)}{hi}\\
    &=\frac{\partial v}{\partial y}-\frac{\partial u}{\partial y}i\text{.}
\end{align*}
Approaching the function from the real axis,
\begin{align*}
    \frac{\partial f}{\partial z}=\lim_{h=0} \frac{f(z+h)-f(z)}{h}&=\lim_{h=0} \frac{u(x+h,y)+iv(x+h,y)-u(x,y)-iv(x,y)}{h}\\
    &=\frac{\partial u}{\partial x}+i\frac{\partial v}{\partial x}\text{.}
\end{align*}
If the limit above exists, the function is \textit{complex differentiable} at the point $z$.
The above equations set the necessary criteria for a complex function to be 
\end{document}
