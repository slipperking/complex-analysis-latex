...
\end{definition}
\begin{theorem}[Bolzano--Weierstrass Theorem]\label{thm:bolzanoweierstrassforsets}
...
\begin{proof}
    ...
    First we will show that \(X\) is bounded. Suppose that \(\forall R>0\), \(\exists z\in X\) where \(\norm{z}>R\). Consider the collection of open sets \[\mathcal{U}=\{D(0,k)\mid k\in\mathbb{N}\}.\] \(\mathcal{U}\) forms an open cover of \(X\). Then there exists a finite subcover \(\{D(0,k_1),\ldots,D(0,k_m)\}\) covering \(X\). Then, \[X\subseteq\bigcup_{i=1}^mD(0,k_i)=D(0,\max(k_1,\ldots k_m)).\] By contradiction, \(X\) must be bounded.
...
\begin{definition}[Epsilon--N]\label{def:epsilonn}
    Let \(\qty{a_n}_{n\in\mathbb{N}}\in\mathbb{R}\) be a sequence. If \(\exists a_\infty\subset\mathbb{R}\) such that \(\forall\varepsilon>0\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(\abs{a_n-a_\infty}<\varepsilon\), then \(\qty{a_n}\) \textit{converges} to \(a_\infty\).
\end{definition}
...
\begin{proof}
    ...
    Then from the first conditions of \autoref{def:limsup} and \autoref{def:liminf}, we get \(\limsup_{n\to\infty}a_n\geq a\) and \(\liminf_{n\to\infty} a_n\leq a\). By the second conditions, we get \(\limsup_{n\to\infty}a_n\leq a\) and \(\liminf_{n\to\infty} a_n\geq a\). Therefore, \[\limsup_{n\to\infty}a_n=\liminf_{n\to\infty}a_n.\]
...
\begin{theorem}[Bolzano--Weierstrass Theorem]\label{thm:bolzanoweierstrass}
    Any bounded infinite sequence has a convergent subsequence.
\end{theorem}
\begin{proof}
    Let \(\qty{a_n}\subset\mathbb{R}\) be the bounded sequence. In other words, \(\exists m,M\in\mathbb{R}\) such that \(\qty{a_n}\subset[m,M]\). By the infiniteness of \(\qty{a_n}\), at least one of \(\qty[m,\frac{m+M}{2}]\) and \(\qty[\frac{m+M}{2},M]\) contains an infinite subsequence. 
    
    Let \(m_1\) be the lower bound of the selected interval and \(M_1\) be the upper. Then at least one of \(\qty[m_1,\frac{m_1+M_1}{2}]\) and \(\qty[\frac{m_1+M_1}{2},M_1]\) contains an infinite subsequence.

    Continuing this process, we obtain a sequence of intervals \(\qty{\qty[m_n, M_n]}\) satisfying \[m_1\leq m_2\leq m_3\leq\cdots\leq M_3\leq M_2\leq M_1.\]
    The length of the \(k\)th interval is equal to \[M_k-m_k=\frac{1}{2^k}\qty(M-m).\] Then since \(\lim_{k\to\infty}M_k-m_k=0\), \[\lim_{k\to\infty}M_k=\lim_{k\to\infty}m_k=a.\]

    \(\forall k\in\mathbb{N}\), \(\qty[m_k,M_k]\) contains infinitely many elements of \(\qty{a_n}\). Let \(a_{n_k}\) be any arbitrary one of them. We then aim to show \(\forall \varepsilon>0\), \(\exists N\in\mathbb{N}\) such that \(\forall k>N\), \(\abs{a_{n_k}-a}<\varepsilon\). From \(a_{n_k},a\in\qty[m_k,M_k]\), we get \(\abs{a_{n_k}-a}\leq\frac{1}{2^k}(M-m)<\varepsilon\). It follows that \(k>\log_2\qty(\frac{M-m}{\varepsilon})\), and we can let \(N=\ceil{\log_2\qty(\frac{M-m}{\varepsilon})}\). Therefore, \(\qty{a_{n_k}}_{k\in\mathbb{N}}\) is a convergent subsequence.
\end{proof}
\begin{theorem}\label{thm:continuousfunctionboundedoncompact}
    Any continuous function on a compact set \(U\) is bounded on \(U\).
\end{theorem}
\begin{proof}
    Suppose for the sake of contradiction that \(f:U\to\mathbb{R}\) is continuous and unbounded on compact \(U\). Then for each \(n\in\mathbb{N}\), there exists \(x_n\in U\) such that \(|f(x_n)|>n\). The sequence \(\cbraces{x_n}\) lies in \(U\), which is compact, so by the Bolzano--Weierstrass Theorem (\autoref{thm:bolzanoweierstrass}), there exists a convergent subsequence \(\cbraces{x_{n_k}}\) with \(x_{n_k}\to x_\infty\in U\).

    Since \(f\) is continuous, \(f(x_{n_k})\to f(x_\infty)\), so the sequence \(\cbraces{f\paren{x_{n_k}}}\) is bounded. But this contradicts \(\qty|f\paren{x_{n_k}}|>n_k\to\infty\), hence \(f\) must be bounded on \(U\).
\end{proof}
...
\end{equation}
Equations \eqref{eq:cauchyriemanneqs1} and \eqref{eq:cauchyriemanneqs2} are known as the \textit{Cauchy--Riemann equations}. Although this condition is necessary, it is not sufficient. Consider the function \(f(z)=\sqrt{\left|\Re(z)\Im(z)\right|}\). Let \(z=x+yi\), \(x=\alpha t\), and \(y=\beta t\). Then
...
A univariate function formed by taking compositions, sums, products, and powers of finitely many functions of the following form:
\begin{enumerate}
...
\begin{enumerate}
    \item Because of the periodicity of \(z=e^w\), \(\log\) is a multi-valued function (infinite-valued).
...
Lastly, we will examine the power function. Let \(\alpha=u+iv\) where \(u,v\in\mathbb{R}\). Then, \[z^\alpha=\exp(\alpha\log(z))=\exp((u+iv)\paren{\ln\abs{z}+i\arg\paren{z}})=e^{u\ln\abs{z}-v\arg\paren{z}}e^{i\paren{v\ln\abs{z}+u\arg\paren{z}}}.\]
...
\begin{theorem}[Abel's Theorem]\label{thm:abelpowerseries}
...
\begin{proof}
    We will prove that the value in equation \eqref{eq:cauchyhadamard} satisfies the criteria in \autoref{thm:abelpowerseries}.

    Assume \(\abs{z}<R\). Then \(\forall\rho\in(|z|,R)\), \(\frac{1}{\rho}>\frac{1}{R}\), By equation \eqref{eq:cauchyhadamard}, \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(\sqrt[n]{\abs{a_n}}<\frac{1}{\rho}\), and \(\abs{a_n}<\frac{1}{\rho^n}\). It follows that \(\abs{a_nz^n}<\frac{\abs{z}^n}{\rho^n}<1\) for all \(n>N\). Then by the Weierstrass \(M\)--Test (\autoref{thm:weierstrassmtest}), \[\sum_{n=0}^\infty a_nz^n\] converges absolutely and uniformly.
\end{proof}
...
\end{theorem}
\begin{proof}\label{thm:uniformlimit}
...
\begin{proof}
    ...
    \end{align*}
    where the remainder terms have a form resembling \[O\paren{\abs{z-z_0}^2}=\paren{z-z_0}^2\frac{k!}{2\pi i}\brackets{k+1+\binom{k+1}{2}}\oint_{\partial U}\frac{f(\zeta)}{\paren{\zeta-z_0}^{k+3}}\ddzeta+O\paren{\abs{z-z_0}^3}.\]
...
\begin{theorem}\label{thm:nthderivativeboundedl1norm}
    Let \(U\subseteq\mathbb{C}\) be open, let \(K\subset U\) be compact and \(V\supset K\) be open such that \(\overline{V}\subset U\) (\(V\) is a neighborhood of \(K\) that is relatively compact in \(U\)). Let \(f(z)\) be holomorphic in \(U\). Then there exists a sequence \(\cbraces{c_n}\) such that \(\forall n\in\mathbb{N}\), \begin{equation}
...
\begin{definition}[Analytic Continuation]
    ...
    \end{enumerate}
\end{definition}
\begin{theorem}[Riemann's Theorem on Removable Singularities]\label{thm:analyticcontinuationremovablesingularities}
...
\begin{proof}
    ...
    Let \(\{U_j\}\) be the collection formed by enumerating all such \(U_{z_{n,i},n}\) for \(n\in\mathbb{N}\) and \(i=1,\ldots,k_n\). Then \(\{U_j\}\subset\mathfrak{B}\) is a countable collection whose union covers \(\Omega\), proving condition 1.

    For condition 2, let \(K\subset\Omega\) be compact. Then \(\exists N\in\mathbb{N}\) such that \(K\subset K_N^\circ\), and hence \(K\) is disjoint from \(V_n\) for all \(n>N+1\). Since each \(V_n\) intersects only finitely many \(U_j\), it follows that \(K\) intersects only finitely many of the \(U_j\). Thus, the collection is \textit{locally finite}.
\end{proof}
...
\begin{lemma}[Schwarz Lemma]\label{lemma:schwarz}
    If a holomorphic function \(f(z)\) maps \(D(0,1)\) to itself and \(f(0)=0\), then \[\abs{f(z)}\leq\abs{z},\quad\abs{f'(0)}\leq 1.\]
     One of the inequalities become equalities if and only if \(f(z)\) is in the form of \(ze^{i\tau}\), where \(\tau\in\mathbb{R}\). In other words, \(f\) is a pure rotation.
\end{lemma}
\begin{proof}
...
\begin{equation}
    \varphi_a(z)=\frac{a-z}{1-\overline{a}z}\in\Aut( D(0,1)).\label{eq:mobiustransformationgroupofholomorphicautomorphismsstatement}
\end{equation}
...
\[\abs{\varphi_a(z)}=\abs{\frac{1}{z}}\abs{\frac{a-z}{\frac{1}{z}-\overline{a}}}=\abs{\frac{a-z}{\overline{z}-\overline{a}}}=1.\]
...
Let us first confirm injectivity. \(\forall z_1,z_2\in D(0,1)\), we will observe when \[\frac{a-z_1}{1-\overline{a}z_1}=\frac{a-z_2}{1-\overline{a}z_2}\] is satisfied. It follows that \[\qty(a-z_1)\qty(1-\overline{a}z_2)=\qty(a-z_2)\qty(1-\overline{a}z_1)\Longleftrightarrow a+\overline{a}z_1z_2-\abs{a}^2z_2-z_1=a+\overline{a}z_1z_2-\abs{a}^2z_1-z_2.\]
Then, \[\abs{a}^2\paren{z_2-z_1}=z_2-z_1.\Longleftrightarrow\qty\big(\qty|a|^2-1)\qty(z_2-z_1)=0.\] Since \(\abs{a}<1\), we get \(z_2-z_2=0\). This proves the univalence of \(\varphi_a(z)\).

Next, we will solve for the inverse of \(\varphi_a\). Let \(z=\varphi_a(w)=\frac{a-w}{1-\overline{a}w}\). Then,
\begin{equation}
    z-\overline{a}zw=a-w\Longleftrightarrow w=\frac{a-z}{1-\overline{a}z}.\label{eq:involutivemobiustransformation}
\end{equation}
It follows that \(\varphi_a=\qty(\varphi_a)^{-1}\). Thus \(\varphi_a\) is surjective and a bijective automorphism. It follows that equation \eqref{eq:mobiustransformationgroupofholomorphicautomorphismsstatement} is true. Functions in the form of \(\varphi_a\) (where \(a\in D(0,1)\)) are known as \textit{Möbius transformations}, and the group of all such transformations is known as the \textit{Möbius transformation group on the unit disk}, which is a subgroup of \(\Aut(D(0,1))\). Functions in the form of \(\rho_\tau(z)=z\exp(i\tau)\), where \(\tau\in\mathbb{R}\) is constant, form a group known as the \textit{rotation group}, which is also a subgroup of \(\Aut(D(0,1))\).
\begin{theorem}[The Holomorphic Automorphism Group on the Unit Disk]
...
\begin{proof}
    ...
    Then, \(\abs{\psi'(0)}=1\), and by the equality statement of \autoref{lemma:schwarz}, \(\psi(z)=ze^{i\tau}=\rho_\tau(z)\) for some constant \(\tau\in\mathbb{R}\), and \(f(z)=\varphi_{f(0)}^{-1}\circ\rho_\tau(z)\). By equation \eqref{eq:involutivemobiustransformation}, \(\varphi_{f(0)}\) is involutive, and it follows that \(f(z)=\varphi_{f(0)}\circ\rho_\tau(z)\)
\end{proof}
\section{The Theory of Weierstrass}
\subsection{Laurent Series}
Laurent Series are a generalization of Taylor Series specifically for holomorphic functions on a region punctured by a non-removable singularity. Whereas the Taylor Series applies on a disk, the Laurent Series applies on an annulus around the singularity.
