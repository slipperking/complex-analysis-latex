...
\end{example}
\begin{example}
    Let \(U\subseteq\mathbb{C}\) be open and simply connected such that \(0\in U\). Suppose \(f:U\setminus\qty{0}:\mathbb{C}\) is holomorphic. If \(\forall \theta\in [0,2\pi]\), \(\lim_{r\to 0^+}f\qty(re^{i\theta})=\infty\), prove that \(0\) cannot be an essential singularity.
\end{example}
\begin{proof}
    Although seemingly trivial at first glance, we are not guaranteed a uniform limit with respect to varying \(\theta\). Namely, for fixed \(\theta\) and \(\forall\varepsilon>0\), \(\exists\delta\) such that \(\forall 0<r<\delta\), we have \(\qty|f\qty(re^{i\theta})|>\varepsilon\). However, \(\delta\) is a function of \(\theta\), namely \(\delta_\varepsilon(\theta)\). Let this function be the supremum for possible values of \(\delta\), or \(\delta_\varepsilon(\theta)=\sup\qty(\cbraces{\delta>0}{\forall 0<r<\delta\Rightarrow\abs{f\qty(re^{i\theta})}>\varepsilon})\). If we can show that \(\delta_\varepsilon(\theta)\) has a positive infimum, or that \(\delta_\varepsilon=\inf_{\theta\in[0,2\pi]}\delta_\varepsilon(\theta)>0\), the conclusion follows.

    Fix \(\theta\in[0,2\pi]\) arbitrarily. First, assume that \(\delta_\varepsilon\) is discontinuous at \(\theta\). That means \(\exists\varepsilon'>0\) such that \(\forall\delta>0\), \(\exists\vartheta\) satisfying \(|\vartheta-\theta|<\delta\) and \(|\delta_\varepsilon(\vartheta)-\delta_\varepsilon(\theta)|>\varepsilon'\).
\end{proof}
