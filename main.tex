...
\end{theorem}
\begin{proof}
    Choose an arbitrary \(z_0\in U\) and the corresponding value of \(\varepsilon>0\). Since \(f\) is continuous on \(\partial D\qty(z_0,\varepsilon)\), there exists a harmonic function \(u(z)=\int_0^{2\pi}\varphi(\zeta)P(\zeta,z)\dd{\tau}\) that is identically equal to \(f\) on \(\partial D\qty(z_0,\varepsilon)\). Consider the function \(f-u\) on \(\overline{D\qty(z_0,\varepsilon)}\), which vanishes on \(\partial D\qty(z_0,\varepsilon)\). 
\end{proof}
