...
\begin{enumerate}
    ...
    \item Let \(z=re^{i\theta}\) and \(w=u+iv\), where \(r,\theta,u,v\in\mathbb{R}\). Then,
          ...
          where \(\arg(z)\) is the multi-valued argument function. The \textit{principle branch} of the argument is denoted \(\Arg:\mathbb{C}\to(-\pi,\pi]\). The principle branch of \(\log(z)\), or \(\Log(z)\), can be defined such that \(\Im\paren{\Log}\in(-\pi,\pi]\).
\end{enumerate}
