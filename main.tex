...
For a set $A\in\mathbb{C}^n$, define the closure of $A$, or $\overline{A}$ to be the intersection of all closed sets containing $A$. In other words, it is the union of $A$ and every accumulation point. (A point is an accumulation point of $A$ if )
\end{definition}
...
\begin{proof}
    ...
    We then prove the converse. By the assumption that \(X\) is bounded, \(\exists R>0\) such that the $X$ is contained within the closed cube \[X\subseteq\left\{z\;\middle|\; z\in\mathbb{C}^n, \max_{i\in\{1,\ldots,n\}}\left|\Re(z_i)\right|\le R,\max_{i\in\{1,\ldots,n\}}\left|\Im(z_i)\right|\le R\right\}.\] 
\end{proof}
