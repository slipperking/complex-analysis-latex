...
The zeros of \[\qty(1-\frac{z}{a_1})\prod_{n=2}^\infty E_{k_n}\qty(\frac{z}{a_n})\] are \(a_1,\ldots, a_{N-1}\) and lie in \(\overline{D(0,2R)}\). To prove the absolute convergence of \(\prod_{n=N}^\infty E_{k_n}\qty(\frac{z}{a_n})\) on \(\overline{D(0,R)}\), we will show that \(\sum_{n=N}^\infty\abs{E_{k_n}\qty(\frac{z}{a_n})-1}\) is convergent. Trivially, when \(\zeta\in\mathbb{D}\), we have \(\abs{\exp(\zeta)-1}\leq\exp\qty|\zeta|-1\leq\paren{e-1}\abs{\zeta}\).

Therefore, we have \[\abs{E_{k_n}\qty(\frac{z}{a_n})-1}=\abs{\exp(Q_{k_n}\qty(\frac{z}{a_n}))-1}\]
