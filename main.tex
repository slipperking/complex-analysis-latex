...
\let\bracesdefault\braces
\renewcommand{\braces}{\bracesdefault*}
...
\begin{definition}[Cover of a Set]
    A cover \(\mathcal{C}\) of a set \(X\) is any collection of sets \(\paren{U_n}\) such that \[\bigcup_{n\in\mathbb{N}}U_n\supseteq X.\] A cover is \textit{open} if every set in the collection is open.
\end{definition}
...
\begin{proof}
    ...
    By the Weierstrass \(M\) Test, the series uniformly converges, and we can justify \[\frac{1}{2\pi i}\oint_{\partial U}\sum_{j=0}^{\infty}\paren{z-z_0}^j\frac{f(\zeta)}{\paren{\zeta-z_0}^{j+1}}\ddzeta=\frac{1}{2\pi i}\sum_{j=0}^{\infty}\oint_{\partial U}\paren{z-z_0}^j\frac{f(\zeta)}{\paren{\zeta-z_0}^{j+1}}\ddzeta=\sum_{j=0}^\infty a_j\paren{z-z_0}^j,\]
...
\end{proof}
\begin{remark}
    By induction, we have shown that the assuming existence of the first order derivative of a holomorphic function \(f\), the \(n\)th order derivative of \(f\) exists \(\forall n\in\mathbb{N}\) and is holomorphic over the same region as \(f^{(n-1)}\). Furthermore, if \(f\) is holomorphic, then \(\forall z\in U\), there exists an open disk enclosing \(z\) such that \(f\) has a convergent Taylor Series expansion. This property is known as \textit{analyticity}, and \autoref{thm:cauchydifferentiationformula} tells us that all holomorphic functions are analytic. Analytic functions can be expanded into power series, which are termwise differentiable, and therefore complex differentiable. Thus, analyticity and holomorphy are logically equivalent, which is a fundamental difference between real and complex functions.
\end{remark}
...
\begin{lemma}\label{lemma:locallyfiniteopencoverexistence}
    Let \(\Omega\subseteq\mathbb{C}\) be an open set and \(\mathfrak{B}\) be an (open) basis of (the standard topology under) \(\Omega\). Then there exists a collection of sets \(\left\{U_n\right\}_{n\in\mathbb{N}}\) in \(\mathfrak{B}\) such that \begin{enumerate}
        \item \(\bigcup_{n\in\mathbb{N}}U_n=\Omega\).\label{item:locallyfiniteopencoverexistence_coveringstatement}
        \item \(\forall K\subset\Omega\) that is compact, \(K\) intersects only a finite number of sets in \(\left\{U_n\right\}_{n\in\mathbb{N}}\).\label{item:locallyfiniteopencoverexistence_locallyfinitestatement}
    \end{enumerate}
\end{lemma}
\begin{proof}
    Let \(K_{-1}=K_0=\emptyset,K_1,K_2,\cdots\subset\Omega\) exhaust \(\Omega\). Define two sequences, where \(\forall n\in\mathbb{N}\), \(W_n=K_{n+1}^\circ\setminus K_{n-2}\) be open and \(V_n=K_n\setminus K_{n-1}^\circ\) be compact.

    By properties of the exhaustion, \(W_n\supseteq V_n\). \(\paren{V_n}\) forms compact shells around \(K_{n-1}\), and therefore, \[\bigcup_{n\in\mathbb{N}} V_n=\Omega.\]

    For every \(r\in\mathbb{N}\), \(\forall z\in V_r\), there exists an open set \(U_{z,r}\in\mathfrak{B}\) where \(z\in U_{z,r}\subseteq W_r\). By the compactness of \(V_r\) and openness of \(U_{z,r}\), the collection \(\left\{U_{z,r}\mid z\in V_r\right\}\) has a finite subcover by \autoref{thm:heineborel}. Then, there exist finitely many points \(z_{r,1},\ldots,z_{r,k_r}\) such that \[V_r\subset\bigcup_{i=1}^{k_r}U_{z_{r,i},r}\subseteq W_r.\]
    Let \(\paren{U_n}\) be a collection formed by enumerating \(U_{z_{r,i},r}\) where \(r\in\mathbb{N}\) and \(i=1,\ldots,k_r\). Obviously, \autoref{item:locallyfiniteopencoverexistence_coveringstatement} is true since each of \(V_r\) is covered by some subcollection of \(\left\{U_j\right\}\), which means that the entire collection covers \(\Omega\). Since \(\forall K\subset\Omega\) that is compact, \(K\) intersects each set of some finite subcollection of \(\left\{W_j\right\}\), with each set of the subcollection containing the union of only a finite subcollection of \(\left\{U_j\right\}\). This property is known as \textit{local finiteness}.
\end{proof}
\begin{theorem}[Partition of Unity]\label{thm:partitionofunity}
    A 
\end{theorem}
\begin{lemma}[Existence of Bump Functions]\label{thm:bumpfunctionexistence}
    Let \(\Omega\subset\mathbb{C}\) be open, \(K\subset\Omega\) be compact, and \(V\subseteq\Omega\) be an open neighborhood of \(K\). Then there exists a bump function \(\varphi\in C^\infty\paren{\mathbb{C}}\) such that \(\forall z\in\mathbb{C}\), \(0\leq\varphi(z)\leq 1\), and there exists an open neighborhood of \(K\) such that \(\varphi\equiv1\) and satisfies \(\supp(\varphi)\subset V\), where \(\supp(\varphi)\) is compact.
\end{lemma}
