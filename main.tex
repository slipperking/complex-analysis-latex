...
\end{proof}
The following is also a direct result from Rouché's Theorem (\autoref{thm:rouche}). Unlike \autoref{thm:hurwitz}, which concerns the stability of zeros under perturbations of \(f_n(z)\) for increasing \(n\), we can directly consider perturbations of \(f(z)+c\) for vanishing \(c\).
\begin{theorem}\label{thm:zerosofashiftedholomorphicfunctionoverneighborhood}
    ...
    Then, for any compact set \(K\subset U\) where \(f(z)\) has no zeros on \(\partial K\), and any open neighborhood \(V\) of the set \(\cbraces{z\in K}{f(z)=0}\), \(\exists c\in\mathbb{R}\) such that \(\forall n>N\),
    \begin{enumerate}
        \item The zeros of \(f_n(z)\) in \(K\) lie in \(V\), that is, \(\cbraces{z\in K}{f_n(z)=0}\subset V\).
        \item The number of zeros (counting multiplicity) of \(f_n(z)\) in \(K\) is the same as that of \(f(z)\) in \(K\).
    \end{enumerate}
