...
\end{theorem}
\begin{proof}
    Assume that \(U\) never attains its supremum \(M\). Then, \(\abs{f(x)}\leq M\). Define \(\psi(x)=\frac{1}{f(x)-M}\), which is continuous as the denominator never reaches 0. By \autoref{thm:continuousfunctionboundedoncompact}, \(\psi(x)\) is bounded with some value of \(\mu>0\) satisfying \(\abs{\psi(x)}<\mu\). 
\end{proof}
