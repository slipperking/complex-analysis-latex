...
\end{equation}
is a Laurent Series at the point \(a\). The series can be separated into a power series with nonnegative exponents, \(\sum_{n=0}^\infty c_n(z-a)^n\), and a power series with negative exponents, \(\sum_{n=1}^{\infty}c_{-n}(z-a)^{-n}\). Equation~\eqref{eq:laurentseries} is said to be convergent at \(z=z_0\) if the two power series are both convergent.
