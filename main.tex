...
\end{proof}
We have the following result on the uniform convergence of infinite products:
\begin{theorem}\label{thm:infiniteproductweierstrassmtest}
    Let \(\prod_{n=1}^\infty f_n(z)\) be an infinite product. If there exists a sequence of complex numbers \(\qty{M_n}_{n\in\mathbb{N}}\) such that \(\sum_{n=1}^\infty M_n\) converges and \(\abs{\log\qty[f_n(z)]}\leq M_n\) for all \(z\in U\) and \(n\in\mathbb{N}\), then \(\prod_{n=1}^\infty f_n(z)\) converges uniformly on \(U\).
\end{theorem}
\begin{proof}
    Since \(\forall N\in\mathbb{N}\), \[\abs{\prod_{n=1}^N f_n(z)}=\prod_{n=1}^N\abs{f_n(z)}=\exp\qty[\sum_{n=1}^N\abs{\log{f_n(z)}}]\leq\exp\qty[\sum_{n=1}^N M_n],\]
    as \(N\to\infty\), it follows that the infinite product uniformly converges.
\end{proof}
...
\end{example}
\begin{proof}
    By the Blaschke condition, it is satisfied that \(\lim_{k\to\infty}1-\abs{a_k}\to 0\). In other words, \(\forall 0<R<1\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(1-\abs{a_k}<1-R\), or that \(\abs{a_k}>R\). It is evident that \[\prod_{k=1}^N \qty[\frac{\abs{a_k}}{a_k}\frac{a_k-z}{1-\overline{a_k}z}]=\exp\qty[\sum_{k=1}^N\log\qty[\frac{\abs{a_k}}{a_k}\frac{a_k-z}{1-\overline{a_k}z}]]\] 
\end{proof}
