...
\end{theorem}
\begin{proof}
    Suppose for the sake of contradiction that \(f:U\to\mathbb{R}\) is continuous and unbounded on compact \(U\). Then for each \(n\in\mathbb{N}\), there exists \(x_n\in U\) such that \(|f(x_n)|>n\). The sequence \(\cbraces{x_n}\) lies in \(U\), which is compact, so by the Bolzano--Weierstrass Theorem (\autoref{thm:bolzanoweierstrass}), there exists a convergent subsequence \(\cbraces{x_{n_k}}\) with \(x_{n_k}\to x^*\in U\).

    Since \(f\) is continuous, \(f(x_{n_k})\to f(x^*)\), so the sequence \(\cbraces{f\paren{x_{n_k}}}\) is bounded. But this contradicts \(\abs{f\paren{x_{n_k}}}>n_k\to\infty\), hence \(f\) must be bounded on \(U\).
\end{proof}
