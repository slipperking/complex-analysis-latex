...
\begin{enumerate}
    ...
    Write \(e^z=e^{x+iy}=e^x\paren{\cos(y)+i\sin(y)}\) where \(x,y\in\mathbb{R}\). Let \(u(x,y)=\real\paren{e^z}=e^x\cos(y)\) and \(v(x,y)=\imaginary\paren{e^z}=e^x\sin(y)\). The first order derivatives are
\end{enumerate}
