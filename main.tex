...
\begin{example}
	Evaluate the improper integral \(I=\int_{-\infty}^\infty \frac{1}{\qty(x^2+1)^2}\ddx\).
\end{example}
\begin{proof}
	\begin{figure}
		\centering
	...
	\end{figure}
	Consider \(\gamma\) to be a closed semicircle with radius \(R\) as in \autoref{fig:semicircularcontour}. We can split the curve into the constituent arc and the straight segment.

	We can evaluate \(\int_{\gamma}\frac{1}{\qty(z^2+1)^2}\ddz=\int_{-R}^R\frac{1}{\qty(x^2+1)^2}\ddx+\int_0^{\pi}\frac{Ri}{\qty(R^2e^{2i\theta}+1)^2}e^{i\theta}\dd{\theta}\). Notice that the main integral has poles at \(z=i\) and \(z=-i\), both of which are inside the contour and have order 2. The residue at \(z=i\) is \[\residue_{z=i}\frac{1}{\qty(z^2+1)^2}=\frac{1}{2\pi i}\int_{\partial D(i,\rho)}\frac{1}{\qty(z^2+1)^2}\ddz.\]
\end{proof}
