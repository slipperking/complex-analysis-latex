...
\(\Aut(U)\) indeed is a group under composition. For any two \(f,g\in\Aut(U)\), \(f\circ g\) is a function from \(U\) to \(U\) and therefore also lies in \(\Aut(U)\).

For \(f,g,h\in\Aut(U)\), \(\paren{f\circ g}\circ h=f\circ\paren{g\circ h}\).

Consider the identity function \(\mathrm{id}(z)=z\). Obviously, \(\forall g\in\Aut(U)\), \(\mathrm{id}\circ g=g\circ \mathrm{id}=g\).

Lastly, the inverse function exists as \(f\in\Aut(U)\) is bijective, and \(f\circ f^{-1}=\mathrm{id}=f^{-1}\circ f\).

We will consider a subgroup of \(\Aut(U)\).
s
In particular, \(\forall z\in U\), first consider an element in the form \[\varphi_a(\zeta)=\frac{z-\zeta}{1-\overline{z}\zeta}\in\Aut(U).\]
