...
\begin{proof}
    ...
    Since the integrand is dominated by a function that has a convergent integral, such as \(\int_{\supp(\psi)}M\dd{\xi}\wedge\dd{\overline{\xi}}\), equation \eqref{eq:onedimensionalpartialconjugatesolution_differenceexpr} uniformly converges and the limit \(h\to0\) may commute with the integral. Let \(\zeta=\alpha+i\beta\). From the real axis, \begin{equation}
...
\end{equation}
Firstly, the function is holomorphic on \(\overline{D(0,1)}\) as \(|z|\le1\), \(\abs{\overline{a}}<1\), the denominator never vanishes. Additionally, \(\varphi_a(a)=0\).
