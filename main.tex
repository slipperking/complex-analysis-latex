...
\end{proof}
\begin{remark}
    In \autoref{sec:differentialgeometry}, we will introduce the \textit{hyperbolic metric} on \(D(0,1)\), defined as \[\dd{s}^2=\frac{4\dd{\overline{z}}\ddz}{\qty(1-\abs{z}^2)^2}.\]
    From equation \eqref{eq:schwarzpick_nonincreasingmetric}, we get that the hyperbolic metric decreases with a holomorphic mapping of \(D(0,1)\) to itself. This metric is invariant (the equality condition) through all functions in \(\Aut(D(0,1))\).
\end{remark}
