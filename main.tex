...
\end{example}
\begin{proof}
    Fix \(\xi\in\mathbb{R}\) and let \(f(z)=\frac{\exp(-2\pi iz\xi)}{\cosh(\pi z)}\). Its poles occur when \(e^{\pi z}+e^{-\pi z}=0\), or equivalently, when \(z=i\qty(n+\frac{1}{2})\), where \(n\in\mathbb{Z}\).
    ...
    \end{figure}Since \(\cosh(\pi z)=\cosh(\pi(z+2i))\), and \(\exp(-2\pi iz\xi)=\exp(-4\pi\xi)\exp(-2\pi i(z+2i)\xi)\), we have that \(f(z)\) is a constant multiple of \(f(z+2i)\). Therefore, we can use a rectangular contour as shown in \autoref{fig:rectangularcontour}. Let the sides be labeled respectively by 
    \begin{gather*}
        ...
        \overleftarrow{\Gamma}=\cbraces{-R+iy}{y\in[0,2\pi]},\quad\overrightarrow{\Gamma}=\cbraces{R+iy}{y\in[0,2\pi]}
    \end{gather*}
