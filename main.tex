...
\begin{theorem}[Extended Residue Theorem]\label{thm:extresiduethm}
...
\end{theorem}
\begin{proof}
    Let \(R>\max_{j\in\mathbb{N}_{\leq n}}\qty|z_n|\) be arbitrary. Then there exist circles centered at each isolated singularity contained in \(D(0,R)\), or that \(\forall k\in\mathbb{N}_{\leq n}\), \(\exists\delta_k>0\) such that \(\partial D\qty(z_k,\delta_k)\subset D(0,R)\). By the Cauchy--Goursat Integral Theorem, \[\int_{\bigcup_{k=1}^nD\qty(z_k,\delta_k)^-\cup\partial D(0,R)^+}f(z)\ddz=0\Longleftrightarrow\qty(\sum_{k=1}^n\oint_{\partial D\qty(z_k,\delta_k)}-\oint_{\partial D(0,R)})f(z)\ddz=0\Leftrightarrow\sum_{k=1}^n\residue_{z=z_k}f(z)+\residue_{z=\infty}f(z)=0.\]
\end{proof}
