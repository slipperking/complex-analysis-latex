...
\end{example}
\begin{proof}
    It is evident that \(\csc^2(\pi z)\) has double poles at each of \(z=n\in\mathbb{Z}\). Therefore, \((z-n)^2\csc^2(\pi z)\) is holomorphic (or has an analytic continuation that is holomorphic) on \(D(n,1)\). Therefore, we can write its Taylor expansion on \(D(n,1)\):
    \begin{align*}
        ...
        &\quad+(z-n)\lim_{\zeta\to n}\frac{-4\pi\qty[\pi\qty(\zeta-n)\qty(\pi\qty({\zeta}-n)\cot(\pi{\zeta})-2)\csc[2](\pi\zeta)+\cot(\pi\zeta)]}{2\pi^2\cos(2\pi\zeta)}
    \end{align*}
\end{proof}
