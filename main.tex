...
\[f(z)=\sum_{j=m_k}^\infty c_j{\qty(z-a_k)}^{j},\] where \(c_{m_k}\neq 0\). Therefore, we can divide \(f(z)\) by \((z-a_k)^j\) to obtain a new entire function with no additional zeros and no zero at \(a_k\). Repeating this for every root, we get that \(g(z)=\frac{f(z)}{p(z)}\) is entire and has no zeros, where \[p(z)=z^{m_0}\qty(1-\frac{z}{a_1})^{m_1}\cdots\qty(1-\frac{z}{a_n})^{m_n}.\]
