...
\begin{proof}
    Define the auxiliary function \[g(z)=\begin{cases}
        ...
        f'(0)&z=0
    \end{cases}.\]
    Because \(\lim_{z\to0}\frac{f(z)}{z}=f'(0)\), \(g(z)\) is holomorphic on \(D(0,1)\). We can then apply the Maximum Modulus Principle (\autoref{thm:maximummodulus}) on every disk (\(\forall0<\varepsilon<1\)) in the form \(D(0,1-\varepsilon)\), we get \[g(z)\not>\eval{\frac{f(z)}{z}}_\]
\end{proof}
