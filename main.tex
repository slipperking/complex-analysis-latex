...
This series is not necessarily convergent (hence the usage of "\(\sim\)"). Notice how \(u\) is continuous on a compact set and therefore is bounded by some \(M\) (\autoref{thm:continuousfunctionboundedoncompact}). Therefore, the integrand of \(a_n\) is bounded by \(M\geq\abs{u\qty(e^{i\tau})e^{-in\tau}}\). It follows that \(\qty|a_n|\leq M\). By introducing a factor of \(\abs{\rho}<1\), we obtain the convergent Abel sum of \[\sum_{n=-\infty}^\infty a_ne^{in\tau}\rho^{|n|}\]
TO BE CONTINUED
