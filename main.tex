...
\begin{theorem}
    Let \(V=\cbraces{z\in\mathbb{C}}{r<\abs{z-a}<R}\) for some \(0\leq r<R\leq\infty\). Let \(f\) be holomorphic on \(V\). Then \(f\) has the unique \textit{Laurent expansion} \[f(z)=\sum_{n=-\infty}^\infty c_n(z-a)^n\] when \(z\in V\), where \(c_n=\frac{1}{2\pi i}\oint_{\gamma}\frac{f(\zeta)\ddzeta}{(\zeta-a)^{n+1}}\), where \(\gamma\subset V\) is a simple closed curve enclosing \(a\). 
\end{theorem}
\begin{proof}
    By the openness of \(V\), there exist two circles \(\gamma_1\subset V\) with radius \(r'\) and \(\gamma_2\subset V\) with radius \(R'\) centered at \(a\) such that \(\gamma\) encloses \(\gamma_1\) and \(\gamma_2\) encloses \(\gamma\) both without intersection. Let \(W=\cbraces{z\in V}{r'<|z-a|<R'}\) and \(z\in W\) be arbitrary. By the Cauchy--Goursat Integral Formula (\autoref{thm:cauchygoursatformula}), \[f(z)=\frac{1}{2\pi i}\qty(\int_{\gamma_2}\frac{f(\zeta)\ddzeta}{\zeta-z}-\int_{\gamma_1}\frac{f(\zeta)\ddzeta}{\z})\]
\end{proof}
