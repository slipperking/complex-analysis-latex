...
\end{theorem}
\begin{proof}
    For the sake of contradiction, given that \(\paren{n_k\in\mathbb{N}}_{k\in\mathbb{N}}\) is a strictly increasing sequence, assume that a subsequence of the zeroes \(\paren{z_n}_{n\in\mathbb{N}}\), \(\paren{z_{n_k}}\) has an accumulation point \(z_0\in U\). By the holomorphy of \(f\) over \(U\), there exists an open set containing \(U\) that \(f\) is holomorphic on. Then, \(\exists\varepsilon>0\) such that \(f\) is holomorphic over \(D\paren{z_0,\varepsilon}\). Then over this disk, \(f\) has the Taylor expansion \[f(z)=\sum_{n=0}^\infty a_n\paren{z-z_0}^n.\] By the definition of an accumulation point, \(\exists N\in\mathbb{N}\) such that \(\forall k>N\), \(z_{n_k}\in D\paren{z_0,\varepsilon}\). Then \(\lim_{k\to\infty}f\paren{z_{n_k}}\)
\end{proof}
