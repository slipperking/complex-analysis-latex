...
\begin{proof}
    ...
    Through the proof of \autoref{thm:identityaccumulationofzeros}, if \(f\not\equiv0\) over \(U\), then the zeros of \(f\) do not have an accumulation point in \(U\) and are therefore discrete. In this case, let \(\gamma\subset U\) be a curve that does not pass through the zeros of \(f\). Since each function in the sequence \(f_n\) does not contain zeros in \(U\), by \autoref{thm:argumentprincipleholomorphic}, 
    \begin{equation}
        \lim_{n\to\infty}\int_{\gamma}\frac{f_n'(z)}{f_n(z)}\ddz=0.\label{eq:hurwitzsimplecase_argumentprinciple}
    \end{equation}
    Since \(f\) is continuous, \(\exists\lambda>0\) such that \(\forall0<\delta<\lambda\), \(\forall z\in\gamma\), \(\abs{f(z)}>\delta\). By the uniform convergence of \(\cbraces{f_n(z)}\) and \(\cbraces{f'_n(z)}\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(\forall z\in\gamma\), \(\abs{f_n(z)-f(z)}<\frac{\delta}{2}\) and \(\abs{f'_n(z)-f'(z)}<\frac{\delta}{2}\). Then \(\abs{f_n(z)}>\frac{\delta}{2}\) on \(\gamma\). It follows that \(\frac{1}{f_n(z)}\) and its limit are uniformly bounded: \[\abs{\frac{1}{f(z)}}<\frac{1}{\delta},\quad\abs{\frac{1}{f_n(z)}}<\frac{2}{\delta},\quad\forall z\in\gamma,\forall n>N.\]
    Since \(f\) and \(f'\) are holomorphic over \(\gamma\). By \autoref{thm:continuousfunctionboundedoncompact}, there exists a finite value \(M>0\) such that \(\forall z\in\gamma\), \(\max\paren{\abs{f(z)},\abs{f'(z)}}<M\). Then,
    \begin{align*}
        \abs{\frac{f'}{f}-\frac{f'_n}{f_n}}&=\abs{\frac{f'f_n-f'_nf}{f_nf}}\\
        &<\frac{\abs{f'f_n-f'f}+\abs{f'f-f'_nf}}{\frac{2}{\delta^2}}\\
        &<M\delta^2\frac{\abs{f_n-f}+\abs{f'-f'_n}}{2}\\
        &<\frac{M\delta^3}{2}.
    \end{align*}
    ...
    \[\lim_{n\to\infty}\int_\gamma \frac{f'_n(z)}{f_n(z)}\ddz=\]
\end{proof}
