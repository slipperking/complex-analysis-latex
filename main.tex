...
\begin{example}
    Show that the functions defined by the series \(f(z)=\sum_{n=0}^\infty\alpha^n z^n\) and \(\widetilde{f}(z)\sum_{n=0}^\infty\frac{(\alpha-1)^n z^n}{(1-z)^{n+1}}\) are analytic continuations of each other.
\end{example}
\begin{proof}
    The analytic element \(\qty(D\qty(0,|\alpha|),f)\) can be extended to the analytic element \(\qty(\mathbb{C}\setminus\qty{\frac{1}{\alpha}},z\mapsto\frac{1}{1-\alpha z})\)
\end{proof}
