...
\end{theorem}
\begin{proof}
    Let \(\cbraces{z_n}_{n\in\mathbb{N}}\) be a subset of \(\cbraces{z}{z\in U,f(z)=0}\) with an accumulation point \(z_0\) in \(U\). Since \(f\) is holomorphic over \(U\), \(\exists\varepsilon>0\) such that \(f\) is holomorphic over \(D\paren{z_0,\varepsilon}\subseteq V\). Then over this disk, \(f\) has the Taylor Series representation
    ...
    Similarly, \(a_1=0\). We recursively obtain that \(a_0,a_1,\ldots=0.\) Then \(f(z)\)
\end{proof}
