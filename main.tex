...
where \(\zeta=Re^{i\tau}\). Two harmonic functions are said to be \textit{conjugate} if they are the real and imaginary parts of a holomorphic function. On simply connected open regions, any harmonic function will admit a unique conjugate, (up to an additive constant). For a harmonic function \(u\), we can construct its harmonic conjugate from equation~\eqref{eq:harmonicconjugate}.
