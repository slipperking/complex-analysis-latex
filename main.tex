...
\end{proof}
\begin{example}
    Evaluate the integrals \(\int_0^{2\pi}\Phi(\cos\theta,\sin\theta)\dd{\theta}\), where \(\Phi(\xi,\eta)\) is a rational function of \(\xi\) and \(\eta\) that is continuous on \([0,2\pi]\).
\end{example}
\begin{proof}
    Let \(z=e^{i\theta}\). Consequently, we have \(\cos\theta=\frac{z+z^{-1}}{2}\), \(\sin\theta=\frac{z-z^{-1}}{2i}\), and \(\ddz=ie^{i\theta}\dd{\theta}\), implying that \(\dd{\theta}=\frac{\ddz}{iz}\). Therefore, we have \[\int_0^{2\pi}\Phi(\cos\theta,\sin\theta)\dd{\theta}=\int_{\partial\mathbb{D}}\Phi\qty(\frac{z+z^{-1}}{2},\frac{z-z^{-1}}{2i})\frac{\ddz}{iz}.\]
\end{proof}
