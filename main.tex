...
\end{theorem}
\begin{proof}
    Let \(z_1,z_2,\ldots,z_n\) be the zeros enclosed within \(\gamma\), with multiplicities \(k_1,k_2,\ldots,k_n\) respectively. There exist disks centered around each zero with radii \(\varepsilon_1,\ldots,\varepsilon_n\) respectively, such that each disk is disjoint and contained in the interior of \(\gamma\). The function \[\frac{f'(z)}{f(z)}\] is holomorphic over \[\mathrm{int}\paren{\gamma}\setminus\bigcup_{k=1}^n D\paren{z_k,\varepsilon_k},\] where \(\mathrm{int}\paren{\gamma}\) is the interior with respect to the Jordan curve \(\gamma\).
    This region has the oriented boundary of \(\bigcup_{k=1}^n\partial D\paren{z_k,\varepsilon_k}^-\cup\gamma\). By the Cauchy--Goursat Integral Theorem (\autoref{thm:cauchygoursattheorem}), \[\int_{\bigcup_{k=1}^n\partial D\paren{z_k,\varepsilon_k}^-\cup\gamma}\frac{f'(z)}{f(z)}\ddz=0,\]
    and from rearrangement, \[\int_\gamma \frac{f'(z)}{f(z)}\ddz=\sum_{k=1}^n\int_{\partial D\paren{z_k,\varepsilon_k}}\frac{f'(z)}{f(z)}\ddz.\]
\end{proof}
