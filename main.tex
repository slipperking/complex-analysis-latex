...
\begin{example}
    Let \(f\) be entire and \(\forall z\in\mathbb{C}\), \(|f(z)|\leq Me^{|z|}\). Then \(|f(0)|\leq M\) and \[\abs{f^{(n)}(0)}\leq Mn!\qty(\frac{e}{n})^n.\]
\end{example}
\begin{proof}
    \(\forall R>0\), by Cauchy's Estimate (\autoref{thm:cauchysestimate}), \[\]
\end{proof}
