...
\begin{proof}
    ...
    with a singularity at \(\zeta=z\). Consider the region \(U\setminus D(z,\varepsilon)\). Applying the Stokes--Cartan Theorem (\autoref{theorem:stokescartan}), \begin{equation}
        \int_{U\setminus D(z,\varepsilon)}\dd(\frac{f(\zeta)\ddzeta}{\zeta-z})=\int_U\frac{f(\zeta)\ddzeta}{\zeta-z}-\int_{D(z,\varepsilon)}\frac{f(\zeta)\ddzeta}{\zeta-z}.\label{eq:pompeiuproofdirectintermediate}
    \end{equation}
    ...
    The rightmost term in equation \eqref{eq:pompeiuproofdirectintermediate} can be parameterized with \(\zeta=z+\varepsilon e^{it}\), \(t\in[0,2\pi)\). Then, \[\int_{D(z,\varepsilon)}\frac{f(\zeta)\ddzeta}{\zeta-z}=\int_0^{2\pi}\frac{f(z+\varepsilon e^{it})}{\varepsilon e^{it}}\cdot i\varepsilon e^{it}\dd{t}=i\int_0^{2\pi}f(z+\varepsilon e^{it})\dd{t}.\]
    By combining and rearranging, we take the limit as \(\varepsilon\to0\): \[i\lim_{\varepsilon\to0}\int_0^{2\pi}f(z+\varepsilon e^{it})\dd{t}=\int_U\frac{f(\zeta)\ddzeta}{\zeta-z}-\int_{U\setminus D(z,\varepsilon)}\pdv{f}{\overline{\zeta}}\cdot\frac{f(\zeta)}{\zeta-z}\dd{\overline{\zeta}}\wedge\ddzeta.\] Because $f$ is continuous, 
\end{proof}
\end{document}