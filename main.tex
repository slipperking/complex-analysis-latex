...
\begin{proof}
    ...
    Assume that \(\abs{z}>R\). \(\forall\rho\in\qty(R,|z|)\), \(\frac{1}{\rho}<\frac{1}{R}\). By \autoref{def:limsup}, \(\forall N\in\mathbb{N}\), \(\exists n>N\) such that \(\sqrt[n]{\abs{a_n}}>\frac{1}{\rho}\). It follows that \(\abs{a_nz^n}>\frac{\abs{z^n}}{\rho^n}>1\). Therefore, \(\sum_{n=0}^\infty a_nz^n\) diverges since \(a_nz^n\) does not approach 0 as \(n\to\infty\), confirming \ref{itm:abelradius_divergence}.
