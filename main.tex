...
\begin{theorem}[Partition of Unity]\label{thm:partitionofunity}
    For a nonempty open set \(\Omega\subseteq\mathbb{C}\), let \(\cbraces{\Omega_i}_{i\in\mathbb{Z}_{\ge0}}\) be an open cover of \(\Omega\). Then there exists a set of bump functions known as a \(C^\infty\) partition of unity, \(\alpha_1(z),\alpha_2(z),\ldots\in C^\infty(\mathbb{C})\) with compact support within \(\Omega\) satisfying
    \begin{enumerate}
...
\begin{proof}
    ...
    Define a set of functions \(\cbraces{\beta_j}_{j\in\mathbb{N}}\) with \(\beta_j=\theta_{r_{z_j}}\paren{z-z_j}\), which is a bump function with the support \(\overline{D\paren{z_j, r_{z_j}}}\subset\Omega_{k_{z_j}}\). Equivalently, \(\cbraces{\supp\paren{\beta_j}}_{j\in\mathbb{N}}\) is locally finite on \(\Omega\). Consequently, a point \(z\in\Omega\) lies only in a finite number of sets of \(\cbraces{\supp\paren{\beta_j}}_{j\in\mathbb{N}}\). Then the sum \[S(z)=\sum_{j=1}^\infty\beta_j(z)\] is convergent, and since \(S(z)\) is strictly positive, and the sequence defined by \[\cbraces{\alpha_j(z)}=\cbraces{\frac{\beta_j(z)}{S(z)}}\] is also \(C^\infty\) and is compactly supported in \(\Omega\)
\end{proof}
