...
\begin{lemma}
    If \(f(z)=\prod_{n=1}^\infty f_n(z)\) is an infinite product of holomorphic functions \(\qty{f_n}\) on some open set \(U\subseteq\mathbb{C}\) such that each \(\log\qty(f_n)\) is holomorphic and \(\sum_{n=1}^\infty\log\qty(f_n(z))\) converges uniformly on a compact subset \(K\subset U\), then \(f(z)\) converges uniformly on \(K\).
\end{lemma}
\begin{proof}
