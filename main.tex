...
\begin{proof}
    ...
    Next, assume only that \(\abs{f'(0)}=1\). It follows that \(\abs{g(0)}=1\). Since \(\abs{g(z)}\leq 1\) over \(D(0,1)\), it follows from \autoref{thm:maximummodulus} that \(g\) is constant with magnitude 1, or in the form of \(\exp(i\tau)\), where \(\tau\in\mathbb{R}\) is a constant.
\end{proof}
