...
Let \(P(\zeta,z)=\frac{1-\abs{z}^2}{2\pi\qty|1-\overline{z}\zeta|^2}\). Then, \begin{equation}
...
Indeed, we can apply the transformation \(\tilde{\varphi}_a\qty(z)=R\varphi_a\qty(\frac{z}{R})\) to extend the automorphism to \(D(0,R)\). Let \(\Phi\) instead be harmonic on \(D(0,R)\) and continuous on \(\overline{D(0,R)}\). Then, \[\Phi(0)=\frac{1}{2\pi R}\int_0^{2\pi}\Phi\qty(Re^{i\theta})\dd{\theta}.\] It follows that \(\phi=\Phi\circ\tilde{\varphi}_a\) is also harmonic on \(D(0,R)\), and under the bijectivity of  \(Re^{i\theta}=\tilde{\varphi}_a\qty(Re^{i\tau})\), \[\]
