...
\begin{proof}
    ...
    By \autoref{thm:weierstrassuniformlimit}, the uniform limit \(\sum_{k=1}^\infty\log\qty(f_k(z))\) is holomorphic on \(U\). Therefore, it is bounded on \(K\). It follows that each partial sum is uniformly bounded on \(K\). Since the exponential function is Lipschitz continuous on compact subsets of \(\mathbb{C}\), there exists a finite constant \(M>0\) such that \[\abs{\exp[\sum_{k=1}^n \log\qty(f_k(z))]-\exp[\sum_{k=1}^m \log\qty(f_k(z))]}\leq M\abs{\sum_{k=m+1}^n\log\qty(f_k(z))}<M\varepsilon.\] The criterion of uniform convergence on compact subsets is also known as \textit{locally uniform convergence}. 
\end{proof}
