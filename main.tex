...
\begin{proof}
    ...
    \(\forall k\in\mathbb{N}\), \(\qty[m_k,M_k]\) contains infinitely many elements of \(\qty{a_n}\). Let \(a_{n_k}\) be any arbitrary one of them. We then aim to show \(\forall \varepsilon>0\), \(\exists N\in\mathbb{N}\) such that \(\forall k>N\), \(\abs{a_{n_k}-a}<\varepsilon\). From \(a_{n_k},a\in\qty[m_k,M_k]\), we get \(\abs{a_{n_k}-a}\leq\frac{1}{2^k}(M-m)<\varepsilon\). It follows that \(k>\log_2\qty(\frac{M-m}{\varepsilon})\), and we can let \(N=\ceil{\log_2\qty(\frac{M-m}{\varepsilon})}\). Therefore, \(a_{n_k}\) is a convergent subsequence.
\end{proof}
