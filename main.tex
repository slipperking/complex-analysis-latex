...
\end{theorem}
\begin{proof}
    Since \(P(\zeta,z)=\frac{1}{4\pi}\paren{\frac{\zeta+z}{\zeta-z}+\frac{\overline{\zeta}+\overline{z}}{\overline{\zeta}-\overline{z}}}\), it follows that from equation~\eqref{eq:laplaciancomplexform}, \(\Delta_z P(\zeta,z)=4\pdv{P(\zeta,z)}{z}{\overline{z}}=0\) (since each term is independent of either \(z\) or \(\overline{z}\)). Then equation~\eqref{eq:dirichletproblemwithlaplaceequationsolution} becomes \[\Delta u(z)=\Delta\int_0^{2\pi}\varphi(\zeta)P(\zeta,z)\dd{\tau}=\int_0^{2\pi}\Delta\qty[\varphi(\zeta)P(\zeta,z)]\dd{\tau}=\]
\end{proof}
