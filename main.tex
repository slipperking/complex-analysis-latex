...
\end{example}
\begin{example}
    Let \(U\subseteq\mathhbb{C}\) be open and simply connected such that \(0\in U\). Suppose \(f:U\setminus\qty{0}:\mathbb{C}\) is holomorphic. If \(\forall \theta\in [0,2\pi]\), \(\lim_{r\to 0^+}f(re^{i\theta})=\infty\), prove that \(0\) cannot be an essential singularity.
\end{example}
\begin{proof}
    Although seemingly trivial at first glance,
\end{proof}
