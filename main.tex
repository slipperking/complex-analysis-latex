...
where \(0<\rho<1\). By the uniform continuity of \(\Phi\) on \(\overline{D(0,1)}\) (\autoref{lemma:uniformcontinuousovercompactset}), \(\forall\varepsilon>0\), \(\exists\delta>0\) such that \(\forall\rho\in\qty(\frac{1}{2},1)\) satisfying \(1-\rho<\delta\), \(\forall\theta\in[0,2\pi]\), \(\abs{\Psi\qty(e^{i\theta})-\Psi\qty(\rho e^{i\theta})}<\varepsilon\). It then follows that \[\abs{\frac{1}{2\pi\rho}\int_0^{2\pi}\Phi\qty( e^{i\theta})\dd{\theta}-\frac{1}{2\pi\rho}\int_0^{2\pi}\Phi\qty(\rho e^{i\theta})\dd{\theta}}<\frac{\varepsilon}{\rho}<2\varepsilon.\]
...
\end{equation}
Let \(\phi(z)=\Phi\circ\varphi_a(z)\), which is also harmonic on \(D(0,1)\). By the univalence of \(\varphi_a\), let \(e^{i\theta}=\varphi_a\qty(e^{i\tau})\). It follows that \[ie^{i\theta}\dd{\theta}=i\frac{1-\overline{a}a}{(1-\overline{a}e^{i\tau})^2}e^{i\tau}\dd{\tau}\Longleftrightarrow\dd{\theta}=\frac{1-\overline{a}a}{(1-\overline{a}e^{i\tau})^2}\frac{1-\overline{a}e^{i\tau}}{e^{i\tau}-a}e^{i\tau}\dd{\tau}=\frac{1-\abs{a}^2}{\abs{1-\overline{a}e^{i\tau}}^2}\dd{\tau}.\]
Then from equation \eqref{eq:harmonicfunctionmeanvalueoverboundaryofunitdisk}, since \(\Phi(0)=\phi\circ\varphi_{-a}(0)=\phi(a)\), \[\Phi(0)=\frac{1}{2\pi}\int_0^{2\pi}\phi\qty( e^{i\tau})\frac{1-\abs{a}^2}{\abs{1-\overline{a}e^{i\tau}}^2}\dd{\tau}=\phi(a).\]
Let \(P(\zeta,z)=\frac{1-\abs{z}^2}{2\pi\qty|1-\overline{z}\zeta|^2}=\frac{1-\abs{z}^2}{2\pi\abs{\zeta-z}^2}\). Then, \begin{equation}
    \phi(z)=\int_0^{2\pi}\phi\qty(\zeta)P(\zeta,z)\dd{\tau},\label{eq:poissonintegralformula}
\end{equation}
where \(\zeta=e^{i\tau}\). Equation \eqref{eq:poissonintegralformula} is also known as the \textit{Poisson Integral Formula}.
\(\forall a\in D(0,R)\), \(\forall R>0\), we can apply the transformation \(\tilde{\varphi}_a\qty(z)=R\varphi_{\flatfrac{a}{R}}\qty(\frac{z}{R})\) to extend the automorphism to \(D(0,R)\). Let \(\Phi\) instead be harmonic on \(D(0,R)\) and continuous on \(\overline{D(0,R)}\). Then, \[\Phi(0)=\frac{1}{2\pi R}\int_0^{2\pi}\Phi\qty(Re^{i\theta})\dd{\theta}.\] It follows that \(\phi=\Phi\circ\tilde{\varphi}_a\) is also harmonic on \(D(0,R)\) with \(\Phi(0)=\phi\circ\tilde{\varphi}_{-a}(0)=\phi(a)\), and from the bijectivity of  \(Re^{i\theta}=\tilde{\varphi}_a\qty(Re^{i\tau})\), 
\[\dd{\theta}=\frac{1-\frac{\abs{a}}{R^2}}{\paren{1-\frac{\overline{a}}{R}e^{i\tau}}^2}e^{i\tau}e^{-i\theta}\dd{\tau}=\dd{\theta}=\frac{1-\frac{\abs{a}}{R^2}}{\paren{1-\frac{\overline{a}}{R}e^{i\tau}}^2}\frac{1-\frac{\overline{a}}{R}e^{i\tau}}{1-\frac{a}{R}e^{-i\tau}}e^{i\tau}\dd{\tau}=\frac{R^2-\abs{a}}{\abs{1-\frac{}{}}^2}\]
\[\Phi(0)=\frac{1}{2\pi}\int_0^{2\pi}\phi\qty(e^{i\tau})\frac{1-\abs{a}^2}{\abs{1-\overline{a}e^{i\tau}}^2}\dd{\tau}=\phi(a).\]
