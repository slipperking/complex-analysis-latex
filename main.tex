...
\NewDocumentCommand{\cbraces}{m g}{%
    \ensuremath{%
        \left\lbrace#1%
        \IfNoValueTF{#2}{}{\,\middle|\,#2}%
        \right\rbrace
    }%
}
...
\begin{proof}
    ...
    For each \(k\), \(Q_k\) contains infinitely many points of \(A\). Because the side length of \(Q_k\) tends to zero, for any \(\varepsilon>0\), \(\exists N\in\mathbb{N}\) such that \(\forall k\geq N\), \(Q_k\subset B(z_\infty,\varepsilon)\) where \(B(z,r)\) is the \textit{ball} with radius \(r\) centered at \(z\), or \[B(a,r)=\cbraces{\qty(z_1,z_2,\ldots,z_n)\in\mathbb{C}^n}{\sum_{j=1}^n\abs{z_j-a_j}^2<r^2}.\]
    Then, \(B(z_\infty, \varepsilon)\) also contains infinitely many points of \(A\). Therefore, \(z_\infty\) is an accumulation point of \(A\).

    We now show that \(z_\infty\in X\). Suppose for contradiction that \(z_\infty\notin X\). Since \(X\) is closed, \(\mathbb{C}^n\setminus X\) is open, and \(\exists\delta>0\) such that \[B(z_\infty,\delta)\subset\mathbb{C}^n\setminus X.\] But then, for sufficiently large \(k\), we have \(Q_k \subset B(z_\infty, \delta)\), and hence \(Q_k\cap X=\emptyset\). This contradicts the construction of \(Q_k\), which ensures that \(Q_k\) contains infinitely many points of \(A \subset X\).
\end{proof}
...
\begin{proof}
    ...
    First we will show that \(X\) is bounded. Suppose that \(\forall R>0\), \(\exists z\in X\) where \(\norm{z}>R\). Consider the collection of open sets \[\mathcal{U}=\{B(0,k)\mid k\in\mathbb{N}\}.\] \(\mathcal{U}\) forms an open cover of \(X\). Then by the assumption, there exists a finite subcover in \(\mathcal{U}\), namely \(\{B(0,k_1),\ldots,B(0,k_m)\}\) which covers \(X\). Then, \[X\subseteq\bigcup_{i=1}^mB(0,k_i)=B(0,\max(k_1,\ldots k_m)).\] By contradiction, \(X\) must be bounded.
    ...
    \[\mathcal{U}=\left\{\mathbb{C}^n\setminus\overline{B}\left(z_0,\frac{1}{k}\right)\;\middle|\; \forall k\in\mathbb{N}\right\}.\]
    There then exists a finite subcover \(\mathcal{C}=\left\{\mathbb{C}^n\setminus\overline{B}\left(z_0,\frac{1}{k_i}\right)\;\middle|\; i=1,2,\ldots,m\right\}\). Then, \[X\subseteq\mathbb{C}^n\setminus\overline{B}\left(z_0,\frac{1}{\max(k_1,\ldots,k_m)}\right),\]
...
\end{lemma}
It is true that for any set \(U\subseteq \mathbb{R}^n\), regardless of contractibility, that for a differential form \(\omega\) defined on \(U\), \(\dd\dd\omega=0\).
...
\begin{definition}[Limit Superior]\label{def:limsup}
    ...
    \end{enumerate}
    then the \textit{superior limit} of \(\qty{a_n}\) is \(a\), denoted by \(\limsup_{n\to\infty}a_n=a\).
\end{definition}
\begin{definition}[Limit Inferior]\label{def:liminf}
    ...
    \begin{enumerate}
        ...
        \item \(\forall\varepsilon>0\), \(\forall N\in\mathbb{N}\), \(\exists n>N\) such that \(a_n<a+\varepsilon\),
    \end{enumerate}
    then the \textit{inferior limit} of \(\qty{a_n}\) is \(a\), denoted by \(\liminf_{n\to\infty}a_n=a\).
\end{definition}
\begin{lemma}
    A number sequence \(\qty{a_n}\) is convergent if and only if \(\limsup_{n\to\infty} a_n=\liminf_{n\to\infty}a_n\).
\end{lemma}
\begin{proof}
    We first prove that \(a=\lim_{n\to\infty}a_n\) implies \(\limsup_{n\to\infty}a_n=\liminf_{n\to\infty}a_n=a\).
    ...
    Then from the first conditions of \autoref{def:limsup} and \autoref{def:liminf}, we get that \(\limsup_{n\to\infty}a_n\geq a\) and \(\liminf_{n\to\infty} a_n\leq a\). By the second conditions, we get \(\limsup_{n\to\infty}a_n\leq a\) and \(\liminf_{n\to\infty} a_n\geq a\). Therefore, \[\limsup_{n\to\infty}a_n=\liminf_{n\to\infty}a_n.\]

    For the converse, assume \(\limsup_{n\to\infty}a_n=\liminf_{n\to\infty}a_n\). Since \(\exists N_1\in\mathbb{N}\) such that \(\forall n>N_1\), \(a_n<a+\varepsilon\). \(\exists N_2\in\mathbb{N}\) such that \(\forall n>N_2\), \(a_n>a-\varepsilon\). Then \(\forall n>\max\qty(N_1, N_2)\), \(\qty|a_n-a|<\varepsilon\), as expected.
\end{proof}
...
\begin{proof}
    ...
    In particular, \(\forall n\in\mathbb{N}\), let \(\delta=\frac{1}{n}\). Then \(\exists x_n,y_n\in U\) such that \(|x_n-y_n|<\frac{1}{n}\) and \(|f(x_n)-f(y_n)|\geq\varepsilon_0\). Since \(U\) is compact, the sequence \(\cbraces{x_n}\) has a convergent subsequence \(x_{n_k}\to x_{\infty}\in U\). Because \(|x_{n_k}-y_{n_k}|<\frac{1}{n_k}\to 0\), we also have \(y_{n_k}\to x_{\infty}\).

    By continuity, \(f(x_{n_k})\to f\paren{x_\infty}\) and \(f(y_{n_k})\to f\paren{x_\infty}\), so \(|f(x_{n_k})-f(y_{n_k})|\to 0\), contradicting \(|f(x_{n_k})-f(y_{n_k})|\geq\varepsilon_0\). Therefore, \(f\) is uniformly continuous on \(U\).
\end{proof}
\begin{definition}
...
For simplicity's sake, if we mention that a function is holomorphic on a region, it is implied for that region to be connected.
\begin{theorem}\label{thm:holomorphycondition}
    The necessary and sufficient criterion for a function \(f(z)\) to be holomorphic on an open region \(U\) is if \(f(z)\in C^1(U)\) and it satisfies the Cauchy--Riemann equations.
\end{theorem}
\begin{proof}
    The first part is to prove that any holomorphic function on \(U\) has continuous first-order partial derivatives in \(U\). This requires an argument that will be covered later (specifically in \autoref{sec:analyticityandholomorphy}), which claims that the complex derivative of any holomorphic function is also holomorphic over the region.
...
This type of equation is called the \textit{Laplace equation}, which is a basic example of an elliptic partial differential equation. Define the operator (the \textit{Laplacian}) \[\Delta=\laplacian=\pdv[2]{}{x}+\pdv[2]{}{y}.\] A function \(u\) satisfying the Laplace equation \(\Delta u=0\) is a \textit{harmonic function}. Thus, the real and complex parts of a holomorphic function are harmonic functions.
...
\begin{enumerate}
    ...
    where \(\arg(z)\) is the multi-valued argument function. The \textit{principle branch} of the argument is denoted \(\Arg:\mathbb{C}\to(-\pi,\pi]\). Then principle branch of \(\log(z)\), or \(\Log(z)\), can be defined such that \(\Im\paren{\Log}\in(-\pi,\pi]\).
\end{enumerate}
...
\begin{theorem}[Abel's Theorem]\label{thm:abelradius}
    ...
    \begin{enumerate}
        \item \(f(z)\) absolutely converges on \(\abs{z}<R\), and \(\forall 0\leq\rho<R\), uniformly converges on \(\abs{z}\leq\rho\).\label{itm:abelradius_absoluteanduniformconvergence}
        \item \(f(z)\) diverges on \(\abs{z}>R\).\label{itm:abelradius_divergence}
        \item \(f(z)\) is holomorphic over \(\abs{z}<R\) and \(f'(z)\) can be obtained by termwise differentiation \(f'(z)=\sum_{n=1}^\infty na_nz^{n-1}\) and also has the radius of convergence \(R\).\label{itm:abelradius_differentiation}
    \end{enumerate}
\end{theorem}
...
\begin{theorem}[Cauchy--Hadamard Theorem]\label{thm:cauchyhadamard}
    The radius of convergence of the power series in the form of \(\sum_{n=0}^\infty a_nz^n\) can be determined by \begin{equation}
        R=\frac{1}{\limsup_{n\to\infty}\sqrt[n]{\abs{a_n}}}. \label{eq:cauchyhadamard}
    \end{equation}
\end{theorem}
...
\begin{proof}
    ...
    To prove \ref{itm:abelradius_differentiation}, first observe that \(\sum_{n=1}^\infty na_nz^n\) and \(\sum_{n=1}^\infty a_nz^n\) have the same convergence radius since \(\limsup_{n\to\infty}\sqrt[n]{n}=1\). For \(z\in D(0, R)\), let \(f(z)=S_n(z)+R_n(z)\), where \[S_n(z)=\sum_{k=0}^{n-1}a_kz^k,\quad R_n(z)=\sum_{k=n}^\infty a_kz^k.\] 
...
\begin{proof}
    ...
    Because the integrand is dominated by a function that has a convergent integral, such as the integral \(\int_{\supp(\psi)}M\dd{\xi}\wedge\dd{\overline{\xi}}\), equation \eqref{eq:onedimensionalpartialconjugatesolution_differenceexpr} uniformly converges, and the limit \(h\to0\) may commute with the integral. Let \(\zeta=\alpha+i\beta\). From the real axis, \begin{equation}
    ...
    On the bounded domain \(\supp\paren{\psi}\), and by the Cauchy--Pompeiu Theorem (\autoref{thm:pompeiu}), the integral above is equal to \[\pdv{u}{\overline{z}}=\psi(z)-\frac{1}{2\pi i}\oint_{\partial\supp(\psi)}\frac{\psi(\zeta)}{\zeta-z}\ddzeta.\]
    Since \(\psi\) is continuous and compactly supported, it vanishes on \(\partial\supp\paren{\psi}\): \[\pdv{u}{\overline{z}}=\psi(z),\] as expected.
\end{proof}
\begin{remark}
...
\begin{proof}
    ...
    \end{align*}
    where the remainder terms \(O\paren{\abs{z-z_0}^2}\) resemble
    \[\paren{z-z_0}^2\frac{k!}{2\pi i}\brackets{k+1+\binom{k+1}{2}}\oint_{\partial U}\frac{f(\zeta)}{\paren{\zeta-z_0}^{k+3}}\ddzeta+O\paren{\abs{z-z_0}^3}.\]
...
\begin{definition}[Analytic Continuation]
    ...
    \end{enumerate}
\end{definition}
\begin{theorem}\label{thm:analyticcontinuationremovablesingularities}
...
Two clopen sets in any topological space \(X\) are \(\emptyset\) and \(X\). \(X\) is \textit{connected} if and only if \(X\) and \(\emptyset\) are the only clopen subsets of \(X\). The proof of this characterization is non-trivial and is omitted here.
\begin{example}
    The topological space of \((\mathbb{R},\tau)\) where \(\tau\) is the standard topology only contains two clopen sets: \(\mathbb{R}\) and \(\emptyset\).
...
\begin{proof}
    ...
    \(\forall n\in\mathbb{N}\) and \(\forall z\in V_n\), since \(W_n\) is an open set containing \(z\) and \(\mathfrak{B}\) is a basis, there exists \(U_{z,n}\in\mathfrak{B}\) such that \(z\in U_{z,n}\subseteq W_n\) (in short, \(U_{z,n}\) is an open neighborhood of \(z\) that is contained in \(W_n\)). The collection \(\cbraces{U_{z,n}}{z\in V_n}\) is an open cover of the compact set \(V_n\), so by the Heine-Borel Theorem (\autoref{thm:heineborel}) it admits a finite subcover of \(V\). That is, there exist finitely many points \(z_{n,1},\ldots,z_{n,k_n}\) such that \(V_n\subset\bigcup_{i=1}^{k_n}U_{z_{n,i},n}\subseteq W_n\).
...
For a region \(U\subseteq\mathbb{C}\) and a holomorphic function \(f:U\to\mathbb{C}\), a point \(z_0\in U\) is a \textit{zero} of \(f\) if and only if \(f\paren{z_0}=0\). Furthermore, if \(f\) has the Taylor expansion at \(z_0\) of \[a_m\paren{z-z_0}^m+a_{m+1}\paren{z-z_0}^{m+1}+\cdots,\quad\quad m\in\mathbb{N},a_m\neq0,\] then the zero at \(z_0\) has multiplicity \(m\).
...
\begin{proof}
    ...
    Let \(z=z_n\neq z_\infty\) for some \(n>N\). Then \(f_1\) vanishes, leaving \[0=a_1+O_{n\to\infty}\paren{z_n-z_\infty}.\]
    ...
    Since \(S^*\) is nonempty and clopen in the connected set \(U\), \(S^*=U\). It follows that \(f\equiv 0\) on \(U\).
\end{proof}
\begin{remark}
...
\begin{proof}
    ...
    Let \(K\subset U\) be arbitrary and compact and \(V\supset K\) be open and relatively compact in \(U\). Since \(\cbraces{f'_n(z)}\) is holomorphic, we can define a holomorphic sequence \(\varphi_n(z)=f_n(z)-f(z)\). By \autoref{crl:nthderivativeboundedsupremum}, there exists a finite constant \(c>0\) such that \[\lim_{n\to\infty}\sup_{z\in K}\abs{\varphi'_n(z)}\leq c\lim_{n\to\infty}\sup_{z\in V}\abs{f_n(z)-f(z)}.\]
    ...
    By the continuity of \(f\), \(\exists\lambda>0\) such that \(\forall z\in\gamma\), \(\abs{f(z)}>\lambda\). By the uniform convergence of \(\cbraces{f_n(z)}\), \(\exists N\in\mathbb{N}\) such that
...
\begin{proof}
    ...
    Let \(k_f\) and \(k_g\) denote the number of zeros of \(f\) and \(g\) enclosed by \(\gamma\), respectively. By the argument principle,
    ...
    From \eqref{eq:rouche}, by dividing both sides by \(f(z)\), we obtain \(\abs{w-1}<1\). Then \(\Gamma\) lies in the open disk \(D(1,1)\), which will never intersect or enclose 0. Then by \autoref{lemma:cauchyintegraltheoremoversimplyconnectedset}, \[k_g-k_f=\int_\Gamma\frac{\dd{w}}{w}=0,\] as desired.
\end{proof}
...
Indeed, suppose a holomorphic function attained a local minimum of its modulus at a zero. Then, by adding a sufficiently small constant (i.e., a shift), the zero could disappear from the region, violating the stability of zeros under small additive perturbations established by \autoref{thm:holomorphiczerosstabilityundershifts}. 

For comparison, consider the real function \(x^2\). Shifting it upward removes the zero of multiplicity 2 at \(x=0\) in \(\mathbb{R}\), while shifting it downward causes that zero to split into two simple zeros. The latter behavior agrees with \autoref{thm:holomorphiczerosstabilityundershifts}, and provides a helpful way to visualize the stability of zeros in two dimensions. However, the fact that the zero is not preserved under a positive shift highlights a key difference between real differentiable functions and holomorphic functions.
...
\end{proof}
Since the real and imaginary parts of holomorphic functions are harmonic functions, they also satisfy the mean-value property. Furthermore, if a real continuous function satisfies the mean-value property, it is harmonic (we will prove this in \autoref{sec:}) TO BE CONTINUED. This equivalence allows for the alternative definition of harmonic functions.
\begin{theorem}[Maximum Modulus Principle]\label{thm:maximummodulus}
    Let \(f(z)\) be holomorphic over an open connected region \(U\subseteq\mathbb{C}\). If \(\exists z_0\in U\) such that for some open neighborhood \(V\subseteq U\) of \(z_0\) and \(\forall z\in U\), \(\abs{f\paren{z_0}}\geq\abs{f(z)}\), then \(f\) is a constant function on \(U\).
\end{theorem}
\begin{proof}
    ...
    \begin{align*}
        ...
        &\leq\frac{1}{2\pi}\int_0^{2\pi}\abs{f(z)}\dd{t}\leq \abs{f(z)}.
    \end{align*}
...
\begin{lemma}[Schwarz Lemma]\label{lemma:schwarz}
    If \(f:D(0,1)\to D(0,1)\) is holomorphic and \(f(0)=0\), then \[\abs{f(z)}\leq\abs{z},\quad\abs{f'(0)}\leq 1.\]
...
\begin{proof}
    Define the auxiliary function \[g(z)=\begin{cases}
        ...
        f'(0)&z=0
    \end{cases}.\]
    Because \(\lim_{z\to0}\frac{f(z)}{z}=f'(0)\), \(g(z)\) is holomorphic on \(D(0,1)\). Since \(f\) is an automorphism on the open disk, \(\forall\abs{z}<1\), \(\abs{f(z)}<1\). By the Maximum Modulus Principle \autoref{thm:maximummodulus}, \(\forall0<\varepsilon<1\), \(\forall z\in D(0,\varepsilon)\), \[\abs{g(z)}\leq\max_{z_\varepsilon\in \partial D(0,\varepsilon)}\frac{\abs{f\paren{z_\varepsilon}}}{\varepsilon}<\frac{1}{\varepsilon}.\] As \(\varepsilon\to 1^-\), we obtain that \(\forall z\in D(0,1)\), \(\abs{g(z)}\leq 1\), or that \(\abs{f(z)}\leq\abs{z}\). Let \(z=0\). Then we get \(\abs{g(0)}=f'(0)\leq 1\). 

    For the sake of the equality condition, assume \(|f(z)|=|z|\). Then \(\abs{g(z)}\equiv 1\) on the unit open disk. By \autoref{thm:maximummodulus}, \(g(z)=\exp(i\tau)\) where \(\tau\in\mathbb{R}\) and \(f(z)=z\exp(i\tau)\) on \(D(0,1)\). 

    Next, assume only that \(\abs{f'(0)}=1\). It follows that \(\abs{g(0)}=1\). Since \(\abs{g(z)}\leq 1\) for all \(z\in D(0,1)\), it follows from \autoref{thm:maximummodulus} that \(g\) is constant with magnitude 1, or in the form of \(\exp(i\tau)\), where \(\tau\in\mathbb{R}\) is a constant. Consequently, \(f(z)=z\exp(i\tau)\).
\end{proof}
...
First we will show that \(\forall a\in D(0,1)\),
\begin{equation}
    \varphi_a(z)=\frac{z-a}{1-\overline{a}z}\in\Aut( D(0,1)).\label{eq:mobiustransformationgroupofholomorphicautomorphismsstatement}
\end{equation}
Firstly, the function is holomorphic on \(D(0,1)\) as \(|z|\le1\), \(\abs{\overline{a}}<1\), the denominator never vanishes. Additionally, \(\varphi_a(a)=0\).

First, we will observe the image of \(\partial D(0,1)\). Let \(\abs{z}=1\). Then,
\[\abs{\varphi_a(z)}=\abs{\frac{1}{z}}\abs{\frac{z-a}{\frac{1}{z}-\overline{a}}}=\abs{\frac{z-a}{\overline{z}-\overline{a}}}=1.\]
Therefore, the image of \(\partial D(0,1)\) lies on \(\partial D(0,1)\), and since \(f\) is holomorphic and non-constant, by the Maximum Modulus Principle (\autoref{thm:maximummodulus}), for \(\abs{z}<1\), \(\abs{\varphi_a(z)}<1\). Therefore, \(f\) maps \(D(0,1)\) to \(D(0,1)\). We next aim to show that \(f:D(0,1)\to D(0,1)\) is bijective.

Let us first confirm injectivity. \(\forall z_1,z_2\in D(0,1)\), we will observe when \[\frac{z_1-a}{1-\overline{a}z_1}=\frac{z_2-a}{1-\overline{a}z_2}\] is satisfied. It follows that \[\qty(z_1-a)\qty(1-\overline{a}z_2)=\qty(z_2-a)\qty(1-\overline{a}z_1)\Longleftrightarrow z_1-a-\overline{a}z_1z_2+\abs{a}^2z_2=z_2-a-\overline{a}z_1z_2+\abs{a}^2z_1.\]
...
\end{equation}
It follows that \(\varphi_{-a}=\qty(\varphi_a)^{-1}\). Thus \(\varphi_a\) is surjective and a bijective automorphism. It follows that equation \eqref{eq:mobiustransformationgroupofholomorphicautomorphismsstatement} is true. Functions in the form of \(\varphi_a\) (where \(a\in D(0,1)\)) are known as \textit{Möbius transformations}, and the group of all such transformations is known as the \textit{Möbius transformation group on the unit disk}, which is a subgroup of \(\Aut(D(0,1))\). Functions in the form of \(\rho_\tau(z)=z\exp(i\tau)\), where \(\tau\in\mathbb{R}\) is constant, form a group known as the \textit{rotation group}, which is also a subgroup of \(\Aut(D(0,1))\).
\begin{theorem}[The Holomorphic Automorphism Group on the Unit Disk]\label{thm:holomorphicautomorphismgrouponunitdisk}
    \(\forall f\in \Aut(D(0,1))\), \(f\) is a composition between a Möbius transformation and a rotation. In other words, \(\exists |a|<1\) and \(\exists\tau\in\mathbb{R}\) such that \[f(z)=\varphi_a\circ\rho_\tau(z).\]
\end{theorem}
\begin{proof}
    Define the auxiliary function \(\psi(z)=\varphi_{f(0)}\circ f(z)\). It follows that \(\psi\in\Aut(D(0,1))\). Furthermore, \(\psi(0)=\varphi_{f(0)} f(0)=0\). By the Schwarz Lemma (\autoref{lemma:schwarz}), \(\qty|\psi'(0)|\leq1\). Since \(\psi^{-1}\in\Aut(D(0,1))\) with \(\psi^{-1}(0)=0\), \(\abs{\qty(\psi^{-1})'(0)}\leq1\). Then, \[\abs{\qty(\psi^{-1})'(0)}=\frac{1}{\psi'\qty(\psi^{-1}(0))}=\frac{1}{\psi'(0)}\leq 1.\]
...
\begin{lemma}[Schwarz--Pick Lemma]\label{lemma:schwarzpick}
    Let \(f:D(0,1)\to D(0,1)\) be holomorphic. \(\forall z_1,z_2\in D(0,1)\), let \(w_1=f\qty(z_1)\) and \(w_2=f\qty(z_2)\). Then,
    \begin{equation}
    ...
    \end{equation}
    The equalities hold if and only if \(f\in\Aut(D(0,1))\).
\end{lemma}
\begin{proof}
    Let \[\varphi_{-z_1}(z)=\frac{z+z_1}{1+\overline{z_1}z}\in\Aut(D(0,1)),\quad\varphi_{w_1}(z)=\frac{z-w_1}{1-\overline{w_1}z}\in\Aut(D(0,1)).\]
    It follows that \(\varphi_{w_1}\circ f\circ\varphi_{-z_1}(0)=\varphi_{w_1}\qty(w_1)=0\). By the Schwarz Lemma (\autoref{lemma:schwarz}), \[\abs{\varphi_{w_1}\circ f\circ\varphi_{-z_1}(z)}\leq\abs{z}\] for \(z\in D(0,1)\). Let \(z_2=\varphi_{-z_1}(z)\). Then, \(\abs{\varphi_{w_1}\circ f\qty(z_2)}\leq\abs{\varphi_{z_1}\qty(z_2)}\Leftrightarrow\abs{\varphi_{w_1}\qty(w_2)}\leq\abs{\varphi_{z_1}\qty(z_2)}\), confirming equation \eqref{eq:schwarzpick_statement1}. By the second statement of the Schwarz Lemma (\autoref{lemma:schwarz}), \(\abs{\qty(\varphi_{w_1}\circ f\circ\varphi_{-z_1})'(0)}\leq1\).
    ...
    Since both derivatives are positive, \(\abs{f'(z_1)}\leq\frac{1-\overline{w_1}w_1}{1-\overline{z_1}z_1}\). Since \(z_1\in D(0,1)\) is arbitrary, it follows that 
    \begin{equation}
    ...
    \end{equation}
    By the Schwarz Lemma (\autoref{lemma:schwarz}), under the equality condition \(\abs{\varphi_{w_1}'\qty(w_1)f'\qty(z_1)\varphi_{-z_1}'(0)}=1\), \(\varphi_{w_1}\circ f\circ\varphi_{-z_1}=e^{i\tau}\), where \(\tau\in\mathbb{R}\) is constant. It follows that \[f=\varphi_{-w_1}\circ e^{i\tau}\circ\varphi_{z_1}\in\Aut(D(0,1)).\]
\end{proof}
\begin{remark}
    In \autoref{sec:differentialgeometry}, we will introduce the \textit{hyperbolic metric} on \(D(0,1)\), defined as \[\dd{s}^2=\frac{4\abs{\ddz}^2}{\qty(1-|z|^2)^2}.\]
    From equation \eqref{eq:schwarzpick_nonincreasingmetric}, we get that the hyperbolic metric does not increase under a holomorphic mapping of \(D(0,1)\) to itself. This metric is invariant (the equality condition) under all functions in \(\Aut(D(0,1))\). This gives a geometric explanation for \autoref{lemma:schwarz}.
\end{remark}
...
Let \(z\in D(0,1)\) and notice that \(\varphi_z(\zeta)=\frac{\zeta-z}{1-\overline{z}\zeta}\in\Aut(D(0,1))\) maps \(\partial D\) to \(\partial D\) bijectively. Let \(\Phi:D(0,1)\to\mathbb{R}\) be harmonic such that \(\Phi\) is continuous on \(\overline{D(0,1)}\). By the mean-value property introduced in \autoref{lemma:holomorphicmeanvalueproperty}, we have \[\Phi (0)=\frac{1}{2\pi\rho}\int_0^{2\pi}\Phi\qty(\rho e^{i\theta})\dd{\theta},\]
where \(0<\rho<1\). By the uniform continuity of \(\Phi\) on \(\overline{D(0,1)}\) (\autoref{lemma:uniformcontinuousovercompactset}), \(\forall\varepsilon>0\), \(\exists\delta>0\) such that \(\forall\rho\in\qty(\frac{1}{2},1)\) satisfying \(1-\rho<\delta\), \(\forall\theta\in[0,2\pi]\), \(\abs{\Phi\qty(e^{i\theta})-\Phi\qty(\rho e^{i\theta})}<\varepsilon\). It then follows that \[\abs{\frac{1}{2\pi\rho}\int_0^{2\pi}\Phi\qty( e^{i\theta})\dd{\theta}-\frac{1}{2\pi\rho}\int_0^{2\pi}\Phi\qty(\rho e^{i\theta})\dd{\theta}}<\frac{\varepsilon}{\rho}<2\varepsilon.\]
Hence, \begin{equation}
    \lim_{\rho\to1^-}\frac{1}{2\pi\rho}\int_0^{2\pi}\Phi\qty(\rho e^{i\theta})\dd{\theta}=\frac{1}{2\pi}\int_0^{2\pi}\Phi\qty( e^{i\theta})\dd{\theta}=\Phi(0).\label{eq:harmonicfunctionmeanvalueoverboundaryofunitdisk}
\end{equation}
Let \(u(\zeta)=\Phi\circ\varphi_z(\zeta)\), which is also harmonic on \(D(0,1)\). By the univalence of \(\varphi_z\), let \(e^{i\theta}=\varphi_z\qty(e^{i\tau})\). It follows that \[ie^{i\theta}\dd{\theta}=i\frac{1-\overline{z}z}{(1-\overline{z}e^{i\tau})^2}e^{i\tau}\dd{\tau}\Longleftrightarrow\dd{\theta}=\frac{1-\overline{z}z}{(1-\overline{z}e^{i\tau})^2}\frac{1-\overline{z}e^{i\tau}}{e^{i\tau}-z}e^{i\tau}\dd{\tau}=\frac{1-\abs{z}^2}{\abs{1-\overline{z}e^{i\tau}}^2}\dd{\tau}.\]
...
\(\forall z\in D(0,R)\), \(\forall R>0\), we can apply the transformation \(\tilde{\varphi}_z\qty(\zeta)=R\varphi_{\flatfrac{z}{R}}\qty(\frac{\zeta}{R})\) to extend the automorphism to \(D(0,R)\). Let \(\Phi\) instead be harmonic on \(D(0,R)\) and continuous on \(\overline{D(0,R)}\). Then, \[\Phi(0)=\frac{1}{2\pi}\int_0^{2\pi}\Phi\qty(Re^{i\theta})\dd{\theta}.\]
It follows that \(u=\Phi\circ\tilde{\varphi}_z\) is also harmonic on \(D(0,R)\) with \(\Phi(0)=u\circ\tilde{\varphi}_{-z}(0)=u(z)\), and from the bijectivity of  \(Re^{i\theta}=\tilde{\varphi}_z\qty(Re^{i\tau})\), 
\[\dd{\theta}=\frac{1-\frac{\abs{z}^2}{R^2}}{\paren{1-\frac{\overline{z}}{R}e^{i\tau}}^2}e^{i\tau}e^{-i\theta}\dd{\tau}=\dd{\theta}=\frac{1-\frac{\abs{z}^2}{R^2}}{\paren{1-\frac{\overline{z}}{R}e^{i\tau}}^2}\frac{1-\frac{\overline{z}}{R}e^{i\tau}}{1-\frac{z}{R}e^{-i\tau}}\dd{\tau}=\frac{R^2-\abs{z}^2}{\abs{Re^{i\tau}-z}^2}\dd{\tau}.\]
...
The expression \(P(\zeta,z)=\frac{R^2-\abs{z}^2}{2\pi\abs{\zeta-z}^2}\) is a general form of the Poisson kernel. Then with \(\zeta=Re^{i\tau}\),
\begin{equation}
    u(z)=\int_0^{2\pi}u(\zeta)P(\zeta,z)\dd{\tau}.\label{eq:poissonintegralformula2}
\end{equation}
Since \(R=\abs{\zeta}\), \[P(\zeta,z)=\frac{|\zeta|^2-|z|^2}{2\pi\paren{\zeta-z}\paren{\overline{\zeta}-\overline{z}}}=\frac{1}{4\pi}\paren{\frac{\zeta+z}{\zeta-z}+\frac{\overline{\zeta}+\overline{z}}{\overline{\zeta}-\overline{z}}}=\frac{1}{2\pi}\Re\qty(\frac{\zeta+z}{\zeta-z}).\]
...
where \(z\in D(0,R)\). Since \(R>0\) and \(\zeta-z\neq0\), the function \(\frac{1}{2\pi i}\int_{\partial D(0,R)}\frac{u(\zeta)}{\zeta}\frac{\zeta+z}{\zeta-z}\ddzeta\) is holomorphic on \(D(0,R)\). Therefore, \(u(z)\) is the real part of a holomorphic function \(f(z)=\frac{1}{2\pi i}\int_{\partial D(0,R)}\frac{u(\zeta)}{\zeta}\frac{\zeta+z}{\zeta-z}\ddzeta+ic\), where \(c\in\mathbb{R}\). Since \(c\in\mathbb{R}\) is holomorphic and real-valued, by \autoref{lemma:realvaluedholomorphicfunctionconstant}, \(c\) is constant. For \(f(z)=u(z)+iv(z)\), \[v(z)=c+\frac{1}{2\pi i}\int_{\partial D(0,R)}\frac{u(\zeta)}{\zeta}\Im\qty(\frac{\zeta+z}{\zeta-z})\ddzeta.\] Letting \(z=0\), the integral vanishes, and we obtain \(c=v(0)=\Im(f(0))\).
...
Laurent Series are a generalization of Taylor Series specifically for holomorphic functions on a region punctured by a non-removable singularity. Whereas the Taylor Series applies to a disk, the Laurent Series applies to an annulus around the singularity.

We have touched the subject of Weierstrass' Theorem for the uniform convergence of the derivatives of a holomorphic function sequence, and we will give the full result below.
...
\begin{proof}
    ...
    \[\lim_{n\to\infty}\sup_{z\in K}\abs{f_n^{(k)}(z)}\leq c_k\lim_{n\to\infty}\sup_{z\in V}\abs{f_n(z)}.\] Since \(\cbraces{f_n(z)}\) is uniformly convergent, the limit on the right-hand side vanishes. Then,
    \[\lim_{n\to\infty}\sup_{z\in K}\abs{f_n^{(k)}(z)}=0,\] and therefore \(\qty{f^{(k)}_n(z)}\) uniformly converges.
\end{proof}
