...
\begin{proof}
    ...
    Therefore, \(D\paren{z_\infty,\varepsilon}\subseteq E\). For any sequence \(\cbraces{z'_n}\in E\) converging to some \(z'_\infty\in U\), by the continuity of \(f\), \[\lim_{n\to\infty} f\paren{z'_n}=f\paren{\lim_{n\to\infty}z'_n}=f\paren{z'_\infty}=0,\] and \(z'_\infty\in E\). Thus, \(E\) contains all of its accumulation points in \(U\) and is therefore closed (if \(z'_\infty\notin U\), then it is no longer relevant; we are concerned with it being closed within \(U\)). \(E\) is also open 
\end{proof}
\begin{remark}
