...
where \(0<\rho<1\). By the uniform continuity of \(\Phi\) on \(\overline{D(0,1)}\) (\autoref{lemma:uniformcontinuousovercompactset}), \(\forall\varepsilon>0\), \(\exists\delta>0\) such that \(\forall\rho\in(0,1)\) satisfying \(\abs{\rho-1}<\delta\), \(\forall\theta\in[0,2\pi]\), \(\abs{\Psi\qty(e^{i\theta})-\Psi\qty(\rho e^{i\theta})}<\varepsilon\). It follows that \[\abs{\frac{1}{2\pi\rho}\int_0^{2\pi}\Phi\qty( e^{i\theta})\dd{\theta}-\frac{1}{2\pi\rho}\int_0^{2\pi}\Phi\qty(\rho e^{i\theta})\dd{\theta}}\]
