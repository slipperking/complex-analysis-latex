...
where \(I=\mqty(\imat{2})\). The quotient group (a group of "cosets" in the form \(\qty{A,-A}\)) is taken because \(\mqty(a&b\\c&d)\) refers to the same transformation as \(\mqty(-a&-b\\-c&-d)\). This group, denoted by \(\mathrm{PSL}(2,\mathbb{C})=\mathrm{SL}(2,\mathbb{C})/\qty{\pm I}\cong\Aut\qty(\riemannsphere)\), is known as the \textit{projective special linear group} of order 2, and is \textit{isomorphic} (one-to-one) to the \textit{group of Möbius transformations}, consisting of all fractional linear transformations.
...
\[f(z)=c_{-m}(z-a)^{-m}+c_{1-m}(z-a)^{1-m}+\cdots+c_1(z-a)+\cdots.\]
Multiplying by \((z-a)^{m}\), we obtain that \[(z-a)^mf(z)=c_{-m}+c_{1-m}(z-a)+\cdots+c_1(z-a)^{m+1}+\cdots,\]
and by differentiating \(m+1\) times, we get:
\[\dv[m+1]{(z-a)^{m+1}}{z}=c_1+\mathcal{O}_{z\to a}(z-a)\Leftrightarrow\residue_{z=a}f(z)=\]
