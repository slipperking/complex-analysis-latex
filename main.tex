...
\begin{theorem}[Hurwitz's Theorem]\label{thm:hurwitz}
    ...
    Then, for any compact set \(K\subset U\) where \(f(z)\) has no zeros on \(\partial K\), and any open neighborhood \(V\) of the set \(\cbraces{z\in K}{f(z)=0}\) relatively compact in \(U\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\),
    \begin{enumerate}
...
\end{theorem}
\begin{proof}
    By the same logic as with the proof of \autoref{thm:hurwitzsimplecase}, \(f(z)\) is holomorphic over \(U\). Since \(f\not\equiv 0\) on \(U\), the zeros in \(K\), \(\cbraces{z_j}\), are disjoint, and there exist disjoint open disks \(D\paren{z_j,r_j}\subset V\) around each zero such that \(f\) does not vanish on their positively oriented boundaries \(\cbraces{\gamma_j}\).

    Since \(f\) does not vanish on \(\partial K\), \(\varepsilon=\min_{z\in\partial K}\abs{f(z)}>0\). Since \(f_n\rightrightarrows f\), \(\exists N\in\mathbb{N}\) such that \[\abs{f(z)-f_n(z)}<\varepsilon.\] By Rouché's Theorem (\autoref{thm:rouche}), \(f\) and \(f_n\) have the same number of zeros in \(K\).
\end{proof}
