...
\begin{theorem}[Abel's Theorem]\label{thm:abelradius}
    ...
    \begin{enumerate}
        ...
        \item \(S(z)\) diverges on \(\abs{z}>R\).\label{itm:abelradius_divergence}
        \item \(S(z)\) is holomorphic over \(\abs{z}<R\) and \(S'(z)\) can be obtained by termwise differentiation \(S'(z)=\sum_{n=1}^\infty na_nz^{n-1}\) and also has the radius of convergence \(R\).\label{itm:abelradius_differentiation}
    \end{enumerate}
\end{theorem}
...
\begin{proof}
    ...
    To prove \ref{itm:abelradius_differentiation}, first observe that \(\sum_{n=1}^\infty na_nz^n\) and \(\sum_{n=1}^\infty a_nz^n\) have the same convergence radius since \(\lim\sup_{n\to\infty}\sqrt[n]{n}=1\). For \(z\in D(0, R)\), let \(S(z)=S_n(z)+R_n(z)\), where \[S_n(z)=\sum_{k=0}^{n-1}a_kz^k,\quad R_n(z)=\sum_{k=n}^\infty a_kz^k.\] 
    Let \(\lim_{n\to\infty}S_n'(z)=\sum_{n=0}^\infty na_nz^{n-1}\)
\end{proof}
