...
\begin{proof}
    ...
    Since the integrand is dominated by a function that has a convergent integral (\(\int_{\supp(\psi)}M\dd{\xi}\wedge\dd{\overline{\xi}}\)), equation \eqref{eq:onedimensionalpartialconjugatesolution_differenceexpr} uniformly converges and the limit \(z_0\to z\) may commute with the double integral, or \[\pdv{\psi\paren{\xi+z}}{z}=\frac{1}{2\pi i}\int_{\mathbb{C}}\pdv{\psi(\xi+z)}{z}\cdot\frac{1}{\xi}\dd{\xi}\wedge\dd{\overline{\xi}}.\] Since \(\pdv{\psi(\xi+z)}{z}\) is continuous over a compact region, it is bounded, and we can invoke the same argument as we did 
\end{proof}
