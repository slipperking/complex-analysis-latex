...
\begin{proof}
    ...
    Since \(f\) is continuous, \(\lim_{k\to\infty}f\qty(x_{n_k})=f\qty(\lim_{k\to\infty}x_{n_k})\), which is defined since \(\lim_{k\to\infty} x_n_k\in U\). Therefore, the sequence \(\cbraces{f\paren{x_{n_k}}}\) is bounded. But this contradicts \(\qty|f\paren{x_{n_k}}|>n_k\to\infty\), hence \(f\) must be bounded on \(U\).
\end{proof}
