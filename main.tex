...
\begin{proof}
    ...
    or in other words, the partial products are uniformly bounded on \(K\). Let \(0<\varepsilon<1\) be arbitrary. By definition, there exists \(N\in\mathbb{N}\) such that \(\forall n>m>N\), \(\abs{\sum_{k=m+1}^n f_k(z)}<\varepsilon\) for all \(z\in K\). The difference between the partial products \(F_n(z)\) satisfies \begin{align*}
        \abs{F_n(z)-F_m(z)}&\leq \abs{F_m(z)}\abs{\prod_{k=m+1}^n\qty(1+\abs{f_k(z)})-1}\\
        &\leq\abs{F_m(z)}\abs{\exp\qty(\sum_{k=m+1}^n\abs{f_k(z)})-1}\\
        &\leq e^M\qty(\ee^\varepsilon -1).
    \end{align*}
    Since \(\ee^{\varepsilon}-1\to 0^+\), it follows that \(\qty{F_n(z)}_{n\in\mathbb{N}}\) is uniformly convergent on \(K\).
\end{proof}
