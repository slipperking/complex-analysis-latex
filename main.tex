...
\begin{proof}
    ...
    The difference between the two is equal to \begin{align*}
        \int_{\overrightarrow{PM}}f(z)\ddz-\int_{\overrightarrow{P_1M_1}}f(z)\ddz&=i\int_{y_{\varphi}}^{y_{\psi}}\paren{f\paren{a+iy}-f\paren{a+\varepsilon+iy}}\ddz\\
        &\quad+i\paren{\int_{\psi(a)}^{y_\varphi}-\int_{\psi(a+\varepsilon)}^{y_\varphi}+\int_{y_\psi}^{\varphi(a)}-\int_{y_\psi}^{\varphi\paren{a+\varepsilon}}
    \end{align*}
\end{proof}
