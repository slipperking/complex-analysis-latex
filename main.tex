...
\begin{lemma}\label{lemma:locallyfiniteopencoverexistence}
    Let \(\Omega\subseteq\mathbb{C}\) be an open set and \(\mathfrak{B}\) be an (open) basis of (the standard topology under) \(\Omega\). Then there exists a sequence of sets \(\paren{U_n}_{n\in\mathbb{N}}\) in \(\mathfrak{B}\) such that \begin{enumerate}
...
\begin{proof}
    ...
    Let \(\paren{U_n}\) be a sequence enumerating \(U_{z_{r,i},r}\) where \(r\in\mathbb{N}\) and \(i=1,\ldots,k_r\). Obviously, \autoref{item:locallyfiniteopencoverexistence_coveringstatement} is true since each of \(V_r\) is covered by some subset
\end{proof}
