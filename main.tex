...
\end{definition}
It then follows that the residue at a removable singularity is 0. Let \(z=\infty\) be an isolated singularity of \(f(z)\), which is holomorphic in \(\mathbb{C}\setminus\overline{D(0,R)}\), for sufficiently large finite \(R\). Then for finite \(\rho>R\), the residue at \(z=\infty\) is \textit{defined} as \[\Res(f,\infty)=-\int_{\partial D(0,\rho)}f(z)\ddz.\]
At a neighborhood of \(\infty\), if \(f\) has the Laurent expansion \[f(z)=\sum_{n=-\infty}^\infty c_nz^n,\]
then we have \(\Res_{z=\infty}f(z)=-c_{-1}\). If \(\zeta=\frac{1}{z}\) and \(g\qty(\frac{1}{\zeta})=f\qty(z)\), from division, we obtain the expansion \[\frac{1}{\zeta^2}g(\zeta)=\sum_{n=-\infty}^\infty c_n\zeta^{-n-2}.\] In terms of residues, we have \[c_{-1}=\Res_{\zeta=0}\frac{g(\zeta)}{\zeta^2}\]
