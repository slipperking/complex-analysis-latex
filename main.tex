...
\end{equation}
Firstly, the function is holomorphic on \(\overline{D(0,1)}\) as \(|z|\le1\), \(\abs{\overline{z}}<1\), the denominator never vanishes. 
...
It follows that \(\varphi_z=\qty(\varphi_z)^{-1}\). Thus \(\varphi_z\) is surjective and consequently bijective. It follows that equation \eqref{eq:mobiustransformationgroupofholomorphicautomorphismsstatement} is true. Functions in the form of \(\varphi_z\) (where \(z\in D(0,1)\)) are known as \textit{Möbius transformations}, and the group of all such transformations is known as the \textit{Möbius transformation group} is a subgroup of \(\Aut(D(0,1))\). Functions in the form of \(\rho_\tau(\zeta)=\zeta\exp(i\tau)\), where \(\tau\in\mathbb{R}\) is constant, form a group known as the \textit{rotation group}, which is also a subgroup of \(\Aut(D(0,1))\).
\begin{theorem}[The Holomorphic Automorphism Group on the Unit Disk]
    \(\forall f\in \Aut(D(0,1))\), \(f\) is a composition between a Möbius transformation and a rotation. In other words, \(\exists z\in\)
\end{theorem}
