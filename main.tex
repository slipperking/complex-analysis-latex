...
\begin{proof}
    ...
    Let \(S_\infty=\cbraces{z\in U}{\forall n\in\mathbb{Z}_{\geq0}, f^{(n)}(z)=0}\). \(\forall z\in D\paren{z_\infty,\varepsilon}\), since \(f(z)\) locally vanishes (and has vanishing derivatives as a consequence), \(D\paren{z_\infty,\varepsilon}\subseteq S_\infty\). Furthermore, \(\forall z'\in S_\infty\), \(\exists\varepsilon'>0\) such that \(f(z)\) has a convergent Taylor Series with vanishing coefficients on \(D(z',\varepsilon')\subseteq U\). Then \(f\equiv0\) on \(D\paren{z',\varepsilon'}\). Then \(\forall z\in D(z',\varepsilon')\), since \(f\) is constant at \(z\), it also has vanishing derivatives. It follows that \(D(z',\varepsilon')\subseteq S_\infty\). Since every point in \(S_\infty\) has an open neighborhood also in \(S_\infty\), \(S_\infty\) is open.

    \(\forall k\in\mathbb{Z}_{\geq0}\), \(f^{(k)}\) is continuous in \(U\) by the holomorphy of \(f\). Let \(S_k=\cbraces{z\in U}{f^{(k)}(z)=0}\). For any sequence \(\cbraces{z_n}\in S_k\) converging to some \(z_\infty\in U\), by the continuity of \(f\), \[\lim_{n\to\infty} f^{(k)}\paren{z_n}=f\paren{\lim_{n\to\infty}z_n}=f\paren{z_\infty}=0,\] and \(z_\infty\in E\). Thus, \(E\) contains all of its accumulation points in \(U\) and is therefore closed (if \(z'_\infty\notin V\), then it is no longer relevant; we are concerned with it being closed within \(U\)). 
\end{proof}
\begin{remark}
