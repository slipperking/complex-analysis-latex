...
\begin{theorem}
    Let \(\lambda>0\) and let \(a\in\mathbb{C}\). Suppose \(f(z)\) is a holomorphic function on \(D^*(a,\lambda)\) with a simple pole at \(z=a\in U\). Then for all \(0<\varepsilon<\lambda\), the boundary \(\partial D(a,\varepsilon) \subset U\).
    
    Let \(\gamma_\varepsilon \subseteq \partial D(a, \varepsilon) \) be a counterclockwise-oriented, connected arc subtending an angle \(\theta\). Then,
    \[\lim_{\varepsilon\to 0}\int_{\gamma_\varepsilon}f(z)\ddz=i\theta\cdot\residue_{z=a}f(z).\]
\end{theorem}
\begin{proof}
