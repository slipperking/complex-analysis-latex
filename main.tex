...
\begin{proof}
	...
	Notice that \(\frac{2}{\pi}\theta\leq\abs{\sin(\theta)}\leq\theta\) over the integration range. We want to observe the behavior as \(R\to\infty\): \[\abs{i\int_0^\pi e^{iR\cos\theta}e^{-R\sin\theta}\dd{\theta}}\leq\int_0^{\pi}e^{-R\sin\theta}\dd{\theta}=2\int_0^{\frac{\pi}{2}}e^{-R\sin\theta}\dd{\theta}<2\int_0^{\frac{\pi}{2}}e^{-R\frac{2}{\pi}\theta}\dd{\theta}.\]
\end{proof}
