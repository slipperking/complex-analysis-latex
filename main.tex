...
\begin{proof}
    ...
    \]
    Informally, the Poisson kernel becomes small when \(z\) and \(\zeta\) are sufficiently far apart. Since \(\varphi\) is continuous on a compact set \(\partial D(0,R)\), it is bounded by \autoref{lemma:uniformcontinuousovercompactset}, or \(M=\sup_{\qty|\zeta|=R}\abs{\varphi(\zeta)}\) that is finite, such that \(\abs{\varphi\qty(Re^{i\vartheta})-\varphi(\zeta)}<2M\).
