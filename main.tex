...
\end{theorem}
In 3 dimensional $\mathbb{R}^3$ space, define a scalar valued function to be a 0-form, a linear combination of \(\ddx\), \(\dd{y}\), and \(\dd{z}\) to be a 1-form, and a linear combination between \(\dd{x}\wedge\dd{y}\), \(\dd{y}\wedge\dd{z}\), and \(\dd{z}\wedge\ddx\) to be a 2-form, and \(\ddx\wedge\dd{y}\wedge\dd{z}\) to be a 3-form, where $\wedge$ denotes an anti-commutative and associative product, where for any two differential forms $\omega_1$ and $\omega_2$
...
which is the definition of the total differential. For a 1-form $\omega_1=P\ddx+Q\dd{y}+R\dd{z}$, we can define the exterior derivative in a similar way: \[\dd{\omega_1}=\dd{P}\wedge\ddx+\dd{Q}\wedge\dd{y}+\dd{R}\wedge\dd{z}.\]
The following lemma is essential to simplify the above equation:
...
\begin{theorem}[Pompeiu's Theorem]\label{theorem:pompeiu}
    Let \(U\subseteq\mathbb{C}\) where $\partial U$ is piecewise smooth. Let \(f(z)\in C^1(\overline{U})\). Then $\forall z\in U$, \begin{equation}
        f(z)=\frac{1}{2\pi i}\left(\int_{\partial U}\frac{f(\zeta)}{\zeta-z}\mathop{\dd{\zeta}}-\int_{U}\pdv{f(\zeta)}{\overline{\zeta}}\frac{\dd{\overline{\zeta}}\wedge\dd{\zeta}}{\zeta-z}\right).
    \end{equation}
\end{theorem}
\begin{proof}
