...
\begin{definition}[Epsilon Delta]\label{def:epsilondelta}
    Let a function \(f:U\to\mathbb{R}\) be defined on an open set \(U\subseteq\mathbb{R}\) around \(a\) (not necessarily containing \(a\) itself). If \(\exists L\in\mathbb{R}\) such that $\forall\varepsilon>0$, \(\exists\delta>0\) such that $\forall |x-a|<\delta$ where \(x\in U\) and \(|f(x)-L|<\varepsilon\), then \(\lim_{x\to a}f(x)=L\).

    If $U\in\mathbb{R}$ is any open set such that \(\exists x\in U\) such that \(x-a>0\), then if \(\exists L\in\mathbb{R}\) such that $\forall\varepsilon>0$, \(\exists\delta>0\) such that $\forall 0<x-a<\delta$ where \(x\in U\) and \(|f(x)-L|<\varepsilon\), then \(\lim_{x\to a^+}f(x)=L\).
    
    Similarly, if $U\in\mathbb{R}$ is any open set such that \(\exists x\in U\) such that \(x-a<0\), then if \(\exists L\in\mathbb{R}\) such that $\forall\varepsilon>0$, \(\exists\delta>0\) such that $\forall{-\delta}<x-a<0$ where \(x\in U\) and \(|f(x)-L_1|<\varepsilon\), then \(\lim_{x\to a^-}f(x)=L_1\).
\end{definition}
...
\end{proof}
\begin{definition}[Uniform Continuity]\label{def:continuity}
    A function \(f:U\to\mathbb{R}\), defined on a set \(U\subseteq\mathbb{R}\), is uniformly continuous if and only if $\forall\varepsilon>0$, $\exists\delta>0$ such that $\forall t,s\in U$ where \(|t-s|<\delta\), \(\left|f(t)-f(s)\right|<\varepsilon\).
\end{definition}
\begin{lemma}
...
\end{proof}
\begin{definition}
    A function \(f\) is Lipschitz continuous over \(U\) if \(\exists C>0\) such that \(\forall x,y \in U\), \(|f(x)-f(y)| \leq C|x-y|\).
\end{definition}
...
\begin{proof}
    Let \(f:U\to\mathbb{R}\) be \(C^1\). Since \(U\) is compact and \(f'\) is continuous, \(\exists M>0\) such that \(\forall x\in U\), \(|f'(x)| \leq M\).

    By the Mean Value Theorem, \(\forall x,y\in U\), \(\exists c\) between \(x\) and \(y\) such that \(f(x)-f(y) = f'(c)(x-y)\). Taking absolute values gives \(|f(x)-f(y)| = |f'(c)||x-y| \leq M|x-y|\), proving \(f\) is Lipschitz continuous with Lipschitz constant \(M\).
\end{proof}
...
\begin{proof}
    ...
    \[\int_{U\setminus D(z,\varepsilon)}\pdv{f}{\overline{\zeta}}\cdot\frac{f(\zeta)}{\zeta-z}\dd{\overline{\zeta}}\wedge\ddzeta.\]
    The rightmost term in equation \eqref{eq:pompeiuproofdirectintermediate} can be parameterized with \(\zeta=z+\varepsilon e^{it}\), \(t\in[0,2\pi)\). Then, \[\int_{\partial D(z,\varepsilon)}\frac{f(\zeta)\ddzeta}{\zeta-z}=\int_0^{2\pi}\frac{f(z+\varepsilon e^{it})}{\varepsilon e^{it}}\cdot i\varepsilon e^{it}\dd{t}=i\int_0^{2\pi}f(z+\varepsilon e^{it})\dd{t}.\]
    By combining and rearranging, we take the limit as \(\varepsilon\to0\): \[i\lim_{\varepsilon\to0}\int_0^{2\pi}f(z+\varepsilon e^{it})\dd{t}=\int_U\frac{f(\zeta)\ddzeta}{\zeta-z}-\int_{U\setminus D(z,\varepsilon)}\pdv{f}{\overline{\zeta}}\cdot\frac{f(\zeta)}{\zeta-z}\dd{\overline{\zeta}}\wedge\ddzeta.\] Because $f$ is continuous, $\exists\delta(\varepsilon)$ such that 
\end{proof}
\end{document}