...
\begin{proof}
    ...
    \begin{align*}
            \abs{\varphi(\xi)-u(z)}&=\abs{\qty(\int_{\abs{\vartheta-\tau}<\delta}+\int_{\abs{\vartheta-\tau}\ge\delta})P(\zeta,z)\paren{\varphi\qty(Re^{i\vartheta})-\varphi(\zeta)}\dd{\tau}}\\&=\abs{I_1+I_2}.
    \end{align*}
    For \(I_1\), we can assume \(\delta<1\) (since \(\delta\) can be arbitrarily smaller than necessary), and hence, \[
    I_1<\varepsilon\int_{\abs{\vartheta-\tau}<\delta}P(\zeta,z)\dd{\tau}<\varepsilon.
    \]
\end{proof}
