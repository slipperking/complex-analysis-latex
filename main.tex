...
\end{proof}
Let \(f(z)\) be an entire function. First, assume that \(f\) has no zeros in \(\mathbb{C}\). Then the function defined by \(z\mapsto\frac{f'(z)}{f(z)}\) is entire, 
...
\begin{proof}
	...
	\end{align*}
	Let us evaluate the integral on \(\gamma_\varepsilon\) as \(\varepsilon\to 0\). Extend the semicircle to a full circle of radius \(\varepsilon\) centered at \(0\), and let the bottom half be clockwise oriented, denoted by \(\gamma_{\varepsilon}'\).
	\begin{align*}
		\ointclockwise_{\partial D(0,\varepsilon)}f(z)\ddz&=-2\pi i\residue_{z=0}f(z)=\int_{\gamma_\varepsilon}f(z)\ddz+\int_{\gamma_\varepsilon'}f(z)\ddz.
	\end{align*}
	The residue can be calculated via the Laurent expansion of \(f(z)\) at \(z=0\):
	\[\frac{e^{iz}}{z}=\frac{1}{z}\sum_{n=0}^\infty\frac{(iz)^n}{n!}=\sum_{n=0}^\infty\frac{i^nz^{n-1}}{n!},\]
	or the coefficient when \(n=0\), which is \(1\).
	Hence, 
	\begin{align*}
		\int_{\gamma_\varepsilon}f(z)\ddz=i\int_\pi^0e^{\varepsilon\qty(i\cos\theta-\sin\theta)}\dd{\theta}&=-2\pi i-i\int_{0}^{-\pi}e^{\varepsilon\qty(i\cos\theta-\sin\theta)}\dd{\theta}\\
		&=-2\pi i-i\int_\pi^0e^{-\varepsilon\qty(i\cos\theta-\sin\theta)}\dd{\theta}
	\end{align*}
\end{proof}
