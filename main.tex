...
\begin{theorem}\label{thm:holomorphiczerosstabilityundershifts}
    ...
    \begin{enumerate}
        \item The zeros of \(f(z)+\varepsilon\) in \(K\) lie in \(V\), that is, \(\cbraces{z\in K}{f(z)+\varepsilon=0}\subset V\).
        \item The number of zeros (counting multiplicity) of \(f(z)+\varepsilon\) in \(K\) is the same as that of \(f(z)\) in \(K\).
    \end{enumerate}
\end{theorem}
\begin{proof}
    ...
    By Rouché's Theorem (\autoref{thm:rouche}), \(f\) and \(f+\varepsilon\) have the same number of zeros, counting multiplicity, over \(K\).

    Similarly, \(\delta_2=\max_{z\in K\setminus V}\abs{f(z)}>0\), and \(\forall|\varepsilon|<\delta_2\), \(\forall z\in K\setminus V\),
    \[\abs{f(z)-\paren{f(z)+\varepsilon}}<\delta_2\leq\abs{f(z)}.\]
    \(K\setminus V\) can be covered by a collection of simply connected sets (with a union equal to \(K\setminus V\)). Thus, by \autoref{thm:rouche}, \(f\) and \(f+\varepsilon\) never vanish on \(K\setminus V\), and therefore all zeros of \(f+\varepsilon\) lie in \(V\).
...
\end{lemma}
\begin{proof}
    Suppose, for the sake of contradiction, that \(f\) is univalent on \(U\) such that \(\exists z_0\in U\) such that \(f'\qty(z_0)=0\). In other words, \(f(z)-f\qty(z_0)\) has a zero at \(z_0\) with multiplicity of at least 2. TO BE CONTINUED 
\end{proof}
Conversely, we have the following statement on local univalence.
\begin{lemma}
    Let \(U\subseteq\mathbb{C}\) be a region and suppose \(f:U\to\mathbb{C}\) is holomorphic. If \(f'\qty(z_0)\neq0\) for some \(z_0\in U\), then there exists an open neighborhood \(V\) of \(z_0\) such that \(f\) is univalent on \(V\).
\end{lemma}
