...
Let \(f(z)\in\Aut\qty(\riemannsphere)\) such that \(f(\infty)\neq\infty\). Then, \(g(z)=\frac{1}{f(z)-f(\infty)}\in\Aut\qty(\riemannsphere)\) and \(g(\infty)=\infty\). By the property above, \(g(z)=cz+d\) for some complex \(d\) and nonzero \(c\). Hence, \(f(z)=\frac{f(\infty)(cz+d)+1}{cz+d}\). Let \(a=cf(\infty)\), \(b=df(\infty)+1\), \(f(z)=\frac{az+b}{cz+d}\). Since \(ad-bc=-c\neq0\). Let the matrix \(\mqty(a&b\\c&d)\) correspond to this transformation, where for any nonzero scalar \(k\), \(k\mqty(a&b\\c&d)\) corresponds to \(\mqty(a&b\\c&d)\). Therefore, there exists a one-to-one correspondence between \(\Aut\qty(\riemannsphere)\) and the group \[\cbraces{\mqty(a&b\\c&d)}{\det{\mqty(a&b\\c&d)}=1}/\qty{\pm I},\]
where \(I=\mqty(\imat{2})\). The quotient group is taken because \(\mqty(a&b\\c&d)\) refers to the same transformation as \(\mqty(-a&-b\\-c&-d)\). This group, denoted by \(\mathrm{PSL}(2,\mathbb{C})=\mathrm{SL}(2,\mathbb{C})/\qty{\pm I}\), is known as the \textit{group of Möbius transformations}, consisting of all fractional linear transformations.
