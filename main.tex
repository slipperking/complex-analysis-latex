...
\begin{example}
    Find the group of holomorphic automorphisms on the upper half-plane \(\mathbb{H}^+=\cbraces{z\in\mathbb{C}}{\Im(z)>0}\).
\end{example}
\begin{proof}
...
\end{proof}
\begin{example}
    Evaluate \[\int_0^\infty\frac{\ln(x^2+1)}{x^2+1}\ddx.\]
\end{example}
\begin{proof}
    We choose a branch of the logarithm that encloses the positive real axis, or the principal branch when \(-\pi<\Arg(z)\leq \pi\). Let \(f(z)=\frac{\Log\qty(z^2+1)}{z^2+1}\). The function has singularities at \(z=\pm i\). Since \(f\) is even, we can consider a semicircular contour. Without loss of generality, choose the contour as in \autoref{fig:semicircularcontour}, made of \[\Gamma=\cbraces{r\in\mathbb{R}}{-R\leq r\leq R},\qquad C_R=\cbraces{Re^{i\theta}}{0\leq\theta\leq\pi},\] where \(R>1\). Without loss of generality, we can let \(R\ge2\). The residue at \(z=i\) is equal to \[\residue_{z=i}f(z)=\residue_{z=i}\qty(\frac{\Log(z+i)}{z^2+1})+\residue_{z=i}\qty(\frac{\Log(z-i)}{z^2+1})\]
\end{proof}
...
\begin{proof}
    By the Cauchy--Hadamard Theorem (\autoref{thm:cauchyhadamard}), the series has the convergence radius of \[\varlimsup_{n\to\infty}\sqrt[n]{
            \begin{cases}
                ...
                0&\text{otherwise}
    \end{cases}}=1.\]
    It follows that \(f\) is holomorphic on \(\mathbb{D}\). Let \(\qty|z_0|=\frac{1}{2}\) be arbitrary. It follows that \(f(z)\) has a Taylor expansion at \(z=z_0\) of \[f(z)=\sum_{n=0}^\infty\frac{f^{(n)}\qty(z_0)}{n!}{\qty(z-z_0)}^n.\] Let \(\rho\) be the convergence radius of the series above. If we can show that \(\rho=\frac{1}{2}\), then it follows that by tangency, the intersection of \(\partial\mathbb{D}\) and \(\partial D\qty(z_0,\frac{1}{2})\), or \(\zeta_0\), is a singularity of \(f\).

    For the sake of contradiction, assume that \(\rho>\frac{1}{2}\). 
\end{proof}
