...
\begin{proof}
    ...
    For the sake of contradiction, assume that \(\rho>\frac{1}{2}\). Hence, \(D\qty(\zeta,\rho-\frac{1}{2})\cap\mathbb{D}\neq\emptyset\). Obviously, the arc defined by \(S=D\qty(\zeta,\rho-\frac{1}{2})\cap\partial\mathbb{D}\) has positive length. Since \(\exp(2\pi i\mathbb{Q})\) is dense in \(\partial\mathbb{D}\), \(\exists\zeta'\in S\) such that \(\zeta'=\exp(2\pi i\frac{p}{q})\) for some \(p,q\in\mathbb{Z}\). Since \(\).
\end{proof}
