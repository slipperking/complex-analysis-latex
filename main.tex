...
\end{theorem}
\begin{proof}
    Suppose, for contradiction, that \(p(z)\) has no complex roots. Then the function \(f(z)=\frac{1}{p(z)}\) is entire, since \(p(z)\) has no roots in \(\mathbb{C}\). Moreover, as \(|z|\to\infty\), \(p(z)\to\infty\), so \(f(z)\to 0\), implying that \( f(z) \) is bounded. By Liouville's Theorem (\autoref{thm:liouville}), every bounded entire function is constant. Thus, \( f(z) \) is constant, and so \( p(z) \) must also be constant — a contradiction. Therefore, \( p(z) \) has at least one complex root.
\end{proof}
...
\end{theorem}
\begin{proof}
    Since the integral vanishes any closed curve \(\gamma\), \(\forall z_0,z\in U\), the integral is path independent with endpoints \(z_0\) and \(z\):
