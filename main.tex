...
\begin{theorem}[Pompeiu's Theorem]\label{theorem:pompeiu}
    Let \(U\subset\mathbb{C}\) where $\partial U$ is piecewise smooth. Let \(f(z)\in C^1(\overline{U})\). Then $\forall z\in U\setminus\partial U$, \begin{equation}
...
\begin{proof}
    ...
    with a singularity at \(\zeta=z\). Since \(z\in U\), \(\exists\) \(U\setminus D(z,\varepsilon)\). Applying the Stokes--Cartan Theorem (\autoref{theorem:stokescartan}), \begin{equation}
