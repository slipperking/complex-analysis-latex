...
\begin{theorem}[Morera's Theorem]\label{thm:morera}
    Let \(U\subseteq\mathbb{C}\) and \(f:U\to\mathbb{C}\) be continuous over \(U\). If for any rectifiable closed curve \(\gamma\), \[\oint_{\gamma}f(\zeta)\ddzeta\] vanishes, then \(f\) is holomorphic over \(U\).
\end{theorem}
\begin{proof}
    ...
    As \(f(z)\) is continuous, \(F(z)\) is continuous as well. \(F(z)\) is holomorphic (\(F'(z)=f(z)\)), and by \(\) 
\end{proof}
\end{document}