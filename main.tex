...
\end{theorem}
\begin{proof}
    Choose an arbitrary \(z_0\in U\) and the corresponding value of \(\varepsilon>0\). Since \(f\) is continuous on \(\partial D\qty(z_0,\varepsilon)\), there exists a harmonic function \(u(z)=\int_0^{2\pi}f(\zeta)P(\zeta,z)\dd{\tau}\) that is identically equal to \(f\) on \(\partial D\qty(z_0,\varepsilon)\). Consider the function \(\psi=f-u\) on \(\overline{D\qty(z_0,\varepsilon)}\), which vanishes on \(\partial D\qty(z_0,\varepsilon)\).

    The Maximum Principle can be generalized to \(\psi\) since it satisfies the mean-value property (refer to the proof of the Maximum Modulus Principle (\autoref{thm:maximummodulus}), which uses the mean-value property of holomorphic functions). It follows that the maximum of \(\psi\) is assumed to be on \(\partial D\qty(z_0,\varepsilon)\), and \(\psi\equiv 0\) on \(\overline{D\qty(z_0,\varepsilon)}\). Then \(f\) is harmonic on the closed disk (and on \(z_0\)). Since \(z_0\in U\) was chosen arbitrarily, the harmonicity is extendable to all of \(U\).
\end{proof}
