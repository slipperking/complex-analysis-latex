...
Consider a holomorphic function \(f:U\to\mathbb{C}\) where \(U\subseteq\mathbb{C}\) is open and connected and a point \(z_0\in U\) such that \(f'\paren{z_0}\neq 0\). Let \(\gamma(t)\in C^1([0,1])\) be a curve with \(\gamma(0)=z_0\). Then at \(z_0\), the angle of \(\gamma\) is equal to \(\Arg\paren{\gamma'\paren{z_0}}\). The image of \(\gamma\) under \(f\) is also a smooth curve, \(\sigma(t)=f(\gamma(t))\), which passes through \(w_0=f\paren{z_0}\). It follows from the chain rule that the angle of \(\sigma(0)\)
