...
\begin{proof}
    ...
    By the continuity of \(f\), \(\exists\varepsilon>0\) such that \(\forall z\in\gamma\), \(\abs{f(z)}>\varepsilon\). By the uniform convergence of \(\cbraces{f_n(z)}\), \(\exists N\in\mathbb{N}\) such that
    \[\abs{f_n(z)-f(z)}<\frac{\varepsilon}{2},\quad\forall n>N,\forall z\in\gamma.\]
    Then \(\abs{f_n(z)}>\frac{\varepsilon}{2}\) on \(\gamma\). It follows that \(\frac{1}{f_n(z)}\) and its limit are uniformly bounded: \[\abs{\frac{1}{f(z)}}<\frac{1}{\varepsilon},\quad\abs{\frac{1}{f_n(z)}}<\frac{2}{\varepsilon},\quad\forall z\in\gamma,\forall n>N.\]
    Then,
    \begin{align*}
