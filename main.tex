...
\begin{definition}[Epsilon Delta]\label{def:epsilondelta}
    Let a function \(f:U\to\mathbb{R}\) be defined on an open interval \(U\subseteq\mathbb{R}\) around \(a\) (not necessarily containing \(a\) itself). If \(\exists L\in\mathbb{R}\) such that $\forall\varepsilon>0$, \(\exists\delta>0\) such that $\forall |x-a|<\delta$ where \(x\in U\) and \(|f(x)-L|<\varepsilon\), then \(\lim_{x\to a}f(x)=L\).
...
\begin{definition}[Continuity]\label{def:continuity}
    A function \(f:U \to \mathbb{R}\), defined on an open interval \(U \subseteq \mathbb{R}\) containing a point \(a \in U\), is said to be continuous at \(a\) if
    \[\lim_{x \to a} f(x) = f(a).\]
    Equivalently, for every \(\varepsilon > 0\), there exists \(\delta > 0\) such that for all \(x \in U\), \[|x - a| < \delta \implies |f(x) - f(a)| < \varepsilon.\]
\end{definition}
