...
\begin{proof}
    ...
    For a constant harmonic function identically equal to 1, we get \(1=\int_0^{2\pi}P(\zeta,z)\dd{\tau}\) from equation~\eqref{eq:poissonintegralformula2}. Hence, \[\abs{\varphi(\xi)-u(z)}=\abs{\int_0^{2\pi}P(\zeta,z)\qty(\varphi\qty(Re^{i\vartheta})-\varphi(\zeta))}.\]
    ...
    \end{align*}
    Since the Poisson kernel is nonnegative, \[\qty|I_1|<\varepsilon\int_{\abs{\vartheta-\tau}<\delta}P(\zeta,z)\dd{\tau}\leq\varepsilon.\]
    \begin{figure}
        \centering
        \begin{tikzpicture}
    ...
    where \(\zeta=Re^{i\tau}\) and \(z=\rho e^{i\theta}\), with \(\abs{\vartheta-\tau}>\delta\). Then \(\exists\eta>0\) such that when \(|\xi-z|<\eta\), \(\abs{\theta-\tau}>\delta\) and \(\rho>\frac{R}{2}\) (this can be arbitrarily chosen for different resulting bounds) as in \autoref{fig:dirichletproblemwithlaplaceequationsolution_secondintegral}. Then by the Law of Cosines, \(|\zeta-z|^2>4\rho^2\sin[2](\frac{\delta}{2})>\frac{1}{2}R^2\qty(1-\cos(\delta))\). 

    Since \(\abs{\varphi\qty(Re^{i\vartheta})-\varphi(\zeta)}<2M\), we can set \(\int_{\abs{\vartheta-\tau}>\delta}\frac{R^2-\rho^2}{\pi R^2\qty(1-\cos(\delta))}\dd{\tau}\leq2\frac{R^2-\rho^2}{R^2\qty(1-\cos(\delta))}< \frac{\varepsilon}{2M}\), and \[R^2-\rho^2<\frac{\varepsilon}{4M}R^2\paren{1-\cos(\delta)}.\]
    Under these existent conditions, we have \(\abs{\varphi(\xi)-u(z)}<2\varepsilon\). Then equation~\eqref{eq:dirichletproblemwithlaplaceequationsolution_limittoboundary} follows.

    We will now show that \(u(z)\) is unique. Assume that \(v\not\equiv u\) on \(\overline{D(0,R)}\) also solves the problem. Then \(u-v\) is harmonic and vanishes on \(\partial D(0,R)\). By the Poisson Integral Formula (\eqref{eq:poissonintegralformula2}), \(\forall z\in D(0,R)\), \(u(z)-v(z)=\int_0^{2\pi}P(\zeta,z)\paren{u(\zeta)-v(\zeta)}\dd{\tau}=0\).
\end{proof}
