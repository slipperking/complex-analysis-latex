...
\begin{theorem}[Looman–Menchoff Theorem]
    If \(f(z)\) is continuous over \(D\) and differentiable over \(D\setminus N\) for a set \(N\) with measure zero, then \(f(z)\) is holomorphic over \(D\) \textit{if and only if} it satisfy the Cauchy--Riemann equations.
\end{theorem}
Clearly 
