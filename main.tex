...
\end{proof}
\begin{remark}
    Let \(\varphi_n(z)=f_{n+1}(z)-f_n(z)\) be a holomorphic function sequence on \(U\). Then \(\forall\varepsilon>0\), \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \(\forall z\in K\), \(\abs{\varphi_n(z)}<\varepsilon\), where \(K\) is an arbitrary compact subset of \(U\) and \(N\) is independent of \(z\). It follows that \(f_n(z)=f_1(z)+\sum_{k=1}^{n-1}\varphi_k(z)\). By the Maximum Modulus Principle (\autoref{thm:maximummodulus}), 
\end{remark}
