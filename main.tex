...
\begin{proof}
    ...
    By the continuity of \(\varphi\), it follows that \(\forall\varepsilon>0\), \(\exists\delta>0\) such that \(\forall\qty|\vartheta-\tau|<\delta\), \(\abs{\varphi(Re^{i\vartheta})-\varphi(\zeta)}<\varepsilon\). Therefore,
    \begin{align*}
    ...
    \end{align*}
    Since the Poisson kernel is nonnegative, \[
    \qty|I_1|<\varepsilon\int_{\abs{\vartheta-\tau}<\delta}P(\zeta,z)\dd{\tau}\leq\varepsilon.
    \]
    Since \(\varphi\) is continuous on a compact set \(\partial D(0,R)\), it is bounded by \autoref{lemma:uniformcontinuousovercompactset}, or \(M=\sup_{\qty|\zeta|=R}\abs{\varphi(\zeta)}\) that is finite.
    ...
    \begin{figure}
        ...
        \end{tikzpicture}
        \caption{\(\zeta\), \(\xi\), and \(z\) when \(\abs{\vartheta-\tau}>\delta\), with angles and distances marked. \(+\) and \(-\) denote a value less than the proceeding value.}
        \label{fig:dirichletproblemwithlaplaceequationsolution_secondintegral}
    \end{figure}

    Since \[P(\zeta,z)=\frac{R^2-\rho^2}{2\pi\qty|\zeta-z|^2},\]
    where \(\zeta=Re^{i\tau}\) and \(z=\rho e^{i\theta}\), assume that \(\abs{\vartheta-\tau}>\delta\). Then \(\exists\eta>0\) such that when \(|\xi-z|<\eta\), \(\abs{\theta-\tau}>\delta\) and \(\rho>\frac{R}{2}\) (this can be arbitrarily chosen for different resulting bounds). Then by the Law of Cosines, \(|\zeta-z|^2>4\rho^2\sin[2](\frac{\delta}{2})>\frac{1}{2}R^2\qty(1-\cos(\delta))\). 
    ...
    We will now show that \(u(z)\) is unique.
\end{proof}
