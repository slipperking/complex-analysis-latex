...
\end{theorem}
Generalizations of the differential and integral exists in multiple variables. The partial differentials of \(f(x,y,z)\), $\pdv{f}{x}\ddx$, $\pdv{f}{y}\ddy$, and \(\pdv{f}{z}\ddz\) sum up to form the total differential, denoted by \(\dd{f}\). Three classical theorems exist, relating line and surface integrals in 2 and 3 dimensions, and the surface and volume integrals in 3 dimensions:
\begin{theorem}[Green's Theorem]\label{theorem:realgreen}
    Let \(\partial R\) be a positively oriented, piecewise smooth, simple closed curve in the plane, and let \(R\) be the region bounded by \(\partial R\). If \(P(x,y)\) and \(Q(x,y)\) are functions defined on an open region containing \(R\) and have continuous partial derivatives there, then:
    \[\oint_{\partial R} P\ddx+Q\ddy=\iint_R\left(\pdv{Q}{x}-\pdv{P}{y}\right)\ddx\ddy.\]
\end{theorem}
\begin{theorem}[Stokes' Theorem]\label{theorem:stokes}
...
Then, Green's, Stokes', and Gauss' Theorems can be generalized into:
