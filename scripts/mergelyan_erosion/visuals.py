...
def write_input_disks(folder: str, disk_points: List[np.ndarray], disk_r: float):
    ...
    for i, center in enumerate(disk_points, 1):
        ...
        path_str = coords_to_tikz_path(
            coords) + f" ({circle_x[0]:.6f},{circle_y[0]:.6f})"
...
def write_subcover_centers(folder: str, sub_centers: List[np.ndarray]):
    ...
    if sub_centers:
        ...
        print(
            f"  Written {len(sub_centers)} subcover centers to {centers_path}")
    else:
        with open(centers_path, "w") as f:
...
def write_wkt(folder: str, name: str, geom):
    ...
    with open(path, "w") as f:
        f.write(content)
    print(
        f"Written WKT for {name} to {path} (empty: {geom is None or geom.is_empty})")
        ...
        for f in sorted(files):
            if f.lower().endswith(".tex") and f.lower() != "imports.tex":
                rel = os.path.relpath(os.path.join(
                    dirpath, f), root).replace(os.sep, "/")
                tex_files.append(rel)
    lines = ["% AUTO-GENERATED", "% DO NOT EDIT", ""]
    for rel in tex_files:
        d, f = os.path.split(rel)
        lines.append(
            f"\\subimport{{{d}/}}{{{f}}}" if d else f"\\subimport{{}}{{{f}}}")
...
def prune_redundant_disks(centers: List[np.ndarray], r_sub: float, covered_region, nocenters_region) -> List[np.ndarray]:
    ...
    print(
        f"Pruned subcover disks: {len(centers)} → {len(pruned)} (strict: zero-contribution only)")
    return pruned
...
def main():
    ...
    CoveredRegion = Support.difference(
        Eroded) if Support and Eroded else Support
    ...
    write_input_disks(str(OUTPUT_ROOT / "input_disks"),
    ...
    H_prime = CoveredRegion.buffer(
        l, resolution=80) if CoveredRegion else Polygon()
    H_doubleprime = H_prime.boundary
    N = NoCentersRegion.buffer(
        l, resolution=80) if NoCentersRegion else Polygon()
    N_prime = N.boundary
    ...
    for _ in range(10):
        sub_disks = [Point(p).buffer(r_sub, resolution=50)
                     for p in sub_centers]
    ...
    for center in sub_centers:
        ...
        if contrib.is_empty:
            covered_disjoint_union_region.append(
                Polygon())  # empty for consistency
            continue
