...
\begin{proof}
    ...
    disk \(D(\zeta,\delta)\). Since \(\exp(2\uppi\ii\mathbb{Q})\) is dense in
    \(\partial\mathbb{D}\), there exists \(\zeta'=\exp(2\uppi\ii\frac{p}{q})\in
...
\begin{example}\label{ex:complexlogarithmanalyticcontinuation}
    Let \(\Log(z)\) denote the principal branch of \(\log(z)\), with \(-\uppi<\Arg(z)\leq\uppi\). The analytic function elements \[\qty(\Log,D(1,1))\qand\qty(\Log+2\uppi\ii,D(1,1))\] are analytic continuations of each other.
\end{example}
\begin{example}
