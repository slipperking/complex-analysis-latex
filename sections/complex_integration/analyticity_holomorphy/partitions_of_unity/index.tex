...
\begin{example}
    Let \(I=[0,2\uppi)\) be a topological space with a metric \(\abs{\cdot}\) under the standard topology (generated by the basis formed with open ``balls'' or more specifically, symmetric intervals around each point, intersected with \([0,2\uppi)\)). Equip the unit circle \(S^1=\cbraces{z\in\mathbb{C}}{\abs{z}=1}\) generated by the metric defined by arc-length (\(d_{S^1}\). Then the continuous bijection \(f:I\to S^1\) defined by \(f(t)=\ee^{\ii t}\) is not a homeomorphism.
\end{example}
\begin{proof}
...
\end{proof}
In a topological space \(X\), a subset can be open, closed (the complement of some open set), both (clopen), or neither. The only clopen sets that exist in any topological space \(X\) are \(\varnothing\) and \(X\) if \(X\) is connected. A technique pertinent to many future proofs relies on the following fact:
