...
\begin{theorem}[Holomorphic Argument Principle]\label{thm:argumentprincipleholomorphic}
    ...
    \[k=\frac{1}{2\uppi\ii}\oint_{\gamma}\frac{f'(z)}{f(z)}\ddz.\]
    Let \(\Gamma\) be the image of \(\gamma\) under the map \(w=f(z)\). Then \(k=\frac{1}{2\uppi}\Delta_\Gamma\arg(w)\), where \(\Delta_\Gamma\arg(w)\) denotes the total change in argument of \(w\) as it traverses \(\Gamma\).
\end{theorem}
\begin{proof}
    ...
    \[\oint_{\partial D\paren{z_j,\varepsilon_j}}\frac{k_j}{z-z_j}\ddz=2\uppi\ii k_j.\]
    Combining results,
    \[\oint_{\gamma}\frac{f'(z)}{f(z)}\ddz=\sum_{j=1}^n2\uppi\ii k_j=2\uppi\ii k.\]
    Finally, parameterize \(\Gamma\) by \(w=f(z)\). Then \(\dd{w}=f'(z)\ddz\), and
    \[k=\frac{1}{2\uppi\ii}\oint_{\Gamma}\frac{\dd{w}}{w}=\frac{1}{2\uppi\ii}\Delta_\Gamma\log(w)=\frac{1}{2\uppi}\Delta_\Gamma\arg(w),\] which proves the result.
\end{proof}
Thus, one defines the \textit{winding index} to quantify how many times a closed curve winds counterclockwise around a given point in the complex plane. Formally, if \(\gamma=\gamma([0,1])\) is a counterclockwise-oriented closed curve and \(z\) is a point satisfying \(z\notin\gamma\), then \[\mathrm{Ind}_{\gamma}(z)=\frac{1}{2\uppi\ii}\int_\gamma\frac{\ddzeta}{\zeta-z}=\frac{1}{2\uppi\ii}\int_0^1\frac{\gamma'(t)\ddt}{\gamma(t)-z}.\]
