...
\begin{gather}
    u(z)=\frac{1}{2\muppi}\int_0^{2\muppi} u(\zeta)\frac{1-\abs{z}^2}{\abs{\zeta-z}^2}\dd{\tau}=\frac{1}{2\muppi}\int_0^{2\muppi}\frac{\qty(1-\rho^2)u\qty(\ee^{\ii\tau})\dd{\tau}}{\qty(\ee^{\ii\tau}-\rho\ee^{\ii\theta})\qty(\ee^{-\ii\tau}-\rho\ee^{-\ii\theta})}\nonumber \\
    =\frac{1}{2\muppi}\int_0^{2\muppi}\frac{\qty(1-\rho^2)u\qty(\ee^{\ii\tau})\dd{\tau}}{1+\rho^2-2\rho\cos(\theta-\tau)}.\label{eq:poissonintegralformulatrigonometricsubstitution}
\end{gather}
Since \(u\qty(z)\) is continuous on \(\partial\mathbb{D}\) and \(u\circ\exp(\ii\theta)\) is periodic with period \(2\muppi\), it admits a Fourier series representation
\begin{equation}
    u\qty(\ee^{\ii\theta})\sim\sum_{n=-\infty}^\infty a_n\ee^{\ii n\theta},\qquad a_n=\frac{1}{2\muppi}\int_0^{2\muppi} u\qty(\ee^{\ii\tau})\ee^{-\ii n\tau}\dd{\tau}. \label{eq:poissonintegralformulafourierseries}
\end{equation}
...
\begin{align*}
    \sum_{n=-\infty}^\infty a_n\ee^{\ii n\theta}\rho^{\abs{n}} & =\sum_{n=-\infty}^\infty\qty(\frac{1}{2\muppi}\int_0^{2\muppi}u\qty(\ee^{\ii\tau})\ee^{-\ii n\tau}\dd{\tau})\ee^{\ii n\theta}\rho^{\abs{n}} \\
                                                               & =\frac{1}{2\muppi}\sum_{n=-\infty}^\infty\int_0^{2\muppi}\rho^{\abs{n}} u\qty(\ee^{\ii\tau})\ee^{\ii n(\theta-\tau)}\dd{\tau}.
\end{align*}
By \cref{thm:weierstrassmtest,thm:limitintegralswitch},
\begin{equation}
    \frac{1}{2\muppi}\sum_{n=-\infty}^\infty \int_0^{2\muppi} \rho^{\abs{n}}u\qty(\ee^{\ii\tau}) \ee^{\ii n(\theta-\tau)} \dd{\tau}=\frac{1}{2\muppi} \int_0^{2\muppi} u\qty(\ee^{\ii\tau})\sum_{n=-\infty}^\infty \rho^{\abs{n}} \ee^{\ii n(\theta-\tau)}\dd{\tau}.\label{eq:poissonintegralformulafourierseriespostintegralsummationswitch}
\end{equation}
...
\[\sum_{n=-\infty}^\infty a_n \ee^{\ii n\theta}\rho^{\abs{n}}=\frac{1}{2\muppi}\int_0^{2\muppi}\frac{\qty(1-\rho^2)u\qty(\ee^{\ii\tau})}{1+\rho^2-2\rho\cos(\theta-\tau)}\dd{\tau}=u\qty(\rho \ee^{\ii\theta}).\]
...
\begin{theorem}\label{thm:meanvaluepropertysolutionsareharmonic}
    ...
    \[f\qty(z_0)=\frac{1}{2\muppi}\int_0^{2\muppi} f\qty(z_0+\varepsilon \ee^{\ii t})\dd{t}.\]
...
\begin{proof}
    ...
    \[u(z)=\int_0^{2\muppi}f(\zeta)P\qty(\zeta,z) \dd{\tau},\]
