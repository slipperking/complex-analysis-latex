...
\begin{theorem}[\textsc{Poisson--Jensen Formula}]\label{thm:poissonjensenformula}
    ...
    \begin{align}
        \log\abs{f(z)} & =\int_0^{2\uppi}\log\abs{f(\zeta)}P\qty(\zeta,z)\dd{\theta}+\sum_{j=1}^m\log\abs{\frac{r\qty(z-a_j)}{r^2-\overline{a_j}z}}-\sum_{k=1}^n\log\abs{\frac{r\qty(z-b_k)}{r^2-\overline{b_k}z}},\label{eq:poissonjensenformula_statement}
    \end{align}
...
\begin{proof}
    ...
    \begin{equation}
        \pdv{\qty(\Re{g(z)})}{z}=\frac12\qty(\pdv{u(z)}{x}-\ii\pdv{u(z)}{y})=\frac12\qty(\pdv{u(z)}{x}+\ii\pdv{v}{x})=\frac12\pdv{g(z)}{x}=\frac{g'(z)}{2}.\label{eq:wirtingerderivativeofrealpartofholomorphicfunction}
    \end{equation}
