...
\begin{proof}
    ...
    \[M(r,f)\leq\exp\qty(r^{\lambda(f)+\varepsilon})\] for some \(r'\) and any \(r>r'\). Taking logarithms twice we have \[\frac{\log\qty(\log M(r,f))}{\log{r}}\leq\limsup_{r\to\infty}\frac{\log\qty(\log M(r,f))}{\log{r}}\leq\lambda(f)+\varepsilon\to\lambda(f)\]
...
\begin{theorem}\label{thm:entirefunctionfiniteorderzerossummationconvergence}
    For an entire function \(f\) (\(f(0)=1\)) of finite order \(\lambda(f)\) whose zeros are at \(\qty{a_k}_{k\in\mathbb{N}}\) counting multiplicities (such that \(\abs{a_1}\leq\abs{a_2}\), etc.), the sum \[\sum_{k=1}^\infty\frac1{\abs{a_k}^{\lambda(f)+\eta}}\] converges for any \(\eta>0\).
\end{theorem}
\begin{proof}
...
converges. Then by the Weierstrass Factorization Theorem (\cref{thm:weierstrassfactorization}), \[f(z)=z^m\ee^{\varphi(z)}\prod_{k=1}^\infty E_{\floor{\lambda}}\qty(\frac z{a_k})\]
locally uniformly converges on \(\mathbb{C}\), where \(\varphi\) is entire. This particular Weierstrass factorization is the \textit{Weierstrass canonical factorization} of \(f\) (the portion corresponding to the product of elementary factors itself is the \textit{Weierstrass canonical product}). Now that we have indulged in the implications of \(\lambda(f)\) to its zero distribution, we now turn to the function \(\varphi\) in the exponential.
...
\begin{theorem}\label{thm:poissonjensenlogdiffintegralterm}
    Let \(f\) be entire with \(f(0)=1\). Then for \(p>\lambda(f)-1\) (\(p\) integer) and \(z\in\mathbb{C}\),
...
\begin{proof}
    ...
    by the Residue Theorem (\cref{thm:residuethm}). Since \(p+2>\lambda(f)+1\geq 1\) where \(p\) is an integer, we must have \(p+2\geq 2\) and thus
    ...
    Now by assumption, we have \[\log M(r,f)\leq r^{\lambda+\varepsilon}\] for any \(\varepsilon>0\) and sufficiently large \(r\). Hence, \[\abs{\int_0^{2\uppi}\frac{r\ee^{\ii\theta}\log\abs{f\qty(r\ee^{\ii\theta})}}{\qty(r\ee^{\ii\theta}-z)^{p+2}}\dd{\theta}}\leq 2^{p+4}r^{\lambda(f)+\varepsilon-p-1}\uppi=2^{p+4}r^{\frac{\lambda(f)-p-1}{2}}\uppi\] at \(\varepsilon=\frac{p+1-\lambda(f)}{2}\). Then since \(\frac{\lambda(f)-p-1}{2}<0\), the expression vanishes as \(R\to\infty\).
\end{proof}
\begin{proposition}\label{prop:entirefunctionfiniteorderlogdiffderivatives}
    Let \(f\) be entire, non-constant, and of finite order such that \(f(0)=1\). Let \(\qty{a_k}_{k\in\mathbb{N}}\) be the zeros of \(f\) counted according to multiplicities such that \(\abs{a_1}\leq\abs{a_2}\leq\abs{a_3}\leq\cdots\). If \(p>\lambda(f)-1\) is an integer, then \[\dv[p]{}{z} \qty(\frac{f'(z)}{f(z)})\equiv-\sum_{k=1}^\infty\frac{p!}{\qty(a_k-z)^{p+1}}\] for all \(z\in\mathbb{C}\).
\end{proposition}
\begin{proof}
...
\begin{lemma}[Logarithmic Factorization]\label{lem:entirefunctionweierstrassproductfiniteorderlogdiffderivatives}
    Let \(f\) be entire, non-constant, and of finite order \(\lambda\) such that \(f(0)=1\). Let \[P(z)=\prod_{k=1}^\infty E_{\floor{\lambda}}\qty(\frac z{a_k})\] be the associated canonical product. If \(p>\lambda(f)-1\) is an integer, then \[\dv[p]{}{z} \qty(\frac{P'(z)}{P(z)})\equiv-\sum_{k=1}^\infty\frac{p!}{\qty(a_k-z)^{p+1}}\] for all \(z\in\mathbb{C}\).
\end{lemma}
\begin{proof}
    Let \(P_n\) be the \(n\)-th partial product of \(P\). Then \[\frac{P'_n(z)}{P_n(z)}=\sum_{k=1}^n\frac{\dv{}{z}E_{\floor{\lambda}}\qty(\frac{z}{a_k})}{E_{\floor{\lambda}}\qty(\frac{z}{a_k})}=\sum_{k=1}^{\floor{\lambda}}\qty[\frac{1}{z-a_k}+\sum_{j=1}^{\floor{\lambda}}\frac{z^{j-1}}{a_k^j}],\] implying that \[\dv[p]{}{z} \qty(\frac{P_n'(z)}{P_n(z)})=-\sum_{k=1}^n\frac{p!}{\qty(a_k-z)^{p+1}}+\dv[p]{}{z}\sum_{j=1}^{\floor{\lambda}}\sum_{k=1}^n\frac{z^{j-1}}{a_k^j}.\]
    Since the polynomial in the rightmost term has degree \(\max j-1\leq\floor{\lambda}\), since \(p>\floor{\lambda}-1\), after \(p\) derivatives each term of the expression vanishes. For an arbitrarily chosen compact \(K\subset\mathbb{C}\) avoiding \(a_k\), some \(N\in\mathbb{N}\) such that \(\abs{a_k}\geq\max_{z\in K}\abs{z}\) for all \(k>N\), we have \(\forall z\in K\), \(\abs{a_k-z}\leq\abs{a_k}+\abs{z}\leq2\abs{a_k}\). Then the convergence of \[\sum_{k=1}^\infty\frac1{2\abs{a_k}^{p+1}}\] from \cref{thm:entirefunctionfiniteorderzerossummationconvergence} implies the absolute convergence of \[\sum_{k=1}^\infty\frac1{\qty(a_k-z)^{p+1}}\] in \(K\). Moreover, it can be shown that the uniform convergence of \(P_n\to P\) (from the Weierstrass factorization) and \(P'_n\to P'_n\) (by the Weierstrass Convergence Theorem, \cref{thm:weierstrassconvergence}) in \(K\) implies that of \(\flatfrac{P'(z)}{P(z)}\). Hence, the Weierstrass Convergence Theeorem implies that
...
\begin{theorem}\label{thm:entirefunctionfiniteordercanonicalweierstrassfactorizationpolynomialdegree}
    Let \(f(z)=\ee^{\varphi(z)}P(z)\) be the Weierstrass canonical factorization of \(f\), where \(f\) is entire with finite order \(\lambda=\lambda(f)\) and \(\lambda(0)=1\). Then \(\varphi\) is a polynomial of degree \(\leq\lambda\).
\end{theorem}
\begin{proof}
    By logarithmic differentiation and by taking \(p>\lambda-1\) subsequent derivatives, we have
    ...
    Hence, \(\varphi\) is a polynomial of degree \(\leq p\). Choosing \(p=1+\floor{\lambda-1}>\lambda-1\) so that \(p\leq\lambda\), the assertion follows.
\end{proof}
\begin{corollary}\label{cor:hadamardfactorizationpolynomial}
    ...
    \(\lambda=\lambda(f)\). Then \(\varphi\) is a
    polynomial of degree \(\leq\lambda\).
\end{corollary}
\begin{proof}
    Let \(f(z)=z^m g(z)\), where \(f\) and \(g\) are entire, \(g(0)\neq 0\), and \(f\) has finite order \(\lambda(f)\) and \(M(r,f)=r^mM(r,g)\) for \(r>0\). For \(\varepsilon>0\), \(\exists r'>0\) such that \(r>r'\) implies \[M(r,f)=r^m M(r,g)\leq\ee^{r^{\lambda(f)+\varepsilon}}\implies M(r,g)\leq\ee^{r^{\lambda(f)+\varepsilon}}.\]
    Thus, \(\lambda(g)\leq\lambda(f)\) by letting \(\varepsilon\to 0^+\). Additionally, for any \(\varepsilon>0\), \(\forall r'>0\), \(\exists r>r'\) such that \[M(r,f)\geq\ee^{r^{\lambda(f)-\varepsilon}}\implies M(r,g)\geq\exp\paren{r^{\lambda(f)-2\varepsilon}\qty(r^\varepsilon-\frac{m\log{r}}{r^{\lambda(f)-2\varepsilon}})}\geq\ee^{r^{\lambda(f)-2\varepsilon}}\]
    because for sufficiently large \(r\),
    \[r^\varepsilon-\frac{m\log{r}}{r^{\lambda(f)-2\varepsilon}}>1.\]
    Hence, \(\lambda(g)\geq\lambda(f)-2\varepsilon\). Letting \(\varepsilon\to 0^+\) implies \(\lambda(f)=\lambda(g)\). Let \(g(z)=ch(z)\) where \(c\) is a constant, so that \(h(0)=1\). It is also trivial that \(\lambda(g)=\lambda(h)\). Explicitly, we have \(h(z)=\ee^{\varphi-\Log c}P(z)\).

    By \cref{thm:entirefunctionfiniteordercanonicalweierstrassfactorizationpolynomialdegree} on \(h\), \(\varphi-\Log c\) is a polynomial of degree \(\leq\lambda\), and so is \(\varphi\).
\end{proof}
\begin{definition}
    The \textit{rank} of an entire function is the smallest \(p\in\mathbb{Z}_{\geq0}\) for which the associated sum \[\sum_{k=1}^\infty\frac1{\abs{a_k}^{p+1}}\] converges.
\end{definition}
The conclusion of \cref{thm:entirefunctionfiniteorderzerossummationconvergence}, the rank of an entire function with finite order exists. Moreover, the rank \(\leq\floor{\lambda}\).

The \textit{genus} of an entire function \(f\) of finite order is then defined as \(\max\qty{p,q}\), where \(p=\operatorname{rank}(f)\) and \(q\) is the degree of \(\varphi\) in the canonical factorization \(z^m\ee^{\varphi(z)}P(z)\). Then the results of \cref{cor:hadamardfactorizationpolynomial,thm:entirefunctionfiniteorderzerossummationconvergence} may be consolidated into a single statement:
\begin{theorem}[Hadamard Factorization Theorem]\label{thm:hadamardfactorization}
    Let \(\mu\) be the genus of \(f\) and let \(\lambda\) be the order of \(f\), where \(f\) is entire with finite order. Then \(\mu\leq\lambda\).
\end{theorem}
...
\begin{proof}
    ...
    Since \(\lim_{\zeta\to 0}\frac{\sin\zeta}\zeta=1\), we have \[\lim_{z\to0}\ee^b\prod_{k=1}^\infty\qty(1-\frac{z^2}{\uppi^2 k^2})\equiv 1\implies b=0.\qedhere\]
\end{proof}
