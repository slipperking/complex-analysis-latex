...
\end{corollary}
\begin{proof}
    Let \(f(z)=z^m g(z)\), where \(f\) and \(g\) are entire, \(g(0)\neq 0\), and \(f\) has finite order \(\lambda(f)\). It follows that \[M(r,f)=r^m M(r,g)\] for all \(r>0\). Assume \(\lambda(g)>\lambda(f)\). Then the inequality \(M(r,g)\leq\exp(r^{\lambda(g)-\varepsilon})\) fails for small \(\varepsilon>0\), but from \[M(r,g)=\frac{M(r,f)}{r^m}\leq\frac{\exp(r^{\lambda(f)+\varepsilon})}{r^m}=\exp(r^{\lambda(f)+\varepsilon}-m\log r).\] For large \(r\), \(r^{\lambda(f)+\varepsilon}\) dominates \(m\log r\), so \[M(r,g)\leq\exp(r^{\lambda(f)+2\varepsilon}),\] implying \(\lambda(g)\leq\lambda(f)+2\varepsilon\). As \(\varepsilon\to 0^+\), \(\lambda(g)\leq\lambda(f)\), a contradiction. Assume \(\lambda(g)<\lambda(f)\). Then \(M(r,g)\leq\exp(r^{\lambda(g)+\varepsilon})\), so \[M(r,f)\leq r^m\exp(r^{\lambda(g)+\varepsilon})=\exp\qty(m\log r+r^{\lambda(g)+\varepsilon}).\] For large \(r\), this is \(\leq\exp(r^{\lambda(g)+2\varepsilon})\), implying \(\lambda(f)\leq\lambda(g)+2\varepsilon\). As \(\varepsilon\to 0^+\), \(\lambda(f)\leq\lambda(g)\), a contradiction. Thus, \(g\) has finite order and \(\lambda=\lambda(g)=\lambda(f)\).

    Since \(z\mapsto\flatfrac{g(z)}{g(0)}\) is also of order \(\lambda\) and satisfies the hypothesis of \cref{thm:entirefunctionfiniteordercanonicalweierstrassfactorizationpolynomialdegree}, \(\varphi\) is a polynomial of degree \(\leq\lambda\).
\end{proof}
