...
\begin{proof}
    ...
    Recognizing the forms of \cref{eq:jensensformula_integratedlogsingularity,eq:jensensformula_formulanonvanishingboundary}, we thus have \[\log\abs{f(0)}=\frac1{2\uppi}\int_0^{2\uppi}\qty(\log\abs{g\qty(R\ee^{\ii\vartheta_j})}+\sum_{j=1}^m\log\qty[\abs{R\ee^{\ii\theta}-b_j}^{k_j}])\dd{\theta}+\sum_{j=1}^n\abs{\frac{a_j}R}=\frac1{2\uppi}\int_0^{2\uppi}\log\abs{f\qty(R\ee^{\ii\vartheta_j})}\dd{\theta}+\sum_{j=1}^n\abs{\frac{a_j}R}+\sum_{j=1}^m\abs{\frac{b_j}R}.\qedhere\]
\end{proof}
