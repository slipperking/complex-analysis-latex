...
\end{theorem}
\begin{proof}
    First assume that no zeros and poles lie on \(\partial D(0,r)\). It follows that \[\varphi_{\frac{a_k}{r}}\qty(\frac{z}{r})=\frac{r\qty(z-a_k)}{r^2-\overline{a_k}z}\quad\forall 1\leq k\leq m,\qquad\varphi_{\frac{b_k}{r}}\qty(\frac{z}{r})=\frac{r\qty(z-a_k)}{r^2-\overline{a_k}z}\quad\forall 1\leq k\leq n\]
    are holomorphic on \(\overline{D(0,r)}\), and have simple zeros at \(z=a_k,b_k\). Additionally, the transformation maps \(\partial D(0,r)\) to \(\partial\mathbb{D}\). The function \[g(z)=f(z)\frac{\prod_{k=1}^n\varphi_{\frac{b_k}{r}}\qty(\frac{z}{r})}{\prod_{k=1}^m\varphi_{\frac{a_k}{r}}\qty(\frac{z}{r})}\]
    is then holomorphic and non-vanishing in \(\overline{D(0,r)}\). Hence, by \cref{lem:nonvanishingholomorphiclogarithmabsolutemeanvalueproperty}, \[\log\qty|g(0)|=\frac{1}{2\uppi}\int_0^{2\uppi}\log\abs{g\qty(r\ee^{\ii\theta})}\dd{\theta},\]
    implying that
    \begin{align}
        \log\abs{f(0)}-\sum_{k=1}^m\log\abs{\varphi_{\frac{a_k}{r}}(0)}+\sum_{k=1}^n\log\abs{\varphi_{\frac{b_k}{r}}(0)} & =\frac{1}{2\uppi}\int_0^{2\uppi}\log\abs{f\qty(r\ee^{\ii\theta})}\dd{\theta}\nonumber                                                                     \\
        & \quad-\frac{1}{2\uppi}\sum_{k=1}^m\int_0^{2\uppi}\log\abs{\varphi_{\frac{a_k}{r}}\qty(\ee^{\ii\theta})}\dd{\theta}\nonumber\\
        &\qquad+\frac{1}{2\uppi}\sum_{k=1}^n\mathop{\mathmakebox[\widthof{\(\int\)}][l]{\int_0^{2\uppi}}}\log\abs{\varphi_{\frac{b_k}{r}}\qty(\ee^{\ii\theta})}\dd{\theta}.\label{eq:jensensformula_intermediate}
    \end{align}
    Since for fixed \(k\), 
    \begin{align*}
        \log\abs{\varphi_{\frac{a_k}{r}}(0)}=\log\abs{\frac{a_k}{r}},&\qquad\log\abs{\varphi_{\frac{a_k}{r}}\qty(\ee^{\ii\theta})}=\log{1}=0\\
        \log\abs{\varphi_{\frac{b_k}{r}}(0)}=\log\abs{\frac{b_k}{r}},&\qquad\log\abs{\varphi_{\frac{b_k}{r}}\qty(\ee^{\ii\theta})}=\log{1}=0,
    \end{align*}
    \cref{eq:jensensformula_intermediate} becomes \[\log\abs{f(0)}=\frac{1}{2\uppi}\int_0^{2\uppi}\log\abs{f\qty(r\ee^{\ii\theta})}\dd{\theta}+\sum_{k=1}^m\log\abs{\frac{a_k}{r}}-\sum_{k=1}^n\log\abs{\frac{b_k}{r}}.\]
    Suppose \(f\) has additional zeros at each \(\textstyle\cbraces{a'_j=r\ee^{\ii\vartheta_j}}_{1\leq j\leq m'}\) and poles at each \(\textstyle\cbraces{b'_j=r\ee^{\ii\vartheta'_j}}_{1\leq j\leq n'}\subset\partial D(0,r)\) (where each \(0\leq\vartheta_j,\vartheta'_j<2\uppi\)), each with multiplicity \(k_j\) and order \(k'_j\), respectively. Then \(f\) can be written as \[f(z)=g(z)\frac{\prod_{j=1}^{m'}\qty(z-a'_j)^{k_j}}{\prod_{j=1}^{n'}\qty(z-b'_j)^{k'_j}},\]
    where \(g\) does not vanish or diverge at the boundary. By the previous result, we have
    \begin{equation}
        \log\abs{g(0)}=\frac1{2\uppi}\int_0^{2\uppi}\log\abs{g\qty(r\ee^{\ii\vartheta_j})}\dd{\theta}+\sum_{j=1}^m\log\abs{\frac{a_j}r}-\sum_{j=1}^n\log\abs{\frac{b_j}r}.\label{eq:jensensformula_formulanonvanishingboundary}
    \end{equation}
    Fix \(1\leq j\leq m'\). Consider the integral \[\frac1{2\uppi}\int_0^{2\uppi}\log\abs{r\ee^{\ii\theta}-a'_j}^{k_j}\dd{\theta}=\lim_{\varepsilon\to 0^+}\frac1{2\uppi}\int_{\substack{0\leq\theta\leq2\uppi\\\abs{\vartheta_j-\theta}>\varepsilon\\\abs{\vartheta_j-2\uppi-\theta}>\varepsilon\\\abs{\vartheta_j+2\uppi-\theta}>\varepsilon}}k_j\log\abs{r\ee^{\ii\theta}-r\ee^{\ii\vartheta_j}}\dd{\theta}.\]
    Observe that
    \begin{align*}
        ...
        &=\log\abs{2r\sin\qty(\frac{\theta-\vartheta_j}{2})}=\log(2r)+\log\abs{\sin\qty(\frac{\theta-\vartheta_j}{2})}.
    \end{align*}
    ...
    \end{align*}
    Therefore, (applying the same logic for each \(b'_j\))
    \begin{equation}
        \frac1{2\uppi}\int_0^{2\uppi}\log\abs{r\ee^{\ii\theta}-a'_j}^{k_j}\dd{\theta}=k_j\log{r},\quad\frac1{2\uppi}\int_0^{2\uppi}\log\abs{r\ee^{\ii\theta}-b'_j}^{k'_j}\dd{\theta}=k'_j\log{r}\label{eq:jensensformula_integratedlogsingularity}
    \end{equation} and the factorization form yields \[\log\abs{f(0)}=\log\abs{g(0)}+\sum_{j=1}^{m'}k_j\log{r}-\sum_{j=1}^{n'}k'_j\log r.\]
    ...
    \begin{align*}
        \log\abs{f(0)}&=\frac1{2\uppi}\int_0^{2\uppi}\qty(\log\abs{g\qty(r\ee^{\ii\vartheta_j})}+\sum_{j=1}^{m'}\log\abs{r\ee^{\ii\theta}-a'_j}^{k_j}-\sum_{j=1}^{n'}\log\abs{r\ee^{\ii\theta}-a'_j}^{k_j})\dd{\theta}\\
        &\quad+\sum_{j=1}^m\abs{\frac{a_j}r}-\sum_{j=1}^n\abs{\frac{b_j}r}\\
        &=\frac1{2\uppi}\int_0^{2\uppi}\log\abs{f\qty(r\ee^{\ii\vartheta_j})}\dd{\theta}+\sum_{j=1}^m\abs{\frac{a_j}r}-\sum_{j=1}^n\abs{\frac{b_j}r}+\sum_{j=1}^{m'}\abs{\frac{a'_j}r}-\sum_{j=1}^{n'}\abs{\frac{b'_j}r}.\qedhere
    \end{align*}
\end{proof}
...
\end{proof}
\begin{proposition}
    Let \(f\) be entire, non-constant, and of finite order such that \(f(0)=1\). Let \(\qty{a_k}_{k\in\mathbb{N}}\) be the zeros of \(f\) counted according to multiplicities such that \(\abs{a_1}\leq\abs{a_2}\leq\abs{a_3}\leq\cdots\). If \(p>\lambda(f)-1\) is an integer, then \[\dv[p]{}{z} \qty(\frac{f'(z)}{f(z)})\equiv-p!\sum_{k=1}^\infty\frac{1}{\qty(a_k-z)^{p+1}}\] for all \(z\in\mathbb{C}\).
\end{proposition}
\begin{proof}
