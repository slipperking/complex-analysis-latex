...
Therefore, there exists a one-to-one correspondence between \(\Aut\qty(\extcomplex)\) and the group (under matrix multiplication) of \[\left.\cbraces{\mqty(a&b\\c&d)}{\det\mqty(a&b\\c&d)=1}\middle/\cbraces{\pm\vb{I}},\right.\]
where \(\vb{I}=\mqty(\imat{2})\). The quotient group (a group of elements in the form \(\cbraces{\vb{A},-\vb{A}}\)) is taken because the matrix \(\mqty(a&b\\c&d)\) corresponds to the same transformation as \(\mqty(-a&-b\\-c&-d)\). This group, denoted by \(\mathrm{PSL}\qty(2,\mathbb{C})=\left.\mathrm{SL}(2,\mathbb{C})\middle/\cbraces{\pm \vb{I}}\right.\cong\Aut\qty(\extcomplex)\), is known as the \textscsl{projective special linear group} of order 2, and is isomorphic to the \textscsl{group of Möbius transformations}, consisting of all complex linear fractional transformations.
