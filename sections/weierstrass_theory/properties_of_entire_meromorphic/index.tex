...
\begin{theorem}\label{thm:generalizedargumentprinciple}
    ...
    \[\frac{1}{2\uppi\ii}\oint_{\gamma}\frac{\psi(z)f'(z)}{f(z)}\dd z=\sum_{i=1}^k\alpha_i\psi\qty(a_i)-\sum_{j=1}^m\beta_j\psi\qty(b_j).\]
\end{theorem}
\begin{proof}
    ...
    \begin{align*}
        \oint_{\partial D\qty(a_i,\varepsilon_i)}g(z)\ddz
         & =\oint_{\partial D\qty(a_i,\varepsilon_i)}\psi(z)\qty(\frac{\alpha_i}{z-a_i}+\frac{h'(z)}{h(z)})\ddz=2\uppi\ii\alpha_i\psi\qty(a_i),
    ...
    \[\oint_{\partial D\qty(b_j,\varepsilon'_j)}g(z)\ddz=-2\uppi\ii\beta_j\psi\qty(b_j).\]
    Combining these,
    \begin{align*}
        \oint_{\gamma}\frac{\psi(z)f'(z)}{f(z)}\ddz
         & = \sum_{i=1}^k 2\uppi\ii\alpha_i \psi\qty(a_i)-\sum_{j=1}^m-2\uppi\ii\beta_j\psi\qty(b_j)      \\
         & = 2\uppi\ii\qty(\sum_{i=1}^k \alpha_i\psi\qty(a_i)-\sum_{j=1}^m \beta_j\psi\qty(b_j)).\qedhere
    \end{align*}
\end{proof}
\begin{theorem}[Argument Principle]\label{thm:argumentprinciplemeromorphic}
    ...
    \[k-k'=\frac{1}{2\uppi\ii}\oint_{\gamma}\frac{f'(z)}{f(z)}\ddz.\]
...
\begin{proof}
    ...
    \[\frac{1}{2\uppi\ii}\oint_{\gamma}\frac{f'(z)}{f(z)}\ddz=k-k'\]

    Parametrize \(\Gamma\) by \(w=f(z)\). Then \(\dd{w}=f'(z)\ddz\), and
    \[k-k'=\frac{1}{2\uppi\ii}\oint_{\Gamma}\frac{\dd{w}}{w}=\operatorname{Ind}_{\Gamma}(0).\qedhere\]
\end{proof}
