...
\begin{example}\label{ex:csc^2poleexpansion}
    Prove the pole expansion formula of \[\csc^2(\muppi z)=\frac{1}{\muppi^2}\sum_{n=-\infty}^\infty \frac{1}{\qty(z-n)^2}\] for \(z\in\mathbb{C}\setminus\mathbb{Z}\).
\end{example}
\begin{proof}
    It is evident that \(\csc^2(\muppi z)\) has double poles at each of \(z=n\in\mathbb{Z}\). Therefore, \((z-n)^2\csc^2(\muppi z)\) is holomorphic (or has an analytic continuation that is holomorphic) on \(D(n,1)\). Therefore, by repeatedly applying L'Hôpital's Rule, we can write its Taylor expansion that converges on \(D(n,1)\):
    \begin{align*}
        (z-n)^2\csc^2(\muppi z) & =\lim_{\zeta\to n}\frac{\qty(\zeta-n)^2}{\sin[2](\muppi\zeta)}+\lim_{\zeta\to n}\dv{\zeta}(\frac{\qty(\zeta-n)^2}{\sin[2](\muppi\zeta)})(z-n)+\order{(z-n)^2} \\
                               & =\lim_{\zeta\to n}\frac{2\qty(\zeta-n)}{\muppi\sin(2\muppi\zeta)}+\order{(z-n)^2}                                                                             \\
                               & =\lim_{\zeta\to n}\frac{2}{2\muppi^2\cos(2\muppi\zeta)}+\order{(z-n)^2}                                                                                       \\
                               & =\frac{1}{\muppi^2}+\order{(z-n)^2},
    \end{align*}
    where the linear term vanishes since \(\frac{\qty(\zeta-n)^2}{\sin[2](\muppi\zeta)}\) is even around \(\zeta=n\). Hence, the principal part of \(\csc^2(\muppi z)\) at \(z=n\) is \(\psi_n(z)=\frac{1}{\muppi^2(z-n)^2}\). Let \(z=x+\ii y\). Since
    \begin{align*}
        \abs{\sum_{n=-\infty}^\infty\psi_n(z)} & \leq\frac{1}{\muppi^2}\sum_{n=-\infty}^\infty\frac{1}{\qty|z-n|^2}\leq\frac{1}{\muppi^2}\sum_{n=-\infty}^\infty\frac{1}{\qty(x-n)^2}                                                \\
                                               & =\frac{1}{\muppi^2}\qty[\sum_{n=2}^\infty\frac{1}{\qty[n-\cbraces{x}]^2}+\sum_{n=1}^\infty\frac{1}{\qty[n+\cbraces{x}]^2}+\frac{1}{\cbraces{x}^2}+\frac{1}{\qty[1-\cbraces{x}]^2}] \\
                                               & \leq\frac{1}{\muppi^2}\qty[2\sum_{n=1}^\infty\frac{1}{n^2}+\frac{1}{\cbraces{x}^2}+\frac{1}{\qty[1-\cbraces{x}]^2}]
    \end{align*} (where \(\cbraces{x}\) is the fractional part of the real component of \(z\)), it converges uniformly on compact subsets of \(\mathbb{C}\setminus\mathbb{Z}\) by the Weierstrass \(M\)--Test (\cref{thm:weierstrassmtest}). It follows that this sum is holomorphic on \(\mathbb{C}\setminus\mathbb{Z}\) by \cref{thm:weierstrassconvergence}. The difference \[f(z)=\csc[2](\muppi z)-\frac{1}{\muppi^2}\sum_{n=-\infty}^\infty\frac{1}{(z-n)^2}\] is holomorphic on \(\mathbb{C}\setminus\mathbb{Z}\). Since the principal parts of \(f\) at each \(z=n\) are zero, it follows that \(f\) is entire. Therefore, it is bounded on every compact subset of the complex plane. Since \(f\) is bounded on the compact rectangle \(K=\cbraces{z\in\mathbb{C}}{0\leq\Re(z)\leq 1,-1\leq\Im(z)\leq 1}\), for some \(M>0\), we have \(\abs{f}\leq M\) on \(K\). For \(0\leq x=\Re(z)\leq 1\) satisfying \(\abs{y}=\abs{\Im(z)}>1\), by the reverse triangle inequality, we have
    \begin{align}
        \abs{f(z)} & \leq\abs{\csc[2](\muppi z)}+\frac{1}{\muppi^2}\qty[\sum_{n=2}^\infty\frac{1}{\qty[n-\cbraces{x}]^2}+\sum_{n=1}^\infty\frac{1}{\qty[n+\cbraces{x}]^2}+\frac{1}{\abs{z}^2}+\frac{1}{\abs{1-z}^2}]\nonumber \\
                   & \leq \frac{1}{\abs{\sin^2\qty[\muppi(x+\ii y)]}}+\frac{1}{\muppi^2}\qty[\frac{\muppi^2}{3}+\frac{2}{y^2}]\nonumber                                                                                        \\
                   & =\frac{1}{\abs{\sin(\muppi x)\cosh(\muppi y)+\ii\sinh(\muppi y)\cos(\muppi x)}^2}+\frac{1}{3}+\frac{2}{\muppi^2 y^2}\nonumber                                                                               \\
                   & \leq \frac{1}{\sin[2](\muppi x)\cosh[2](\muppi y)+\sinh[2](\muppi y)\qty[1-\sin[2](\muppi x)]}+\frac{1}{3}+\frac{2}{\muppi^2}\nonumber                                                                      \\
                   & \leq \frac{1}{\sin[2](\muppi x)+\sinh[2](\muppi y)}+\frac{1}{3}+\frac{2}{\muppi^2}\leq\csch[2](\muppi)+\frac{1}{3}+\frac{2}{\muppi^2}.\label{eq:csc^2poleexpansionuniformbound}
    \end{align}
    Since \[\abs{f}<\max\cbraces{M,\csch[2](\muppi)+\frac{1}{3}+\frac{2}{\muppi^2}}\] on \(\mathbb{C}\), Liouville's Theorem (\cref{thm:liouville}) gives that \(f\) is constant. At \(z=\frac{1}{2}\), we have
    \begin{align*}
        f\qty(\frac{1}{2}) & =\csc[2](\frac{\muppi}{2})-\frac{8}{\muppi^2}\sum_{n=1}^\infty\frac{1}{\qty[2n-1]^2}=1-\frac{8}{\muppi^2}\qty(\sum_{n=1}^\infty\frac{1}{n^2}-\sum_{n=1}^\infty\frac{1}{(2n)^2}) \\
                           & =1-\frac{8}{\muppi^2}\qty(\frac{\muppi^2}{6}-\frac{\muppi^2}{24})=0,
    \end{align*}
