...
\begin{lemma}\label{lem:infiniteproductlocallyuniformconvergencecriterion}
    Let \(U\subseteq\mathbb{C}\) be open and connected. Suppose \(\textstyle\sum_{k=1}^\infty f_k(z)\) uniformly converges on compact subsets of \(U\) such that each \(f_k\) is holomorphic on \(U\). Then the infinite product \(\textstyle\prod_{k=1}^\infty \exp\qty[f_k(z)]\) is uniformly convergent on compact subsets of \(U\).
\end{lemma}
\begin{proof}
...
\begin{proof}
    ...
    \end{align*}
    where the second inequality can be easily verified by expanding the product \(\textstyle\prod_{k=m+1}^n\qty(1+{f_k(z)})-1\) and the triangle inequality.

    Since \(\ee^{\varepsilon}-1\to 0^+\), it follows that \(\textstyle F(z)=\prod_{k=1}^\infty \qty[1+f_k(z)]\) is uniformly convergent on \(K\). Let \(\xi\in U\) be a point satisfying \(F(\xi)=0\). Since there exists an \(m\in\mathbb{N}\) such that \[\prod_{k=m+1}^\infty \qty(1+f_k(z))\] is non-vanishing at \(z=\xi\), and from the fact that \[F(z)=\prod_{k=1}^m \qty(1+f_k(z))\cdot\prod_{k=m+1}^\infty \qty(1+f_k(z)),\] we can analyze the zeros of the finite product to obtain the conclusion.
\end{proof}
...
\begin{enumerate}
    ...
    \item If \(f(z)\) is entire and has infinitely many zeros such that \(f\) is not identically zero. It follows that \(f\) has countably many zeros (since the zeros are isolated). Let the zeros be indexed by \(\mathbb{N}\), namely \(a_1,a_2,\ldots\). Without loss of generality, assume that \(\forall n\in\mathbb{N}\), \(0<\abs{a_n}\leq\abs{a_{n+1}}\) (repeated elements representing multiplicities), and \(\lim_{n\to\infty}a_n=\infty\). The case for a zero at \(0\) will be treated differently.
        ...
        By \(\varepsilon\)--\(N\), for a fixed \(R>0\), \(\exists N\in\mathbb{N}\) such that \(\forall n\geq N\), \(\abs{a_n}>2R\). Consider the product \(\textstyle\prod_{n=N}^\infty E_{p_n}\qty(\frac{z}{a_n})\). For \(z\in \overline{D(0,R)}\) and \(n\ge N\), we have \(\textstyle\qty|\flatfrac{z}{a_n}|\le\flatfrac{1}{2}\). The Taylor expansion \(\textstyle\log(1-w)=-\sum_{k=1}^\infty\frac{w^k}{k}\) has a convergence disk of \(D(0,1)\). Then,
        \begin{align}
            ...
            & \leq\sum_{k=p_n+1}^\infty\abs{\frac{z}{a_n}}^k=\abs{\frac{z}{a_n}}^{p_n+1}\frac{1}{1-\abs{\frac{z}{a_n}}}\leq 2\abs{\frac{R}{a_n}}^{p_n+1}.\label{eq:infiniteproductweierstrassfactorizationuniformbound}
        \end{align}
        By the definition of \(\cbraces{p_n}_{n\in\mathbb{N}}\), the series \(\textstyle 2\sum_{n=1}^\infty\abs{\frac{R}{a_n}}^{p_n+1}\) converges. Therefore, \(\textstyle \sum_{n=1}^\infty Q_{p_n}\qty(\frac{z}{a_n})\) is uniformly and absolutely convergent on \(\overline{D(0,R)}\) by the Weierstrass \(M\)--Test (\cref{thm:weierstrassmtest}). We then get that \(\textstyle\prod_{n=N}^\infty E_{p_n}\qty(\frac{z}{a_n})=\exp(\sum_{n=N}^\infty Q_{p_n}\qty(\frac{z}{a_n}))\), and it uniformly converges on \(\overline{D(0,R)}\) to a nonzero holomorphic function \(f(z)\) on \({D(0,R)}\) by \cref{lem:infiniteproductlocallyuniformconvergencecriterion}, \cref{thm:weierstrassconvergence}, and \cref{thm:hurwitzsimplecase}.

        The zeros of \[\prod_{n=1}^{N-1} E_{p_n}\qty(\frac{z}{a_n})\] are \(a_1,\ldots, a_{N-1}\) and lie in \(\overline{D(0,2R)}\). To prove the absolute convergence of \(\textstyle\prod_{n=N}^\infty E_{p_n}\qty(\frac{z}{a_n})\) on \(\overline{D(0,R)}\), we will show that \(\textstyle\sum_{n=N}^\infty\abs{E_{p_n}\qty(\frac{z}{a_n})-1}\) is convergent. Trivially, when \(\zeta\in\overline{\mathbb{D}}\), we have \(\abs{\exp(\zeta)-1}\leq\exp\qty|\zeta|-1\leq\paren{\ee-1}\abs{\zeta}\). By \cref{eq:infiniteproductweierstrassfactorizationuniformbound} above, we get that \(\textstyle\qty|Q_{p_n}\qty(\frac{z}{a_n})|\leq 1\) when \(n\geq N\).
        ...
        \end{align*}
        which has a convergent series by definition. Therefore, \(\prod_{n=N}^\infty E_{p_n}\qty(\frac{z}{a_n})\) is absolutely convergent on \(\overline{D(0,R)}\). Letting \(R\to\infty\), we obtain the following result:
        \begin{theorem}[\textsc{Weierstrass Product Theorem}]\label{thm:weierstrassproduct}
            Let \(\cbraces{a_n}_{n\in\mathbb{N}}\) be a sequence of nonzero complex numbers satisfying \(a_n\to\infty\) as \(n\to\infty\) and \(0<\qty|a_n|\leq\qty|a_{n+1}|\) (equality of \(a_n\) and \(a_{n+1}\) treated as multiplicities) for all \(n\). Then there exists a sequence \(\cbraces{p_n}_{n\in\mathbb{N}}\) of nonnegative integers such that \(\forall R>0\), \(\textstyle\sum_{n=1}^\infty{\abs{\frac{R}{a_n}}}^{p_n+1}\) converges. For such a prescribed sequence, the function
            \begin{equation}
                f(z)=\prod_{n=1}^\infty E_{p_n}\qty(\frac{z}{a_n})\label{eq:entirefunctionconstructedfrominfinitelymanyzeros}
            \end{equation} defines an entire function with zeros at each element of the sequence of multiplicities matching the number of times an element is repeated. Moreover, the product converges uniformly on any compact disk \(\overline{D(0,R)}\).
        \end{theorem}
        ...
        \end{theorem}
        \begin{proof}
            By the Weierstrass Product Theorem, construct \(\psi(z)\) to be entire and have zeros at \(\cbraces{a_n}_{n\in\mathbb{N}}\). Thus, \(z^m\psi(z)\) and \(f(z)\) have the same zeros and corresponding multiplicities. Then the function \(z^m\flatfrac{\psi(z)}{f(z)}\) has removable singularities on all of \(\cbraces{a_n}_{n\in\mathbb{N}}\cup\cbraces{0}\) and has an analytic continuation (\cref{thm:riemannremovablesingularities}) to an entire and non-vanishing function. Therefore, it can be written as \(z^m\flatfrac{\psi(z)}{f(z)}=\ee^{\phi(z)}\), where \(\phi\). Let \(\varphi=-\phi\), and from rearrangement, we obtain \cref{eq:weierstrassfactorization_statement}
        \end{proof}
\end{enumerate}
...
\end{corollary}
\begin{proof}
    Let \(\varphi(z)\) be any entire function with zeros only at each pole of \(f\) (with multiplicities matching the order of each pole). If there are infinitely many poles, we can explicitly construct such a \(\varphi\) by the Weierstrass Product Theorem (\cref{thm:weierstrassproduct}). If there are finitely many poles, construct \(\varphi\) using \cref{eq:weierstrassfactorization_finitezeros}. It follows that \(\varphi f\) has can be analytically continued (on its removable singularities) to an entire function \(\phi(z)\) with the same zeros as \(f(z)\). Hence, \[f(z)\varphi(z)=\phi(z)\Longleftrightarrow f(z)=\frac{\phi(z)}{\varphi(z)},\]
...
\begin{example}\label{ex:cscpoleexpansion}
    Prove the pole expansion formula of \[\uppi\csc(\uppi z)=\PV{\mathop{\mathmakebox[\widthof{\(\sum_{k=\infty}\)}][c]{\sum_{k=-\infty}^\infty}}\frac{\qty(-1)^k}{z+k}}=\frac1{z}+\sum_{k=1}^\infty\frac{2z\qty(-1)^k}{z^2-n^2}\] for \(z\in\mathbb{C}\setminus\mathbb{Z}\).
\end{example}
\begin{proof}
