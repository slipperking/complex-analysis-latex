...
\begin{definition}[Residue]\label{def:residue}
    ...
    \begin{equation}
        \residue_{z=a}f(z)=\frac{1}{2\muppi\ii}\oint_{\partial D(a,\rho)}f(z)\ddz,\label{eq:residue}
    \end{equation}
    where \(0<\rho<r\) is arbitrary. Since \(f\) has a Laurent expansion at \(a\), being \[\sum_{n=-\infty}^\infty c_n(z-a)^n,\qquad c_n=\frac{1}{2\muppi\ii}\oint_{\partial D(a,\rho)}\frac{f(z)\ddz}{(z-a)^{n+1}},\] we get that the residue of \(f\) at \(a\) is equal to the first term \(c_{-1}\) of the principal part of its Laurent expansion.
\end{definition}
...
\end{equation}
Let \(z=\infty\) be an isolated singularity of \(f(z)\), which is holomorphic in \(\mathbb{C}\setminus\overline{D(0,R)}\), for sufficiently large finite \(R\). Then for finite \(\rho>R\), the residue at \(z=\infty\) is \textit{defined} as (notice the orientation) \[\residue_{z=\infty}f(z)=\frac{1}{2\muppi\ii}\ointclockwise_{\partial D(0,\rho)}f(z)\ddz.\]
Let \(\zeta=\frac{1}{z}\). Then we get that
\begin{align*}
    \residue_{z=\infty}f(z) & =-\frac{1}{2\muppi\ii}\ointctrclockwise_{\partial D(0,\rho)}f\qty(\frac{1}{\zeta})\dd(\frac{1}{\zeta})                                                                         \\
                            & =\frac{1}{2\muppi\ii}\ointclockwise_{\partial D\qty(0,\frac{1}{\rho})}\frac{f\qty(\frac{1}{\zeta})}{\zeta^2}\ddzeta=-\residue_{\zeta=0}\frac{f\qty(\frac{1}{\zeta})}{\zeta^2}.
\end{align*}
...
\begin{theorem}[\textsc{Residue Theorem}]\label{thm:residuethm}
    Let \(U\subset\mathbb{C}\) be an open set with a simple closed boundary curve \(\partial U\). Suppose \(\cbraces{z_n}\subset U\) is a finite set and \(f(z)\) is holomorphic on \(U\setminus\cbraces{z_n}\) and continuous on \(\overline{U}\setminus\cbraces{z_n}\). Then, \[\oint_{\partial U}f(z)\ddz=2\muppi\ii\sum_{k=1}^n\residue_{z=z_k}f(z)\]
\end{theorem}
\begin{proof}
...
\end{theorem}
\begin{proof}
    Let \(R>\max_{j\in\mathbb{N}_{\leq n}}\qty|z_n|\) be arbitrary. By the Residue Theorem (\cref{thm:residuethm}), \[-\residue_{z=\infty}f(z)=\frac{1}{2\muppi\ii}\oint_{\partial D(0,R)}f(z)\ddz=\sum_{k=1}^n\residue_{z=z_k}f(z)\]
...
\begin{proof}
    ...
    \begin{align*}
        \oint_{\gamma} \frac{1}{\qty(z^2+1)^{n+1}}\ddz & =\int_{-R}^R\frac{1}{\qty(x^2+1)^{n+1}}\ddx+\int_0^\muppi\frac{R\ii}{\qty(R^2\ee^{2\ii\theta}+1)^{n+1}}\ee^{\ii\theta}\dd{\theta} \\
                                                       & =2\muppi\ii\residue_{z=\ii}\frac{1}{\qty(z^2+1)^{n+1}}=\frac{(2n)!\muppi}{2^{2n}(n!)^2}.
    \end{align*}
    We will now show that the integral over the semicircle vanishes as \(R\to\infty\). Under the assumption that \(R\geq 2\), since \[\abs{\frac{R\ii \ee^{\ii\theta}}{\qty(R^2\ee^{2\ii\theta}+1)^{n+1}}}=\frac{R}{\abs{R^2\ee^{2\ii\theta}+1}^{n+1}}\leq\frac{R}{\abs{R^2-1}}\leq\frac{2}{3},\] which is integrable over \([0,\muppi]\), and we can commute the limit with the integral. Therefore, we have
    \begin{align*}
        \int_{-\infty}^\infty \frac{1}{\qty(x^2+1)^{n+1}}\ddx & =\lim_{R\to\infty}\oint_{\gamma}\frac{1}{\qty(z^2+1)^{n+1}}\ddz                                                                                        \\
                                                              & \quad-\int_0^{\muppi}\lim_{R\to\infty}\frac{R\ii}{\qty(R^2\ee^{2\ii\theta}+1)^{n+1}}\ee^{\ii\theta}\dd{\theta}=\frac{(2n)!\muppi}{2^{2n}(n!)^2}.\qedhere
    \end{align*}
\end{proof}
...
\begin{proof}
    ...
    We will now analyze each integral. The first integral is \[\int_{C_R}f(z)\ddz=\int_0^\muppi\frac{\exp(\ii R\ee^{\ii\theta})}{R\ee^{\ii\theta}}Ri\ee^{\ii\theta}\dd{\theta}=\ii\int_0^{\muppi}\ee^{\ii R\cos\theta}\ee^{-R\sin\theta}\dd{\theta}.\]
    Notice that \(\frac{2}{\muppi}\theta\leq\sin(\theta)\leq\theta\) over the integration range. We want to observe the behavior as \(R\to\infty\):
    \begin{align*}
        \abs{\ii\int_0^\muppi \ee^{\ii R\cos\theta}\ee^{-R\sin\theta}\dd{\theta}} & \leq\int_0^{\muppi}\ee^{-R\sin\theta}\dd{\theta}=2\int_0^{\frac{\muppi}{2}}\ee^{-R\sin\theta}\dd{\theta}                                      \\
                                                                                 & <2\int_0^{\frac{\muppi}{2}}\ee^{-R\frac{2}{\muppi}\theta}\dd{\theta}=\eval{-\frac{\muppi}{R}\ee^{-R\frac{2}{\muppi}\theta}}_0^{\frac{\muppi}{2}} \\
                                                                                 & =\frac{\muppi}{R}\qty(1-\ee^{-R})\to 0.
    \end{align*}
    Let us evaluate the integral on \(\gamma_\varepsilon\) as \(\varepsilon\to 0\): \[\int_{C_\varepsilon}f(z)\ddz=\ii\int_{\muppi}^0\exp(\varepsilon\qty(\ii\cos\theta-\sin\theta))\dd{\theta}=\ii\int_\muppi^0\ee^{-\varepsilon\sin\theta}\ee^{\ii\varepsilon\cos\theta}\dd{\theta}.\]
    Obviously, \[\abs{\ee^{-\varepsilon\sin\theta}\ee^{\ii\varepsilon\cos\theta}}\leq 1,\]
    and therefore, the integral and the limit may commute:
    \[\lim_{\varepsilon\to 0}\int_{C_\varepsilon}f(z)\ddz=\ii\int_{\muppi}^0\lim_{\varepsilon\to 0^+}\ee^{-\varepsilon\sin\theta}\ee^{\ii\varepsilon\cos\theta}\dd{\theta}=\ii\int_{\muppi}^0\dd{\theta}=-\ii\muppi.\]
    ...
    \end{align*}
    Hence, \[-\ii\muppi+2\ii\int_{0}^\infty\frac{\sin(z)}{z}\ddz=0\Longleftrightarrow\int_{0}^\infty\frac{\sin{z}}{z}\ddz=\frac{\muppi}{2}.\qedhere\]
\end{proof}
...
\begin{proof}
    \begin{figure}
        ...
        \begin{tikzpicture}[>=stealth,
            ...
            \node[anchor=west] at (0.4,0.25) {\(\tfrac{\muppi}{4}\)};
    ...
    \begin{gather*}
        \Gamma_1=\cbraces{x\in\mathbb{R}}{0\leq x\leq R},\qquad\Gamma_2=\cbraces{r\ee^{\ii\frac{\muppi}{4}}}{0\leq r\leq R},\\
        C_R=\cbraces{R\ee^{\ii\theta}}{0\leq\theta\leq\frac{\muppi}{4}}
    \end{gather*}
    ...
    \end{equation} The third integral can be written as \[\int_{C_R}f(z)\ddz=R\ii\int_0^{\frac{\muppi}{4}}\exp[\ii{\qty(R\ee^{\ii\theta})}^2]\ee^{\ii\theta}\dd{\theta}.\]
    Using the fact that \(\frac{4}{\muppi}\theta<\sin(2\theta)\) on the integration range, it can be bounded as
    \begin{align*}
        \abs{\int_{C_R}f(z)\ddz} & \leq R{\int_0^{\frac{\muppi}{4}}\ee^{-R^2\sin(2\theta)}\dd{\theta}}<R\int_0^{\frac{\muppi}{4}}\ee^{-\frac{4}{\muppi}R^2\theta}\dd{\theta} \\
                                 & =-\frac{\muppi}{4R}\eval{\ee^{-\frac{4}{\muppi}R^2\theta}}_0^{\frac{\muppi}{4}}=\frac{\muppi}{4R}\qty(1-\ee^{-R^2}).
    \end{align*}
    As \(R\to\infty\), this integral tends to 0. Let \(z=r\ee^{\ii\frac{\muppi}{4}}\) on \(\Gamma_2\). Then, we have \[\lim_{R\to\infty}\int_{\Gamma_2}f(z)\ddz=\int_\infty^0\exp[\ii\qty(r\ee^{\ii\frac{\muppi}{4}})^2]\ee^{\ii\frac{\muppi}{4}}\dd{r}=\ee^{\ii\frac{\muppi}{4}}\int_\infty^0\exp(-r^2)\dd{r}.\]
    From \cref{eq:fresnelwedgecontourintegral}, we have that \[\int_0^\infty \ee^{\ii r^2}\dd{r}=\ee^{\ii\frac{\muppi}{4}}\int_0^\infty \ee^{-r^2}\dd{r}.\] Since \(\int_0^\infty \ee^{-r^2}\dd{r}=\frac{\sqrt{\muppi}}{2}\), we have \[\int_0^\infty \ee^{\ii r^2}\dd{r}=\qty(\frac{\sqrt{2}}{2}+\ii\frac{\sqrt{2}}{2})\frac{\sqrt{\muppi}}{2}.\]
    Since \(\ee^{\ii r^2}=\cos(r^2)+\ii\sin(r^2)\), we have
    \begin{gather*}
        \int_0^\infty\cos(r^2)\dd{r}=\Re\qty[\int_0^\infty \ee^{\ii r^2}\dd{r}]=\frac{\sqrt{2\muppi}}{4},\\
        \int_0^\infty\sin(r^2)\dd{r}=\Im\qty[\int_0^\infty \ee^{\ii r^2}\dd{r}]=\frac{\sqrt{2\muppi}}{4},
    \end{gather*}
    as desired.
\end{proof}
\begin{example}
    Evaluate the integrals \(\int_0^{2\muppi}\Phi(\cos\theta,\sin\theta)\dd{\theta}\), where \(\Phi(\xi,\eta)\) is a rational function of \(\xi\) and \(\eta\) that is continuous on \(\theta\in[0,2\muppi]\).
\end{example}
\begin{proof}
    Let \(z=\ee^{\ii\theta}\). Consequently, we have \(\cos\theta=\frac{z+z^{-1}}{2}\), \(\sin\theta=\frac{z-z^{-1}}{2\ii}\), and \(\ddz=\ii\ee^{\ii\theta}\dd{\theta}\), implying that \(\dd{\theta}=\frac{\ddz}{\ii z}\). Therefore, by the Residue Theorem (\cref{thm:residuethm}), letting \(f(z)=\frac{1}{\ii z}\Phi\qty(\frac{z+z^{-1}}{2},\frac{z-z^{-1}}{2\ii})\), we have \[\int_0^{2\muppi}\Phi(\cos\theta,\sin\theta)\dd{\theta}=\oint_{\partial\mathbb{D}}f(z)\ddz=2\muppi\ii\sum_{k=1}^n\residue_{z=z_k}f(z),\]
...
\end{example}
\begin{proof}
    Let \(f(z)=\frac{z^\alpha}{1+z^\beta}\) and let \(-\muppi<\Arg(z)\leq \muppi\) in the principal branches of \(z^\alpha=\ee^{\alpha\Log(z)}\) and \(z^\beta=\ee^{\beta\Log(z)}\). Then except for at the zeros of \(1+z^\beta\), \(f\) is holomorphic.
    ...
    \end{figure}The solutions to \(z^\beta=-1\) are \(z=\exp(\ii\frac{\muppi}{\beta}+2\ii k\frac{\muppi}{\beta})\). Choose an indented wedge contour (as there is a logarithmic branch point singularity at the origin) with an angle of \(\frac{2\muppi}{\beta}\) (as in \cref{fig:wedgecontour}). The only singularity it encloses is \(\exp(\ii\frac{\muppi}{\beta})\). Since it is a simple zero of \(\frac{1}{f}\), this singularity is a simple pole.

    The contour is the union of the following curves:\
    \begin{gather*}
        \Gamma_1=\cbraces{x\in\mathbb{R}}{\varepsilon\leq x\leq R},\qquad\Gamma_2=\cbraces{r\exp(\ii\frac{2\muppi}{\beta})}{\varepsilon\leq r\leq R},\\
        C_R=\cbraces{R\ee^{\ii\theta}}{0\leq\theta\leq \frac{2\muppi}{\beta}},\qquad C_\varepsilon=\cbraces{\varepsilon\ee^{\ii\theta}}{0\leq\theta\leq \frac{2\muppi}{\beta}}
    \end{gather*}
    where \(R>1\) and \(0<\varepsilon<1\). By the Residue Theorem (\cref{thm:residuethm}), we get that \[\lim_{\varepsilon\to 0}\lim_{R\to\infty}\qty(\int_{\Gamma_1}+\int_{\Gamma_2}+\int_{C_R}+\int_{C_\varepsilon})f(z)\ddz=2\muppi\ii\Res[f,\exp(\ii\frac{\muppi}{\beta})].\]
    By \cref{eq:residueatpole}, it follows that
    \begin{align*}
        \residue\qty[f,\exp(\ii\frac{\muppi}{\beta})] & =\lim_{z\to\exp(\ii\frac{\muppi}{\beta})}\flatfrac{\qty[z-\exp(\ii\frac{\muppi}{\beta})]}{\qty[\frac{1+z^\beta}{z^\alpha}]} \\
                                                     & =\lim_{z\to\exp(\ii\frac{\muppi}{\beta})}{\dv{z}(z^{-\alpha}+z^{\beta-\alpha})}^{-1}                                       \\
                                                     & =\lim_{z\to\exp(\ii\frac{\muppi}{\beta})}\frac{z^{\alpha+1}}{(\beta-\alpha)z^{\beta}-\alpha}                               \\
                                                     & =-\frac{1}{\beta}\exp(\ii\frac{\muppi}{\beta}(\alpha+1)).
    \end{align*}
    We can write the integral on \(\Gamma_2\) in terms of \(I\):
    \begin{align*}
        \lim_{\substack{R\to\infty                                                                 \\\varepsilon\to 0}}\int_{\Gamma_2}f(z)\ddz & =\lim_{\substack{R\to\infty\\\varepsilon\to 0}}\int_R^0 f\qty[r\exp(\ii\frac{2\muppi}{\beta})]\exp(\ii\frac{2\muppi}{\beta})\dd{r}   \\
         & =-\exp[\ii\frac{2\muppi}{\beta}(1+\alpha)]\int_0^\infty\frac{r^\alpha}{1+r^\beta}\dd{r}.
    \end{align*}
    We also have \[\int_{C_R}f(z)\ddz=R\ii\int_{0}^{\frac{2\muppi}{\beta}}f\qty(R\ee^{\ii\theta})\ee^{\ii\theta}\dd{\theta}=\ii\int_0^{\frac{2\muppi}{\beta}}\frac{R^{\alpha+1}}{1+R^\beta \ee^{\ii\beta\theta}}\exp[\ii\theta(1+\alpha)]\dd{\theta}.\] It can also be shown that the integral is bounded by a vanishing function as \(R\to\infty\):
    \[\abs{\int_0^{\frac{2\muppi}{\beta}}\frac{R^{\alpha+1}}{1+R^\beta \ee^{\ii\beta\theta}}\exp[\ii\theta(1+\alpha)]\dd{\theta}}\leq\int_0^{\frac{2\muppi}{\beta}}\frac{R^{\alpha+1}}{R^\beta-1}\dd{\theta}=\frac{2\muppi}{\beta}\frac{R^{\alpha+1}}{R^\beta-1}\to 0.\]
    Similarly, as \(\varepsilon\to 0\), \[\abs{\int_{C_\varepsilon}f(z)\ddz}\leq\varepsilon\int_0^{\frac{2\muppi}{\beta}}\abs{f\qty(\varepsilon\ee^{\ii\theta})}\dd{\theta}=\int_0^{\frac{2\muppi}{\beta}}\frac{\varepsilon^{\alpha+1}}{1-\varepsilon^\beta}\dd{\theta}=\frac{2\muppi}{\beta}\frac{\varepsilon^{\alpha+1}}{1-\varepsilon^{\beta}}\to 0.\]
    By letting \(R\to\infty\) and \(\varepsilon\to 0\), we have \[\qty[1-\exp(\ii\frac{2\muppi}{\beta}(1+\alpha))]I=-\frac{2\muppi\ii}{\beta}\exp(\ii\frac{\muppi}{\beta}(\alpha+1)).\]
    It follows that \[I=\frac{2\muppi\ii}{\beta}\brackets{\exp(\ii\frac{\muppi}{\beta}(\alpha+1))-\exp(-\ii\frac{\muppi}{\beta}(\alpha+1))}^{-1}=\frac{\muppi}{\beta}\csc(\frac{\muppi}{\beta}(\alpha+1)).\qedhere\]
\end{proof}
\begin{example}
    Prove that the Fourier transform of \(\sech(\muppi x)\) is itself, or that \[I(\xi)=\int_{-\infty}^\infty\exp(-2\muppi \ii x\xi)\sech(\muppi x)\ddx=\sech(\muppi\xi).\]
\end{example}
\begin{proof}
    Fix \(\xi\in\mathbb{R}\) and let \(f(z)=\frac{\exp(-2\muppi \ii z\xi)}{\cosh(\muppi z)}\). Its poles in \(\mathbb{C}\) occur when \(\ee^{\muppi z}+\ee^{-\muppi z}=0\), or equivalently, when \(z=\ii\qty(n+\frac{1}{2})\), where \(n\in\mathbb{Z}\).
    ...
    \end{figure}Since \[\cosh(\muppi(z+\ii))=-\cosh(\muppi z),\qquad\exp(-2\muppi\ii(z+\ii)\xi)=\exp(2\muppi\xi)\exp(-2\muppi \ii z\xi),\] we have that \(f(z)\) is a constant multiple of \(f(z+\ii)\). In particular, \(f(z+\ii)=-\exp(2\muppi\xi)f(z)\). Therefore, we can use a rectangular contour as shown in \cref{fig:rectangularcontour}. Let the sides be denoted by
    \begin{gather*}
    ...
    \begin{equation}
        \qty(\int_{\overrightarrow{\Gamma}}+\int_{\overset{\uparrow}{\Gamma}}+\int_{\overleftarrow{\Gamma}}+\int_{\overset{\downarrow}{\Gamma}})f(z)\ddz=2\muppi\ii\Res(f,\frac{\ii}{2}).\label{eq:fouriertransformofsechpix_rectangularcontourintegral}
    \end{equation}
    By \cref{eq:residueatpole}, we have
    \begin{align*}
        \Res(f,\frac{\ii}{2}) & =\lim_{z\to\frac{\ii}{2}}\qty(z-\frac{\ii}{2})\frac{\exp(-2\muppi \ii z\xi)}{\cosh(\muppi z)}            \\
                              & =\lim_{z\to\frac{\ii}{2}}\dv{z}(\frac{\cosh(\muppi z)}{\exp(-2\muppi \ii z\xi)})^{-1}                    \\
                              & =\lim_{z\to\frac{\ii}{2}}\frac{\exp(-2\muppi \ii z\xi)}{\muppi\sinh(\muppi z)+2\muppi\ii\xi\cosh(\muppi z)} \\
                              & =\frac{\exp(\muppi\xi)}{\muppi \ii}.
    \end{align*}
    The sum of the horizontal line integrals is equal to
    \begin{align*}
        \int_{-R}^R f(z)\ddz+\int_R^{-R}f(z+\ii)\ddz & =\int_{-R}^R f(z)\ddz-\int_R^{-R}\ee^{2\muppi\xi}f(z)\ddz \\
                                                     & =\qty(1+\ee^{2\muppi\xi})\int_{-R}^R f(z)\ddz.
    \end{align*} As \(R\to\infty\), we have \(\int_{\overrightarrow{\Gamma}}f(z)\ddz+\int_{\overleftarrow{\Gamma}}f(z)\ddz\to\qty(1+\ee^{2\muppi\xi})I(\xi)\).
    The remaining two integrals can be written as
    \begin{align*}
        \int_{\overset{\uparrow}{\Gamma}}f(z)\ddz=\int_0^1 \frac{\exp(-2\muppi\ii(R+\ii z)\xi)}{\cosh(\muppi(R+\ii z))}\ddz \\
        \int_{\overset{\downarrow}{\Gamma}}f(z)\ddz=\int_1^0 \frac{\exp(2\muppi\ii(R-\ii z)\xi)}{\cosh(\muppi(-R+\ii z))}\ddz.
    \end{align*}
    They can be bounded with
    \begin{align*}
        \abs{\int_0^1 \frac{\exp(-2\muppi\ii(R+\ii z)\xi)}{\cosh(\muppi(R+\ii z))}\ddz} & \leq 2\int_0^1\frac{\exp(2\muppi z\xi)}{\abs{\ee^{\muppi R}\ee^{\muppi \ii z}+\ee^{-\muppi R}\ee^{-\muppi \ii z}}}\ddz \\
                                                                                      & \leq2\int_0^1\frac{\exp(2\muppi z\xi)}{\abs{\ee^{\muppi R}-\ee^{-\muppi R}}}\ddz
    \end{align*} and
    \begin{align*}
        \abs{\int_1^0 \frac{\exp(2\muppi\ii(R-\ii z)\xi)}{\cosh(\muppi(-R+\ii z))}\ddz} & \leq 2\int_0^1\frac{\exp(2\muppi z\xi)}{\abs{\ee^{-\muppi R}\ee^{\muppi \ii z}+\ee^{\muppi R}\ee^{-\muppi \ii z}}}\ddz \\
                                                                                      & \leq2\int_0^1\frac{\exp(2\muppi z\xi)}{\abs{\ee^{\muppi R}-\ee^{-\muppi R}}}\ddz
    \end{align*}
    Since the integrands are continuous and uniformly convergent to \(0\) with respect to \(z\), we have \[\int_{\overset{\uparrow}{\Gamma}}f(z)\ddz+\int_{\overset{\downarrow}{\Gamma}}f(z)\ddz\to 0\] as \(R\to\infty\). By rearrangement of \cref{eq:fouriertransformofsechpix_rectangularcontourintegral}, \[I(\xi)\qty(1+\ee^{2\muppi\xi})=2\exp(\muppi\xi),\]
    or that \[I(\xi)=\frac{2}{\ee^{-\muppi\xi}+\ee^{\muppi\xi}}=\sech(\muppi\xi),\] which proves the result.
\end{proof}
...
\begin{proof}
    ...
    \end{align}
    Let \(\gamma=\Gamma\cup C_{R}\), where concretely, \[\Gamma=\cbraces{x\in\mathbb{R}}{-R\leq x\leq R},\qquad C_{R}=\cbraces{R\ee^{\ii\theta}}{0\leq\theta\leq\muppi}\] and \(R>2\), and let \(f(z)=\frac{\Log(z+\ii)}{z^2+1}\), where the branch for \(\Log\) is chosen to satisfy \([0,\muppi]\subset\Im\log\qty(\mathbb{C}^*)\), such as the principal branch. The only singularity of \(f\) in the upper half plane is a simple pole at \(z=\ii\). By the Residue Theorem (\cref{thm:residuethm}), we have \[\lim_{R\to\infty}\oint_{\gamma}f(z)\ddz=\lim_{R\to\infty}\qty(\int_\Gamma+\int_{C_R})f(z)\ddz=2\muppi\ii\residue_{z=\ii}f(z).\]
    By \cref{eq:residueatpole}, we have \[\residue_{z=\ii}f(z)=\lim_{z\to\ii}\qty(z-\ii)\frac{\log(z+\ii)}{z^2+1}=\lim_{z\to\ii}\frac{\log(z+\ii)}{z+\ii}=\frac{\log(2\ii)}{2\ii}=\frac{\muppi}{4}-\ii\frac{\log(2)}{2}.\]
    Additionally, for \(z\in C_R\), since as \(R\to\infty\), \(\abs{f(z)}=\abs{\frac{\Log(z+i)}{z^2+1}}\leq\frac{\abs{\log\abs{z+i}}+\muppi}{R^2-1}\leq\frac{\log\abs{R+1}+\muppi}{R^2-1}<\frac{R+1+\muppi}{R^2-1}\to 0\) by virtue of \(R>2\), it follows that \(\int_{C_R}f(z)\ddz\to 0\).

    Since \(\lim_{R\to\infty}\int_\Gamma f(z)\ddz=\int_{-\infty}^\infty f(z)\ddz\) and \[\int_{-\infty}^\infty f(z)\ddz=\frac{\muppi^2\ii}{2}+\muppi\log(2),\] by \cref{eq:branchpointpoleconcurrenceintegral_rewrite}, we have \(I=\Re\int_{-\infty}^\infty f(z)\ddz=\muppi\log(2)\).
\end{proof}
