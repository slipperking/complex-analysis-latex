\subsection{Nevanlinna's Counting, Proximity, and Characteristic Functions}
Let \(f:D(0,R)\to\extcomplex\) be meromorphic such that \[f(z)=\sum_{j=k}^\infty c_jz^j\] where \(c=c_k\neq0\). Then \(z\mapsto\flatfrac{f(z)}{z^k}\) is holomorphic on a neighborhood of 0 and is nonzero at 0. By Jensen's formula (\cref{thm:jensensformula}) on \(\flatfrac{f(z)}{z^k}\),
\begin{equation}
...
\end{equation}
Define the \textit{nonnegative part of the logarithm}, denoted \(\logp (x)\), to be \[\logp (x)=\max\qty{\log(x),0}.\]
\begin{proposition}\label{prop:lognonnegativepartproperties}
    The following easily verifiable properties hold:
    \begin{enumerate}
        \item \(\logp  x\geq\log x\) for \(x>0\),
        \item \(\logp  x\leq\logp  y\) for \(x\leq y\) (nondecreasing),
        \item \(\log x=\logp x-\logp \tfrac1x\) for \(x>0\),
        \item \(\abs{\log x}=\logp x+\logp \tfrac1x\) for \(x>0\),\label{itm:lognonnegativepartproperties_sumofreciprocallogs}
        \item \(\textstyle\logp \qty(\prod_k x_k)\leq\sum_k\logp x_k\) for (finitely many) positive \(x_k\),\label{itm:lognonnegativepartproperties_multiplicativesubadditivity}
        \item \(\textstyle\logp \qty(\sum_{k=1}^n x_k)\leq\log n+\sum_{k=1}^n\logp x_k\) for (finitely many) positive \(x_k\).\label{itm:lognonnegativepartproperties_weaksubadditivity}
    \end{enumerate}
\end{proposition}
\begin{proof}
    The first five properties are trivial. We now prove \cref{itm:lognonnegativepartproperties_weaksubadditivity}. Observing that \(x_k\leq\ee^{\logp  x_k}\), it follows that \[\logp \sum_{k=1}^n x_k\leq\logp \sum_{k=1}^n\ee^{\logp  x_k}\mathmakebox[\widthof{\(\quad\)}][c]{\overset{\text{\raisebox{0.7\baselineskip}{(\cref{itm:lognonnegativepartproperties_multiplicativesubadditivity})}}}{\leq}}\logp n+\logp \qty(\ee^{\max_{k}\logp x_k})=\log n+\max_k\logp x_k.\] The last step uses the fact that \(\logp \) is simply \(\log\) when the argument \(\geq 1\).
\end{proof}
\begin{definition}
    For \(r>0\), \(a\in\mathbb{C}\), define the \textit{Nevanlinna proximity function} by \[m(r,a,f)=\frac1{2\uppi}\int_0^{2\uppi}\logp \abs{\frac1{f\qty(r\ee^{\ii\theta})-a}}\dd{\theta}\] and \[m(r,f)=m(r,\infty,f)=\frac1{2\uppi}\int_0^{2\uppi}\logp \abs{f\qty(r\ee^{\ii\theta})}\dd{\theta}\] in the infinite case. Observe that \(m(r,a,f)=m\qty(r,\flatfrac1{(f-a)})\).
\end{definition}
Since
\begin{gather*}
    \logp \abs{f\qty(r\ee^{\ii\theta})}-\logp \abs{\frac1{f\qty(r\ee^{\ii\theta})}}=\log\abs{f\qty(r\ee^{\ii\theta})},
\end{gather*}
this implies \[m(r,\infty,f)-m(r,0,f)=\frac1{2\uppi}\int_0^{2\uppi}\log\abs{f\qty(r\ee^{\ii\theta})}\dd{\theta}.\]
\Cref{eq:nevanlinnacountingjensensformulaexposition2} then can be written as
\begin{equation}
    N(r,0,f)+m(r,0,f)=N(r,\infty,f)+m(r,\infty,f)-\log\abs{c}.\label{eq:nevanlinnafirsttheorematzero}
\end{equation}
\begin{definition}
    This motivates the sum \(T(r,f)\), known as the \textit{Nevanlinna characteristic}, defined by \(T(r,f)=m(r,f)+N(r,f)\) (we will also use the notation \(T(r,a,f)=m(r,a,f)+N(r,a,f)\)).
\end{definition}
Then \cref{eq:nevanlinnafirsttheorematzero} can be rewritten as:
\begin{proposition}\label{prop:nevanlinnafirsttheorematzero}
    For any (non-constant) meromorphic \(f\) on \(D(0,R)\) with the innermost Laurent series \(\textstyle f(z)=\sum_{j=k}^\infty c_jz^j\) and any \(0<r<R\), \[T(r,0,f)=T(r,\infty,f)-\log\abs{c_k}.\]
\end{proposition}
