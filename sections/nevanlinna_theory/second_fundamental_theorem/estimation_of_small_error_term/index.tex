...
\begin{theorem}[name=\textsc{Second Fundamental Theorem of Nevanlinna Theory, Part 2},store=thm:nevanlinnasecondfundamentaltheorempart2]\label{thm:nevanlinnasecondfundamentaltheorempart2}
    ...
    \begin{enumerate}
        ...
        \item If \(R_0=\infty\) and \(f\) has infinite order, then \cref{eq:nevanlinnasecondfundamentaltheorempart2_intermediate_entirecomplexplane} as \(r\to\infty\) through all values of \(r\) outside a set \(E\) of finite linear measure not exceeding 2.
...
\begin{theorem}\label{thm:nevanlinnaadmissibilityerrortermgrowth}
    ...
    \begin{enumerate}
        ...
        \item If \(R_0<\infty\), then \(S(r,f)=o\qty[T(r,f)]\) for \(r\to R_0^-\) through any sequence \(\qty{r_n}_n\) dependent on \(f\) but not on \(\qty{a_\nu}\) and \(q\).
    \end{enumerate}
\end{theorem}
\begin{proof}
    ...
    \begin{enumerate}
        \item Assume \(R_0=\infty\) and \(f\) has finite order. Then \(T(r,f)=\order{r^{\rho+\varepsilon}}\), \(r^{\rho-\varepsilon}=o\qty[T(r,f)]\) (assuming that \(\rho>0\)), so \(\log r=o\qty[T(r,f)]\). Therefore, by \cref{thm:nevanlinnasecondfundamentaltheorempart2}, it follows that through all values as \(r\to\infty\),
            ...
            If \(\rho=0\), either \(\log r=o\qty(T(r,f))\) or \(T\qty(r,f)=\order{\log r}\). The former case is the same as above.
