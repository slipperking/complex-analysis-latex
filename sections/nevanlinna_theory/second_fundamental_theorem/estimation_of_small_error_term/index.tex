...
\begin{proof}
    ...
    \begin{enumerate}
        \item Assume \(R_0=\infty\) and \(f\) has finite order. Then \(T(r,f)=\order{r^{\rho+\varepsilon}}\), \(r^{\rho-\varepsilon}=o\qty[T(r,f)]\) (assuming that \(\rho>0\)), so \(\log T(r,f)=\order{\log r}\). Therefore, by \cref{thm:nevanlinnasecondfundamentaltheorempart2}, it follows that through all values as \(r\to\infty\),
