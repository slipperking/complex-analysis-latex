...
\begin{proof}
    ...
    \begin{enumerate}
        \item \(\exists\nu\) such that \(\textstyle\abs{f(z)-a_\nu}<\flatfrac{\delta}{(3q)}\).
            ...
            \end{align}
        \item There exists no such \(\nu\). Then all \(\nu\) have the property that \(\textstyle\abs{f(z)-a_\nu}\geq\flatfrac{\delta}{3q}\). It follows that \[\logp\frac1{\abs{f(z)-a_\nu}}\leq\logp\frac{3q}\delta\implies\sum_{\mu=1}^q\logp\frac1{\abs{f(z)-a_\mu}}-q\logp {\frac{3q}\delta}\leq0\leq \log^+\abs{F(z)}.\]
