...
\end{align*}
where the precise formulation allows for a sequence \(\qty{r_n}\) diverging to \(\infty\) to be chosen so that \(S(r,f)=o\qty(T(r,f))\) for \(r\) in this sequence. This implies \(\textstyle\limsup_{r\to\infty}\frac{N(r,c,f)}{T(r,f)}=1\). This in turn then proves Picard's theorems (see \cref{sec:deficiencyrelation}).
