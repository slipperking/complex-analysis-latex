...
\begin{theorem}[\textsc{Second Fundamental Theorem of Nevanlinna Theory, Part 1}]\label{thm:nevanlinnasecondfundamentaltheorempart1}
    ...
    and
    \begin{equation}
        S(r,f)=m\qty(r,\frac{f'}f)+m\qty(r,\sum_{\nu=1}^q\frac{f'}{f-a_\nu})+q\logp \frac{3q}{\delta}+\log 2-\log\abs{c'},\label{eq:nevanlinnasecondfundamentaltheorempart1_smallerrorterm}
    \end{equation}
...
\begin{proof}
    ...
    \begin{enumerate}
        \item \(\exists\nu\) such that \(\textstyle\abs{f(z)-a_\nu}<\flatfrac{\delta}{(3q)}\).
            ...
            \begin{align}
                ...
                &\geq\sum_{\mu=1}^q\logp\frac1{\abs{f(z)-a_\mu}}-q\logp {\tfrac{3q}\delta}-\log2.\label{eq:nevanlinnasecondfundamentaltheorempart1_logpFlowerbound}
            \end{align}
        \item There exists no such \(\nu\). Then all \(\nu\) have the property that \(\textstyle\abs{f(z)-a_\nu}\geq\flatfrac{\delta}{3q}\). It follows that \[\frac1{\abs{f(z)-a_\nu}}\leq\tfrac{3q}\delta\implies\sum_{\mu=1}^q\logp\frac1{\abs{f(z)-a_\mu}}-q\logp {\tfrac{3q}\delta}\leq0\leq \logp\abs{F(z)}.\]
            Then \cref{eq:nevanlinnasecondfundamentaltheorempart1_logpFlowerbound} holds for this case trivially.
    \end{enumerate}
    Integrating the relation \cref{eq:nevanlinnasecondfundamentaltheorempart1_logpFlowerbound} (which holds in all cases) gives that
    \begin{equation}
        m(r,F)\geq\sum_{\nu=1}^qm\qty(r,a_\nu,f)-q\logp\tfrac{3q}\delta-\log 2.\label{eq:nevanlinnasecondfundamentaltheorempart1_proximityFlowerbound}
    \end{equation}
    ...
    Combining this with \cref{eq:nevanlinnasecondfundamentaltheorempart1_proximityFlowerbound} gives
    \begin{align*}
    ...
    \end{align*}
    where \(S(r,f)\) is defined as in \cref{eq:nevanlinnasecondfundamentaltheorempart1_smallerrorterm}. This concludes the proof.
\end{proof}
