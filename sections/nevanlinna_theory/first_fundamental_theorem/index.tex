...
\begin{proof}
    ...
    Then by \cref{prop:nevanlinnafirsttheorematzero}, and the fact that the poles of \(f\) match those of \(f-a\) (more importantly, \(N(r,0,f-a)=N(r,0,f)\)), 
    \begin{align*}
...
\begin{corollary}
    Let \(f\) be (non-constant) meromorphic on \(D(0,R)\) where \(0<r\leq\infty\). Then,
    \[\frac1{2\uppi}\int_0^{2\uppi}m\qty(r,\ee^{\ii\theta})\dd{\theta}\leq\log 2.\]
\end{corollary}
\begin{proof}
    By the First Fundamental Theorem (\cref{thm:nevanlinnafirstfundamentaltheorem}), we have \[N\qty(r,\ee^{\ii\theta},f)+m\qty(r,\ee^{\ii\theta},f)=T(r,f)-\log\abs{c}+\varepsilon\qty(r,\ee^{\ii\theta},f),\] where \(\abs{\varepsilon\qty(r,\ee^{\ii\theta},f)}\leq\log 2\) and \(c\) is the first nonzero coefficient of the Laurent expansion for \(f-\ee^{\ii\theta}\). Hence, \[T(r,f)=\frac1{2\uppi}\int_0^{2\uppi}\qty(N\qty(r,\ee^{\ii\theta},f)+m\qty(r,\ee^{\ii\theta},f))\dd{\theta}+\log\abs{c}-\frac1{2\uppi}\mathop{\mathmakebox[\widthof{\(\int\)}][l]{\int_0^{2\uppi}}}\varepsilon\qty(r,\ee^{\ii\theta},f)\dd{\theta}\]
\end{proof}