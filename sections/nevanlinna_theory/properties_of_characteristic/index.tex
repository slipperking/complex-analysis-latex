...
\begin{proof}
    ...
    Recognizing that \[N\qty(r,\sum_kf_k)=\sum_j\log\abs{\frac r{b^{(j)}}}+n\qty(0,\sum_kf_k)\log r,\quad\sum_{k=1}^n N\qty(r,f_k)=\sum_{k=1}^n\sum_j\log\abs{\frac r{b_k^{(j)}}}+\sum_kn\qty(0,f_k),\] \cref{itm:nevanlinnathreefunctionsmiscproperties_Nsubadd} then follows.
