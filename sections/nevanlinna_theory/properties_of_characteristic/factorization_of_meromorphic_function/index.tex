...
\begin{proof}
    ...
    Since \(\abs{\flatfrac{z}{b_n}}<\flatfrac12\) for \(P_2^>\), \[\abs{\Log E_q\qty(\frac{z}{b_n})}\leq\sum_{k>q}^\infty\frac1{k}\abs{\frac{z}{b_n}}^{q+1}\leq\abs{\frac{z}{b_n}}^{q+1}\sum_{k>q}^\infty\frac1{k}\abs{\frac12}^{k-q-1}\leq cr^{q+1}\abs{b_n}^{-q-1},\] where \(c\) is a constant dependent only on \(q\) and \(r=\abs{z}\) (see). Thus,
    \begin{align*}
    ...
    and thus \(g\) is of order \(\rho'\leq\rho\). For \(q=0\), the assertion is direct from \[\log\abs{E_q\qty(\frac{z}{b_n})}\leq\log\qty(1+\abs{\frac{z}{b_n}})\le\abs{\frac{z}{b_n}}\implies\log\abs{P_2^{\leq}}=\order{r}.\] % figure out a better estimate maybe
    Similar logic then derives that \(g\) is of order \(incomplete\). There then exists an integer \(p\leq\rho'\) such that \(P_1\) converges and \(g(z)=\ee^{\varphi(z)}P_1(z)\) has the property that \(\varphi\) is a polynomial of degree \(g\leq\rho'\). Writing \(f=\flatfrac{g}{P_2}\) then yields the desired representation.
\end{proof}
