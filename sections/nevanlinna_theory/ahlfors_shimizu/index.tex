...
\begin{proof}
    ...
    \end{equation}
    Letting \(R=\abs{f(z)}=\sqrt{f(z)\overline{f(z)}}\) and adopting the elliptic notation \(\partial_z=\pdv*{z},\partial_{\overline{z}}=\pdv*{\overline{z}}\) etc.,
...
Define 
\begin{equation}
    m_0(r,a,f)=\frac1{2\uppi}\int_0^{2\uppi}\log\qty(\frac1{k\qty(f\qty(r\ee^{\ii\theta}),a)})\dd{\theta}=\frac1{2\uppi}\int_0^{2\uppi}\log\sqrt{1+\abs{F\qty(r\ee^{\ii\theta})^2}}\dd{\theta}
\end{equation}
Assume \(F(0)\neq\infty\), or when \(f(0)\neq a\), Applying \cref{eq:ahlforsshimizuderivation_regularcase} on the function \(F\), we have \[T_0(r,F)=N(r,F)+\frac1{2\uppi}\int_0^{2\uppi}\log\sqrt{1+\abs{F\qty(r\ee^{\ii\theta})^2}}\dd{\theta}-\frac1{2\uppi}\int_0^{2\uppi}\log\sqrt{1+\abs{F\qty(0)^2}}\dd{\theta},\]
which in terms of \(f\), gives \[T_0(r,f)=N(r,a,f)+m_0(r,a,f)-m_0(0,a,f).\] %make a theorem box
% give singular case?