...
Taking the limit \(\varepsilon\to 0^+\) hence gives \[\frac{1}{2\uppi}\int_0^{2\uppi}\nabla_{\vu{n}}{G\qty(\abs{f\qty(r\ee^{\ii\theta})})}r\dd{\theta}+n(r,f)=\frac1{\uppi}\int_0^{2\uppi}\int_0^r \frac{\abs{f'\qty(\rho\ee^{\ii\theta})}^2}{\qty(1+\abs{f'\qty(\rho\ee^{\ii\theta})}^2)^2}\rho\dd{\rho}\dd{\theta}.\]
