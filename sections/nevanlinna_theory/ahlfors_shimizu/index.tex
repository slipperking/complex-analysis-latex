...
where \(f^\sharp\) is the spherical derivative (\cref{def:sphericalderivative}). Let the expression on the right-hand side by denoted by \(A(r)\). We thus derive \[A(r)=\frac{r}{2\uppi}\int_0^{2\uppi}\pdv{r}\log\sqrt{1+\abs{f\qty(r\ee^{\ii\theta})}^2}\dd{\theta}+n(r,f).\]
By dividing by \(r\), changing variables, and integrating from 0 to \(r\), we have 
\begin{align*}
    \int_0^r\frac{A(t)\dd{t}}t&=\lim_{\varepsilon\to 0^+}\int_\varepsilon^r\frac{1}{2\uppi}\dv{t}\int_0^{2\uppi}\log\sqrt{1+\abs{f\qty(t\ee^{\ii\theta})}^2}\dd{\theta}\ddt+\int_\varepsilon^r\frac{n(0,f)}{t}\ddt\\
    ...
    &\quad+N(r,f)-n(0,f)\log r+\frac{1}{2\uppi}\int_0^{2\uppi}\log\sqrt{1+\abs{f\qty(r\ee^{\ii\theta})}^2}\dd{\theta}.
\end{align*}
The integral expression is written in its present form to ensure convergence. 