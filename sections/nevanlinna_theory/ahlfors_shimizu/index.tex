...
Taking the limit \(\varepsilon\to 0^+\) hence gives \[\frac{1}{2\uppi}\int_0^{2\uppi}\nabla_{\vu{n}}{G\qty(\abs{f\qty(r\ee^{\ii\theta})})}r\dd{\theta}+n(r,f)=\frac1{\uppi}\int_0^{2\uppi}\int_0^r \frac{\abs{f'\qty(\rho\ee^{\ii\theta})^2}}{\qty(1+\abs{f'\qty(\rho\ee^{\ii\theta})^2}^2)^2}\rho\dd{\rho}\dd{\theta}.\]
Let the expression on the right-hand side by denoted by \(A(r,f)\). We thus derive \[A(r,f)=\frac{r}{2\uppi}\int_0^{2\uppi}\pdv{r}\log\sqrt{1+\abs{f\qty(r\ee^{\ii\theta})}^2}\dd{\theta}+n(r,f).\]
...
\begin{align}
    m_0(r,F)&=\frac1{2\uppi}\mathop{\mathmakebox[\widthof{\(\int_{\uppi}\)}][l]{\int_0^{2\uppi}}}\log\sqrt{1+\abs{F\qty(r\ee^{\ii\theta})}^2}\dd{\theta}\nonumber\\
    &=m_0(r,a,f)=\frac1{2\uppi}\mathop{\mathmakebox[\widthof{\(\int^{\uppi}\)}][l]{\int_0^{2\uppi}}}\log\qty(\frac1{k\qty(f\qty(r\ee^{\ii\theta}),a)})\dd{\theta}\label{eq:ahlforsshimizuproximity}
\end{align}
...
\end{theorem}
\begin{proof}
    Observe that for \(z\in\partial D(0,r)\), \[\logp \abs{f(z)}\leq\log\sqrt{1+\abs{f(z)}^2}=\tfrac12\log\qty(1+\abs{f(z)}^2).\]
    Since \(1+x^2\leq2\max\qty{1,x^2}\) for any real \(x\), \[\tfrac12\log\qty(1+\abs{f(z)}^2)\leq\tfrac12\log2+\logp \abs{f(z)}.\] Integrating and adding \(N\), we have \[T(r,f)\leq m_0(r,f)+N(r,f)\leq\tfrac12\log 2+T(r,f).\]
...
\end{align*}
The surface area of a surface \(E\subseteq S\) is given by \[\int_E\dd{A}_S=\iint_{\symbf{\sigma}^{-1}(E)}\frac{\dd{u}\dd{v}}{\qty(\abs{w}^2+1)^2}.\]
For \(w=f(z)\), pulling back to \(z\) gives \[A(r)=\frac1{\uppi}\mathop{\mathmakebox[\widthof{\(\iint\)}][l]{\iint_{\symbf{\sigma}(f\qty(D(0,r)))}}}{\mathmakebox[\widthof{\(\iint_{\symbf{\sigma}(f\qty(D(0,r)))}\)}][l]{\dd{A}}}\] or the area of the image of \(f\) on the Riemann sphere, counted according to multiplicities (referring to overlaps when not univalent), divided by the area of the Riemann sphere.

This completes the geometric explanation for \cref{thm:nevanlinnafirstfundamentaltheoremahlforsshimizu}. This area is invariant under rotations of \(S\). In particular, we rotate the north pole \((0,0,1)\) corresponding to \(\infty\) to the point corresponding to \(a\).
