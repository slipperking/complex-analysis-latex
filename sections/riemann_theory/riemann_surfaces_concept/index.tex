...
The continuity of the inverse is essential. Consider the two topological spaces \(S^1=\cbraces{(x_1,x_2)\in\mathbb{R}^2}{x_1^2+x_2^2=1}\) and \([0,2\muppi)=\cbraces{t\in\mathbb{R}}{0\leq t<2\muppi}\) under the standard topology.

The function \(f:[0,2\muppi)\to S^1\) with \(f(t)=\qty(\cos(t),\sin(t))\) is indeed continuous, but the inverse \(f^{-1}\qty(x_1,x_2)\) is discontinuous at \(\qty(x_1,x_2)=(1,0)\).
...
Consider \(\log(z)=\log\abs{z}=\ii\arg(z)\) on \(\mathbb{C}^*\), where \(\arg\) is multi-valued. The most common solution to this problem is achieved by restricting the range of \(\arg\), say to \((-\muppi,\muppi]\), effectively selecting a branch of \(\log\), inducing a discontinuity along the branch cut (here, along \(\mathbb{R}_{\leq0}\)).
