...
The continuity of the inverse is essential. Consider the two topological spaces \(S^1=\cbraces{(x_1,x_2)\in\mathbb{R}^2}{x_1^2+x_2^2=1}\) and \([0,2\uppi)=\cbraces{t\in\mathbb{R}}{0\leq t<2\uppi}\) under the standard topology.
...
Consider \(\log(z)=\log\abs{z}=\ii\arg(z)\) on \(\mathbb{C}^*\), where \(\arg\) is multi-valued. The most common solution to this problem is achieved by restricting the range of \(\arg\), say to \((-\uppi,\uppi]\), effectively selecting a branch of \(\log\), inducing a discontinuity along the branch cut (here, along \(\mathbb{R}_{\leq0}\)).
...
A point that connects multiple branches or sheets is known as a \textit{branch point}. In the case of the above construction, the origin is a branch point. In general, a branch point does not need to connect \emph{all} sheets; if it connects \(n\) sheets, it is known as a branch point of order \(n-1\).

The set \(W=\cbraces{(z,w)}{w\in\mathbb{C},z=\ee^w}\) under the one chart \((W,\varphi)\) where \((z,w)\mapsto w\). Then the function \(F:W\to\mathbb{C}\) corresponding to the complex logarithm is simply \(\varphi\). We define the \textit{projection map} \(\pi:W\to\mathbb{C}\) via \((z,w)\mapsto z\), which simply recovers the original variable. Then we have the relationship (written slightly informally) \(F(\mathfrak{z})=\log\circ\pi(\mathfrak{z})\). Indeed, the most trivial method for defining a Riemann surface as the domain of a multi-valued function is the set of \(z,w\) points.

We now provide another definition for a potential Riemann surface for the logarithm. Let \(W=\cbraces{(r,\theta)\in\mathbb{R}^2}{r>0}\), with the chart \(\qty(W,\qty(r,\theta)\mapsto\log(r)+\ii\theta)\). Here the corresponding function is \(F:(r,\theta)\mapsto\log(r)+\ii\theta\), and the projection map is \(\pi(r,\theta)=r\ee^{\ii\theta}\). Once more, the relationship can be heuristically described by \(F(\mathfrak{z})=\log\circ\pi(\mathfrak{z})\), or more accurately by \(\log^{-1}\circ F(\mathfrak{z})=\pi(\mathfrak{z})\).

Similarly, a single-valued holomorphic function can be made injective by mapping to Riemann surfaces.
