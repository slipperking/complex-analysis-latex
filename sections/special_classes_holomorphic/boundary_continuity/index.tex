...
\begin{proof}
    ...
    \begin{proof}
        ...
        \begin{align*}
            \int_0^{\frac12}\frac{l_r^2}{\uppi r}\dd{r} & =&&\int_0^{\frac12}\frac{1}{\uppi r}\qty[\int_{-\theta_0(r)}^{\theta_0(r)}\abs{\varphi'\qty(1-r\ee^{\ii\theta})}r\dd{\theta}]^2\dd{r}\\
            &=&&\int_0^{\frac12}\frac{1}{\uppi r}\qty[\int_{-\theta_0(r)}^{\theta_0(r)}\abs{\varphi'\qty(1-r\ee^{\ii\theta})}\sqrt{r}\sqrt{r}\dd{\theta}]^2\dd{r}\\
            &\underset{\mathclap{\substack{\text{Cauchy}\\\text{Schwarz}}}}{\leq}&&\int_0^{\frac12}\frac{1}{\uppi r}\qty[\int_{-\theta_0(r)}^{\theta_0(r)}\abs{\varphi'\qty(1-r\ee^{\ii\theta})}^2 r\dd{\theta}]\qty[\int_{-\theta_0(r)}^{\theta_0(r)}r\dd{\theta}]\dd{r}\\
    ...
    \begin{proof}
        ...
        \begin{equation}
            \operatorname{area}W_j\leq\operatorname{area}D_j=\uppi\qty(l_{r_j}+\eta\qty(l_{r_j}))^2\to0\qq{as}j\to\infty.\label{eq:osgoodtaylorcaratheodory_vanishingareasanddiameters_areaestimation2}
