...
\begin{proof}
    ...
    Let \(\widetilde{\varphi}\) be a conformal mapping from \(\extcomplex\setminus K\) to \(\mathbb{D}\), such that \(\widetilde{\varphi}(\infty)=0\) and \(\alpha=\widetilde{\varphi}'(\infty)\in\mathbb{R}_{>0}\). Let \(\varphi(z)=\frac{1}{\alpha}\widetilde{\varphi}(z)\). It follows that \(\varphi'(\infty)=1\), \(\varphi(\infty)=0\). By \cref{prop:complementbiholomorphismquarterestimate}, \[\abs{\alpha}\geq\frac{1}{4}\diam(K)\Longleftrightarrow\abs{\varphi(z)}\leq\frac{4\abs{\widetilde{\varphi}(z)}}{\diam(K)}.\]
    Consequently, we have the crucial estimate of \(\varphi\qty(\extcomplex\setminus K)\subseteq D\qty(0,\frac{4}{\diam(K)})\subseteq D\qty(0,\frac{8}{r})\). For fixed \(\zeta\in D(0,r)\), define \[\phi_\zeta(z)=\varphi(z)+(\zeta-\beta)\varphi^2(z),\qquad z\in\extcomplex\setminus K\]
...
\begin{proposition}\label{prop:diracdeltaapproximation}
    ...
    \begin{equation}
        ...
        \begin{dcases}
            ...
            0                   & \qif*\abs{z}\ge1,
        \end{dcases}\qquad\lambda_r(z)=\frac{3}{\uppi r^2}\lambda\qty(\frac{z}{r})\quad\forall r>0\label{eq:diracdeltaapproximation_lambdadefinition}
    \end{equation}
    ...
    \begin{enumerate}
        ...
        \item \(\int_{\mathbb{C}}\abs{\pdv{\lambda_r}{\overline{\zeta}}}\dd{\xi}\wedge\dd{\eta}\leq\frac{2\uppi}{r}\).\label{itm:diracdeltaapproximation_absoluteantiholomorphicderivativeintegral}
...
\begin{proof}
    ...
    \begin{align*}
        \iint_{\mathbb{C}}\lambda_r(\zeta)\dd{\xi}\dd{\eta} & =\int_{0}^{2\uppi}\int_0^r\lambda_r\qty(\rho\ee^{\ii\theta})\rho\dd{\rho}\dd{\theta}=\int_{0}^{2\uppi}\int_0^r\frac{3\rho}{\uppi r^2}\qty(1-\qty(\frac{\rho}{r})^2)^2\dd{\rho}\dd{\theta} \\
    ...
    \begin{align*}
        \iint_{D(0,R)}f(z-\zeta)\lambda_r(\zeta)\dd{\xi}\dd{\eta} & =\int_0^r\frac{3\rho}{\uppi r^2}\qty(1-\qty(\frac{\rho}{r})^2)^2\int_0^{2\uppi}f\qty(z-\rho\ee^{\ii\theta})\dd{\theta}\dd{\rho} \\
        & =2\uppi f(z)\int_0^r\frac{3\rho}{\uppi r^2}\qty(1-\qty(\frac{\rho}{r})^2)^2\dd{\rho}=f(z)
    ...
    Hence, \[\norm{\grad\lambda_r(z)}=\frac{3}{\uppi
        r^2}\norm{\grad\qty[\lambda\qty(\frac{z}{r})]}=\frac{3}{\uppi
        r^2}\norm{(\grad\lambda)\qty(\frac{z}{r})}\frac{1}{r}=\frac{12}{\uppi
    r^3}\qty(1-\abs{z}^2)\abs{z}<\frac{4}{r^3},\] which confirms \cref{itm:diracdeltaapproximation_gradientstatement}. Since \(\abs{\pdv{\lambda_r}{\overline{\zeta}}}=\abs{\frac{1}{2}\qty(\pdv{\lambda_r}{\xi}+\ii\pdv{\lambda_r}{\eta})}=\frac{1}{2}\norm{\grad\lambda_r(\zeta)}<\frac{2}{r^3}\), we have \[\iint_{\mathbb{C}}\abs{\pdv{\lambda_r}{\overline{\zeta}}}\dd{\xi}\dd{\eta}=\iint_{D(0,r)}\abs{\pdv{\lambda_r}{\overline{\zeta}}}\dd{\xi}\dd{\eta}<\uppi r^2\frac{2}{r^3}=\frac{2\uppi}{r}\] since \(\supp\qty(\lambda_r)=\overline{D(0,r)}\) which verifies the inequality in \cref{itm:diracdeltaapproximation_absoluteantiholomorphicderivativeintegral}.
...
\begin{proof}
    ...
    Let \(\textstyle g_0(z)=\frac13\eta_{A_0,B_0}(z)\). It is clear that \(\textstyle \abs{g_0}\leq\frac13\) on \(\mathbb{C}\). If \(z\in A_0\), then \(\textstyle -1\leq f(z)\leq-\frac13\), \(\textstyle g_0(z)=-\frac13\), and hence \(\textstyle \abs{f-g_0}\leq\frac23\). If \(z\in B_0\), then \(\textstyle \frac13\leq f(z)\leq1\), \(\textstyle g_0(z)=\frac13\), and thus \(\textstyle \abs{f-g_0}\leq\frac23\). If \(z\notin A_0\cup B_0\), then \(\textstyle -\frac13<f(z)<\frac13\) and \(\textstyle \abs{f-g_0}\leq\abs{f}+\abs{g_0}<\frac13+\frac13=\frac23\). Thus, \(\forall z\in K\), \[\abs{f(z)-g_0(z)}\leq\frac23.\]
...
\begin{proposition}\label{prop:integralofcontinuousextensionofholomorphicproperties}
    ...
    \begin{enumerate}
        \item \(\Phi\in C^1(\mathbb{C})\) and has compact support.\label{itm:integralofcontinuousextensionofholomorphicproperties_continuousdifferentiabilitycompactsupport}
        ...
        \item For all \(z\in\mathbb{C}\), \(\abs{\pdv{\Phi}{\overline{z}}(z)}\leq\frac{4\uppi\omega_f(r)}{r}\).\label{itm:integralofcontinuousextensionofholomorphicproperties_antiholomorphicderivativebound}
        \item \(\Phi(z)=-\frac1\uppi\iint_H\pdv{\Phi}{\overline{\zeta}}(\zeta)\frac{\dd{\xi}\dd{\eta}}{\zeta-z}\) for \(z\in\mathbb{C}\), where \(H=\supp(\Phi)\setminus U\).\label{itm:integralofcontinuousextensionofholomorphicproperties_integralformula}
    \end{enumerate}
\end{proposition}
\begin{proof}
    ...
    \begin{align*}
        ...
        & \leq\frac{4\omega_f(r)}{r^3}\iint_{D(0,r)}\dd{\xi}\dd{\eta}\leq\frac{4\omega_f(r)}{r^3}\cdot\uppi r^2=\frac{4\uppi\omega_f(r)}{r},
    \end{align*}
...
\end{theorem}
\begin{proof}
    \import{build/visual_output/mergelyan_erosion}{imports.tex}
    \begin{figure}
        \centering
        \begin{tikzpicture}
            ...
            \draw[thin, |-|, line cap=round, shorten >=1pt] (1.97,-1.16) -- (1.93,-0.775) node[midway, anchor=east] {\small\(r\)};
            \draw[thin, |-|, line cap=round, shorten >=1pt] (-0.4,0.045) -- (-0.18,0.36) node[midway, anchor=north west] {\small\(r\)};
    ...
    \begin{figure}
        ...
        \begin{minipage}{0.48\textwidth}
            ...
            \begin{tikzpicture}
                ...
                \draw[thick, solid] (0,0) node[point, label={below:{\(p_k\)}}] {} -- (-0.5,1) -- (0.25,1.5) -- (1,0.8) -- (0.9,0.1) node[anchor=west] {\(\gamma_{k}^{(j)}\)} -- (0.6,-0.4) -- (1.7,-0.7) -- ({1.7*cos(-55)},{1.7*sin(-55)}) -- ({2*cos(-55)},{2*sin(-55)});
                \draw[thick, dashed] ({2*cos(-55)},{2*sin(-55)}) node[point] {} -- ({2.5*cos(-55)},{2.5*sin(-55)}) -- (0.2,-1.6) -- (-0.4,-1.7) -- (-1.7,-1.9) node[point,label={left:{\(\zeta^{(j)}_k\)}}] {};
            \end{tikzpicture}
        ...
        \begin{minipage}{0.48\textwidth}
            ...
            \end{tikzpicture}
            \caption{A conceptual construction of \(\qty{H_k^{(j)}}\), which unions to \(H\).}\label{fig:mergelyan_hkjsets}
        \end{minipage}
    ...
    Thus the union \[\bigcup_{k=1}^n\bigcup_{j=1}^{m_k}H_k^{(j)}=H\cap\qty(\bigcup D\qty(\zeta_k^{(j)},\frac54r))=H\] since the set of all \(D\qty(\zeta_k^{(j)},\frac54r)\) covers \(H\). Let \[\Psi(z)=\frac{1}{\uppi}\sum_{k=1}^n\sum_{j=1}^{m_k}\int_{H_k^{(j)}}\pdv{\Phi}{\overline{\zeta}}\phi_{\zeta,k}^{(j)}(z)\dd{\xi}\wedge\dd{\eta}\qq{where}\zeta=\xi+\ii\eta,\quad\forall z\notin\bigcup E_k^{(j)}.\]
    Because \[\frac{\Psi(z+\Delta x)-\Psi(z)}{\Delta x}=\frac{1}{\uppi}\sum_{k=1}^n\sum_{j=1}^{m_k}\int_{H_k^{(j)}}\pdv{\Phi}{\overline{\zeta}}\frac{\phi_{\zeta,k}^{(j)}(z+\Delta x)-\phi_{\zeta,k}^{(j)}(z)}{\Delta x}\dd{\xi}\wedge\dd{\eta},\]
    and both \(\pdv{\Phi}{\overline{\zeta}}\) and the integrand is continuous on a set (we only need to consider the factors involving \(\phi_{\zeta,k}^{(j)}\) since \(\pdv{\Phi}{\overline{\zeta}}\) is independent from \(z\)) by Cauchy's Estimates and the first bound of \cref{eq:mergelyan_familybounds}, Lebesgue's Dominate Convergence gives that
    \[\pdv{\Psi}{x}=\frac{1}{\uppi}\sum_{k=1}^n\sum_{j=1}^{m_k}\int_{H_k^{(j)}}\pdv{\Phi}{\overline{\zeta}}\pdv{\phi_{\zeta,k}^{(j)}}{x}\qty(z)\dd{\xi}\wedge\dd{\eta},\] and in analogous fashion, \[\pdv{\Psi}{y}=\frac{1}{\uppi}\sum_{k=1}^n\sum_{j=1}^{m_k}\int_{H_k^{(j)}}\pdv{\Phi}{\overline{\zeta}}\pdv{\phi_{\zeta,k}^{(j)}}{y}\qty(z)\dd{\xi}\wedge\dd{\eta}.\]
    ...
    \begin{align*}
        \abs{\Psi(z)-\Phi(z)} & =\abs{\frac{1}{\uppi}\sum_{k=1}^n\sum_{j=1}^{m_k}\iint_{H_k^{(j)}}\pdv{\Phi}{\overline{\zeta}}\phi_{\zeta,k}^{(j)}(z)\dd{\xi}\dd{\eta}-\frac1\uppi\iint_H\pdv{\Phi}{\overline{\zeta}}\frac{\dd{\xi}\dd{\eta}}{z-\zeta}} \\
        & =\frac1{\uppi}\abs{\sum_{k=1}^n\sum_{j=1}^{m_k}\iint_{H_k^{(j)}}\pdv{\Phi}{\overline{\zeta}}\qty(\phi_{\zeta,k}^{(j)}(z)-\frac{1}{z-\zeta})\dd{\xi}\dd{\eta}}                                                           \\
        & \leq\frac1{\uppi}\sum_{k=1}^n\sum_{j=1}^{m_k}\iint_{H_k^{(j)}}\abs{\pdv{\Phi}{\overline{\zeta}}}\abs{\phi_{\zeta,k}^{(j)}(z)-\frac{1}{z-\zeta}}\dd{\xi}\dd{\eta}                                                        \\
        & \leq\frac{1}{\uppi}\sum_{k=1}^n\sum_{j=1}^{m_k}\qty(\iint_{H_k^{(j)}\cap D(z,2r)}+\iint_{H_k^{(j)}\setminus D(z,2r)})\abs{\pdv{\Phi}{\overline{\zeta}}}\abs{\phi_{\zeta,k}^{(j)}(z)-\frac{1}{z-\zeta}}\dd{\xi}\dd{\eta}
    \end{align*}
    ...
    \begin{multline*}
        18704r\omega_f(r)\int_0^{2\uppi}\int_{2r}^\infty\frac{\dd{\rho}\dd{\theta}}{\rho^2}+\frac{4\omega_f(r)}{r}\qty(\frac{584}{r}\operatorname{area}D(0,2r)+\int_0^{2\uppi}\int_0^{2r}\dd{\rho}\dd{\theta})\\
        =18704r\omega_f(r)2\uppi\eval{\frac{1}{\rho}}_{\infty}^{2r}+\frac{9344\uppi\omega_f(r)r^2}{r^2}+\frac{4\omega_f(r)}{r}4r\uppi\\
        =18704\uppi\omega_f(r)+9344\uppi\omega_f(r)+16\uppi\omega_f(r)=28064\uppi\omega_f(r).
    \end{multline*}
    Runge's Theorem (\cref{thm:runge}) provides the existence of some rational function \(\psi\) with poles in \(E\) such that \[\sup_{z\in K}\abs{\psi(z)-\Psi(z)}\leq\uppi\omega_f(r)\] since \(\Psi\) is holomorphic on a neighborhood of \(K\) (to assure this bound is positive, we assume \(f\) is not identically zero, otherwise the assertion is trivial). Therefore, for all \(z\in K\), we have (from \cref{itm:integralofcontinuousextensionofholomorphicproperties_differbymodulusofcontinuity} of \cref{prop:integralofcontinuousextensionofholomorphicproperties})
    \begin{align*}
        \sup_{z\in K}\abs{\psi(z)-f(z)}&\leq\sup_{z\in K}\abs{\psi(z)-\Psi(z)}+\abs{\Psi(z)-\Phi(z)}+\abs{\Phi(z)-f(z)}\\
        &\leq 28065\uppi\omega_f(r) + \omega_f(r)=28066\uppi\omega_f(r).
    \end{align*} Because \(\lim_{r\to0^+}\omega_f(r)=0\), for any \(\varepsilon>0\), there exists a \(r>0\) such that \[\omega_f(r)<\frac{\varepsilon}{28066\uppi}.\]
    Hence for any such \(\varepsilon\), we now construct \(\psi\) in accordance with an \(r\) satisfying \(28066\uppi\omega_f(r)<\varepsilon\).
\end{proof}
