...
\begin{proof}
    \begin{figure}
        \centering
        \import{build/erosion_output/}{imports.tex}
        \begin{tikzpicture}
    ...
    Hence, the intersection \(\overline{D\qty(\zeta_k^{(j)},\frac34r)}\cap\gamma_{k}^{(j)}\) consists of at least one connected component joining \(\zeta_k^{(j)}\) to a point on \(\partial D\qty(\zeta_k^{(j)},\frac34 r)\). Denote this intersection by \(E_k^{(j)}\), satisfying \(\diam E_k^{(j)}\geq\frac34r>\frac r2\) and \(E_k^{(j)}\cap K=\varnothing\).
