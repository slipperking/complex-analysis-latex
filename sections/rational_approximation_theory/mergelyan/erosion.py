...
def write_raw_outputs(rings):
    folder = "erosion_output/eroded"
    ...
    for i, ring in enumerate(rings, start=1):
        ...
        macro = "\\ErosionOutput" + number_to_letters(i)
        macro_names.append(macro)

        coords = "\\def" + macro + "{" + " ".join(
            f"({x:.4f},{y:.4f})" for x, y in pts
        ) + "}"
    ...
    # Write the combined macro list file
    list_filename = os.path.join(folder, "all.tex")
    with open(list_filename, "w") as f:
        f.write("\\def\\ErosionOutputList{" + ",".join(macro_names) + "}")

    print(f"Wrote {len(rings)} rings and macro list to '{folder}'")


def write_original_inputs(curves):
    folder = "erosion_output/original"
    os.makedirs(folder, exist_ok=True)
    ...
    for i, c in enumerate(curves, start=1):
        pts = c["points"]
        macro = "\\ErosionInput" + number_to_letters(i)
        macro_names.append(macro)
    ...
    list_filename = os.path.join(folder, "all.tex")
    with open(list_filename, "w") as f:
        f.write("\\def\\ErosionInputList{" + ",".join(macro_names) + "}")

    print(f"Wrote {len(curves)} original curves and macro list to '{folder}'")

def write_metadata_tex(num_original, num_eroded):
    folder = "erosion_output"
    os.makedirs(folder, exist_ok=True)
    filename = os.path.join(folder, "imports.tex")

    lines = ["% THIS FILE IS AUTO_GENERATED.", "% DO NOT EDIT THIS FILE.", ""]
    lines.append(f"\\subimport{{eroded/}}{{all.tex}}")
    lines.append(f"\\subimport{{original/}}{{all.tex}}")
    for i in range(1, num_original + 1):
        lines.append(f"\\subimport{{original/}}{{{i}.tex}}")
    for i in range(1, num_eroded + 1):
        lines.append(f"\\subimport{{eroded/}}{{{i}.tex}}")

    with open(filename, "w") as f:
        f.write("\n".join(lines) + "\n")

    print(f"Wrote imports file to '{filename}'")
...
if __name__ == "__main__":
    ...
    write_metadata_tex(len(curves), len(rings))