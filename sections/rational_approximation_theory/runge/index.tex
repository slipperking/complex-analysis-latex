...
\begin{proof}
    ...
    \begin{equation}
        \frac{1}{2\muppi\ii}\oint_{\partial\widetilde{K}}\frac{f(\zeta)\ddzeta}{\zeta-z}=f(z)\label{eq:rungesimplepolesandremovablesingularityatinfinity_cauchygoursat}
    \end{equation} in the case that \(z\in\widetilde{K}\). The boundary \(\partial\widetilde{K}\) may be written as the union of \(n\) lines parameterized by \(0\leq t\leq 1\); more concretely, we have \(\partial\widetilde{K}=\bigcup_{j\in\mathbb{N}_{\leq n}}\gamma_j([0,1])\). Hence we have in equivalent formulation, \[f(z)=\frac{1}{2\muppi\ii}\sum_{j=1}^n\int_{\gamma_j([0,1])}\frac{f(\zeta)}{\zeta-z}\ddzeta=\frac{1}{2\muppi\ii}\sum_{j=1}^n\int_0^1\frac{f\qty(\gamma_j(t))\gamma'_j(t)}{\gamma_j(t)-z}\ddt.\]
    ...
    \begin{multline*}
        \abs{f(z)-\frac{1}{2\muppi\ii}\sum_{j=1}^n\sum_{k=0}^{m-1}\frac{f\qty(\gamma_j\qty(t_k))\gamma'_j\qty(t_k)}{\gamma_j\qty(t_k)-z}\Delta t_k}\\
        =\frac{1}{2\muppi}\abs{\sum_{j=1}^n\sum_{k=0}^{m-1}\int_{t_k}^{t_{k+1}}\qty[\frac{f\qty(\gamma_j\qty(t))\gamma'_j\qty(t)}{\gamma_j\qty(t)-z}-\frac{f\qty(\gamma_j\qty(t_k))\gamma'_j\qty(t_k)}{\gamma_j\qty(t_k)-z}]\ddt}\\
        \leq\frac{1}{2\muppi}\sum_{j=1}^n\sum_{k=0}^{m-1}\int_{t_k}^{t_{k+1}}\abs{\frac{f\qty(\gamma_j\qty(t))\gamma'_j\qty(t)}{\gamma_j\qty(t)-z}-\frac{f\qty(\gamma_j\qty(t_k))\gamma'_j\qty(t_k)}{\gamma_j\qty(t_k)-z}}\ddt\\
        \leq\frac{\varepsilon}{2n\muppi}\sum_{j=1}^n\sum_{k=0}^{m-1}\Delta t_k=\frac{\varepsilon}{2\muppi}<\varepsilon
    \end{multline*} uniformly in \(z\in K\). The summation \[\psi(z)=\frac{1}{2\muppi\ii}\sum_{j=1}^n\sum_{k=0}^{m-1}\frac{f\qty(\gamma_j\qty(t_k))\gamma'_j\qty(t_k)}{\gamma_j\qty(t_k)-z}\Delta t_k\] defines a rational function with simple poles at each \(\gamma_j\qty(t_k)\in\partial\widetilde{K}\), which is disjoint from \(K\).
\end{proof}
