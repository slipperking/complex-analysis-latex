...
\begin{proposition}\label{prop:homogeneouspolynomialderivatives}
    ...
    \begin{enumerate}
        \item For any multi-index \(\symbf{\alpha}=(\alpha_1,\ldots,\alpha_n)\) with \(\norm{\symbf{\alpha}}=r\leq d\), \[\partial^{\symbf{\alpha}}\symbf{\psi}(\symbf{z})=\frac{\partial^r\symbf{\psi}}{\partial z_1^{\alpha_1}\cdots\partial z_n^{\alpha_n}}(\symbf{z})\] is a homogeneous polynomial of degree \(d-r\).\label{itm:homogeneouspolynomialderivatives_less}
        \item If \(r=d\neq 0\), then \(\partial^{\symbf{\alpha}}\symbf{\psi}\) is constant (and there exists a multi-index \(\symbf{\alpha}\) with \(\norm{\symbf{\alpha}}=d\) such that \(\partial^{\symbf{\alpha}}\symbf{\psi}\) is nonzero).\label{itm:homogeneouspolynomialderivatives_equality}
...
\begin{proof}
    ...
    \[(k_1-\alpha_1)+\cdots+(k_n-\alpha_n)=d-\norm{\symbf{\alpha}}.\]
    Hence, \(\partial^{\symbf{\alpha}}\symbf{\psi}\) is a homogeneous polynomial of degree \(d-\norm{\symbf{\alpha}}\), establishing \cref{itm:homogeneouspolynomialderivatives_less}.
...
\end{theorem}
\begin{proof}
    Let \(\symbf{f}\in\Aut\qty(B^n)\) be arbitrary, and set \(\symbf{\alpha}=\symbf{f}(\symbf{0})\). Then there exists a unitary matrix \(\symbf{U}\) such that \(\symbf{U}\symbf{\alpha}=\qty(\norm{\symbf{\alpha}},0,\ldots,0)\).

    Now let \(\symbf{\varphi}_{\norm{\symbf{\alpha}}}\) be as in \cref{prop:unitballsimpleautomorphism}, mapping \(\qty(\norm{\symbf{\alpha}},0,\ldots,0)\) to \(\symbf{0}\). Then, the map \(\symbf{\varphi}_{\norm{\symbf{\alpha}}}\circ\symbf{U}\symbf{f}\in\Aut\qty(B^n)\) fixes \(\symbf{0}\), so by \cref{prop:unitballautomorphismfixedpointatzero} it is a unitary transformation, say \(\symbf{V}\). Therefore, \[\symbf{\varphi}_{\norm{\symbf{\alpha}}}\circ\symbf{U}\symbf{f}\equiv\symbf{V}\implies\symbf{f}(\symbf{z})\equiv\symbf{U}^{-1}\symbf{\varphi}_{\norm{\symbf{\alpha}}}^{-1}\circ\symbf{Vz}.\] The converse is trivial.
\end{proof}