...
\end{proposition}
\begin{proof}
    Let \(\vb{w}=\qty(w_1,\dots,w_n)\in\Omega\) with each \(w_j\neq0\). Since \(\Omega\) is a Reinhardt domain, for every \(\vb*{\theta}=\qty(\theta_1,\dots,\theta_n)\in[-\muppi,\muppi]^n\) the point
    ...
    \[a_{\mathbf{k}}=\frac{1}{\qty(2\muppi\ii)^n}\int_K\frac{f(\vb{z})}{\vb{z}^{\vb{k}+\qty(1,\dots,1)}}\dd{\vb{z}}=\frac{\vb{w}^{-\vb{k}}}{\qty(2\muppi)^n}\mathmakebox[\widthof{\(\int_{-\muppi}\)}][l]{\int_{[-\muppi,\muppi]^n}}
