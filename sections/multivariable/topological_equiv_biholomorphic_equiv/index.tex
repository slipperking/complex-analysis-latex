...
\begin{proof}
    ...
    The definition of \(\symbf{\Phi}\) ensures that \(\symbf{\Phi}:\mathbb{D}^n\to
    B^n\) and \(\symbf{\Phi}(\symbf{0})=\symbf{0}\). Then
    \(\symbf{\Phi}^{-1}\circ\Aut\qty(B^n)\circ\symbf{\Phi}\) consists of functions
    mapping \(\mathbb{D}^n\) to \(\mathbb{D}^n\), or that \[\symbf{\Phi}^{-1}\circ\Aut\qty(B^n)\circ\symbf{\Phi}\subseteq\Aut\qty(\mathbb{D}^n)\implies\Aut\qty(B^n)\subseteq\symbf{\Phi}\circ\Aut\qty(\mathbb{D}^n)\circ\symbf{\Phi}^{-1}.\]
    Similarly,
    \(\symbf{\Phi}\circ\Aut\qty(\mathbb{D}^n)\circ\symbf{\Phi}^{-1}\subseteq\Aut\qty(B^n)\).
    Hence,
    \(\Aut\qty(B^n)=\symbf{\Phi}\circ\Aut\qty(\mathbb{D}^n)\circ\symbf{\Phi}^{-1}\),
    and
    \begin{equation}
    ...
    \(\Aut'\qty(B^n)\cong\mathrm{U}(n)\), the \(n\times n\) \textit{unitary
    group}.

    For \(\symbf{U}\in\mathrm{U}(n)\), the spectral theorem allows it to be expressed
    in the form of
    \(\symbf{V}\operatorname{diag}\qty(\ee^{\ii\theta_1},\ldots,\ee^{\ii\theta_n})\symbf{V}^\dagger\).
    ...
    the entire expression then simplifies to \(\symbf{U}\). Hence the unitary group is
    divisible.

    Consider the unitary monomial matrix \[\symbf{P}_\tau=\mqty(0&1&0&0&\cdots&0\\1&0&0&0&\cdots&0\\0&0&1&0&\cdots&0\\0&0&0&1&\cdots&0\\\vdots&\vdots&\vdots&\vdots&\ddots&\vdots\\0&0&0&0&\cdots&1).\] inducing the permutation \(\tau\), swapping the first and second entries.
    Assume that there exists some unitary monomial matrix
    \(\symbf{Q}=\symbf{P}_\sigma\symbf{D}\) (where \(\symbf{D}\) is diagonal and
        \(\symbf{P}_\sigma\) is a permutation matrix corresponding to the permutation
    \(\sigma\)) such that \(\symbf{Q}^2=\symbf{P}_\tau\). This is equivalent to \[\symbf{P}_\sigma\symbf{D}\symbf{P}_\sigma\symbf{D}=\symbf{P}_\tau\implies\symbf{P}^2_\sigma\symbf{P}^{-1}_\sigma\symbf{D}\symbf{P}_\sigma\symbf{D}=\symbf{P}_\tau\implies\symbf{P}^2_\sigma\symbf{D}^2=\symbf{P}_\tau\]
    because \(\symbf{P}^{-1}_\sigma\symbf{D}\symbf{P}_\sigma=\symbf{D}\). Thus,
    \(\symbf{P}^2_\sigma=\symbf{P}_\tau\) (and \(\symbf{D}^2=\symbf{I}\)) since their
    permutation parts must match. This is an impossibility since
    \(\symbf{P}^2_\sigma\) corresponds to an even permutation, while \(\symbf{P}_\tau\)
    ...
    biholomorphism \(\symbf{\varphi}\) exists.
\end{proof}
\begin{remark}
    ...
    \begin{quote}
        ...
        For each permutation \(\sigma\in S_n\) (the \textit{symmetric group} of
        permutations) let \(\symbf{P}_\sigma\) be the corresponding permutation matrix and
        define
        ...
        spectral theorem) into \(\symbf{VD}\symbf{V}^\dagger\), where \[\symbf{D}=\operatorname{diag}\qty(\ee^{\ii\theta_1},\ldots,\ee^{\ii\theta_n})\] is a diagonal unitary matrix and \(\symbf{V}\in\mathrm{U}(n)\). Then there exists
        a connected path connecting \(\qty(\theta_1,\dots,\theta_n)\) to
        \(\qty(0,\ldots,0)\), which corresponds to the matrix
        \(\symbf{VI}\symbf{V}^\dagger=\symbf{I}\). Because every matrix is path-connected to the
    ...
    their natural topologies (the \textit{compact-open topology}) by
    \(\symbf{\psi}\mapsto\symbf{\Phi}\circ\symbf{\psi}\circ\symbf{\Phi}^{-1}\). Hence any
