...
\begin{lemma}\label{lem:monomial_matrix_diagonal_multiplication_commute}
    If \(\symbf{P}\) is a permutation matrix, and \(\symbf{D}\) is a diagonal matrix, then there exists a diagonal matrix \(\symbf{D}'\) such that
    \(\symbf{P}\,\symbf{D}=\symbf{D}'\,\symbf{P}\). Similarly, there exists a diagonal matrix \(\symbf{D}''\) such that
    \(\symbf{D}\,\symbf{P}=\symbf{P}\,\symbf{D}''\).
\end{lemma}
\begin{proof}
    Let \(\symbf{D}=\operatorname{diag}(d_1,\dots,d_n)\) and let \(\sigma\) be the permutation corresponding to \(\symbf{P}\), so that \(\symbf{P}\symbf{e}_i=\symbf{e}_{\sigma(i)}\) for each standard basis vector \(\symbf{e}_i\).

    Define \[\symbf{D}'=\operatorname{diag}\bigl(d_{\sigma^{-1}(1)},\dots,d_{\sigma^{-1}(n)}\bigr).\]
    For every \(i\) we have
    \[\symbf{P}\symbf{D}\symbf{e}_i=\symbf{P}(d_i\symbf{e}_i)=d_i\symbf{e}_{\sigma(i)},\]
    while
    \[\symbf{D}'\symbf{P}\symbf{e}_i=\symbf{D}'\symbf{e}_{\sigma(i)}=d_i\symbf{e}_{\sigma(i)}.\]
    Hence \(\symbf{P}\symbf{D}\symbf{e}_i=\symbf{D}'\symbf{P}\symbf{e}_i\) for all \(i\), and therefore
    \(\symbf{P}\symbf{D}=\symbf{D}'\symbf{P}\).

    To obtain the second equality, apply the same argument to \(\symbf{P}^\top\). There exists a diagonal matrix \(\symbf{D}''\) with \(\symbf{P}^\top\symbf{D}=(\symbf{D}''\symbf{P}^\top)\). Taking transposes and using that diagonal matrices are invariant under transposition yields \(\symbf{D}\symbf{P}=\symbf{P}\symbf{D}''\), as required.
\end{proof}
