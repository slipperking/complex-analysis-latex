...
The methods of differential geometry can also be used to prove the statements of the following theorems (which can also be independently used to prove the Picard theorems).
...
\end{theorem}
\begin{proof}
    For \(w\in f(\mathbb{D})\), let \(\phi(w)\) denote the radius of the largest schlicht disk in \(f(\mathbb{D})\) centered at \(w\) (it is mapped to univalently by \(f\) on some subdomain). Trivially, \(\phi\) is \(C^0\) and vanishes at points where \(f'=0\) and towards the boundary of \(f(\mathbb{D})\).
    ...
    At a point (an \textit{algebraic} branch point) where \(f'=0\) corresponding to some \(a\in f(\mathbb{D})\), \(f-a\) has a zero of multiplicity \(n\geq 2\). On a neighborhood of \(a\), we have \(\phi(w)=\abs{w-a}\) (since \(f'=0\) is isolated in the interior of \(f(\mathbb{D})\)). Hence in this neighborhood, letting \(w_1=(w-a)^{\frac1n}\), the metric has equivalent form \(\dd{s}=\frac{n\phi(w)^{\frac12-\frac1n}}{A^2-\phi(w)}\abs{\dd{w_1}}\).

    For every non-singular point \(w_0\), the bounding circle corresponding to \(\phi\qty(w_0)\) passes through a (at least one) branch point or a boundary, denoted by \(b=b_{w_0}\). Let \(\phi_{w_0}(w)=\abs{w-b}\) and let \[\rho_{w_0}(w)=\frac{A}{\sqrt{\phi_{w_0}(w)}\qty(A^2-\phi_{w_0}(w))},\qquad w\in D\qty(w_0,\phi(w_0)).\]
