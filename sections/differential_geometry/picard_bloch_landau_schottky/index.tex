...
\begin{proof}
    ...
    By the definition of \(\phi\), we have \(\phi_{w_0}\geq\phi\) everywhere in this neighborhood. Since \(\rho_{w_0}(w)=\frac{A}{\phi'\qty(A^2-\phi^2)}=\qty(\sqrt{\phi_{w_0}(w)})^*\qty(\phi'\mapsto\frac{2A}{\qty(A^2-\phi^2)})\) is the pullback of the hyperbolic metric in \cref{eq:poincaremetricscaledcurvature}, the metric \(\rho_{w_0}(w)\) has the constant negative curvature of \(-1\). 

    Our goal is to construct \(\rho_{w_0}\) so that it is the function of a supporting metric for \(\rho\) (satisfies the criteria for \cref{thm:schwarzahlforspicksupportingmetric}). For \(\rho_{w_0}\leq\rho\) to be satisfied, we consider \[\sqrt{\phi_{w_0}(w)}\qty(A^2-\phi_{w_0}(w))\geq\sqrt{\phi(w)}\qty(A^2-\phi(w)),\qquad\phi_{w_0}(w)\geq\phi(w).\] In particular, we want \[t\mapsto\sqrt{t}\qty(A^2-t)\] to be increasing 
\end{proof}
\begin{remark}
