...
\begin{proof}
    ...
    Define the metric \[\rho(w)=\frac{A}{\sqrt{\phi(w)}\qty(A^2-\phi(w))},\qquad\dd{s}=\rho(w)\abs{\dd{w}}\] for \(w\in f(\mathbb{D})\) and \(\rho(w)\neq 0\), where \(A^2>\sup_w\phi(w)\) is a constant. We may assume that \(\sup_w\phi(w)\) is finite, since otherwise the theorem is already proved for \(f\).
    ...
    By the definition of \(\phi\), we have \(\phi_{w_0}\geq\phi\) everywhere in this neighborhood. Since \(\rho_{w_0}=\qty(\phi_{w_0}\mapsto\sqrt{\phi_{w_0}})^*\qty(\phi\mapsto\frac{2A}{\qty(A^2-\phi^2)})\) is the pullback of the hyperbolic metric in \cref{eq:poincaremetricscaledcurvature}, the metric \(\rho_{w_0}(w)\) has the constant negative curvature of \(-1\). 
