...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item Consider \(f(z)=\infty\). Assume, for the sake of contradiction, \(z\) is an
            ...
            continuity, \(\exists\delta>0\) such that \[d_\sigma\qty(f(\zeta),\infty)<\frac\uppi2\qquad\forall\zeta\in D(z,\delta).\] Similarly, \(\exists N\in\mathbb{N}\) such that \(\forall n>N\), \[d_\sigma\qty(f(\zeta),f_n(\zeta))<\frac\uppi2.\]
            Hence, we have \[\uppi-d_\sigma\qty(\infty,f_n(\zeta))=d_\sigma(\infty,0)-d_\sigma\qty(\infty,f_n(\zeta))>0.\] By the reverse triangle inequality, we have \[d_\sigma\qty(0,f_n(\zeta))>0.\]
...
\begin{proof}
    ...
    is \textit{uniformly spherically equicontinuous}. Since \(d_\sigma\leq\uppi\)
    ...
    \begin{enumerate}
        ...
        \item \(f\qty(z)=\infty\), then by continuity, \(\exists\delta>0\) such that \(\forall\zeta\in D(z,\delta)\), \[d_\sigma\qty(f(\zeta),\infty)<\frac\uppi2.\] Similarly, \(\exists N\in\mathbb{N}\) such that \(\forall k>N\), \[d_\sigma\qty(f(\zeta),f_{n_k}(\zeta))<\frac\uppi2.\]
            Hence, we have \[\uppi-d_\sigma\qty(\infty,f_{n_k}(\zeta))=d_\sigma(\infty,0)-d_\sigma\qty(\infty,f_{n_k}(\zeta))>0.\] By the reverse triangle inequality, we have \[d_\sigma\qty(0,f_{n_k}(\zeta))>0.\]
