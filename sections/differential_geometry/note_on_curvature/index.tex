...
The \textit{mean curvature} of \(\Sigma\) at \(P\) is defined to be \(H=\tfrac{\kappa_1+\kappa_2}{2}\). Let \(r_1,r_2\) be the radii of curvature corresponding to \(\kappa_1\) and \(\kappa_2\). Hence, we have \[H=\tfrac{\tfrac{1}{r_1}+\tfrac{1}{r_2}}{2}=\frac{r_1+r_2}{2r_1r_2}=\kappa_1\kappa_2\frac{r_1+r_2}{2}.\] The product of the two principal curvatures is known as the \textit{Gaussian curvature} of \(\Sigma\) at \(P\), denoted by \(K=\kappa_1\kappa_2\). We will now heuristically derive the explicit formulas for \(H\) and \(K\) in terms of \(E,F,G,L,M,N\).
...
Hence, the two roots \(\lambda_1,\lambda_2\) are precisely the principal curvatures. Vieta's formulas give that \[K=\lambda_1\lambda_2=\frac{LN-M^2}{EG-F^2},\quad H=\lambda_1+\lambda_2=\frac{EN+GL-2MF}{EG-F^2}.\]
Now, assume a parameterization of \(\Sigma\) by \(u,v\) such that \[\mathrm{I}(u,v)=\rho^2\dd{u}^2+\rho^2\dd{v}^2=\rho^2\qty(\dd{u}^2+\dd{v}^2)\] (which we will later formalize as a conformal metric). Then there is an alternate representation of the Gaussian curvature in terms of \(\rho\).
