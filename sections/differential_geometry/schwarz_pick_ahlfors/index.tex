...
\begin{theorem}\label{thm:schwarzahlforspicksupportingmetric}
    Let \(f:\mathbb{D}\to U\) be holomorphic. Let \(\rho:U\subseteq\mathbb{C}\to\mathbb{R}_{>0}\) be a continuous conformal metric (but not necessarily \(C^2\)) such that at each point \(w\), there exists a neighborhood \(V_w\supset w\) in \(U\) and a regular metric \(\rho_w\) thereon such that \(\rho_w(w)=\rho(w)\) and \(\rho_w\leq\rho\) everywhere else. If each \(K_{\rho_w}\leq-1\) everywhere, then the conclusion of \cref{thm:schwarzahlforspick} continues to hold.
\end{theorem}
\begin{proof}
