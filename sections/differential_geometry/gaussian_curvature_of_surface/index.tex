...
\begin{equation}
    \mathrm{I}=\dd{s}^2=E\dd{u}^2+2F\dd{u}\dd{v}+G\dd{v}^2,\label{eq:firstfundamentalform}
...
\begin{equation}
    \mathrm{I\!I}=L\dd{u}^2+2M\dd{u}\dd{v}+N\dd{v}^2,\label{eq:secondfundamentalform}
...
Suppose \(p\in\Sigma\). Adopt the matrix notation of \(\symbf{I}\), \(\symbf{I\!I}\) as in \[\symbf{I}=\mqty(E&F\\F&G),\qquad\symbf{I\!I}=\mqty(L&M\\M&N),\] to reduce to the optimization problem of \[\kappa_n=\frac{\va{v}^\top\symbf{I\!I}\va{v}}{\va{v}^\top\symbf{I}\va{v}},\qquad\va{v}\in T_p\Sigma.\]
We may restrict \(\va{v}=\qty(v_1,v_2)\) so that the denominator is always \(1\), aiming to optimize the numerator. By the method of Lagrange multipliers, we write \[\mathcal{L}\qty(\va{v},\lambda)=\va{v}^\top\symbf{I\!I}\va{v}-\lambda\qty(\va{v}^\top\symbf{I}\va{v}-1).\]
The equation \(\grad\mathcal{L}=\symbf{0}\) for \(\textstyle\grad{}=\qty(\pdv{v_1},\pdv{v_2},\pdv{\lambda})\) can then be decomposed into (where \(\va{v}=\qty(v_1,v_2)\)):
\begin{gather*}
    2Lv_1+2Mv_2-\lambda\qty(2Ev_1+2Fv_2)=0,\\
    2Mv_1+2Nv_2-\lambda\qty(2Fv_1+2Gv_2)=0,\\
    \qty(\va{v}^\top\symbf{I}\va{v}=1).
\end{gather*}
The first two equations can be written as
\begin{equation}
    \mqty(L-\lambda E&M-\lambda F\\M-\lambda F&N-\lambda G)\va{v}=\symbf{0}.\label{eq:gaussiancurvaturelambdarootsmatrixvectorvanish}
\end{equation}
Let the matrix on the left be denoted by \(\symbf{M}\). In order for non-trivial (\(\symbf{v}\neq\symbf{0}\)) to exist, we must have \(\det\symbf{M}=0\). That is,
\[(L-\lambda E)(N-\lambda G)-(M-\lambda F)^2=\lambda^2\qty(EG-F^2)+\lambda(2MF-EN-GL)+LN-M^2=0.\]
This is a quadratic giving two solutions for \(\lambda\). From \[\grad\qty(\va{v}^\top\symbf{I\!I}\va{v})=\lambda\grad\qty(\va{v}^\top\symbf{I}\va{v})\] it is apparent that the roots \(\lambda_1,\lambda_2\in\mathbb{R}\). Moreover, from \cref{eq:gaussiancurvaturelambdarootsmatrixvectorvanish} we have \[\symbf{I\!I}\va{v}=\lambda\symbf{I}\va{v}\implies\lambda=\frac{\va{v}^\top\symbf{I\!I}\va{v}}{\va{v}^\top\symbf{I}\va{v}}.\]
Hence, the two roots \(\lambda_1,\lambda_2\) are precisely the principal curvatures. Vieta's formulas give that \[K=\lambda_1\lambda_2=\frac{LN-M^2}{EG-F^2},\quad H=\frac{EN+GL-2MF}{2EG-2F^2}.\]
Now, assume a parameterization of \(\Sigma\) by \(\va{r}(u,v)\) (thrice continuously differentiable) such that \[\mathrm{I}(u,v)=\rho^2\dd{u}^2+\rho^2\dd{v}^2=\rho^2\qty(\dd{u}^2+\dd{v}^2)\] (which we will later formalize as a \textit{conformal metric}). Then there is an alternate representation of the Gaussian curvature in terms of \(\rho\).
...
\begin{align*}
    ...
    &=\frac{\det\mqty(\va{r}''_{uu}&\va{r}'_u&\va{r}'_v)\det\mqty(\va{r}''_{vv}&\va{r}'_u&\va{r}'_v)}{EG-F^2}=\frac{1}{\rho^4}\det\mqty(\va{r}''_{vv}\cdot\va{r}''_{uu}&\va{r}''_{vv}\cdot\va{r}'_u&\va{r}''_{vv}\cdot\va{r}'_v\\\va{r}'_u\cdot\va{r}''_{uu}&\va{r}'_u\cdot\va{r}'_u&\va{r}'_u\cdot\va{r}'_v\\\va{r}'_v\cdot\va{r}''_{uu}&\va{r}'_v\cdot\va{r}'_u&\va{r}'_v\cdot\va{r}'_v).
\end{align*}
Similarly,
\begin{align*}
    M^2&=\frac{1}{\rho^4}\det\mqty(\va{r}''_{uv}\cdot\va{r}''_{uv}&\rho\rho'_v&\rho\rho'_u\\\rho\rho'_v&\rho^2&0\\\rho\rho'_u&0&\rho^2)=\frac{1}{\rho^4}\qty[\va{r}''_{uv}\cdot\va{r}''_{uv}\rho^4-\rho^4{\rho'_v}^2-\rho^4{\rho'_u}^2]\\
    &=\va{r}''_{uv}\cdot\va{r}''_{uv}-{\rho'_v}^2-{\rho'_u}^2.
\end{align*}
By differentiation of the equations \[\va{r}'_u\cdot\va{r}'_v\equiv F\equiv 0,\qquad\va{r}'_u\cdot\va{r}'_u\equiv E\equiv G\equiv \va{r}'_v\cdot\va{r}'_v\equiv \rho^2,\]
we have
\begin{equation}
    \va{r}''_{uu}\cdot\va{r}'_v+\va{r}'_u\cdot\va{r}''_{uv}\equiv 0,\qquad\va{r}''_{uv}\cdot\va{r}'_v+\va{r}'_u\cdot\va{r}''_{vv}\equiv 0,\label{eq:gaussiancurvatureofsurface_conformalzerodifferentiation}
\end{equation}
and
\begin{equation}
    2\va{r}''_{uu}\cdot\va{r}'_u\equiv 2\rho\rho'_u\equiv 2\va{r}''_{uv}\cdot\va{r}'_v,\qquad 2\va{r}''_{uv}\cdot\va{r}'_u\equiv 2\rho\rho'_v\equiv 2\va{r}''_{vv}\cdot\va{r}'_v.\label{eq:gaussiancurvatureofsurface_conformalnonzerodifferentiation}
\end{equation}
...
Differentiating the inner two expressions of \cref{eq:gaussiancurvatureofsurface_conformalnonzerodifferentiation}, we have \[\va{r}'''_{uuv}\cdot\va{r}'_v+\va{r}''_{uv}\cdot\va{r}_{uv}={\rho'_u}^2+\rho\rho''_{uu},\qquad\qty(\va{r}'''_{vvu}\cdot\va{r}'_u+\va{r}''_{uv}\cdot\va{r}_{uv}={\rho'_v}^2+\rho\rho''_{vv}).\]
...
\begin{align*}
    LN&=\frac{1}{\rho^4}\det\mqty(\va{r}''_{vv}\cdot\va{r}''_{uu}&-\rho\rho'_u&\rho\rho'_v\\\rho\rho'_u&\rho^2&0\\-\rho\rho'_v&0&\rho^2)=\frac{1}{\rho^4}\qty[\va{r}''_{vv}\cdot\va{r}''_{uu}\rho^4+\rho^4{\rho'_u}^2+\rho^4{\rho'_v}^2]\\
...
\end{align}
To understand the motivation for which \(\dd{s}^2\) is said to be conformal, consider two curves in the \(u-v\) plane, parameterized by \(\symbf{\gamma}_1(t)=\qty(u_1(t),v_1(t))\) and \(\symbf{\gamma}_2(t)=\qty(u_2(t),v_2(t))\) such that \(\symbf{\gamma}_1(0)=\symbf{\gamma}_2(0)=\symbf{w}_0=\qty(u_0,v_0)\). Their images via \(\vb{r}\) are \(\symbf{\alpha}_1(t)=\symbf{r}\circ\symbf{\gamma}_1(t)\) and \(\symbf{\alpha}_2(t)=\symbf{r}\circ\symbf{\gamma}_2(t)\) so that they intersect at some point \(P\in\Sigma\). Let \(\symbf{\gamma}'_1(0)=\vb{v}_1=a\symbf{e}_u+b\symbf{e}_v\) and \(\symbf{\gamma}'_2(0)=\symbf{v}_2=c\symbf{e}_u+d\symbf{e}_v\) be two tangent vectors. Then the corresponding vectors in \(T_P\Sigma\) are \[\dd{\symbf{r}}_{\symbf{w}_0}\qty(\symbf{v}_1)=a\dd{\symbf{r}}_{\symbf{w}_0}\qty(\symbf{e}_u)+b\dd{\symbf{r}}_{\symbf{w}_0}\qty(\symbf{e}_v),\quad\dd{\symbf{r}}_{\symbf{w}_0}\qty(\symbf{v}_2)=c\dd{\symbf{r}}_{\symbf{w}_0}\qty(\symbf{e}_u)+d\dd{\symbf{r}}_{\symbf{w}_0}\qty(\symbf{e}_v).\]
Since\footnote{In more modern formulations of differential geometry, differentials (known as pushforwards) are functions mapping tangent vectors to tangent vectors. The notion came from the realization that ``changes'' of functions are best described in terms of a direction of change. Then basis vectors themselves became partial derivative operators two give the second set of equalities, which is a consequence of the more abstract notion of ``duality.'' For the remaining sections, this structural viewpoint is not considered.} \(\textstyle\vb{e}_u=\pdv{}{u}\), \(\textstyle\vb{e}_v=\pdv{}{v}\) and \(\textstyle\dd{\vb{r}}_{\symbf{w}_0}\qty(\pdv{}{u})=\eval{\pdv{}{u}\qty(\vb{r})}_{\symbf{w}_0}\), \(\textstyle\dd{\vb{r}}_{\symbf{w}_0}\qty(\pdv{}{v})=\eval{\pdv{}{v}\qty(\vb{r})}_{\symbf{w}_0}\). Then
\[\dd{\symbf{r}}_{\symbf{w}_0}\qty(\symbf{v}_1)=a\symbf{r}'_u\qty(\symbf{w}_0)+b\symbf{r}'_v\qty(\symbf{w}_0),\quad\dd{\symbf{r}}_{\symbf{w}_0}\qty(\symbf{v}_2)=c\symbf{r}'_u\qty(\symbf{w}_0)+d\symbf{r}'_v\qty(\symbf{w}_0).\]
The angle \(\theta_{uv}\) between \(\symbf{v}_1\) and \(\symbf{v}_2\) on the \(u-v\) plane satisfies \[\cos\theta_{uv}=\frac{\symbf{v}_1\cdot\symbf{v}_2}{\norm{\symbf{v}_1}\norm{\symbf{v}_2}}=\frac{ac+bd}{\sqrt{a^2+b^2}\sqrt{c^2+d^2}},\]
while the angle \(\theta_\Sigma\) between the two tangent vectors in \(T_P\Sigma\) satisfies \[\cos\theta_\Sigma=\mathmakebox[\widthof{\(\eval{\frac{\qty(a\symbf{r}'_u+b\symbf{r}'_v)\cdot\qty(c\symbf{r}'_u+d\symbf{r}'_v)}{\norm{a\symbf{r}'_u+b\symbf{r}'_v}\norm{c\symbf{r}'_u+d\symbf{r}'_v}}}\)}][l]{\eval{\frac{\qty(a\symbf{r}'_u+b\symbf{r}'_v)\cdot\qty(c\symbf{r}'_u+d\symbf{r}'_v)}{\norm{a\symbf{r}'_u+b\symbf{r}'_v}\norm{c\symbf{r}'_u+d\symbf{r}'_v}}}_{\symbf{w}_0}}=\mathmakebox[\widthof{\(\eval{\frac{\rho^2\qty(ac+bd)}{\sqrt{a^2\rho^2+b^2\rho^2}\sqrt{c^2\rho^2+d^2\rho^2}}}\)}][l]{\eval{\frac{\rho^2\qty(ac+bd)}{\sqrt{a^2\rho^2+b^2\rho^2}\sqrt{c^2\rho^2+d^2\rho^2}}}_{\symbf{w}_0}}=\cos\theta_{uv}.\]
