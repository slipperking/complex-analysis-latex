...
\end{align*}
For a given metric \(\dd{s}=\lambda(z)\abs{\ddz}\), if there is some other parameterization such that \(\dd{s}=\lambda'\qty(z')\abs{\dd{z'}}\), \(z'=f(z)\) is conformal, then the relation is given by \(\lambda=f^*\lambda'\). Under differing parameterizations of a metric \(\dd{s}\), we hence have the invariance of curvature.
