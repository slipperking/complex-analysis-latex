...
\begin{proof}
    ...
    Let the integral in the last expression be a complex function of \(\beta\), more explicitly \(J(\beta)\).
    Notice that for \(c>0\), the integral maybe written as the difference of two convergent integrals for \(-1<\Re\beta<0\): \[J(\beta)=\int_0^\infty\frac{s^\beta\dd{s}}{1+s}-\int_0^\infty\frac{s^\beta c^{\beta}\dd{s}}{1+s}=\qty(1-c^\beta)\operatorname{B}(-\beta,\beta+1)=\qty(c^\beta-1)\uppi\csc\qty(\uppi\beta)\] by the reflection formula (\cref{thm:gammafunction_eulerreflection}), which derives an analytic continuation to everywhere in \(\mathbb{C}\setminus\mathbb{Z}\). 
    ...
    \begin{align*}
        ...
        &={\uppi\sqrt{2}}\int_0^1\frac{\dd{u}}{\qty(1+u)\qty(1+u^2)}=\frac{\uppi\sqrt{2}}2\int_0^1\qty(\frac{1}{1+u}+\frac{1-u}{1+u^2})\\
        &=\frac{\uppi\sqrt{2}}2\qty(\ln{2}+\arctan 1-\frac12\ln2)=\frac{\uppi^2\sqrt2}8+\frac{\uppi\sqrt{2}}4\ln 2.
    \end{align*}
\end{proof}
