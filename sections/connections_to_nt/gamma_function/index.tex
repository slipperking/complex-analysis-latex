...
Therefore, \(\Gamma(x)\) is convergent on \(\mathbb{R}_{>0}\). It follows that \(\Gamma(z)\) is absolutely convergent on the right half-plane \(\cbraces{z\in\mathbb{C}}{\Re(z)>0}\).
\begin{theorem}
...
\begin{theorem}[\textsc{Gauss}]\label{thm:gammafunctiongaussformula}
...
\begin{theorem}[Weierstrass]\label{thm:gammafunction_weierstrassformula}
    ...
    \end{equation} where \(\upgamma=\int_1^\infty\qty(\frac{1}{\floor{x}}-\frac{1}{x})\ddx\).
\end{theorem}
\begin{proof}
    ...
    \begin{align*}
        ...
        & =z\prod_{k=1}^n\qty[\qty(1+\frac{z}{k})\ee^{-\frac{z}{k}}]\exp\qty(z\upgamma).
    \end{align*}
    The constant \(\upgamma=\int_1^\infty\qty(\frac{1}{\floor{x}}-\frac{1}{x})\ddx\) is known as the \textit{Euler--Mascheroni constant}. For each \(x\), \(0\leq\frac1{\floor{x}}-\frac1x<\frac1{\floor{x}}-\frac1{\floor{x}+1}\), and the integral is bounded by \(\sum_{n=1}^\infty\qty(\frac1n-\frac1{n+1})<1\), and hence the integral is convergent. The properties of \(\upgamma\) still largely remain unknown.

    By the Weierstrass Factorization Theorem (\cref{thm:weierstrassfactorization}), if we let \(a_n=-n\) and \(p_n=1\), it follows that \[\sum_{n=1}^\infty\abs{\frac{R}{a_n}}^{p_n+1}=R^2\sum_{n=1}^\infty\frac{1}{n^2}=\frac{R^2\uppi^2}{6}\] is convergent. Thus, the Weierstrass formula defines an entire function with zeros at each of \(\mathbb{Z}_{\leq 0}\).
\end{proof}
...
\begin{theorem}[Euler's Reflection Formula]\label{thm:gammafunction_eulerreflection}
    ...
    \begin{equation}
        \Gamma(z)\Gamma(1-z)=\uppi\csc(\uppi z)\label{eq:gammafunction_eulerreflection}
...
\begin{proof}
    ...
    \begin{equation*}
        \frac{1}{\Gamma(z)}=z\prod_{k=1}^n\qty[\qty(1+\frac{z}{k})\ee^{-\frac{z}{k}}]\ee^{z\upgamma},\quad\frac{1}{\Gamma(-z)}=-z\prod_{k=1}^n\qty[\qty(1-\frac{z}{k})\ee^{\frac{z}{k}}]\ee^{-z\upgamma}.
    \end{equation*}
    Since the Weierstrass elementary factors form an absolutely convergent infinite product, we may rearrange its terms. Hence, by \cref{thm:sinproductformula}, we have \[\frac{1}{\Gamma(z)\Gamma(1-z)}=-\frac{1}{z\Gamma(z)\Gamma(-z)}=z\prod_{k=1}^n\qty(1-\frac{z^2}{k^2})=\frac{\sin(\uppi z)}{\uppi},\] which confirms \cref{eq:gammafunction_eulerreflection}.
\end{proof}
\begin{example}\label{ex:gammafunction_onehalf}
    Evaluate \(\Gamma\qty(\frac{1}{2})\).
\end{example}
\begin{proof}
    By the Reflection Formula (\cref{thm:gammafunction_eulerreflection}), we have that \[\Gamma\qty(\frac{1}{2})^2=\uppi\csc(\frac{\uppi}{2})=\uppi,\]
    and it follows that \(\Gamma\qty(\frac{1}{2})=\sqrt{\uppi}\) as it is positive.
\end{proof}
\begin{theorem}[Legendre's Duplication Formula]\label{thm:gammafunction_legendreduplication}
    ...
    \begin{equation}
        \Gamma(z)\Gamma\qty(z+\frac{1}{2})=2^{1-2z}\sqrt{\uppi}\Gamma(2z).\label{eq:gammafunction_legendreduplication}
    \end{equation}
\end{theorem}
\begin{proof}
    ...
    where the left-hand side is defined since \(z\in\mathbb{C}\setminus\qty(-\frac{\mathbb{N}}{2})\). By expansion of the value, we have
    \begin{align*}
...
\begin{theorem}[Gauss Multiplication Theorem]
    ...
    \begin{equation}
        \Gamma(z)\Gamma\qty(z+\frac{1}{m})\cdots\Gamma\qty(z+\frac{m-1}{m})=\qty(2\uppi)^{\frac{m-1}{2}}m^{\frac{1}{2}-mz}\Gamma(mz).\label{eq:gammafunction_gaussmultiplication}
    \end{equation}
\end{theorem}
The Gamma function as in \cref{eq:gammafunction} is commonly referred to as the \textit{Euler Integral of the Second Kind}. The \textit{Euler Integral of the First Kind} is also known as the \textit{Beta function}, and is defined by \[\operatorname{B}\qty(z_1,z_2)=\int_0^1 t^{z_1-1}(1-t)^{z_2-1}\ddt.\] By a change of variables (by letting \(\tau=1-t\)), we derive the symmetry of the Beta function: \[\operatorname{B}\qty(z_1,z_2)=\int_0^1\tau^{z_2-1}(1-\tau)^{z_1-1}\dd\tau=\operatorname{B}\qty(z_2,z_1).\]
...
\begin{example}[MIT Integration Bee 2023 Finals \#1]
    Evaluate \[\int_0^{\frac{\uppi}{2}}\frac{\sqrt[3]{\tan(x)}}{(\sin(x)+\cos(x))^2}\ddx.\]
\end{example}
\begin{proof}
    By rewriting the integral, and letting \(u=\tan(x)\), we have
    \begin{align*}
        I=\int_0^{\frac{\uppi}{2}}\frac{\sqrt[3]{\tan(x)}}{(\sin(x)+\cos(x))^2}\ddx & =\int_0^\infty\frac{u^{\frac{1}{3}}\sec[2](x)\ddx}{(u+1)^2} \\
        & =\int_0^\infty\frac{u^{\frac{1}{3}}\dd{u}}{(u+1)^2}         \\
        & =\operatorname{B}\qty(\frac{4}{3},\frac{2}{3}),
    \end{align*}
    where the last step recognizes the form of \cref{eq:betagammafunctionrelationship_intermediate}. \Cref{thm:betagammafunctionrelationship} then gives \[I=\frac{\Gamma\qty(\frac{2}{3})\Gamma\qty(\frac{4}{3})}{\Gamma(2)}=\frac{1}{3}\Gamma\qty(\frac{1}{3})\Gamma\qty(\frac{2}{3}).\] Lastly, the Reflection Formula (\cref{thm:gammafunction_eulerreflection}) gives that \[I=\frac{\uppi}{3\sin(\frac{\uppi}{3})}=\frac{2\uppi\sqrt{3}}{9}.\qedhere\]
\end{proof}
\begin{example}
    Evaluate \[I=\int_0^{\frac{\uppi}2}x\sqrt{\tan{x}}\ddx.\]
\end{example}
\begin{proof}
    Apply the substitution \(u=\tan x\), \(s=u^2\) and the integral representation \(\textstyle\arctan{u}=\int_0^u\frac{\dd{v}}{1+v^2}=\int_0^1\frac{u\dd{v}}{1+u^2v^2}\) to get 
    \begin{align*}
        I&=\int_0^{\infty}\arctan{u}\frac{\sqrt{u}\dd{u}}{1+u^2}=\int_0^\infty\qty(\int_0^1\frac{u}{1+u^2v^2}\dd{v})\frac{\sqrt{u}}{1+u^2}\dd{u}\\
        &=\int_0^1\int_0^\infty\frac{u^{\frac32}}{\qty(1+u^2v^2)\qty(1+u^2)}\dd{u}\dd{v}=\int_0^1\int_0^\infty\frac{s^{\frac14}}{2\qty(1+sv^2)\qty(1+s)}\dd{s}\dd{v}.
    \end{align*}
    The integral switch is justified since the integrand \(\geq 0\) throughout. Consider the integral for \[\int_0^\infty\frac{s^{\frac14}\dd{s}}{\qty(1+sv^2)\qty(s+1)}=\frac1{1-v^2}\int_0^{\infty}s^{\beta}\qty(\frac1{1+s}-\frac{1}{c+s})\dd{s},\qquad c=v^{-2},\quad\beta=\frac14\]
    Let the integral in the last expression be a complex function of \(\beta\), more explicitly \(J(\beta)\), defined for \(-1<\Re\beta<1\).

    Notice that the integral maybe written as the difference of two convergent integrals for \(-1<\Re\beta<0\): \[J(\beta)=\int_0^\infty\frac{s^\beta\dd{s}}{1+s}-\int_0^\infty\frac{s^\beta c^{\beta}\dd{s}}{1+s}=\qty(1-c^\beta)\operatorname{B}(-\beta,\beta+1)=\qty(c^\beta-1)\uppi\csc\qty(\uppi\beta)\] by the reflection formula (\cref{thm:gammafunction_eulerreflection}), which derives an analytic continuation of \(J\restriction_{-1<\Re\beta<1}\) to everywhere in \(\mathbb{C}\setminus\mathbb{Z}\). 
    
    One can show by differentiation under the integral sign, that the integral \(J(\beta)\) is holomorphic for \(-1<\Re{\beta}<1\). The identity theorem (\cref{thm:identity}) then allows the computed cosecant expression to be assigned to \(J(\beta)\): 
    \begin{align*}
        I&=\int_0^1\frac{\qty(v^{-\frac12}-1)\uppi\csc\qty(\frac{\uppi}4)}{2-2v^2}\dd{v}=\frac{\uppi\sqrt{2}}2\int_0^1\frac{\qty(v^{-\frac12}-1)}{1-v^2}\dd{v}={\uppi\sqrt{2}}\int_0^1\frac{\qty(1-u)}{1-u^4}\dd{u}\\
        &={\uppi\sqrt{2}}\int_0^1\frac{\dd{u}}{\qty(1+u)\qty(1+u^2)}=\frac{\uppi\sqrt{2}}2\int_0^1\qty(\frac{1}{1+u}+\frac{1-u}{1+u^2})\dd{u}\\
        &=\frac{\uppi\sqrt{2}}2\qty(\ln{2}+\arctan 1-\frac12\ln2)=\frac{\uppi^2\sqrt2}8+\frac{\uppi\sqrt{2}}4\ln 2.\qedhere
    \end{align*}
\end{proof}
