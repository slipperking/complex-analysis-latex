...
\begin{theorem}[Weierstrass]\label{thm:gammafunction_weierstrassformula}
    ...
    \end{equation} where \(\mupgamma=\int_1^\infty\qty(\frac{1}{\floor{x}}-\frac{1}{x})\ddx\).
\end{theorem}
\begin{proof}
    ...
    \begin{align*}
        \frac{1}{\Gamma(z)} & =\lim_{n\to\infty}\frac{\prod_{k=0}^n(z+k)}{n^z n!}                                                                                           \\
                            ...
                            & =z\prod_{k=1}^n\qty[\qty(1+\frac{z}{k})\ee^{-\frac{z}{k}}]\exp\qty(z\mupgamma).
    \end{align*}
    The constant \(\mupgamma=\int_1^\infty\qty(\frac{1}{\floor{x}}-\frac{1}{x})\ddx\) is known as the \textit{Euler--Mascheroni constant}. By the Weierstrass Factorization Theorem (\cref{thm:weierstrassfactorization}), if we let \(a_n=-n\) and \(p_n=1\), it follows that \[\sum_{n=1}^\infty\abs{\frac{R}{a_n}}^{p_n+1}=R^2\sum_{n=1}^\infty\frac{1}{n^2}=\frac{R^2\muppi^2}{6}\] is convergent. Thus, the Weierstrass formula defines an entire function with zeros at each of \(\mathbb{Z}_{\leq 0}\).
\end{proof}
...
\begin{theorem}[Euler's Reflection Formula]\label{thm:gammafunction_eulerreflection}
    ...
    \begin{equation}
        \Gamma(z)\Gamma(1-z)=\muppi\csc(\muppi z)\label{eq:gammafunction_eulerreflection}
...
\begin{proof}
    ...
    \begin{equation*}
        \frac{1}{\Gamma(z)}=z\prod_{k=1}^n\qty[\qty(1+\frac{z}{k})\ee^{-\frac{z}{k}}]\ee^{z\mupgamma},\quad\frac{1}{\Gamma(-z)}=-z\prod_{k=1}^n\qty[\qty(1-\frac{z}{k})\ee^{\frac{z}{k}}]\ee{-z\mupgamma}.
    \end{equation*}
    Since the Weierstrass elementary factors form an absolutely convergent infinite product, we may rearrange its terms. Hence, by \cref{ex:sinproductformula}, we have \[\frac{1}{\Gamma(z)\Gamma(1-z)}=-\frac{1}{z\Gamma(z)\Gamma(-z)}=z\prod_{k=1}^n\qty(1-\frac{z^2}{k^2})=\frac{\sin(\muppi z)}{\muppi},\] which confirms \cref{eq:gammafunction_eulerreflection}.
\end{proof}
\begin{example}\label{ex:gammafunction_onehalf}
    Evaluate \(\Gamma\qty(\frac{1}{2})\).
\end{example}
\begin{proof}
    By the Reflection Formula (\cref{thm:gammafunction_eulerreflection}), we have that \[\Gamma\qty(\frac{1}{2})^2=\muppi\csc(\frac{\muppi}{2})=\muppi,\]
    and it follows that \(\Gamma\qty(\frac{1}{2})=\sqrt{\muppi}\) as it is positive.
\end{proof}
\begin{theorem}[Legendre's Duplication Formula]\label{thm:gammafunction_legendreduplication}
    ...
    \begin{equation}
        \Gamma(z)\Gamma\qty(z+\frac{1}{2})=2^{1-2z}\sqrt{\muppi}\Gamma(2z).\label{eq:gammafunction_legendreduplication}
    \end{equation}
\end{theorem}
\begin{proof}
...
\begin{theorem}[Gauss Multiplication Theorem]
    ...
    \begin{equation}
        \Gamma(z)\Gamma\qty(z+\frac{1}{m})\cdots\Gamma\qty(z+\frac{m-1}{m})=\qty(2\muppi)^{\frac{m-1}{2}}m^{\frac{1}{2}-mz}\Gamma(mz).\label{eq:gammafunction_gaussmultiplication}
    \end{equation}
\end{theorem}
...
\begin{example}[MIT Integration Bee 2023 Finals \#1]
    Evaluate \[\int_0^{\frac{\muppi}{2}}\frac{\sqrt[3]{\tan(x)}}{(\sin(x)+\cos(x))^2}\ddx.\]
\end{example}
\begin{proof}
    By rewriting the integral, and letting \(u=\tan(x)\), we have
    \begin{align*}
        I=\int_0^{\frac{\muppi}{2}}\frac{\sqrt[3]{\tan(x)}}{(\sin(x)+\cos(x))^2}\ddx & =\int_0^\infty\frac{u^{\frac{1}{3}}\sec[2](x)\ddx}{(u+1)^2} \\
                                                                                    & =\int_0^\infty\frac{u^{\frac{1}{3}}\dd{u}}{(u+1)^2}         \\
                                                                                    & =\operatorname{B}\qty(\frac{4}{3},\frac{2}{3}),
    \end{align*}
    where the last step recognizes the form of \cref{eq:betagammafunctionrelationship_intermediate}. \Cref{thm:betagammafunctionrelationship} then gives \[I=\frac{\Gamma\qty(\frac{2}{3})\Gamma\qty(\frac{4}{3})}{\Gamma(2)}=\frac{1}{3}\Gamma\qty(\frac{1}{3})\Gamma\qty(\frac{2}{3}).\] Lastly, the Reflection Formula (\cref{thm:gammafunction_eulerreflection}) gives that \[I=\frac{\muppi}{3\sin(\frac{\muppi}{3})}=\frac{2\muppi\sqrt{3}}{9}.\qedhere\]
\end{proof}