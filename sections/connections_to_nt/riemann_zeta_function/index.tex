...
\[C_{\varepsilon}=\cbraces{\varepsilon\ee^{\ii \theta}}{\asin(\frac{\delta}{\varepsilon})\leq \theta\leq2\uppi-\asin(\frac{\delta}{\varepsilon})},\quad\overset{\pm}{\Gamma}_\varepsilon=\cbraces{x\pm \ii\delta}{\sqrt{\varepsilon^2-\delta^2}\leq x<\infty}.\]
...
\(\delta\le\varepsilon_1<\varepsilon_2<2\uppi\) is arbitrarily chosen. Then we
have
\begin{align*}
...
\begin{align*}
    \abs{\mathrm{I}} & \leq\varepsilon\int_\vartheta^{2\uppi-\vartheta}\abs{\frac{\qty(\varepsilon\ee^{\ii \theta})^{s-1}}{\ee^{\varepsilon\ee^{\ii \theta}}-1}}\dd{\theta}\leq2\uppi\varepsilon^{\Re(s)}\sup_{\vartheta\leq \theta\leq2\uppi-\vartheta}\frac{\abs{\ee^{\ii \theta(s-1)}}}{\frac{\varepsilon}2} \\
    & =4\uppi\varepsilon^{\Re(s)-1}\sup_\theta\abs{\ee^{\theta(\ii\qty(\Re(s)-1)-\Im(s))}}=4\uppi\varepsilon^{\Re(s)-1}\sup_\theta\ee^{-\theta\Im(s)}                                                                                                                                          \\
    & \leq4\uppi\varepsilon^{\Re(s)-1}\ee^{2\uppi\abs{\Im(s)}}\to0\qq{as}\varepsilon\to 0.
\end{align*}
On the contrary,
\begin{align}
    ...
    & \leq\int_{\widetilde{\delta}}^\infty\frac{\ee^{\Log\sqrt{1+x^2}(\Re(s)-1)+\uppi\Im(s)}}{\ee^x-1}\ddx\nonumber                                                                                                                                                 \\
    & \quad+\int_{\widetilde{\delta}}^\infty\frac{\ee^{\Log\sqrt{1+x^2}(\Re(s)-1)+\uppi\Im(s)}}{\ee^x-1}\ddx\nonumber
\end{align} where we choose the principal branch logarithm such that \(\arg(z)\in(-\uppi,\uppi]\) and take \(0<\delta\leq1\). It is trivial to see that the bounding integrals (now independent of \(\delta\)) are convergent for fixed \(s\). Then Lebesgue's Dominated Convergence Theorem applies to \cref{eq:riemannzetafunction_remainingtwointegralshankelcontour} as \(\delta\to 0^+\), resulting in
\begin{align}
    \mathrm{I\!I}+\mathrm{I\!I\!I} & \to\int_{\widetilde{\delta}}^\infty\frac{\ee^{(s-1)\qty[\log\qty(x)+\ii\uppi]}}{\ee^x-1}\ddx-\int_{\widetilde{\delta}}^\infty\frac{\ee^{(s-1)\qty[\log(x)-\ii\uppi]}}{\ee^x-1}\ddx\nonumber                                                      \\
    & =\int_{\widetilde{\delta}}^\infty\frac{x^{s-1}\ee^{\ii\uppi(s-1)}}{\ee^x-1}\ddx-\int_{\widetilde{\delta}}^\infty\frac{x^{s-1}\ee^{-\ii\uppi(s-1)}}{\ee^x-1}\ddx\nonumber                                                                         \\
    & =\qty(\ee^{-\ii\uppi s}-\ee^{\ii\uppi s})\int_{\widetilde{\delta}}^\infty\frac{x^{s-1}\ddx}{\ee^x-1}=-2\ii\sin\qty(\uppi s)\int_{\widetilde{\delta}}^\infty\frac{x^{s-1}\ddx}{\ee^x-1}.\label{eq:riemannzetafunction_remainingtwointegralslimit}
\end{align}
Now under the limiting operations \(\delta\to 0^+\) and \(\varepsilon\to0^+\), we have
\begin{equation}
    F(s)=F_\varepsilon(s)(\forall\varepsilon<2\uppi)=\lim_{\varepsilon\to 0^+}F_\varepsilon(s)=\lim_{\varepsilon\to 0}\lim_{\delta\to0}\mathrm{I}+\mathrm{I\!I}+\mathrm{I\!I\!I}=-2\ii\sin\qty(\uppi s)\Gamma(s)\zeta(s)\label{eq:riemannzetafunctionthreeintegralslimit}
...
\begin{proof}
    ...
    since \(\sin(\uppi)=0\) (in
    \cref{eq:riemannzetafunction_remainingtwointegralslimit}). Now because \[\mathrm{I}\to\oint_{\partial D\qty(0,\varepsilon)}\frac{\ddz}{\ee^z-1}=2\uppi\ii\residue_{z=0}\qty(\frac1{\ee^z-1})\] as \(\delta\to 0^+\), the integrand has a simple pole at \(z=0\). Its residue,
    ...
    Hence, \(F(1)=2\uppi\ii\). Now the expression
    \(\zeta(1)=-\frac{\uppi}{\sin(\uppi)\Gamma(1)}\) shows that the singularity at
    ...
    \begin{align*}
        \residue_{s=1}\zeta(s) & =\lim_{s\to 1}(s-1)\zeta(s)=\lim_{s\to 1}(1-s)\qty(\frac{F_\varepsilon(s)}{2\ii\sin(\uppi s)\Gamma(s)})                          \\
        & =\frac{2\uppi\ii}{2\ii\Gamma(1)}\lim_{s\to 1}\frac{1-s}{\sin(\uppi s)}=-\uppi\lim_{s\to1}\frac{1}{\uppi\cos(\uppi s)}=1.\qedhere
    \end{align*}
\end{proof}
...
\begin{theorem}[\textsc{Riemann's Reflection Formula}]\label{thm:riemannzetafunction_functionalequation}
    For any \(s\neq 0\), we have
    \begin{equation}
        \zeta(1-s)=2\zeta(s)\Gamma(s)\cos(\frac\uppi2s)(2\uppi)^{-s}.\label{eq:riemannzetafunction_functionalequation}
    \end{equation}
\end{theorem}
\begin{proof}
    We restrict our consideration to \(s\neq 0,-1,-2,\ldots\) (and the usage of limits suffices to extend the relation to all of \(\mathbb{C}^*\)). The integrand \(\frac{(-z)^{s-1}}{\ee^z-1}\) (as a function of \(z\)) has a branch cut singularity on \(\mathbb{R}_{\geq 0}\) (a region excluded by all Hankel contours and thus irrelevant to our interest). The denominator vanishes (simply) when \(\ee^z=1\) or when \(z\in2\uppi\ii\mathbb{Z}\) and thus has simple poles at these points.

    Suppose that \(\Re s<0\). Now the Residue Theorem (\cref{thm:residuethm}) gives
    that \[F_{(2n+1)\uppi}(s)-F_{\varepsilon}(s)=2\uppi\ii\sum_{\mathclap{\substack{z\in2\uppi\ii\mathbb{Z}\\z\in\text{ contour}}}}\residue_z\frac{(-z)^{s-1}}{\ee^z-1},\qquad\varepsilon<2\uppi\] where the contour is the keyhole contour in \cref{fig:keyholecontour} with a
    larger radius of \((2n+1)\uppi\) and a smaller one of \(\varepsilon\). Then we
    have, by virtue of \cref{eq:residueatpole}:
    \begin{multline*}
        \frac1{2\uppi\ii}\qty(F_{(2n+1)\uppi}(s)-F_\varepsilon(s))=\sum_{\substack{k=-n\\k\neq 0}}^{n}\lim_{\zeta\to2\pi\ii k}\frac{\qty(\zeta-2\uppi\ii k)(-\zeta)^{s-1}}{\ee^\zeta-1}\\
        =\sum_{\substack{k=-n\\k\neq 0}}^n\lim_{\zeta\to 2\uppi\ii k}\frac{2\uppi\ii k(s-1)(-\zeta)^{s-2}+s(-\zeta)^{s-1}}{\ee^\zeta}\\
        =\sum_{\substack{k=-n\\k\neq0}}^n\qty[2\uppi\ii k(s-1)(-2\uppi\ii k)^{s-2}+s(-2\uppi\ii k)^{s-1}]=\sum_{\substack{k=-n\\k\neq0}}^n(-2\uppi\ii k)^{s-1},
    \end{multline*}
    where we utilize the principal branch logarithm:
    \begin{align*}
        F_{(2n+1)\uppi}(s)-F_\varepsilon(s) & =2\uppi\ii\sum_{k=1}^n\qty[\ee^{\Log(2\uppi\ii k)\qty(s-1)}+\ee^{\Log(-2\uppi\ii k)\qty(s-1)}]                                   \\
        & =2\uppi\ii\sum_{k=1}^n\qty[\ee^{\Log(2\uppi k)\qty(s-1)}\ee^{\Log(\ii)(s-1)}+\ee^{\Log(2\uppi k)\qty(s-1)}\ee^{\Log(-\ii)(s-1)}] \\
        & =2\uppi\ii\sum_{k=1}^n(2\uppi k)^{s-1}\qty[\ee^{\ii\frac{\uppi}{2}(s-1)}+\ee^{-\ii\frac{\uppi}{2}(s-1)}]                         \\
        & =4\uppi\ii\cos\qty(\frac\uppi2(s-1))\sum_{k=1}^n(2\uppi k)^{s-1}.
    \end{align*}
    Now as \(n\to\infty\),
    \begin{align}
        \abs{F_{(2n+1)\uppi}(s)} & \le\abs{\oint_{\partial D(0,R)}\frac{\qty(-z)^{s-1}\ddz}{\ee^z-1}}+2\abs{\sin(\uppi s)\int_{R}^\infty\frac{x^{s-1}\ddx}{\ee^x-1}},\quad R=(2n+1)\uppi\nonumber                                                                                                          \\
        & \leq2\uppi R^{\Re s}\sup_{\theta\in[0,2\uppi]}\abs{\qty(-\ee^{\ii\theta})^{s-1}}\sup_{z\in\partial D(0,R)}\frac{1}{\abs{\ee^z-1}}+2\abs{\sin(\uppi s)\int_{R}^\infty\frac{x^{s-1}\ddx}{\ee^x-1}}.\label{eq:riemannzetafunction_functionalequation_contourintegralbound}
    \end{align}
    ...
    \end{align*} (by the double argument/angle formulas) where \(x^2+y^2=R^2\) for some \(R=(2n+1)\uppi\). Now we observe two cases:
    \begin{enumerate}
        ...
        \item If \(\abs{x}<2\), then \(\ee^x>\ee^{-2}\). Assume, without loss of generality,
            ...
            Because \(\xi=n\uppi+\frac\uppi2\), we deduce that \[\sin(\frac y2)=(-1)^n\cos\eta\implies\sin[2](\frac y2)=\cos^2\eta=\cos[2](\frac R2\qty(1-\sqrt{1-\frac{x^2}{R^2}})).\]
            Since \(\abs{x}<2\), we have that \(0\leq\frac{x^2}{R^2}<\frac4{R^2}<1\), which
            implies that \[\sqrt{1-\frac{x^2}{R^2}}\geq 1-\frac{x^2}{R^2}\] (since concavity of \(\sqrt{1-t}\) for \(t\in(0,1)\) suffices to ensure that the tangent line at \(0\) is an over-approximation). Therefore, we have \[1-\sqrt{1-\frac{x^2}{R^2}}\leq\frac{x^2}{R^2}\implies\frac{R}{2}\qty(1-\sqrt{1-\frac{x^2}{R^2}})\leq\frac{x^2}{2R}\leq\frac{2}{R}\leq\frac{2}{\uppi}\] since \(R\geq\uppi\). On this range \(\cos\) is decreasing and we hence obtain \[\sin^2\frac y2=\cos^2\eta\geq\cos[2](\frac2\uppi)\implies\abs{\ee^z-1}^2\geq\frac4{\ee^2}\cos[2](\frac2\uppi).\]
    \end{enumerate}
    We have thus shown that \(\abs{\ee^z-1}\) is bounded below by a positive constant \[\min\cbraces{1-\ee^{-2},\frac2{\ee}\cos(\frac2\uppi)}>0\] for all \(z\in\bigcup_{n=1}^\infty\partial D\qty(0,(2n+1)\uppi)\) (and hence
    \(\frac1{\abs{\ee^z-1}}\) is bounded). Therefore, from
    \cref{eq:riemannzetafunction_functionalequation_contourintegralbound} we have
    that \(\abs{F_{(2n+1)\uppi}(s)}\to 0\) as \(n\to\infty\) for \(\Re s<0\).
    Hence, we may deduce that for \(\Re s<0\), \[-F_\varepsilon(s)=4\uppi\ii\cos\qty(\frac\uppi2(s-1))(2\uppi)^{s-1}\sum_{k=1}^nk^{s-1}=4\uppi\ii\cos\qty(\frac\uppi2(s-1))(2\uppi)^{s-1}\zeta(1-s).\]
    The left-hand side can be simplified by
    \cref{eq:riemannzetafunction_analyticcontinuationcontourintegral} to give
    \begin{align*}
        \sin(\uppi s)\Gamma(s)\zeta(s)                             & =\cos\qty(\frac\uppi2(s-1))(2\uppi)^{s}\zeta(1-s)=\sin\qty(\frac\uppi2s)(2\uppi)^{s}\zeta(1-s) \\
        2\sin\qty(\frac\uppi2s)\cos\qty(\frac\uppi2s)(2\uppi)^{-s} & =\sin(\frac{\uppi}2s)\zeta(1-s),
    \end{align*}
    which we now extend to all of \(\mathbb{C}^*\) by analytic continuation.
\end{proof}
\begin{remark}
    Suppose that \[\xi(s)=\frac12s(s-1)\uppi^{-\frac s2}\Gamma\qty(\frac s2)\zeta(s).\]
...
\begin{proof}
    ...
    \[\zeta(1-s)=2\zeta(s)\Gamma(s)\cos\qty(\frac\uppi2s)(2\uppi)^{-s}.\] for \(\Re s>1\). On this range, \(\zeta\), \(\Gamma\), and \((2\uppi)^{-s}\)
    are all nonzero. The \(\cos\) function vanishes only at odd multiples of
    \(\frac\uppi2\), or when \(s=1+2n\) for \(n\in\mathbb{N}\). Therefore, the only
