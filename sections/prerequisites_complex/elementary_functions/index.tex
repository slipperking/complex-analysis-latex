...
We can also define the power function for non-integer powers with \(w=z^\alpha=\ee^{\alpha\log(z)}\). Then, power functions can all be written in terms of exponential functions and logarithms. Letting \(x=\uppi\) in \cref{eq:eulersformula} yields \(\ee^{\ii\uppi}=-1\). Furthermore, we can see that exponentiation with an imaginary number is a rotation:
...
\begin{enumerate}
    ...
    \item Since \(\ee^{2\uppi\ii}=1\), it is periodic over \(2\uppi\ii\).
    ...
    \item The region \(\ee^z\) is univalent over:
        ...
        Let \(z_1=x_1+\ii y_1\), \(z_2=x_2+\ii y_2\), \(x_1,y_1,x_2,y_2\in\mathbb{R}\) satisfy \(\ee^{z_1}=\ee^{z_2}\). Then, \[\ee^{x_1}\ee^{\ii y_1}=\ee^{x_2}\ee^{\ii y_2}.\] The moduli are equal, and therefore \(x_1=x_2\), and by the periodic nature of the exponentiation of imaginary numbers, \(y_1-y_2=2\uppi k\), where \(k\in\mathbb{Z}\). To satisfy the univalence over a region \(U\), \(y_1-y_2\neq2\uppi k\), we can select \(U\) to be any horizontal strip \(2\uppi k\leq \Im(z)< 2\uppi (k+1)\) (or \(2\uppi k< \Im(z)\leq 2\uppi (k+1)\)). Similar to the exponential function, any belt region with thickness \(2\uppi\) is a region over which \(\log\) is univalent.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Let \(z=r\ee^{\ii\theta}\) and \(w=u+\ii v\), where \(r,\theta,u,v\in\mathbb{R}\). Then,
        ...
        and \(\ee^u=r\), meaning that \(u=\log(r)\), \(v=\theta+2\uppi k\), where \(k\in\mathbb{Z}\). Then,
        \[w=\log(r)+\ii(\theta+2\uppi k),\]
        and using the modulus-argument notation,
        \[\log(z)=\log\abs{z}+\ii\arg(z),\]
        where \(\arg(z)\) is the multi-valued argument function. We denote the \textit{principal branch} of the argument function by \(\Arg:\mathbb{C}\setminus\cbraces{0}\to(-\uppi,\uppi]\). The principal branch of \(\log(z)\), or \(\Log(z)\), can be defined such that \(\Im\paren{\Log}\in(-\uppi,\uppi]\).
\end{enumerate}
The functions \(\sin\) and \(\cos\), through their exponential form, still satisfy the properties such as their derivatives, periodicity being \(2\uppi\), parity, sum and difference, and the fundamental identities \(\sin^2(z)+\cos^2(z)=1\), \(\sin(z)=\cos(\frac{\uppi}{2}-z)\). However, due to the extension, some properties do not hold. For instance, \(\sin(z)\) and \(\cos(z)\) are unbounded, as along the imaginary axis, they resemble their hyperbolic counterparts, which are unbounded along the real line.
...
\(\xi\) is clearly univalent on \(\mathbb{C}\), as it is a linear map (specifically, a rotation by \(\frac{\uppi}{2}\) radians followed by scaling).\ \(\zeta\) is univalent on any domain \(U\subset\mathbb{C}\) such that for all \(\xi_1,\xi_2\in U\), \(\xi_1-\xi_2\neq2\uppi \ii k\) for any \(k\in\mathbb{Z}\). That is, \(U\) must not contain any pair of points differing by a nonzero integer multiple of \(2\uppi\ii\). If \(\xi_1=\ii z_1\) and \(\xi_2=\ii z_2\), then this translates to \(z_1-z_2\neq2\uppi k\) for \(k\in\mathbb{Z}\).\ \(w(\zeta)=\frac{\zeta+\frac{1}{\zeta}}{2}\) is univalent on regions excluding pairs \((\zeta_1,\zeta_2)\) such that \(\zeta_1=\frac{1}{\zeta_2}\). In terms of \(z\), this condition becomes \(\ee^{\ii z_1}\ee^{\ii z_2}\neq1\), or equivalently, \(z_1+z_2\neq2\uppi k\) for any \(k\in\mathbb{Z}\).

Combining these constraints, we conclude that \(\cos(z)\) is univalent on any vertical strip in the complex plane of width \(\uppi\), such as a region of the form \[\cbraces{z\in\mathbb{C}}{k\uppi<\Re(z)<(k+1)\uppi,k\in\mathbb{Z}}.\] Let us now consider the specific region \(\cbraces{z\in\mathbb{C}}{0<\Re(z)<\uppi}\), and analyze how it is mapped under \(\cos(z)\).
\begin{enumerate}
    \item \(\xi(z)=\ii z\) maps the region \(\cbraces{z\in\mathbb{C}}{0<\Re(z)<\uppi}\) to \(\cbraces{\xi\in\mathbb{C}}{0<\Im(\xi)<\uppi}\).
    \item \(\zeta(\xi)=\ee^\xi\) maps this region to the upper half plane \(\Im(\zeta)>0\) since \(0<\Arg(\zeta)<\uppi\) and \(0<\abs{\zeta}\).
    \item \(w(\zeta)=\frac{\zeta+\frac{1}{\zeta}}{2}\) maps \(\Im\paren{\zeta}>0\) to \(\mathbb{C}\setminus\paren{(-\infty,-1]\cup[1,\infty)}\).
\end{enumerate}
Thus, the composition \(\cos(z)=w\circ\zeta\circ\xi\) is univalent on the strip \[\cbraces{z\in\mathbb{C}}{0<\Re(z)<\uppi},\]
...
\end{align*}
Then \(\arccos\) is also a multi-valued function. We can also define \(\arcsin(z)=\frac{\uppi}{2}-\arccos{z}\).
...
\begin{enumerate}
    ...
    \item If \(\alpha=u\in\mathbb{Q}\) with reduced fractional form \(\frac{p}{q}\), where \(p,q\in\mathbb{Z}\), \(q>0\), and \(\gcd(p,q)=1\), then the multivalued function \(z^\alpha\) is given by
        \[z^\alpha=\abs{z}^{\frac{p}{q}}\exp\qty(\ii\frac{p}{q}(\Arg(z)+2\uppi k))=\abs{z}^{\frac{p}{q}}\exp(\ii\frac{p}{q}\Arg(z))\exp(2\ii\frac{p}{q}\uppi k),\]
        for \(k\in\mathbb{Z}\). These values are periodic with period \(q\), since
        \[\exp(2\ii\frac{p}{q}\uppi(k+q))=\exp(2\ii\frac{p}{q}\uppi k)\exp(2\ii\uppi p)=\exp(2\ii\frac{p}{q}\uppi k),\]
        as \(\exp(2\uppi\ii p)=1\) for integer \(p\). To prove there are exactly \(q\) distinct values, consider \(k=0,1,\dots,q-1\). The exponential factors are \(\exp(2\ii\frac{p}{q}\uppi k)\). These are distinct if, for \(0\leq j<k\leq q-1\),
        \[\exp(2\ii\frac{p}{q}\uppi j)\neq\exp(2\ii\frac{p}{q}\uppi k),\]
