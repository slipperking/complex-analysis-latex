...
It follows that the points \(0\), \(\qty(x,y,1)\), and \(\qty(x_1,x_2,1-x_3)\) are collinear in \(\mathbb{R}^3\). Under the linear map \(\va{v}\mapsto\va{v}\operatorname{diag}\qty(1,1,-1)+\qty(0,0,1)\), we get that \(\qty(0,0,1)\), \(\qty(x,y,0)\), and \(\qty(x_1,x_2,x_3)\) are collinear. In other words, this correspondence is a central projection with center \(\qty(0,0,1)\), projecting the points from \(S^2\setminus \qty(0,0,1)\) onto \(\mathbb{C}\). Let this center correspond to \(\infty\). In this representation, \(\infty\in\extcomplex\) is no longer considered to be ``special''.

It is worth noting that in several geometric context, an alternative paradigm exists where we let \(S\) be the sphere centered at \(\qty(0,0,\tfrac12)\) of diameter 1, and project points from the north pole \((0,0,1)\) onto the horizontal plane of tangency. Later in Nevanlinna Theory (\cref{sec:nevanlinnatheory}), we will observe that in some sense, this is the more natural object to study. The corresponding equations are then \[x_1=\frac{\Re{z}}{\abs{z}^2+1},\quad x_2=\frac{\Im{z}}{\abs{z}^2+1},\quad\frac{\abs{z}^2}{\abs{z}^2+1}.\] The forward projection remains unchanged.

Lastly, we define the upper half-plane; for the following sections, let \(\mathbb{H}^+=\cbraces{z\in\mathbb{C}}{\Im(z)>0}\).
